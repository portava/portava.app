...
Three sites — `routes/messaging.ts:2611` (`disable_messaging` on the ordinary
...
| --- | --- | --- |
| `:1611` | `lib/telegraphThreadWrite.ts:36 → :80#guardTelegraphThreadWrite` | `messagingStopUnknownRefusal` added above it (defect 2) |
| `:3197` | `components/TelegraphInboxScreen.tsx:226 → :250#needsAction` | the accessibility sweep's labels on the unread badge and the mute glyph, both above that render |
