...
**All ten queued migrations are now applied to portava-ci and NONE of them to
production.** CI is a rehearsal surface; an entry in the `CI` column is not an
entry in the `Applied` column, and no row below has moved from one to the other.
...
| 22 | `2722` highlight sources | **2720 → 2721** (ordered, not batched) | **ci ✓** `20260908151954` applied, constraints exercised with a seeded fixture, rollback rehearsed | table, both index names, the unique constraint and all three policy names FREE | **not applied to production** | RLS on; **three policies and deliberately NO UPDATE policy** — "editing which source a link points at, in place, is how provenance quietly changes under an audit" — confirmed as an equality, 0 UPDATE policies. Exactly ONE foreign key (`highlight_id`); `source_id` carries none, which is checked rather than assumed because it is load-bearing: a link to a source that no longer exists MUST remain storable or a deleted Memory leaves nothing for §21 revocation to find. Probes: link accepted with default provenance `USER_ASSERTED`, duplicate link REFUSED, `source_type='POST'` REFUSED, `provenance='GUESSED'` REFUSED, link to an unknown source ACCEPTED (intended), and deleting the highlight took its links (0 left) | **ci only** |
| 23 | `2724` highlight revocation log | **2722** | **ci ✓** `20260908152035` applied, all four statuses and three vocabularies exercised, rollback rehearsed | table, all three index names FREE; `profiles` present | **not applied to production** | RLS on with **ZERO policies** — checked as an equality, because deny-by-default IS the design here and one permissive policy would be the defect. Only `actor_id` carries a foreign key (`ON DELETE SET NULL`, confdeltype `n`); `subject_id` carries none, and must not: the Highlight is already gone by the time the log is written. Probes: all four statuses accepted in one attempt — including `not_implemented` and `not_applicable`, which are the point (a destination that was not reached must be recorded as not reached, never omitted) — `operation='PURGE'` REFUSED, a ninth destination REFUSED, `status='partially_revoked'` REFUSED, and a retry row for the same (attempt, destination) ACCEPTED, which is required: forbidding it would force an in-place edit of the record of a failure | **ci only** |
| 24 | `2723` highlight class / lifecycle / pin | **2720 → 2721 → 2722** | **ci ✓** `20260908152414` applied, NOT VALID semantics proved, rollback rehearsed | **the only one of the five that ALTERS a live table.** All 5 column names, both constraint names and the index name FREE; `highlights` 17 columns, 0 rows, `deleted_at` present (the partial index needs it), 5 RLS policies. The insert-path claim was verified in code, not taken from the header: `routes/highlights.ts:806#.from("highlights")` lists its inserted columns explicitly and uses no `SELECT *`, so a new column cannot break a PostgREST insert (one unknown column fails the WHOLE insert, PGRST204) | **not applied to production** | 17 → 22 columns, every one **nullable with no DEFAULT** — a DEFAULT would write a state onto rows no state machine transitioned. Both CHECKs report `convalidated = false`, i.e. genuinely NOT VALID, **and still refuse a future write**: `lifetime_class='FOREVER'` REFUSED, `lifecycle_state='DELETED'` REFUSED (the §21 deletion machine stays separate from the §5 one), legal values accepted, a row with all five NULL accepted. Policies 5 → 5, rows 0 → 0. Rollback rehearsed: 22 → 17, both CHECKs go WITH their columns, the partial index goes, the 5 policies and the other 7 `highlights_*` indexes untouched | **ci only** |
| 25 | `2450` trip/participant families | **2420** | — | — | queued | — | — |
| 26 | `2500` `JOIN_VIA_LINK` + host | **2450** | — | — | queued | — | — |
| 27 | `2490` destructive privilege boundary | none | ✓ | 375 app-owned offenders → **0**; 3 extension-owned excluded | **APPLIED prod 20260908011416** | vacuity guard ≥300 relations; 3472 `has_table_privilege` probes, 0 held | ✓ |
...
## A premise in 2724's comment that does not hold

2724 justifies putting a foreign key on `actor_id` like this: "an account
deletion that removes the profile is entitled to remove the actor link." The
`ON DELETE SET NULL` is correct and harmless. The premise is not: account
deletion ANONYMISES `public.profiles` rather than deleting the row — measured
against the live production catalogue, `profiles` has 182 inbound `ON DELETE
CASCADE` foreign keys and ZERO outbound ones, and does not reference
`auth.users` at all. So that FK's delete action can never fire from account
deletion, and `actor_id` outlives the actor's erasure exactly as
`memory_command_audit.actor_user_id` does. Both sit on the open **D6 deletion
fate** boundary. Reported, not decided.
...
## 2026-09-22 — the Layover decision record, NOT YET QUEUED FOR PRODUCTION

`2992_layover_decision_record_and_operational_tables.sql` is written and
rehearsed and is applied to NO database — not production, not portava-ci.

**It has a hard prerequisite and says so itself.** The file's own precondition
block refuses with a named reason rather than failing on a missing relation:

> PRECONDITION FAILED (2992): public.layover_certified_computations does not
> exist. Apply 2700_layover_certified_feasibility.sql FIRST -- this migration
> completes that table rather than forking it, and creating it here would fork
> the definition of the one object the whole design turns on.

`2700_layover_certified_feasibility.sql` is the only file in the tree that
creates that table, and it is **not in `production-applied-migrations.json`**;
`layover_certified_computations` is absent from the 2026-09-22 production
capture, as are `layover_constraints` and `layover_time_budgets`. So the
production chain is **2700 then 2992**, in that order, and neither step has run.

**What 2992 does to existing data: nothing.** It has no UPDATE, no DELETE and no
TRUNCATE. Its three `DROP TRIGGER IF EXISTS` statements each sit immediately
before the `CREATE TRIGGER` that replaces them, which is the file's idempotency
pattern rather than a removal. Its three `ADD COLUMN IF NOT EXISTS ... NOT NULL
DEFAULT` statements take constant defaults, so on PostgreSQL 11+ they are
catalog-only and rewrite no rows.

**Until both are applied, `persistDecision` takes its refusal branch** and
`GET /airport/sessions/:id/safety` answers `persisted: { state: "not_stored",
reason }`. That is the honest state, and it is why census-layover L1 is not
graded closed on the strength of the merged code.
