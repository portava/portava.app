...
| CT-02 | `:25`, `:373`, `:488` Consume typed Trip projections (`TripCompassProjection`) rather than duplicating Trip semantics | **N** | `TripCompassProjection`: zero occurrences (census-trips TR360 `N`). Compass reads raw `trip_members`/`trips`/`trip_plan_items` (`CompassTools.ts:416-463`; `CompassTripContext.ts:17-21`). Trips' to publish; not closable from Compass. |
| CT-03 | `:185` Consume Temporal Freedom windows rather than independently calculating free time | **W** | No engine exists (TR131), and Compass calculates free time itself in **two** places: `CompassTools.ts:146-160, 647-701` `check_trip_conflicts`, and `compass/CompassSenseEngine.ts:245-246` `free_time_block` (*"no plan item starts within the next 3 hours"* from `trip_plan_items`). census-trips TR133 scores the *consumption* `N`; this row scores the *independent calculation* the clause forbids, which is built and wrong. |
...
| CL-05 | §12 Twelve layover tools (`getLayoverContext` …) | **N** | L102–L113 `N`: `CompassTools.ts:65-220` declares eleven tools, none layover. |
...
| CTG-07 | §18.3 No cross-participant leak, no impersonation, no silent canonical plan | **C** | T253 `C` (`CompassTools.ts:236` SOCIAL RULES; `requires_confirmation: true`). |
...
| CTG-09 | §30A.13 Private policy signals constrain, never exposed as a score | **C** | T418 `C`: `CompassTools.ts:839-860` consults `overall_score` as a floor and answers uniformly *"not available"* — never why. Made fail-closed on an unreadable table this pass (CC-07). |
...
| CC-20 | CANDIDATE RULE: the model never invents candidates; UI blocks validated against the turn's tool results (`CompassTools.ts:231`; `CompassUiBlocks.ts:7-15`) | **C** | `CompassUiBlocks.ts:11-15` *"any id the tools did not return is silently dropped"*. |
...
|---|---|---|---|
| F1 | Social trust floor fails closed on an unreadable `trust_profiles`; absent row unchanged | `compass/CompassTools.ts:839-860` | CC-07; CTG-09 (T418); CTR-01 (one of A17's four reads made safe) |
...
| F3 | `refreshHiddenUsers` with no snapshot and a failed read now throws (tool answers "failed") instead of using an empty hidden set | `compass/CompassTools.ts:272-313` | CC-08; CTG-02 (T219/T220 Compass leg) |
...
| CC-05 | W | C | The count is now checked instead of asserted: `artifacts/api-server/src/compass/CompassTools.ts:4#Forty-one tools the model may call on demand` states it, `artifacts/api-server/src/compass/CompassTools.ts:127#number of tools the file header states` carries it as a constant, and `artifacts/api-server/src/test/compassToolCountContract.test.ts:1#/**` asserts the constant equals the definition count AND that the header's number WORD parses to the constant. Both halves are needed: pinning only the constant leaves the prose free to lie. |
...
| what | where | closes |
|---|---|---|
| `COMPASS_TOOL_COUNT_IN_HEADER` beside the definitions, and a test asserting it against the definition count and against the header's own number word | `artifacts/api-server/src/compass/CompassTools.ts:127#number of tools the file header states`, `artifacts/api-server/src/test/compassToolCountContract.test.ts:1#/**` | CC-05, for the third time and the first time executably |
...
| CH-03 | W | **C** | **Built.** *"A graph node built from a memory persists after that memory is deleted … no deletion hook, no per-memory prune"* is closed for the node. `artifacts/api-server/src/compass/CompassGraphEngine.ts:1393#export async function pruneOrphanedExperienceNodes` removes an `experience` node and every edge touching it once the source Memory is deleted, unpublished or switched to `only_me`, and runs inside the daily rebuild BEFORE the world models and the confidence index derive anything from it (`artifacts/api-server/src/compass/CompassGraphEngine.ts:1474#const pruned = await pruneOrphanedExperienceNodes(db);`). The scheduler that calls it is registered unconditionally, so this is ungated. **It is still not a HOOK**, and the row's `C` is for the prune, not the window: a node survives until the next daily rebuild. Pinned by block B, six cases, two mutations. |
...
| A per-memory prune of `experience` graph nodes and their edges, inside the daily rebuild | `artifacts/api-server/src/compass/CompassGraphEngine.ts:1393#export async function pruneOrphanedExperienceNodes` | CH-03 |
...
carries `artifacts/api-server/src/compass/CompassGraphEngine.ts:1352#sourceUnavailable: boolean;`
