...
| CC-04 | Every tool result is stripped of coordinate-shaped and private keys before reaching the model (`CompassTools.ts:10-12`) | **C** | `:534-554` `PRIVATE_KEY_RE` + recursive `stripCoordinateFields`; applied at `CompassTools.ts:2032#return sanitizeToolResult(raw);`. |
...
stripper) and `artifacts/api-server/src/compass/CompassTools.ts:2032#sanitizeToolResult(raw)` (the single dispatch exit it is applied at). The verdict is unchanged — the
...
- **`CC-04`'s *"applied at `:1979`"*** cited a line beyond the end of a 1967-line file. The single
  sanitize exit is `artifacts/api-server/src/compass/CompassTools.ts:2032#return sanitizeToolResult(raw);`.
...
`artifacts/api-server/src/compass/CompassTools.ts:1486#const { data: trip, error: tripErr }` — is
...
`artifacts/api-server/src/compass/CompassTools.ts:2047#if (err instanceof TripAccessUnavailableError) {`.
