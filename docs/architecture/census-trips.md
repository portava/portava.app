...
- **Five rows the tree had already earned.** TR189: Today has carried
  ...
  `src/index.ts:172#startTripOutboxWorker()`), and §41's pipeline
...
- **TR440 holds W, with the pieces named.** The command route exists
  ...
  (`src/index.ts:172#startTripOutboxWorker();`); the reservation
...
- **TR440 — `server/trips/`, the five pieces, real.**
  ...
  (`src/index.ts:172#startTripOutboxWorker();`, `src/index.ts:116#startTripProjectionWorkers();`)
