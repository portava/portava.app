...
- **Five rows the tree had already earned.** TR189: Today has carried
  ...
  `src/index.ts:192#startTripOutboxWorker()`), and §41's pipeline
...
- **§21.3 / §5.3 the raw text bounded, and the sweep that enforces both policies (TR403)** —
  ...
  `index.ts` starts it (`src/index.ts:118#startTripProjectionWorkers();`).
...
- **TR440 holds W, with the pieces named.** The command route exists
  ...
  (`src/index.ts:192#startTripOutboxWorker();`); the reservation
...
- **TR440 — `server/trips/`, the five pieces, real.**
  ...
  (`src/index.ts:192#startTripOutboxWorker();`, `src/index.ts:118#startTripProjectionWorkers();`)
