...
| --- | --- | --- |
| **IMPLEMENTED** | 328 of 451 requirements are BUILT (**148 C + 180 W**) — **72.7 % constructed** — ON THE BRANCH `claude/sweet-fermat-fmx7up`, not on `main` | §36 recount, 451/451 rows parsed; §38 and §39 re-derived 78 of them; §40.1–§40.7 BUILT 57 rows into C and 31 N→W, each with file, line, test and a mutation that went red; §41 executed the kernel on a throwaway database built from production's structure dump (3 rows to C) and seven new kernel families on it, 2779–2785 (25 N→W), every migration seen refused before it was applied |
...
The 72.7 % is a **document measuring itself**. `check:census-integrity` states
...
**Trips: 72.7 % constructed, 32.8 % correct, of 451 requirements — on the branch.**
