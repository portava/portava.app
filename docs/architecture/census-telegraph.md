...
| --- | --- | --- |
| T79 | "`routes/messaging.ts` returns media_url/thumbnail/type/duration on a deleted message. Four lines." | **Exactly right, and it was exactly four lines.** Confirmed at the serializer and confirmed to be the ONLY leaking surface: search filters in the query (`artifacts/api-server/src/services/telegraphSearch.ts:164#// §21: deleted objects are excluded in the QUERY`), the drawer filters (`artifacts/api-server/src/routes/telegraphKinds.ts:149#.is("deleted_at", null)`), the inbox preview filters, saved messages filters, and both Memory Note paths refuse a tombstone. |
...
## §31 — TELEGRAPH LANE 4: the coordination session's LIFECYCLE, and the four §13.2 events nothing was firing
...
deployed and no flag was enabled. This lane wrote NO migration** — see §31.7 for
...
### 31.1 What was re-derived before anything was written
...
### 31.2 Retries: the command is idempotent, and it is enforced TWICE
...
### 31.3 Expiry: a mechanism, not a column
...
### 31.4 Cancellation, and membership changes mid-session — the spec's answer
...
### 31.5 Version conflict, and its path is proved
...
| T155 | N | **N** | **`conversation_snapshots` — evidence correction, verdict unchanged.** "Absent" is still true and this lane deliberately did not close it; see §31.7. Recorded here because the row was opened, priced and declined rather than overlooked. |
...
as a mutation — §31.9 — and two of them did not land on first measurement.

### 31.7 `conversation_snapshots` was priced and DECLINED, and the reason is not effort
...
1. **§12's own wording is "rebuild/replay checkpoints WHERE USEFUL"**, and the
   ...
   (§31.6, T150), and a checkpoint would have papered over it at the price of a
   second store.
...
### 31.8 The §9 W rows, enumerated, and which were taken
...
| id | Why not |
|---|---|
| T104 | *Preparing UI: plan card, attendance, leave-by, route, availability conflicts.* Three of five. §21.5b already priced the fourth: in-thread availability-conflict detection is buildable and INERT, because `open_to_plans_windows_enabled` is seeded OFF and on no database. That reason is now half stale — §31.3's substrate, `quick_availability_status`, is NOT flag-gated and IS in production, so a conflict detector over it would be exercisable. It was still not built: this lane holds no client file, and the remaining fifth item (route) belongs to the Map programme. **The blocker is now the drawing, not the data.** |
...
but weak, and is why §31.9's mutation pass is the evidence that each individual
...
file and this lane did not touch it. §31.6 is a re-measurement of the ten rows
...
### 31.12 Headline, restated from the rows

`check:census-integrity` counted this document at **C 231 / W 159 / N 58 / X 3
across 451** when this lane opened it — §27.7's figure. After §31.6 the same
...
§31.6 and nothing else**: T168, T187, T188, T189, T190, T191 and T192 N→C, T150
...
### 31.13 Not claimed
  ...
  - **`availability_windows` is untouched.** §31.3 sweeps
  ...
  - The 151 W rows this section does not name were not re-read, and §31.12's
