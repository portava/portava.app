...
## §27 — DELIVERED becomes a fact the bus already knew, reconnect stops starting from scratch, and the fan-out gets a size it will not exceed
...
read receipts, per-message seen, and the delivery receipts §27.1 just added —
...
| id | exact blocker |
| --- | --- |
| T69, T72, T139, T141, T156, T210, T228, T230, T231 | Columns that exist only in `2810`/`2811`. **No database has run them.** T69 gains DELIVERED as a concept from §27.1 — two of six becomes three of six — and still has no `lifecycleState` column and no UNSENT, so it does not move. |
...
- **A durable event log arriving.** Then T233's event half becomes a live
  question and §27.2's ceiling must be re-derived rather than re-read.
...
append-only. **The delta from §24.2 is exactly the three rows in §27.1–§27.3 and
