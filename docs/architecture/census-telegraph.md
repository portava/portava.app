...
1. **A group-add reads the whole back history.** `GET /threads/:threadId/messages`
   (`routes/messaging.ts:1576-1584`) filters on `thread_id` and active
...
2. **`saved_messages` is write-only.** Both client surfaces offer "Save"
   ...
   (`routes/messaging.ts:2680`) — and **nothing in the repository ever reads the
...
3. **`message_reports` and `thread_reports` are dead tables.** The ground-truth
   ...
   **unified `reports` table** instead (`artifacts/api-server/src/routes/messaging.ts:3824#router.post('/threads/:threadId/report', async (req, res) => {` for threads,
...
`services/media/MediaProjectionService.ts:1399#const { data } = await (sc as any).from(table).select("*").eq(ownerCol, ownerId).limit(1000);`) were opened and confirmed to be
...
| T2 | Pillar **Talk** — text, rich media, voice, GIFs, replies, reactions, seen states, safe message lifecycle | W | Four of eight. Text/media (`routes/messaging.ts:1733`, `:2068`), replies (`:3080#reply_to_id` `reply_to_id`), seen (`:1203`) are real. **Voice, GIF, reactions and safe lifecycle are absent**: `:2079-2082` accepts only `image`/`video`; `message_reactions` does not exist in `baseline/20260819_baseline_structure.sql`; delete is unconditional (`routes/groupChat.ts:340-381`) and unsend does not exist. |
...
| T34 | Stranger flow: safe profile preview → Wave/Request → accepted thread; no automatic unrestricted messaging | C | `lib/messagingPermissions.ts:29-45` resolves `allowed` / `requires_request` / `denied`; `routes/messaging.ts:277-304` refuses `open-thread` fail-closed and names the request route; `:431` files the request and `routes/messaging.ts:890#router.post('/message-requests/:requestId/accept', async (req, res) => {` accepts it into a thread. No path opens an unrestricted stranger thread. There is no separate "Wave" primitive — the request is it. |
...
| T39 | Object family **Services** — Buddy profile/service, eligible booking card, Visa Buddy operational card | W | Booking milestones render (`components/rentabuddy/BookingMilestoneMessage.tsx`, dispatched `app/messages/[id].tsx:1862-1866`) and a booking owns its thread (`rent_buddy_bookings.telegraph_thread_id`, read at `routes/messaging.ts:1886`). No Buddy profile share, no Visa Buddy card. |
| T40 | Object family **Media** — photo, video, voice, GIF, file | W | Photo and video only (`routes/messaging.ts:2079-2082`). No voice, GIF or file. |
...
| T48 | Message kind **TEXT** | C | `routes/messaging.ts:1757` `msgType` defaults to `'text'`; rendered `app/messages/[id].tsx:1899`. |
| T49 | Message kind **IMAGE** | C | `routes/messaging.ts:2079-2082` + `:2175` `media_type`; rendered `components/MessageMediaBubble.tsx`, dispatched `app/messages/[id].tsx:1870`. |
...
| T59 | Message kind **SYSTEM** | C | `routes/messaging.ts:1756` `msgType === 'system' ? 'system' : 'text'`; centred-pill renderer `app/messages/[id].tsx:1892-1898` → `components/TelegraphSystemNotice.tsx:16`. |
...
| T73 | Direct: Sent/Delivered/Seen. Groups: "Seen by N", optionally list viewers when policy allows | N | No receipt UI beyond the inbox's own unread count. `read.updated` is published (`routes/messaging.ts:1230-1235`) but nothing renders per-message receipts. |
| T74 | Do not create permanent row-per-message-per-user receipt explosions | C | The receipt is one row per member per thread with a single timestamp (`baseline:7495-7505`), and unread counts are derived by comparing `created_at > last_read_at` (`routes/messaging.ts:916-961`). The forbidden shape is structurally absent, and the alternative is implemented. |
...
| T77 | The server resolves read-vs-unsend races transactionally | N | No unsend; and the competing writer (`routes/messaging.ts:1743#.update({ last_read_at: threshold })` `last_read_at`) takes no lock. |
| T78 | Preserve a sequence tombstone internally | W | A tombstone is preserved — deleted rows are returned and rendered as a redacted slot, and the body is emptied rather than the row removed (`routes/groupChat.ts:363-372`, reader `routes/messaging.ts:1576`). But there is no `sequence` to preserve continuity of; ordering is `created_at` (`:1580`). |
| T79 | Remove from normal retrieval, search and projections | W | The body is suppressed on `deleted_at` (`routes/messaging.ts:1718` `body: isDeleted ? null : m.body`) — but in **main** the media fields are not: `:1719-1722` returns `media_url`, `media_thumbnail_url`, `media_type` and `media_duration_seconds` unconditionally, so deleting a photo redacts the caption and leaves the asset addressable. PR #472 fixes exactly this; it is not merged. |
| T80 | Text edits retain an Edited marker **and version history** | W | The marker is real (`routes/messaging.ts:2370-2373` sets `edited_at`, surfaced as `editedAt`). **Version history is not**: the update overwrites `body` in place and `message_edits` does not exist in the schema. Translation invalidation on edit is correct (`:2400` `markTranslationsPending`). |
| T81 | Editing message prose never silently mutates a canonical Plan/Event/Trip object | C | The edit handler writes exactly two columns on one row — `body` and `edited_at` (`routes/messaging.ts:2370-2373`) — and touches nothing else. No card is re-parsed, no domain write is triggered. The forbidden path does not merely happen not to run; the handler has no branch that could reach it. |
...
| T119 | A user may explicitly save a message, voice note, place share or media item as a private Memory draft | W | **Persists into a hole.** Both client surfaces offer it (`app/messages/[id].tsx:2227`, `components/GroupChatScreen.tsx:1064`) and the server upserts (`routes/messaging.ts:2655-2692`), but **nothing reads `saved_messages` back** — settled by reading call sites, not greps: the only other references in the entire repository are the deletion cascade (`lib/deletionDispositions.ts:432`) and the RLS ledger (`scripts/rlsDispositions.ts:429`). There is no saved-messages route and no screen. It also lands in `saved_messages`, not in `memories`, so it is not a Memory draft. |
...
| T153 | `conversation_reports` / `blocks` | W | `blocks` is real and cascading (T30). Reporting is real but writes to the **unified `reports` table** (`artifacts/api-server/src/routes/messaging.ts:3824#router.post('/threads/:threadId/report', async (req, res) => {` threads, `:2709` messages) — while the two dedicated tables production actually holds, `message_reports` and `thread_reports`, have **no writer and no reader** anywhere in application code. Settled by reading both handlers; their only other appearances are `scripts/rlsDispositions.ts:310,456`, `scripts/frozenLegacyFiles.ts:45-46` and `lib/deletionDispositions.ts:370,441`. |
| T154 | `conversation_outbox` — transactional domain-event outbox | N | No outbox table and no outbox worker. Events are published **after the HTTP response** and outside any transaction (`routes/messaging.ts:1997` responds, `:2019` publishes). |
...
| --- | --- | --- | --- |
| T159 | `SEND_MESSAGE` | C | `routes/messaging.ts:1733`. |
| T160 | `EDIT_MESSAGE` | C | `routes/messaging.ts:2332`. |
...
| T164 | `ACCEPT_REQUEST` | C | `routes/messaging.ts:890#router.post('/message-requests/:requestId/accept', async (req, res) => {`. |
| T165 | `DECLINE_REQUEST` | C | `routes/messaging.ts:839`; cancel at `:878`. |
...
| T175 | `MUTE_THREAD` | C | `routes/messaging.ts:2539` → `message_thread_members.muted_at` (`baseline:7500`). |
| T176 | `REPORT_MESSAGE` | C | `routes/messaging.ts:2698-2727`, writing the unified `reports` table with `target_type:'message'` and invalidating the reporter's Compass cache at `:2724`. |

| id | Event (§13.2) | V | Evidence |
| --- | --- | --- | --- |
| T177 | `message.sent` | C | Published as `message.created` (`lib/telegraphEvents.ts:24`), emitted at `routes/messaging.ts:2019-2034` with the sender excluded. Renamed, semantically identical. |
| T178 | `message.delivered` | N | No delivered concept exists to emit (T69). |
| T179 | `message.seen` | W | `read.updated` (`lib/telegraphEvents.ts:30`) is published at `routes/messaging.ts:1230-1235` — but it carries a **thread-level** `lastReadAt`, not a per-message seen fact, so no consumer can answer "was *this* message seen". |
| T180 | `message.edited` | C | `message.updated` (`lib/telegraphEvents.ts:33#message.updated`), published `routes/messaging.ts:2393-2398`. |
...
| T195 | Canonical mutation and event-outbox write occur in the **same database transaction** | N | There is no outbox. `routes/messaging.ts:1997` sends the 201, then `:2019` and `:2031` publish, then `:2038` translates — all after the response, all outside any transaction. A crash between insert and publish loses the event with no replay path. |
...
| id | Requirement | V | Evidence |
| --- | --- | --- | --- |
| T197 | Capability `canSendMessage` | C | Resolved server-side and re-checked per send: `lib/messagingPermissions.ts:29-45` (verdict ladder incl. `message_privacy` and mutual-block), enforced at `routes/messaging.ts:1764-1772` (membership) and `:1778-1792` (pairwise block, fail-closed via `isBlockedBetween`). |
...
| T202 | Capability `canRequestPayment` | N | No payment request in Telegraph. The nearest thing is the off-app solicitation *detector* (`routes/messaging.ts:1874-1941`), which is the opposite concern. |
...
| T211 | New members do not automatically receive pre-membership history | W | **Violated.** `routes/messaging.ts:1576-1584` selects from `messages` filtered on `thread_id` and ordered by `created_at`, with the only gate being *current* active membership (`:1559-1569`) — no `joined_at` bound of any kind. `services/groupChatSync.ts:9-13` adds every newly-accepted trip member to the trip thread, and `syncCircleChatMembers` does the same for circles, so a new member can immediately page back through the entire prior conversation. |
...
| T226 | Media scanning/moderation must not block basic text delivery | C | Structurally impossible to block it: text and media are separate endpoints (`routes/messaging.ts:1733` vs `:2068`), and the sniff/size/rate policy runs at *upload* time in `lib/mediaPipeline.ts:187-227` — a path a text send never enters. |
...
| id | Requirement | V | Evidence |
| --- | --- | --- | --- |
| T228 | Per-conversation server sequence ordering, not global ordering | N | No sequence column exists. Ordering is `created_at DESC` with a `created_at` cursor (`routes/messaging.ts:1580-1584`), which cannot distinguish two messages written in the same millisecond and cannot express "after event N". |
| T229 | Client timestamps are advisory only | C | The server generates the timestamp itself and never reads one from the body: `routes/messaging.ts:1821` `const now = new Date().toISOString()`, written at `:1852` `created_at: now`. There is no `createdAtClient` field to trust. |
| T230 | Offline command fields: `clientMessageId`, `idempotencyKey`, `createdAtClient`, `syncState` | W | One of four, and it is **not persisted**. `clientId` is accepted (`routes/messaging.ts:1762`), echoed in the response (`:2006`) and in the realtime payload (`:2030`) — but the insert at `:1847-1860` does not include it, so the server retains no correlation key after the request ends. |
...
| T234 | Seen/delivery state, edits and canonical thread state converge server-side | W | Edits and thread state converge properly (one server-authoritative row; `routes/messaging.ts:2370`). Seen converges at **thread** granularity only. Delivery has no state to converge (T69). |
...
| T237 | Push failure → no loss of canonical messages; push is wakeup/fallback, not consistency authority | C | The row is inserted (`routes/messaging.ts:1847`) and the 201 sent (`:1997`) before any notification work; dispatch is `Promise.allSettled` inside a try/catch explicitly labelled non-fatal (`:1976-1991`). |
| T238 | Translation / AI / maps failure → core messaging continues | C | `services/messageTranslation.ts:11` — *"Falls back to `status: failed` on any error — never throws"* — and the call site adds an outer net (`routes/messaging.ts:2038-2045`). Tagging is wrapped the same way (`:1963-1991`). Suggestion generation is a separate endpoint entirely. |
...
| T260 | Inbox displays "12 unread · 1 needs action" | W | Unread is real and carefully built (`routes/messaging.ts:916-961`, with a schema-drift fallback when migration 0016's `last_read_at` is absent, `:941-944`). **"Needs action" has no representation** — nothing distinguishes a message from an unresolved decision. |
...
| --- | --- | --- | --- |
| T262 | **Trips** — crew threads, shared Trip cards, today/next context, membership authorization, Trip Kernel commands | W | Two of five. Crew threads are real and auto-synced (`routes/messaging.ts:2425` `GET /trips/:tripId/chat`, `services/groupChatSync.ts:9-13`) and membership authorizes (`isAcceptedTripMember`, `routes/telegraphCommands.ts:463`). No shared Trip card, no today/next context, no Trip Kernel commands. |
...
| T269 | **Buddy / Visa Buddy** — protected booking/operational layer; **no chat mutation of price/terms** | C | The booking layer is real (`components/rentabuddy/BookingMilestoneMessage.tsx` dispatched at `app/messages/[id].tsx:1862`; a booking owns its thread via `rent_buddy_bookings.telegraph_thread_id`, read at `routes/messaging.ts:1886`; booking-scoped call eligibility at `routes/calls.ts:26`). The prohibition is **actively enforced, not merely unviolated**: no message write path reaches a booking, and `routes/messaging.ts:1874-1941` runs a 14-pattern off-app solicitation detector that logs an admin-only event and auto-suspends the buddy profile past a threshold — with both writes error-checked because *"a silently failed update here leaves a repeat off-app solicitor ACTIVE with no trace"* (`:1916-1935`). |
| T270 | **Safety** — Safe Return, help states, location scope, block/report | C | All four. Safe Return end-to-end (`routes/safeReturn.ts:6-23`), help states (`circle_presence.needs_help`, `routes/circle.ts:1556-1589`), location scope (`trip_crew_location_sessions.visibility_level`), block/report (`routes/blocks.ts:46`, `routes/messaging.ts:2615`, `:2698`), plus an in-thread safety sheet (`components/ThreadSafetySheet.tsx`). |
...
| T276 | Unsent/deleted/revoked objects removed from normal user search and Compass retrieval | W | Partial, in the one place it can act. Deleted message bodies are suppressed on read (`routes/messaging.ts:1718`) and a report invalidates the reporter's Compass cache (`:2627`, `:2724`). But deleted **media survives** in main (T79) and revoked source objects survive inside cards forever (T46). |
...
| T282 | Travel scam signals: off-platform payment, fake taxi, visa help, ticket resale, fake hotel, urgent money request | W | **One of six, and that one is real and enforced.** `routes/messaging.ts:1874-1894` — a 14-pattern off-app payment detector (`off-app`, `pay outside`, `venmo me`, `cashapp`, `my whatsapp`, …) scoped to buddy-booking threads and attributed only when the sender *is* the buddy (`:1897-1905`), escalating to auto-suspension past `OFF_APP_SUSPENSION_THRESHOLD` (`:1914-1936`). No detector exists for the other five families. |
...
| --- | --- | --- | --- |
| T289 | `TelegraphHomeProjection` — conversation list + badges + Now + nearby summary | W | The list half is a real server-built projection: `artifacts/api-server/src/routes/messaging.ts:1897#router.get('/me/threads', async (req, res) => {` `GET /me/threads` resolves the other participant's display identity under `nameVisibilitySet`, folds in unread counts and thread type. No Now band and no nearby summary (T8). |
| T290 | `ConversationProjection` — renderable ordered thread **with current permissions** | W | The renderable half is real and careful: `routes/messaging.ts:1551-1731` sanitizes sender identities per viewer, joins per-recipient translations, resolves reply context and enriches mention/hashtag spans. It carries **no permissions block**, and it does not bound history (T211). |
...
| T299 | Inventory source-domain FK identities; never substitute semantically similar IDs | C | `scripts/checkSchemaReferences.ts` and `scripts/checkEnumLiterals.ts` are the standing guards; the routes enforce it locally too (`routes/messaging.ts:1835-1845` refuses a reply reference from another thread; `:2669-2678` refuses a cross-thread save). |
...
| --- | --- | --- | --- |
| T311 | Non-member reads conversation → **DENY** | C | `routes/messaging.ts:1559-1569` re-checks active membership on every read and returns 403 before any message query; `migrations/2070_rls_hardening.sql:11` lists `message_thread_members` among the hardened tables; `test/rlsPrivacy.test.ts`, `test/accessControl.test.ts`. |
| T312 | Removed member reads future sequence → **DENY** | C | Achieved by a blunter rule that is strictly stronger: `.is('left_at', null)` (`routes/messaging.ts:1564`) denies a departed member the *entire* thread, not merely future messages. There is no sequence, but the required outcome holds. |
...
| T314 | Blocked sender sends DM → **DENY** | W | Denied on the happy path (`routes/messaging.ts:1787-1791`, fail-closed `isBlockedBetween`) — but the membership read that decides whether to consult the guard drops its error (`:1782`), so a transient read failure yields ALLOW (T220). PR #472 closes it. |
...
| T319 | Trip member loses Trip membership → capabilities downgrade **immediately** | W | The effect is right and the timing is not guaranteed: `services/groupChatSync.ts:9-13` reconciles `message_thread_members` on membership change and reads then deny (`routes/messaging.ts:1564`) — but the sync is invoked fire-and-forget from the membership routes, and there is no capability object to downgrade (T207). |
...
| T333 | Edit while translation/transcript is generating | W | The ordering hazard is handled and tested — `markTranslationsPending` on edit (`routes/messaging.ts:2400`) plus `test/retranslateGate.test.ts`, `test/contentTranslationInvalidation.test.ts`, `test/commentTranslationInvalidation.test.ts` — but as unit coverage of the gate, not as an adversarial race fixture. |
...
| T344 | No silent catch converts a schema/permission failure into a plausible empty inbox/context | W | The ratchet exists and is on point — `test/silentSchemaErrorCatches.test.ts` and `scripts/checkSilentSupabaseWrites.ts` — but **the messaging tree still contains instances**, four of them in one file: `artifacts/api-server/src/routes/messaging.ts:1799#const { data: member, error: memberErr } = await sc` (membership read, error dropped), `:1782` (block-guard membership read, error dropped — the T220 hole), `:1610` (translation read, error dropped), `:2669` (save membership read, error dropped). PR #460 exists for exactly this class and is unmerged. |
...
| T352 | realtime reconnect recovery — no lost confirmed messages | C | Guaranteed architecturally rather than measured: the canonical row is committed and the 201 returned (`routes/messaging.ts:1847`, `:1997`) before any realtime publish (`:2019`), and the client's polling path is the source of truth (`routes/telegraphStream.ts:13-15`, `lib/telegraphEvents.ts:15-16` — *"any missed event self-heals on the next poll"*). A dropped SSE event cannot lose a confirmed message. |
...
| --- | --- | --- | --- |
| T355 | No AI, translation, transcription, maps or push dependency in the core message-delivery path | C | The row is inserted (`routes/messaging.ts:1847`) and the 201 sent (`:1997`) before translation (`:2038`), tagging (`:1963`), notification (`:1976`) and realtime (`:2019`), every one of which is fire-and-forget with its own net; `services/messageTranslation.ts:11` — *"never throws."* Delivery cannot be blocked by any of them. |
...
| T363 | No silent schema failures that become plausible empty state | W | The right ratchet exists (`test/silentSchemaErrorCatches.test.ts`, `scripts/checkSilentSupabaseWrites.ts`) and the messaging tree is one of the places it has not finished: four dropped-error reads in `routes/messaging.ts` (`:1561`, `:1610`, `:1782`, `:2669`), one of which disables a block guard (T220). |
...
| T369 | Derived projections are rebuildable from canonical state + events | W | The half that exists is fully rebuildable — both projections are computed per request from canonical tables and cache nothing (`artifacts/api-server/src/routes/messaging.ts:1897#router.get('/me/threads', async (req, res) => {`, `:1551`), which is stronger than "rebuildable". But there are **no events to rebuild from** (T195), and four of the six named projections do not exist. |
...
| T385 | **§30A.3** Text editing with an Edited marker and internal version history; editing never mutates a canonical object | W | Marker yes (`routes/messaging.ts:2370`), history no (T80). The non-mutation half is fully correct (T81). |
...
| T388 | Deleted and unsent content removed from normal search, Compass retrieval, inbox projections and content drawers per the operation's semantics | W | Inbox/read suppression is real (`routes/messaging.ts:1718`) and Compass is invalidated on report (`:2724`). Media survives deletion in main (T79); there is no search (T272) or drawer (T66) to remove from. |
...
| T398 | Thread notification policy ALL / MENTIONS / IMPORTANT / temporary mute / MUTED; safety-critical delivery governed by safety policy rather than ordinary mute | W | Two of five, but the second is the important one and it is exactly right: `MUTED` (`message_thread_members.muted_at`, `routes/messaging.ts:2539`) and the safety override — `services/notifications/NotificationPreferenceService.ts:6-7`, *"Safety override: urgent + admin priority always deliver even when push is off"*, with a stored `safety_override` flag (`:59`). No ALL/MENTIONS/IMPORTANT selector, no temporary mute. |
...
| T427 | Canonical conversations and messages survive loss of realtime caches, search indexes, translations, notification queues, AI artifacts and read projections | C | All six are strictly derived and none is on the write path. Realtime: in-memory, failures swallowed by design (`lib/telegraphEvents.ts:14-16`). Search index: none. Translations: separate rows with a `failed` status that never throws (`services/messageTranslation.ts:11`). Notifications: `Promise.allSettled` inside a non-fatal try (`routes/messaging.ts:1976-1991`). AI artifacts: their own table. Read projections: a single nullable timestamp, with an explicit fallback when the column itself is missing (`routes/messaging.ts:941-944`). |
| T428 | `ConversationProjection`, `InboxProjection`, `SharedContextProjection`, `ContentIndexProjection`, `SearchIndex` and seen-state derivatives are rebuildable from canonical state and events | W | The two that exist are stronger than rebuildable — computed per request from canonical tables, caching nothing (`artifacts/api-server/src/routes/messaging.ts:1897#router.get('/me/threads', async (req, res) => {`, `:1551`). Four do not exist, and there are no events to rebuild from (T195). |
...
| T71 | 7.2 | "Push delivery, app launch and background rendering do not count as seen." **The server cannot know.** `POST /threads/:threadId/read` (`routes/messaging.ts:1203`) accepts no evidence of foreground visibility and stamps `last_read_at` on any authenticated call. The client calls it from a mount effect on thread open (`app/messages/[id].tsx:1269-1273`, *"Mark thread as read when the user opens it. Fire-and-forget."*), which is the right intent but fires on mount rather than on visible render, and nothing enforces it server-side. This is a *design* gap as much as a verification one — the requirement is not checkable because the protocol carries nothing to check. |
...
2. **Two of production's seven Telegraph tables are orphans.** The seven are
   ...
   table instead (`artifacts/api-server/src/routes/messaging.ts:3824#router.post('/threads/:threadId/report', async (req, res) => {`, `:2709`). Their migrations are
...
6. **Messaging has one kill switch and no rate limit.** `disable_messaging` is
   checked fail-closed on every send (`routes/messaging.ts:1751-1755`), which is
...
- **§5.1's interface exists, verbatim, and is instantiated per family.**
  ...
  (`services/telegraph/shareables.ts:984#shareableFor`) returns one for any of
  fifteen object types across §5's five families —
  `services/telegraph/shareables.ts:974#SHAREABLE_OBJECT_TYPES`. A family with
...
- **§5.2's third layer is the whole point, and it is computed per read.**
  ...
  (`services/telegraph/shareables.ts:1068#resolveShareProjections`) takes a
...
- **The envelope is a REFERENCE.** `PortavaObjectBody`
  (`services/telegraph/shareables.ts:1146#PortavaObjectBody`) carries five
...
| id | was | now | why |
| --- | --- | --- | --- |
| T41 | N | **C** | **`TelegraphShareable` interface — `getSharePreview` / `getCurrentState` / `getAvailableActions` / `getDeepLink`** — all four methods exist under those names (`services/telegraph/shareables.ts:97#TelegraphShareable`) and are implemented for fifteen object types (`:984#shareableFor`), asserted family by family at `test/telegraphShare.test.ts:252`. |
| T44 | N | **C** | **Four-layer model: Share projection — what the recipient is *currently* authorized to see** — the layer exists and is computed per (viewer, object) on every read (`services/telegraph/shareables.ts:1068#resolveShareProjections`); the card is no longer whatever the sender serialised. |
...
| T36 | W | **W** | **Object family Social — profile, post, Highlight, public Memory derivative, Memory Note, Stamp** — profile, post and Memory are now shareable through the contract (`services/telegraph/shareables.ts:974#SHAREABLE_OBJECT_TYPES`). Highlight and Stamp have no loader and `STAMP` is not in the registry, so three of six. |
...
| T39 | W | **W** | **Object family Services — Buddy profile/service, eligible booking card, Visa Buddy operational card** — the booking card now resolves with its two-party authorization (`services/telegraph/shareables.ts:984#shareableFor`), and BUDDY_SERVICE maps to the same loader. There is no Visa Buddy card and no standalone Buddy profile share. |
...
- **`services/telegraph/messageKinds.ts`** declares a zod payload per kind and
  `validateKindMessage` (`services/telegraph/messageKinds.ts:186#validateKindMessage`)
...
- **§6.4's drawer is a CLASSIFIER over rows the thread already has.**
  `drawerTabFor` (`services/telegraph/messageKinds.ts:258#drawerTabFor`) routes
  each row to one of §6.4's seven tabs
  (`services/telegraph/messageKinds.ts:245#DRAWER_TABS`), including the legacy
...
- **§6.4's search is object-aware, and its authorization is not new law.**
  `searchableTextOf` (`services/telegraph/messageKinds.ts:313#searchableTextOf`)
...
| id | was | now | why |
| --- | --- | --- | --- |
| T51 | N | **C** | **Message kind MEDIA_ALBUM** — a validated kind with at least two independent asset references in one message (`services/telegraph/messageKinds.ts:186#validateKindMessage`), sent, stored and rendered (`travel-buddy-standalone/src/features/telegraph/kinds/TypedMessageRenderer.tsx:65#TypedMessageRenderer`). One asset is refused — that is an IMAGE or a VIDEO, not an album (`test/telegraphKinds.test.ts:302`). |
...
| T67 | N `∅` | **C** | **The drawer is a structured index, not a second storage copy** — no longer an unguarded absence. The drawer exists and stores nothing: it classifies `messages` rows in memory (`services/telegraph/messageKinds.ts:258#drawerTabFor`), every id it returns is a `messages.id` that already existed, and the route writes nothing and declares `indexOnly: true` in its own response. |
| T68 | N | **C** | **Object-aware search respects current authorization and unsent/deleted state** — `GET /threads/:id/search` (`routes/telegraphKinds.ts:327`) is member-gated, §14.3-bounded and tombstone-excluding, and matches on a typed object's HUMAN fields rather than on raw JSON (`services/telegraph/messageKinds.ts:313#searchableTextOf`). A deleted message is not findable by its exact text (`test/telegraphKinds.test.ts:531`). |
...
| T53 | N | **N** | **Message kind VOICE** — deliberately NOT moved. It is now REFUSED by name with the constraint that blocks it (`services/telegraph/messageKinds.ts:146#UNSENDABLE_KINDS`), which is honest, but a refusal is not a kind. It needs a migration widening `messages.media_type` and an audio MIME in `lib/mediaPipeline.ts:75`. |
...
| T98 | W | **C** | **Action `CHECK_IN_SAFE`** — no longer circle-only: the §6.2 SAFETY kind carries a check-in from any thread, with four classes and a coarse label (`services/telegraph/messageKinds.ts:186#validateKindMessage`). The circle check-in path is untouched. |
...
(`routes/messaging.ts:2147#const lastReadAt: string | null = mem.last_read_at ?? null;`). `GET /threads/:threadId/read-receipts`
...
only to messages the caller did NOT send (`routes/messaging.ts:2019`). A sender
...
`:240` is the gate the send path now calls (`routes/messaging.ts:2083`), placed
...
| T279 | W | **C** | §22 adaptive rate limits — both halves. The send step now has a limit (`routes/messaging.ts:2083`), and it is adaptive to all five named inputs (`domain/telegraph/policies/sendRateLimit.ts:112`): relationship, verification, trust, account age and open reports, with an unreadable input falling to the strictest tier. The request step's existing adaptive machinery is untouched. |
...
| T282 | W | **W** | §22 travel scam signals — all six families are detected (`domain/telegraph/policies/travelScamSignals.ts:48`, `:167`) and attached to the recipient's read (`routes/messaging.ts:2019`), against a ten-line false-positive corpus. Holds W for one reason: **no client surface renders `safetySignals` yet**, so a traveller does not see the warning. The server half is complete and the traveller-facing half is not. |
...
The read path attaches it at `routes/messaging.ts:2055`, and the thread renders
...
| T282 | W | **C** | §22 travel scam signals, end to end. All six families detected server-side (`domain/telegraph/policies/travelScamSignals.ts:48`, `:167`), attached to the recipient's read (`routes/messaging.ts:2055` area), and now rendered (`MessageSafetyBanner.tsx:87`) with advice and a report action. §11.4 held this at W precisely for the missing client half; that half is here. |
...
(`lib/telegraphEvents.ts:717#export function emitSafetyReported`, called at `routes/messaging.ts:3175` and `:3419`).
...
| T194 | N | **C** | §13.2 `safety.reported`. In the union (`:73`) and emitted to the reporter only through a dedicated emitter whose signature cannot carry a second audience (`lib/telegraphEvents.ts:717#export function emitSafetyReported`; called at `routes/messaging.ts:3175`, `:3419`). |
...
(`routes/messaging.ts:1831`) instead of sending a number. The client renders the
...
| id | was | now | why |
| --- | --- | --- | --- |
| T260 | W | **C** | §19's inbox line. The unread half was already real; the other half now exists end to end and is measured end to end: `domain/telegraph/policies/needsAction.ts:80` computes it from pending RSVPs and unvoted time options, `routes/messaging.ts:1771` calls it inside `GET /me/threads` and `:1831` puts `needsActionCount` + `needsActionReasons` on every row, and `components/TelegraphInboxScreen.tsx:226#needsAction` renders "1 needs action" / "N need action". `telegraphNeedsAction.test.ts` drives the REAL Express route, not just the policy, so "wired" is a measured fact. C rather than W because it needs no migration and no flag — every table it reads is from migration 0013 and has had live writers for the whole life of the meetups feature. |
...
one (`routes/messaging.ts:3227`, `:3454`). A person who has just reported
...
`routes/messaging.ts:2043` takes it straight from the request body; the only
...
- **The roster-refusal proof corrected the test.** Removing the block guard's
  roster-error refusal from `routes/messaging.ts:2079` left the matrix suite
...
| T313 | W | W | **Still W, and the reason changed completely.** The bound now EXISTS: migration `migrations/2400_telegraph_history_bound.sql` adds `message_thread_members.visible_from_at` and the flag `telegraph_history_bound_enabled`, and `services/groupChatHistoryBound.ts` is applied at `routes/messaging.ts:2017#visibleFromOf` and `:1849`. RLS-03 proves both halves at `test/telegraphRlsAuthorizationMatrix.test.ts:222`: with the flag on the pre-membership message is withheld and the bound is applied IN the query so pagination cannot walk past it; with the flag as seeded — FALSE — the whole back history is returned. **Ceiling: no database has 2400 and the flag is seeded off.** BUILT ON BRANCH IS NOT DEPLOYED; this row cannot move from inside the tree. |
| T314 | W | **C** | The fail-open is closed in the tree. `routes/messaging.ts:2079-2083` now REFUSES the send when the roster read fails ("cannot determine whether this is a blocked 1:1 thread") instead of inferring an empty roster and skipping the guard. RLS-04 drives five configurations at `test/telegraphRlsAuthorizationMatrix.test.ts:261` — recipient-blocked, sender-blocked, mutual (the two-row state that used to make the guard raise), blocks-table unreadable, roster unreadable — and all five deny. Shown red by deleting that refusal. |
...
| T444 | W | W | Same substrate as T390, from §30A.20's side, and 2400 cites this clause by number. The window is enforced in the QUERY (`routes/messaging.ts:1862`) rather than filtered after the fact, so pagination cannot walk past it — which is the difference between a bound and a display rule. **Ceiling: the flag is seeded FALSE.** |
...
`GET /api/me/saved-messages` exists at `artifacts/api-server/src/routes/messaging.ts:3920#router.get('/me/saved-messages', async (req, res) => {` and reads
...
| The eighth coordination kind | `artifacts/api-server/src/services/telegraph/coordination.ts:802#ACKNOWLEDGEMENT` |
| The projection | `artifacts/api-server/src/services/telegraph/coordination.ts:504#export function projectAcknowledgements` |
| The write, and its four refusals | `artifacts/api-server/src/routes/telegraphCoordination.ts:301#did not ask to be acknowledged` |
| The announcement reader (one answer for every reason a caller must not distinguish) | `artifacts/api-server/src/routes/telegraphCoordination.ts:125#function readAnnouncementRow` |
| The read | `artifacts/api-server/src/routes/telegraphCoordination.ts:615#/threads/:threadId/announcements` |
...
| piece | where |
| --- | --- |
| The four lines | `artifacts/api-server/src/routes/messaging.ts:2535#mediaUrl: isDeleted ? null : ((m as any).media_url ?? null),` |
...
| piece | where |
| --- | --- |
| The three primary reads, now a refusal | `artifacts/api-server/src/routes/messaging.ts:1967#for (const [label, r] of [` |
| The member-profile batch, now the refusal its own comment had claimed for months | `artifacts/api-server/src/routes/messaging.ts:1997#if (profileErr) {` |
| The message-request list, which returned `sender: null` for everybody | `artifacts/api-server/src/routes/messaging.ts:837#if (profilesErr) {` |
| Trip context: omitted, because `undefined` ("not known") and `null` ("this trip has no city") are different claims | `artifacts/api-server/src/routes/messaging.ts:2208#tripCity: tripCityDegraded ? undefined : tripCity,` |
| The preview says `failed` only when it failed | `artifacts/api-server/src/routes/messaging.ts:2045#let previewTranslationsDegraded = false;` |
| The thread read reports §18's own word for "we did not translate this", instead of inventing a monolingual thread | `artifacts/api-server/src/routes/messaging.ts:2332#status: 'failed' as TranslationStatusValue,` |
...
| --- | --- | --- |
| Highlight | `artifacts/api-server/src/services/telegraph/shareables.ts:583#const loadHighlight` | Public or owner only. The other three `highlights.visibility` values need a relationship read another surface owns, and approximating it is the backdoor §5.3 forbids — the same reasoning `loadMemory` already states. Plus the rule no other family needs: **it expires.** |
| Stamp | `artifacts/api-server/src/services/telegraph/shareables.ts:626#const loadStamp` | Revoked → gone. Hidden from its owner's own passport → not handed to a third party. Two reads, both bound and checked: an unreadable definition is `unknown`, not an untitled stamp. |
| neighborhood | `artifacts/api-server/src/services/telegraph/shareables.ts:685#const loadNeighborhood` | None, and none invented. `neighborhood_areas` is derived public reference data with no owner and no visibility column. §11 said this family "deliberately has no loader ... rather than pretending"; what it refused to pretend about was an authorization model, and this family genuinely has none to get wrong. |
| route | `artifacts/api-server/src/services/telegraph/shareables.ts:717#const loadRoute` | `route_plan_members`, not the trip. Being on the trip a route was planned for is not being on the route. A draft degrades for everyone but its owner. |
| reservation-safe derivative | `artifacts/api-server/src/services/telegraph/shareables.ts:766#const loadReservation` | Owner or accepted trip member — and the projection carries neither `confirmation_ref`, `raw_text` nor `extraction`. See §15.2. |
| layover plan | `artifacts/api-server/src/services/telegraph/shareables.ts:810#const loadLayoverPlan` | Owner, or an accepted member of the trip the session belongs to. A session with no `trip_id` is private to its traveller, full stop. |
| Media | `artifacts/api-server/src/services/telegraph/shareables.ts:865#const loadMedia` | `visibility: 'inherit'` — the column's DEFAULT — degrades to `private`. See §15.2. |
...
2. **T344 and T363 — bigger than §14.6 said, and the correction matters.**
   ...
   - **An unreadable `profiles` becomes a confident 404.** "Circle owner not
     ...
     (`artifacts/api-server/src/routes/messaging.ts:3707#const { data: ownerProfile, error: ownerProfileErr } = await sc`
   ...
   - **A mention notification names "@someone"** when the tagger's profile read
     ...
     (`artifacts/api-server/src/routes/messaging.ts:3014#const { data: taggerProfile, error: taggerProfileErr } = await sc`
...
(`artifacts/api-server/src/routes/messaging.ts:2330#target_language: 'und',`).
...
(`artifacts/api-server/src/routes/messaging.ts:2515#...(replyLinkageUnreadable`).
...
1. **T344 and T363 stay W, and the reason has changed shape.** §16.4 said a row
   ...
   | --- | --- |
   | `artifacts/api-server/src/routes/messaging.ts:634#const { data: profile, error: profileErr } = await client.from('profiles').select('id').eq('id', recipientId).maybeSingle();` | "User not found" |
   | `artifacts/api-server/src/routes/messaging.ts:902#.select('id, sender_id, recipient_id, status, preview_text')` | "Message request not found" (accept) |
   | `artifacts/api-server/src/routes/messaging.ts:1154#.select('id, sender_id, recipient_id, status')` | the same, on decline |
   | `artifacts/api-server/src/routes/messaging.ts:1254#.select('id, sender_id, status')` | the same, on cancel |
   | `artifacts/api-server/src/routes/messaging.ts:3765#if (memberErr) { refuseUnreadableAccess(req, res, 'message_thread_members', { err: memberErr, threadId, userId: user.id }); return; }` | "Thread not found" |
   | `artifacts/api-server/src/routes/messaging.ts:1823#.select('id, thread_type, is_e2ee')` | the same, one read later |
   | `artifacts/api-server/src/routes/messaging.ts:3383#.select('id, thread_id, sender_id, body, deleted_at, original_language')` | "Message not found" (translate retry) |
   | `artifacts/api-server/src/routes/messaging.ts:3515#.select('id, thread_id, sender_id, body, deleted_at')` | the same, on edit |
   | `artifacts/api-server/src/routes/messaging.ts:3625#.select('id, title, destination_city')` | "Trip not found" |
   | `artifacts/api-server/src/routes/messaging.ts:4034#.select('id')` | "Message not found in this thread" (save) |
...
1. **`artifacts/api-server/src/routes/messaging.ts:412#const { data: before, error: beforeErr } = await client` — the prior-preference read.** §17.8 classified it "neither" and
   ...
   **CLOSED 2026-09-15, and the reasoning above is correct about the gate and wrong about
   ...
   now refuses on `beforeErr` (`artifacts/api-server/src/routes/messaging.ts:437#if (newLang && !beforeErr && shouldRetranslateOnLanguageChange({`),
   ...
   (`artifacts/api-server/src/routes/messaging.ts:416#if (beforeErr) req.log.warn`).
2. **`artifacts/api-server/src/routes/messaging.ts:944#const { data: current, error: currentErr } = await sc` — the status re-read inside a lost CAS race.** The claim's
...
3. **`artifacts/api-server/src/routes/messaging.ts:1113#const { data: previewMsg, error: previewErr } = await sc` — the preview-message insert.** A write chain, not a read;
   ...
   **CLOSED 2026-09-15 anyway, because being outside a checker's scope is not the same as
   ...
   logged (`artifacts/api-server/src/routes/messaging.ts:1117#if (previewErr) req.log.error`).
4. **`GET /me/unread-counts` reports `newHighlights` and `meetups` as `0` when their inputs are
   ...
   **HALF-CLOSED 2026-09-15, on the half that was never the owner decision.** The `0` on
   ...
   had (`artifacts/api-server/src/routes/messaging.ts:1565#if (hCountErr) req.log.warn`).
...
`GET /api/threads/:id/layers` serves it (`routes/telegraphCoordination.ts:724#"/threads/:threadId/layers"`).
...
DISRUPTED included (`services/telegraph/coordination.ts:661#export const CoordinationTransitionPayload`).
The projection folds them (`services/telegraph/coordination.ts:722#export function projectCoordinationSession`)
and the route REFUSES an arrow §9 does not have, naming the legal set
(`routes/telegraphCoordination.ts:337#§9 allows`). §9.1's separation survives: `declaredState` and
...
2. **The trip-context select list.** `GET /threads/:id/trip-context` states
   ...
   (`routes/telegraphSharedContext.ts:347#const TRIP_CONTEXT_COLUMNS`). The narrow mutation then
...
`routes/telegraphCoordination.ts:823#"/threads/:threadId/catch-up"`, membership-gated and
...
- **`artifacts/api-server/src/routes/messaging.ts:412#const { data: before, error: beforeErr } = await client` — the prior-language read.** §19.6's analysis of the gate is
...
- **`artifacts/api-server/src/routes/messaging.ts:1113#const { data: previewMsg, error: previewErr } = await sc` — the preview-message insert.** §19.6 placed it outside this class
...
- **`artifacts/api-server/src/routes/messaging.ts:1564#const { count: hCount, error: hCountErr } = await q;` — the highlights count in the unread badge.** The last read in
