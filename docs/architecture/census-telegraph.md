...
| T275 | Private semantic indexes filter access **before** retrieval, not after | N `∅` | Unguarded absence: no semantic index over conversations exists. (Compass's own tools do gate before retrieval — `compass/CompassTools.ts:839` `sharesSocialContext`, `:841` trust floor — but they index no message content.) |
...
(`compass/CompassTools.ts:241`) and reached through the existing dispatcher by
one branch (`compass/CompassTools.ts:1241`), so this pass adds one import, one
