...
`services/media/MediaProjectionService.ts:1249#.from(table) { data } = await (sc as any).from(table).select("*").eq(ownerCol, ownerId).limit(1000);`) were opened and confirmed to be
