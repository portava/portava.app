...
| M42 | Gold marker = Saved / Passport / Memory | **W** | The **Saved** arm is correct: `lib/mapProducers/savedPlaceProducer.ts:388,396` reads the union of `wishlist_places` + `discovery_place_saves` after PR #446 moved it off writerless `saved_places` (`:18-23`). The **Memory** arm was the defect: `lib/mapProducers/memoryProducer.ts:35` filters `memory_projections` to `subject_type = 'place'`, and the projector's PLACE lane read the writerless `saved_places`, so it had never had an eligible subject. **`2963_memory_projector_place_lane_union.sql` repoints that lane** at the union of the two tables saves actually land in, bridged into the `discovery_places` id-space by `id`, `canonical_location_id` and `osm_id`. It is applied on `portava-ci` (PR #511's merge) and on production (2026-09-21). **THIS ROW WAS BRIEFLY MOVED TO C ON 2026-09-21 AND IS MOVED BACK, because the evidence for the move did not test what it claimed to test.** The behavioural proof ran against a local disposable PostgreSQL and showed 2 `subject_type = 'place'` projections for a user with only union-sourced saves. The SQL logic is right and that result stands as far as it goes. But 2963:171 is `DELETE FROM _canon_saves;` — **unqualified** — and while plain PostgreSQL permits it, `portava-ci` and production both run `session_preload_libraries = supautils`, whose `safeupdate` guard raises *"DELETE requires a WHERE clause"* in PostgREST-role sessions. The statement sits mid-body, AFTER the episodic, semantic and social inserts, so every call through the API raised and the whole transaction rolled back: the projector produced **nothing at all**, which is worse than the M42 defect it was written to remove. `main` has been red on `live DB · RLS + role/is_official write boundaries` since #511 merged — 9 of 11 `memoryProjectionLifecycleLive` cases plus `memoryLifecycleLive`, all with that one message — and `lib/memoryProjectionScheduler.ts:81` only `logger.warn`s the rejection, so unattended it fails silently. **`2965_memory_projector_canon_saves_delete_guard.sql`** qualifies the delete by surgery on the installed definition; it is applied to production and verified there (1 overload, signature unchanged, no unqualified form left, PLACE lane intact) and travels to `main` on its own branch. **Turns red when:** 2965 is on `main`, the live memory suites are green on a `main` run, and a `memory_projections` row with `subject_type = 'place'` exists **through the PostgREST path** — not merely in a database with the guard unarmed, which is the distinction that made the first attempt at this row wrong. |
