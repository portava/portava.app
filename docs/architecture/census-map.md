...
| M11 | Layers / Legend | C | `src/components/map/LayersSheet.tsx:2`; opened at `app/map/index.tsx:3246#visible={overlayOpen('LAYERS')}`. |
...
| M201 | Saved items | C | **Moved W→C 2026-09-13.** The row's finding was right: the client carried the whole branch (`mapSearchModel.ts:50`, `:144-148`, `:64`) and the server had no type that could reach it. It has one now — `` `artifacts/api-server/src/routes/discoverySearch.ts:136#saved` `` is wire vocabulary, produced by `` `artifacts/api-server/src/routes/discoverySearch.ts:1314#async function searchSaved(` `` over the two tables saves actually land in (`wishlist_places` + `discovery_place_saves`, as `savedPlaceProducer` reads them after #446), dispatched at `` `artifacts/api-server/src/routes/discoverySearch.ts:2170#case "saved":` ``. The adapter's `saved` key moved out of the tolerated-alias block into the wire table (`` `travel-buddy-standalone/src/features/map/search/searchAdapter.ts:84#saved:` ``) and `savedKind` is now read from the wire rather than hard-coded (`` `travel-buddy-standalone/src/features/map/search/searchAdapter.ts:261#export function savedKindFromMetadata` ``). The map asks for it: `` `travel-buddy-standalone/src/components/map/MapSearchSheet.tsx:164#searchUnified(q,` ``. Executed: `` `artifacts/api-server/src/test/mapSearchSavedItems.test.ts:168#it("dispatchSearch has a` `` (15 cases; deleting the dispatch case reddens 9, dropping either save table reddens 6). **Not in the `all` fan-out** — see the owner decision in §40. |
...
| M269 | `meet_here_created` | **W** | `src/features/map/telemetry/mapTelemetry.ts:653#'meet_here_created',`; 1 emitter: `app/map/index.tsx:3158#emitMapEvent('meet_here_created',`. |
| M270 | `crew_locate_started` | **W** | `src/features/map/telemetry/mapTelemetry.ts:654#'crew_locate_started',`; 1 emitter: `app/map/index.tsx:2177#emitMapEvent('crew_locate_started',`. |
| M271 | `contribution_submitted` | **W** | `src/features/map/telemetry/mapTelemetry.ts:655#'contribution_submitted',`; 1 emitter: `app/map/index.tsx:2877#emitMapEvent('contribution_submitted',`. `src/features/map/telemetry/mapTelemetry.ts:585#sinceRouteStart` carries the banded time since `route_started`. |
| M272 | `alternative_requested` | **W** | `src/features/map/telemetry/mapTelemetry.ts:656#'alternative_requested',`; 1 emitter: `app/map/index.tsx:3179#emitMapEvent('alternative_requested',`. |
...
(`` `artifacts/api-server/src/routes/discoverySearch.ts:2188#// 17 of the 18 non-"all" types run in parallel at FAN_LIMIT items each.` ``).
...
1. **`app/map/index.tsx:2836#emitMapEvent('why_shown_opened',` → use `whyShownOpenedPayload`.** Replace the inline
