...
| A16 | Passport §21 `:213-214` — Discovery variant: *"Identity, verification, availability, Open to Plans, shared context, permitted trust summary"* | **C** | `routes/discoverySearch.ts:2920-2952#router.get("/discovery/people/:userId/passport",` `GET /discovery/people/:userId/passport` → `allowDiscoveryPersonCard` (`:2941#allowDiscoveryPersonCard(sc,`) → `buildConsumerProjection(sc, "discovery_card", …)` (`:2945#buildConsumerProjection(sc,`). **Supersedes census-passport P95 (W — "Nothing calls it")**: it is called, from Discovery, by the route that opens a search row. |
...
| B05 | G277 — CountryResolver | **W** | `routes/discoverySearch.ts:2109-2196#async function searchCountries(` `searchCountries` aggregates `profiles.home_country` (`artifacts/api-server/src/routes/discoverySearch.ts:2121#.ilike("home_country", pat)`): a country with no users in it does not exist as a suggestion. Privacy-filtered (`artifacts/api-server/src/routes/discoverySearch.ts:2129#.eq("allow_profile_discovery", false)` opt-outs), so not a leak; a construction defect. Fix needs a canonical country registry Discovery does not own. |
...
| census-passport **P95 W — "Nothing calls it"** | **Superseded**: `routes/discoverySearch.ts:2945#buildConsumerProjection(sc,` calls the `discovery_card` variant. A16. |
...
service client (`getServiceClient()` at `routes/discovery.ts:1592`, `routes/discoverySearch.ts:2551#getServiceClient();`),
...
2. **`11` §9 and `/discovery/suggest`'s shipping fail-soft contract are in direct conflict.**
   ...
   `artifacts/api-server/src/routes/discoverySearch.ts:2238-2248#offset: number,`. **Affected rows:** C14, DV-74's
...
`artifacts/api-server/src/routes/discoverySearch.ts:2771#discoveryRefusal("validation", "query_too_short", "GET /discovery/suggest")`,
`artifacts/api-server/src/routes/discoverySearch.ts:2817#discoveryRefusal("transient_db", "visibility_state_unreadable", "GET /discovery/suggest")` and
`artifacts/api-server/src/routes/discoverySearch.ts:2901#discoveryRefusal("transient_db", "suggest_failed", "GET /discovery/suggest")`.
...
The 17 buckets are now named (`routes/discoverySearch.ts:2329#const FAN_SOURCES = [`),
a rejected one is collected rather than collapsed
(`routes/discoverySearch.ts:2384#const unreadableSources: string[] = [];`), and the
...
(`routes/discoverySearch.ts:2610#unreadableSources.length === FAN_SOURCES.length ? "nothing" : "partial",`).
...
(`routes/discoverySearch.ts:2832#const unreadableAt = new Array<string | null>(plan.length).fill(null);`)
so the names come out in plan order however the parallel reads finish, and
answers `suggest_sources_unreadable`
(`routes/discoverySearch.ts:2877#suggest_sources_unreadable`),
...
(`routes/discoverySearch.ts:1963#throw new DiscoverySearchReadError("profile_privacy_settings", optOutResult.error);`
...
code (`artifacts/api-server/src/routes/discoverySearch.ts:2169#if (code !== null && takenCodes.has(code)) continue;`).
...
their own ordering (`artifacts/api-server/src/routes/discoverySearch.ts:2295#case "countries":   return searchCountries(sc, q, blockedSet, ageRestrictedSet, offset, fetchLimit);`),
...
(`artifacts/api-server/src/routes/discoverySearch.ts:2181#merged.push(...rankByMatchTier(tail, q));`)
...
- **Discovery consumes it at one site.**
  `artifacts/api-server/src/routes/discoverySearch.ts:2148#const registry = searchCountryRegistry(q, offset + fetchLimit);`
...
- **The profile leg survives and a registry-unresolvable name still lists**, at
  `artifacts/api-server/src/routes/discoverySearch.ts:2120#.select("id, home_country")`
  and `artifacts/api-server/src/routes/discoverySearch.ts:2121#.ilike("home_country", pat)`,
...
- **The refusal is NOT relaxed**, which is the half a reviewer should distrust
  ...
  named read error at
  `artifacts/api-server/src/routes/discoverySearch.ts:2129#.eq("allow_profile_discovery", false),`
...
|---|---|---|
| `` `artifacts/api-server/src/routes/discoverySearch.ts:2121#.ilike("home_country", pat)` `` | the `searchCities` signature — the anchor text is not on that line | `artifacts/api-server/src/routes/discoverySearch.ts:2121#.ilike("home_country", pat)` |
| `` `artifacts/api-server/src/routes/discoverySearch.ts:2129#.eq("allow_profile_discovery", false)` `` | a bare `sc` continuation line inside `searchCities` | `artifacts/api-server/src/routes/discoverySearch.ts:2129#.eq("allow_profile_discovery", false),` |
...
`artifacts/api-server/src/routes/discoverySearch.ts:2109-2196#async function searchCountries(`,
