...
(`artifacts/api-server/src/routes/compass.ts:281#export function enrichFeedWithRecommendationIds`)
...
| `travel-buddy-standalone/app/map/index.tsx:1139#if (res.ok && Array.isArray(res.data?.places)) {` | **NO** | **Defect.** A refusal takes the `ok` branch with `places: []`, clears the pins, and the screen's own `placesEmpty` then renders "no places here" for an outage. |
...
- `travel-buddy-standalone/app/map/index.tsx:1139#if (res.ok && Array.isArray(res.data?.places)) {`
