...
| A16 | Passport §21 `:213-214` — Discovery variant: *"Identity, verification, availability, Open to Plans, shared context, permitted trust summary"* | **C** | `routes/discoverySearch.ts:3069-3101#router.get("/discovery/people/:userId/passport",` `GET /discovery/people/:userId/passport` → `allowDiscoveryPersonCard` (`:3090#allowDiscoveryPersonCard(sc,`) → `buildConsumerProjection(sc, "discovery_card", …)` (`:3094#buildConsumerProjection(sc,`). **Supersedes census-passport P95 (W — "Nothing calls it")**: it is called, from Discovery, by the route that opens a search row. |
...
| B05 | G277 — CountryResolver | **W** | `routes/discoverySearch.ts:2145-2232#async function searchCountries(` `searchCountries` aggregates `profiles.home_country` (`artifacts/api-server/src/routes/discoverySearch.ts:2157#.ilike("home_country", pat)`): a country with no users in it does not exist as a suggestion. Privacy-filtered (`artifacts/api-server/src/routes/discoverySearch.ts:1988#.eq("allow_profile_discovery", false)` opt-outs), so not a leak; a construction defect. Fix needs a canonical country registry Discovery does not own. |
...
| C03 | Blocked users excluded both directions; unknown block state → empty search (header `:17-18`) | **C** | `lib/blocks.ts:21#fetchBlockedSet` (null on error), consumed at `routes/discovery.ts:1722#fetchBlockedSet`, `:2648#fetchBlockedSet` and `:3048#fetchBlockedSet`; every per-type searcher returns `[]` on null. Tests: *"returns empty results when the blocks table returns a DB error"*. |
...
| C22 | Shadow never changes what was served and writes only to `discovery_shadow_serves` (`lib/discoveryShadow.ts` header) | **C** | `routes/discovery.ts:1919#served` (`served: false`, handed a client that cannot write), invoked after the response is sent (`:1971#served`); `lib/discoveryShadow.ts:371#discovery_shadow_serves` the single insert; tests G, I. (Re-anchored 2026-09-08: both line numbers had drifted 30-odd lines when the silent-write burn-down `2550b8ba` edited this file. The claim held; the pointers did not.) |
...
| *"`discovery.ts:1504,2523` are the caller's own `date_of_birth`, weak evidence; real readers are `discoverySearch.ts:441` and `compass.ts:3376`"* | **CONFIRMED** (post-edit lines `routes/discovery.ts:1687#resolveGateAge(`; `routes/discoverySearch.ts:580#.select("id, handle, username, name, display_name, avatar_url`; `routes/compass.ts:4077#buildConsumerProjection(sc, "discovery_card"`). The line numbers inside the QUOTE are the sibling census's own and are stale; they are left as quoted. C33, A15. |
| census-passport **P95 W — "Nothing calls it"** | **Superseded**: `routes/discoverySearch.ts:3094#buildConsumerProjection(sc,` calls the `discovery_card` variant. A16. |
...
service client (`getServiceClient()` at `routes/discovery.ts:1592`, `routes/discoverySearch.ts:2667#getServiceClient();`),
...
`artifacts/api-server/src/services/telegraph/shareables.ts:1005#const LOADERS`. Search behaviour was a
...
|---|---|---|---|---|
| DSV2-01 | **DUPLICATE** | A02 | Same subject: search truth kept independent of recommendation ranking. | A02 **C** holds at the stricter bar — `routes/discoverySearch.ts` never imports the ranker; `lib/discoveryLiveRank.ts:464#if (a.grade.safety.demoted !== b.grade.saf` demotes only inside `GET /discovery`'s window. A quiet venue stays searchable because search reads no live state. |
| DSV2-02 | **DUPLICATE** | A01 | Same subject: live-intelligence axes into ranking. | A01 **W** holds; reason updated. All eight axes now exist (`lib/discoveryLiveRank.ts:142-150#weights: {`, `lib/discoveryLiveRank.ts:209#/** Live-qualified`). *Unknown must not become favourable* is **satisfied**: `FRESHNESS_AXIS` maps `unknown: 0`, `historical: 0` (`lib/discoveryLiveRank.ts:128#const FRESHNESS_AXIS: Readonly<Record<Fres`) and a null axis contributes nothing. `W` on the 2850 gate and on travel time (§11.7.3). |
...
| DSV2-09 | **DUPLICATE** | A07 | Same subject: safety vs opportunity. | A07 **N → W**, §11.6. Separation bar **met**: `lib/compassDecision.ts:298#unsafe: crowdLevel === "unsafe_density",` sets `unsafe` only on the distinct level `unsafe_density`, never on `busy`/`packed` (`lib/compassDecision.ts:313#quiet: { dead: 0.6, quiet: 1.0, moderate:` score `packed: 1.0` for high-energy with `unsafe` false). |
...
| DV-03 | §1.2 / `01` §7 Cache A — must never bypass personalization/ranking | **W** | Built, held OFF — **Still true in the shipping default.** User-independent key: `routes/discovery.ts:299#function cacheKey(dest: string, cat: strin` `cacheKey(dest, cat, radius)` — no viewer term. The bypass: `routes/discovery.ts:1852#let servedFiltered = filtered;` `let servedFiltered = filtered;`, with the per-request ranker at `routes/discovery.ts:1863#const outcome = await rankForViewer(merged` reached only inside `if (pdeCohort?.included …)` (`routes/discovery.ts:1857#if (pdeCohort?.included && callerUserId) {`), whose precondition is `engineMode.mode === "pde"` (`routes/discovery.ts:1854#const pdeCohort = (callerUserId && engineM`). `DISCOVERY_ENGINE_MODE` is seeded `enabled=false, mode='legacy'` (`migrations/2091_discovery_engine_mode_flags.sql:9#1. DISCOVERY_ENGINE_MODE  (capability, ena`) and production reads the same (§5). Serve points 1/2/3 hand the cached order to the user unranked on every deployment. The fix is built and wired — built-and-gated, not absent — behind ROADMAP Phase F gate 2, *"not ruled"* (`docs/discovery/ROADMAP.md:648#discovery_ranking_modifiers_enabled`). |
| DV-04 | §1.2 / `01` §7 Cache B + `06` §5 — preserve model_version, feature_version, candidate source, reasons, ranking timestamp | **W** | 1 of 5 criteria passes — **Still true.** Entry type `routes/discovery.ts:286#const _compassCandidateCache = new Map<str` `Map<string, { places: DiscoveryPlace[]; at: number }>` — ranked order and a write clock, nothing else. Written at `routes/discovery.ts:2213#_compassCandidateCache.set(cCacheKey, {`; replayed at `routes/discovery.ts:2133#const cFiltered = applyFilters(cCacheHit.p` with no re-rank; projection handed `scoredById: null` (`routes/discovery.ts:2141#cacheLevel: "compass_candidate_hit", cache`) so `whyForUser` is empty on every Cache-B serve. **model_version FAIL · feature_version FAIL · candidate source FAIL · reason codes FAIL · ranking timestamp PASS** (as of §11.8; it was `cachedAt: null` before this pass). |
...
| DV-07 | §6 *"fire-and-forget failures are surfaced"* / §10.3 *"remove silent failure catches"* — SPLIT off C25 | **W** | 1 of 3 writers — `lib/discoveryServeLog.ts:444-449#const { error } = await sc.from("rank_even` **PASSES** (branches the error, `logger.warn` with serve point, route and count; a throw warned at `lib/discoveryServeLog.ts:477#logger.warn({ err }, "discoveryServeLog: i`). `lib/rankLog.ts:143-148#if (insertError) {` PASSES for the portavaRank path but its Compass path **FAILS** — `lib/rankLog.ts:255#recommendations response.  Every failure is`, *"All errors are swallowed silently"*, same at `lib/rankLog.ts:81#feed response.  Every failure is REPORTED`. And Discovery's own serve path **FAILS** on a ranking branch: `routes/discovery.ts:1869#} catch (err) {` `} catch { /* fall through to normal rule-based path */ }` wraps the whole Compass block (`routes/discovery.ts:2106#if (category === "for_you" && callerUserId`), so a failure in `rankItemsForDiscovery`, in the Cache-B read, or in serve-point-5 logging degrades to the unranked path with no log line. |
| DV-08 | §8 five states OFF · SHADOW · COMPARE · PARTIAL · ON — SPLIT off C20/C24/A06 | **W** | 3 of 5 — `lib/discoveryEngineMode.ts:70#export type DiscoveryEngineMode = "legacy"` — `"legacy" \ | "shadow" \| "pde"`, validated `lib/discoveryEngineMode.ts:72#const VALID_MODES: readonly string[] = ["l`. **OFF PASS** (= legacy) · **SHADOW PASS** · **ON PASS** (= pde) · **PARTIAL FAIL** — not a state; reached by crossing a cohort gate with a mode (`lib/discoveryCohort.ts`, applied `routes/discovery.ts:1857#if (pdeCohort?.included && callerUserId) {`) · **COMPARE FAIL** — no state; the artefact exists (`lib/discoveryDivergenceReport.ts`) as a consequence of SHADOW, not a selectable state. §8's last clause — no side effect merely because shadow computes — **PASSES** (C22, C23, C24). |
| DV-09 | §9 surface-specific objectives: Pulse · Discovery · Trail · Trip Planning · Trending | **W** | 1 of 5 — **Discovery PASS** — `for_you` runs the Compass pipeline (`routes/discovery.ts:2175#const scored = await rankItemsForDiscovery` `rankItemsForDiscovery`) distinctly from the default. **Pulse FAIL** — shares `portavaRank`'s single `DEFAULT_WEIGHTS` with Discovery; per-surface weights exist in `services/ranking/rankingConfig.ts:77-129#exploration: number;` but Discovery does not vary by them. **Trail FAIL · Trending FAIL** — no such surface (DV-20, DV-28). **Trip Planning FAIL** — no trip-fit or route-fit term in `lib/portavaRank.ts`. |
...
| DV-11 | §10 — never expose block/unfollow reasons | **C** | `lib/blocks.ts` returns a `Set<string>` of ids — there is no reason field to expose. Consumed at `routes/discovery.ts:1722#viewerBlockedIds = blockSc ? await fetchBl`, `routes/discovery.ts:59#import { fetchBlockedSet, submitterIsVisib`, `routes/discoverySearch.ts:437#export { fetchBlockedSet };`. A block removes a row; it never annotates one. Matches `05` §7. Pinned by `discoveryBlockedSubmitter.test.ts` (25) and `discoverySearchBlockedSubmitter.test.ts` (12). |
...
| DV-18 | §11 nine internal reason codes + plain-language explanations — SPLIT off A03 | **N** | **Zero of nine exist**: `grep -rn "trail_affinity\|trip_match\|nearby_now\|trending_local\|creator_affinity\|saved_similar\|social_context\|season_match"` → no Discovery hit. What exists is adjacent and is not this: `lib/discoveryCandidate.ts:304#export function whyForUserFromFeatures(` returns the ranker's own **feature keys** — internal variable names, not a reason vocabulary — and `lib/discoveryLiveRank.ts:332#if (state.unsafe) out.push("crowd_unsafe_d` emits `crowd_unsafe_density`, a claim label. No plain-language string is produced anywhere. |
...
is a declared class prior labelled in source *"Not a measurement"* (`lib/discoveryCandidate.ts:162-162#/** Class prior in [0,1]. Not a measuremen`).
...
| DV-73 | `11` §10 every mutation is authorized — SPLIT off C28 | **C** | 4 of 4 — Four mutating routes, each calling `requireUser(req, res)` as its first statement and returning if absent: `routes/discovery.ts:2377#router.post("/discovery/already-known", as` (`POST /discovery/already-known`), `routes/discovery.ts:3201#router.post("/discovery/community", async` (`POST /discovery/community`), `routes/discovery.ts:3372#router.post("/discovery/community/:placeId` (`.../save`), `routes/discovery.ts:3482#router.post("/discovery/community/:placeId` (`.../report`). Reinforced in the catalogue, not only in code: `discovery_places` grants to `authenticated` and `anon` are **SELECT only** in both databases (C28). |
...
2. **`11` §9 and `/discovery/suggest`'s shipping fail-soft contract are in direct conflict.**
   ...
   `artifacts/api-server/src/routes/discoverySearch.ts:2299-2309#offset: number,`. **Affected rows:** C14, DV-74's
...
3. **DSV2's release gate: *"Provider-backed route time remains unverified until its configured
   ...
   travel axis decays over `TRAVEL_HORIZON_MINUTES = 45` (`lib/discoveryLiveRank.ts:116#export const TRAVEL_HORIZON_MINUTES = 45;`) from a
   `distanceKm` input (`lib/discoveryLiveRankRead.ts:79#_flagCache = { value, at: Date.now() };`) — **a distance estimate on a travel-time
...
**The defect.** `GET /discovery`'s Cache B (`_compassCandidateCache`, `routes/discovery.ts:286#const _compassCandidateCache = new Map<str`)
holds a **final ranked page** for ten minutes, keyed `userId:destination:radius:sortBy` (`routes/discovery.ts:295#function compassCandidateCacheKey(userId:`).
On a hit it replays that page through `applyFilters` and slices it (`routes/discovery.ts:2133#const cFiltered = applyFilters(cCacheHit.p`). `applyFilters`
(`routes/discovery.ts:1752#function applyFilters(raw: DiscoveryPlace[`) applies open-now, minimum-rating, adult-venue and sort — **and no block filter.**
Block eligibility enters Discovery in exactly one place, `viewerBlockedIds` (`routes/discovery.ts:1719#let viewerBlockedIds: Set<string>`), passed to
`loadCuratedAndCanonicalPlaces` **at read time** (`routes/discovery.ts:2069#loadCuratedAndCanonicalPlaces(destination,` cold path, `routes/discovery.ts:1830#const { places: dbPlaces, failedSources: dbFailedSources }` Cache-A path).

The Cache-A path is therefore **safe**, and that asymmetry is what makes the defect real rather than
theoretical: a Cache-A hit re-reads curated rows per request and re-applies blocks at `routes/discovery.ts:1830#const { places: dbPlaces, failedSources: dbFailedSources }`, so a
...
2. **A03's `whyForUser` was empty on a serve point where a ranker DID run, and the row does not say
   ...
   was false. Closed by §12.2's build (`artifacts/api-server/src/lib/discoveryCandidate.ts:422#const signals =`).
...
(`artifacts/api-server/src/routes/compass.ts:292#export function enrichFeedWithRecommendationIds`)
...
because Compass dedupes on it (`artifacts/api-server/src/routes/compass.ts:321#dedupeByRecommendationId`)
...
(`artifacts/api-server/src/routes/discovery.ts:2250#reasonCodesById: reasonCodesByIdFromProvenance(cProvenanceById),`
and, replaying the stored provenance, `artifacts/api-server/src/routes/discovery.ts:2161#reasonCodesById: reasonCodesByIdFromProvenance(cCacheHit.provenanceById),`).
...
`artifacts/api-server/src/routes/discoverySearch.ts:2920#discoveryRefusal("validation", "query_too_short", "GET /discovery/suggest")`,
`artifacts/api-server/src/routes/discoverySearch.ts:2966#discoveryRefusal("transient_db", "visibility_state_unreadable", "GET /discovery/suggest")` and
`artifacts/api-server/src/routes/discoverySearch.ts:3050#discoveryRefusal("transient_db", "suggest_failed", "GET /discovery/suggest")`.
...
|---|---|---|
| A05 | `W` (unchanged) | *"Nothing in routes/discovery\*.ts or lib/discovery\*.ts models an intent mode."* `artifacts/api-server/src/lib/discoveryLiveRank.ts:103#export const DISCOVERY_INTENT_MODES` is exactly the spec's eight, and `artifacts/api-server/src/routes/discovery.ts:1881#mode: parseIntentMode(req.query.intentMode` parses one off the request — twice, the second at `:2251`. Held at `W` by `discovery_live_rank_enabled` (2850), not by absence. |
| A07 | `W` (unchanged) | *"No safety term reaches the ranker."* `artifacts/api-server/src/lib/discoveryLiveRank.ts:413#const unsafe = state.unsafe;` forces influence and opportunity value to zero, and `artifacts/api-server/src/lib/discoveryLiveRank.ts:464#if (a.grade.safety.demoted !== b.grade.saf` sorts every demoted row behind every non-demoted one. Same flag. |
...
migration: `artifacts/api-server/src/migrations/2910_discovery_trails.sql:139#CREATE TABLE IF NOT EXISTS public.trails (`,
`artifacts/api-server/src/migrations/2910_discovery_trails.sql:175#CREATE TABLE IF NOT EXISTS public.content_trails (`
and `artifacts/api-server/src/migrations/2910_discovery_trails.sql:259#CREATE TABLE IF NOT EXISTS public.trail_edges (`.
...
`artifacts/api-server/src/routes/discovery.ts:2796#classifyRefusal(err, "GET /discovery/feed", "feed_assembly_failed"),`
is the feed's, and `artifacts/api-server/src/routes/discovery.ts:2554#classifyRefusal(err, "GET /discovery/counts", "category_counts_failed"),`
...
The 17 buckets are now named (`routes/discoverySearch.ts:2445#const FAN_SOURCES = [`),
a rejected one is collected rather than collapsed
(`routes/discoverySearch.ts:2500#const unreadableSources: string[] = [];`), and the
...
(`routes/discoverySearch.ts:2726#unreadableSources.length === FAN_SOURCES.length ? "nothing" : "partial",`).
...
(`routes/discoverySearch.ts:2981#const unreadableAt = new Array<string | null>(plan.length).fill(null);`)
so the names come out in plan order however the parallel reads finish, and
answers `suggest_sources_unreadable`
(`routes/discoverySearch.ts:3026#suggest_sources_unreadable`),
...
(`routes/discoverySearch.ts:1999#throw new DiscoverySearchReadError("profile_privacy_settings", optOutResult.error);`
...
| id | was | now | evidence |
|---|---|---|---|
| DV-07 | W | **C** | 3 of 3 writers. **(1)** `lib/discoveryServeLog.ts` already passed and is unchanged. **(2)** `lib/rankLog.ts`'s Compass path: §32 established that its FAIL rested on a stale doc-comment — the insert binds its error and `logger.warn`s it, pinned by `rankLogInsertErrors` 4/4, and the prose now says so. **(3)** `routes/discovery.ts:1869#} catch (err) {` was a bare `catch` with no binding and no log line, wrapping the entire Compass block; it now warns with the error, the destination, the category and the engine mode before degrading. |
...
`routes/discovery.ts:1116#address: (row.neighborhood ?? null) as string | null, neighborhood: (row.neighborhood ?? null) as string | null,`
in `queryDbPlaces` and
`routes/discovery.ts:1252#address: (row.neighborhood ?? row.address ?? null) as string | null, neighborhood: (row.neighborhood ?? null) as string | null,`
...
code (`artifacts/api-server/src/routes/discoverySearch.ts:2205#if (code !== null && takenCodes.has(code)) continue;`).
...
their own ordering (`artifacts/api-server/src/routes/discoverySearch.ts:2411#case "countries":   return searchCountries(sc, q, blockedSet, ageRestrictedSet, offset, fetchLimit);`),
...
(`artifacts/api-server/src/routes/discoverySearch.ts:2217#merged.push(...rankByMatchTier(tail, q));`)
...
- **Discovery consumes it at one site.**
  `artifacts/api-server/src/routes/discoverySearch.ts:2184#const registry = searchCountryRegistry(q, offset + fetchLimit);`
...
- **The profile leg survives and a registry-unresolvable name still lists**, at
  `artifacts/api-server/src/routes/discoverySearch.ts:2156#.select("id, home_country")`
  and `artifacts/api-server/src/routes/discoverySearch.ts:2157#.ilike("home_country", pat)`,
...
- **The refusal is NOT relaxed**, which is the half a reviewer should distrust
  ...
  named read error at
  `artifacts/api-server/src/routes/discoverySearch.ts:1988#.eq("allow_profile_discovery", false),`
...
|---|---|---|
| `` `artifacts/api-server/src/routes/discoverySearch.ts:2157#.ilike("home_country", pat)` `` | the `searchCities` signature — the anchor text is not on that line | `artifacts/api-server/src/routes/discoverySearch.ts:2157#.ilike("home_country", pat)` |
| `` `artifacts/api-server/src/routes/discoverySearch.ts:1988#.eq("allow_profile_discovery", false)` `` | a bare `sc` continuation line inside `searchCities` | `artifacts/api-server/src/routes/discoverySearch.ts:1988#.eq("allow_profile_discovery", false),` |
...
`artifacts/api-server/src/routes/discoverySearch.ts:2145-2232#async function searchCountries(`,
