...
| B05 | G277 — CountryResolver | **W** | `routes/discoverySearch.ts:2109-2130#async function searchCountries(` `searchCountries` aggregates `profiles.home_country` (`:1933#.ilike("home_country",`): a country with no users in it does not exist as a suggestion. Privacy-filtered (`:1941#.eq("allow_profile_discovery",` opt-outs), so not a leak; a construction defect. Fix needs a canonical country registry Discovery does not own. |
