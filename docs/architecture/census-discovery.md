...
| A16 | Passport §21 `:213-214` — Discovery variant: *"Identity, verification, availability, Open to Plans, shared context, permitted trust summary"* | **C** | `routes/discoverySearch.ts:2785-2817#router.get("/discovery/people/:userId/passport",` `GET /discovery/people/:userId/passport` → `allowDiscoveryPersonCard` (`:2806#allowDiscoveryPersonCard(sc,`) → `buildConsumerProjection(sc, "discovery_card", …)` (`:2810#buildConsumerProjection(sc,`). **Supersedes census-passport P95 (W — "Nothing calls it")**: it is called, from Discovery, by the route that opens a search row. |
...
| census-passport **P95 W — "Nothing calls it"** | **Superseded**: `routes/discoverySearch.ts:2810#buildConsumerProjection(sc,` calls the `discovery_card` variant. A16. |
...
service client (`getServiceClient()` at `routes/discovery.ts:1592`, `routes/discoverySearch.ts:2416#getServiceClient();`),
...
2. **`11` §9 and `/discovery/suggest`'s shipping fail-soft contract are in direct conflict.**
   ...
   `artifacts/api-server/src/routes/discoverySearch.ts:2112-2122#offset: number,`. **Affected rows:** C14, DV-74's
...
`artifacts/api-server/src/routes/discoverySearch.ts:2636#discoveryRefusal("validation", "query_too_short", "GET /discovery/suggest")`,
`artifacts/api-server/src/routes/discoverySearch.ts:2682#discoveryRefusal("transient_db", "visibility_state_unreadable", "GET /discovery/suggest")` and
`artifacts/api-server/src/routes/discoverySearch.ts:2766#discoveryRefusal("transient_db", "suggest_failed", "GET /discovery/suggest")`.
...
The 17 buckets are now named (`routes/discoverySearch.ts:2194#const FAN_SOURCES = [`),
a rejected one is collected rather than collapsed
(`routes/discoverySearch.ts:2249#const unreadableSources: string[] = [];`), and the
...
(`routes/discoverySearch.ts:2475#unreadableSources.length === FAN_SOURCES.length ? "nothing" : "partial",`).
...
recorded in the assertion message. `test/discoveryRefusalD11.test.ts:746#it("P4 — type=all over readable tables carries NO refusal (control)",`
...
(`routes/discoverySearch.ts:2697#const unreadableAt = new Array<string | null>(plan.length).fill(null);`)
so the names come out in plan order however the parallel reads finish, and
answers `suggest_sources_unreadable`
(`routes/discoverySearch.ts:2742#suggest_sources_unreadable`),
...
`test/discoveryRefusalD11.test.ts:709#const SWALLOWED`; 572/572 across the
