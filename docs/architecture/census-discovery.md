...
| A16 | Passport §21 `:213-214` — Discovery variant: *"Identity, verification, availability, Open to Plans, shared context, permitted trust summary"* | **C** | `routes/discoverySearch.ts:2663-2695#router.get("/discovery/people/:userId/passport",` `GET /discovery/people/:userId/passport` → `allowDiscoveryPersonCard` (`:2684#allowDiscoveryPersonCard(sc,`) → `buildConsumerProjection(sc, "discovery_card", …)` (`:2688#buildConsumerProjection(sc,`). **Supersedes census-passport P95 (W — "Nothing calls it")**: it is called, from Discovery, by the route that opens a search row. |
...
| census-passport **P95 W — "Nothing calls it"** | **Superseded**: `routes/discoverySearch.ts:2688#buildConsumerProjection(sc,` calls the `discovery_card` variant. A16. |
...
service client (`getServiceClient()` at `routes/discovery.ts:1592`, `routes/discoverySearch.ts:2319#getServiceClient();`),
...
`artifacts/api-server/src/routes/discoverySearch.ts:2539#discoveryRefusal("validation", "query_too_short", "GET /discovery/suggest")`,
`artifacts/api-server/src/routes/discoverySearch.ts:2585#discoveryRefusal("transient_db", "visibility_state_unreadable", "GET /discovery/suggest")` and
`artifacts/api-server/src/routes/discoverySearch.ts:2644#discoveryRefusal("transient_db", "suggest_failed", "GET /discovery/suggest")`.
...
The 17 buckets are now named (`routes/discoverySearch.ts:2097#const FAN_SOURCES = [`),
a rejected one is collected rather than collapsed
(`routes/discoverySearch.ts:2152#const unreadableSources: string[] = [];`), and the
...
(`routes/discoverySearch.ts:2378#unreadableSources.length === FAN_SOURCES.length ? "nothing" : "partial",`).
...
Failing first: `test/discoveryRefusalD11.test.ts:599#it("P3 — type=all refuses PARTIAL when a bucket's read fails, instead of a silently short list",`,
red before the change with the served row and the missing `refusal` key both
recorded in the assertion message. `test/discoveryRefusalD11.test.ts:624#it("P4 — type=all over readable tables carries NO refusal (control)",`
