...
| C03 | Blocked users excluded both directions; unknown block state → empty search (header `:17-18`) | **C** | `lib/blocks.ts:21#fetchBlockedSet` (null on error), consumed at `routes/discovery.ts:1650#fetchBlockedSet`, `:2493#fetchBlockedSet` and `:2853#fetchBlockedSet`; every per-type searcher returns `[]` on null. Tests: *"returns empty results when the blocks table returns a DB error"*. |
...
| C22 | Shadow never changes what was served and writes only to `discovery_shadow_serves` (`lib/discoveryShadow.ts` header) | **C** | `routes/discovery.ts:1827#served` (`served: false`, handed a client that cannot write), invoked after the response is sent (`:1862#served`); `lib/discoveryShadow.ts:371#discovery_shadow_serves` the single insert; tests G, I. (Re-anchored 2026-09-08: both line numbers had drifted 30-odd lines when the silent-write burn-down `2550b8ba` edited this file. The claim held; the pointers did not.) |
...
| DV-07 | §6 *"fire-and-forget failures are surfaced"* / §10.3 *"remove silent failure catches"* — SPLIT off C25 | **W** | 1 of 3 writers — `lib/discoveryServeLog.ts:272-277#const { error } = await sc.from("rank_even` **PASSES** (branches the error, `logger.warn` with serve point, route and count; a throw warned at `lib/discoveryServeLog.ts:305#logger.warn({ err }, "discoveryServeLog: i`). `lib/rankLog.ts:143-148#if (insertError) {` PASSES for the portavaRank path but its Compass path **FAILS** — `lib/rankLog.ts:255#recommendations response.  All errors are`, *"All errors are swallowed silently"*, same at `lib/rankLog.ts:81#feed response.  All errors are swallowed s`. And Discovery's own serve path **FAILS** on a ranking branch: `routes/discovery.ts:2060#} catch { /* fall through to normal rule-b` `} catch { /* fall through to normal rule-based path */ }` wraps the whole Compass block (`routes/discovery.ts:2019#if (category === "for_you" && callerUserId`), so a failure in `rankItemsForDiscovery`, in the Cache-B read, or in serve-point-5 logging degrades to the unranked path with no log line. |
...
| DV-11 | §10 — never expose block/unfollow reasons | **C** | `lib/blocks.ts` returns a `Set<string>` of ids — there is no reason field to expose. Consumed at `routes/discovery.ts:1650#viewerBlockedIds = blockSc ? await fetchBl`, `routes/discovery.ts:50#import { fetchBlockedSet, submitterIsVisib`, `routes/discoverySearch.ts:425#export { fetchBlockedSet };`. A block removes a row; it never annotates one. Matches `05` §7. Pinned by `discoveryBlockedSubmitter.test.ts` (25) and `discoverySearchBlockedSubmitter.test.ts` (12). |
...
| DV-36 | §3 **attributable to a surface** | **C** | `surface` is NOT NULL and CHECK-constrained on every row (production `information_schema.columns`), and every writer passes it explicitly: `lib/discoveryServeLog.ts:262#rankedInRequest: RANKED_IN_REQUEST.has(ser`, `lib/rankLog.ts:142#const { error: insertError } = await sc.fr`. |
| DV-37 | §3 **idempotent where retried** | **N** | No idempotency mechanism. `lib/discoveryServeLog.ts:272#const { error } = await sc.from("rank_even` is a bare `.insert(rows)` — no `onConflict`, no unique key, no client token; `lib/rankLog.ts:142#const { error: insertError } = await sc.fr` likewise. A retried batch writes duplicate impressions. DSV2-07 asks for *"retry deduplicates"*; nothing does. |
...
| id | Criterion | Verdict | Evidence |
|---|---|---|---|
| DV-49 | Improves candidate generation | **W** | A graph ships — `CompassGraphEngine`, rebuilt on a schedule, widened by migration `2290` to admit the `circle` and `experience` kinds `05` §2 names. It reaches Discovery as a **bounded** world-model city-confidence input to `portavaRank`, behind `discovery_ranking_modifiers_enabled` (2289, OFF, absent from production). It does not generate candidates — that is Overpass plus curated rows (`routes/discovery.ts:1981#queryOverpassDeduped(coords.lat, coords.ln`) — it re-weights them, and only when an unlit flag is lit. |
...
| DV-73 | `11` §10 every mutation is authorized — SPLIT off C28 | **C** | 4 of 4 — Four mutating routes, each calling `requireUser(req, res)` as its first statement and returning if absent: `routes/discovery.ts:2267#router.post("/discovery/already-known", as` (`POST /discovery/already-known`), `routes/discovery.ts:3001#router.post("/discovery/community", async` (`POST /discovery/community`), `routes/discovery.ts:3172#router.post("/discovery/community/:placeId` (`.../save`), `routes/discovery.ts:3267#router.post("/discovery/community/:placeId` (`.../report`). Reinforced in the catalogue, not only in code: `discovery_places` grants to `authenticated` and `anon` are **SELECT only** in both databases (C28). |
...
`lib/discoveryServeLog.ts:272-277#const { error } = await sc.from("rank_even` branches the error and warns with serve point, route and count;
`lib/discoveryServeLog.ts:305#logger.warn({ err }, "discoveryServeLog: i` warns on a throw. **C28 re-verified in the catalogue** rather than carried: `discovery_places`
...
holds a **final ranked page** for ten minutes, keyed `userId:destination:radius:sortBy` (`routes/discovery.ts:282#function compassCandidateCacheKey(userId:`).
On a hit it replays that page through `applyFilters` and slices it (`routes/discovery.ts:2046#const cFiltered = applyFilters(cCacheHit.p`). `applyFilters`
(`routes/discovery.ts:1680#function applyFilters(raw: DiscoveryPlace[`) applies open-now, minimum-rating, adult-venue and sort — **and no block filter.**
Block eligibility enters Discovery in exactly one place, `viewerBlockedIds` (`routes/discovery.ts:1647#let viewerBlockedIds: Set<string>`), passed to
`loadCuratedAndCanonicalPlaces` **at read time** (`routes/discovery.ts:1982#loadCuratedAndCanonicalPlaces(destination,` cold path, `routes/discovery.ts:1744#const dbPlaces = await loadCuratedAndCanon` Cache-A path).

The Cache-A path is therefore **safe**, and that asymmetry is what makes the defect real rather than
theoretical: a Cache-A hit re-reads curated rows per request and re-applies blocks at `routes/discovery.ts:1744#const dbPlaces = await loadCuratedAndCanon`, so a
