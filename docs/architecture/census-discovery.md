...
| C03 | Blocked users excluded both directions; unknown block state → empty search (header `:17-18`) | **C** | `lib/blocks.ts:21-40` (null on error), consumed at `:1793,1982`; every per-type searcher returns `[]` on null (`:523`). Tests: *"returns empty results when the blocks table returns a DB error"*. |
...
| C09 | Hidden names are not searchable — a hidden-name match survives only if handle/username matched (`:560-571` comment; `.agents/memory/display-name-privacy.md`) | **C** | `:564-571`. |
