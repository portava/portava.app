...
| *"`discovery.ts:1504,2523` are the caller's own `date_of_birth`, weak evidence; real readers are `discoverySearch.ts:438` and `compass.ts:3369`"* | **CONFIRMED** (post-edit lines `routes/discovery.ts:1545,2627#.select("date_of_birth")`; `routes/discoverySearch.ts:568#.select("id, handle, username, name, display_name, avatar_url`; `routes/compass.ts:3509#"id, username, display_name, name, avatar_url,`). The line numbers inside the QUOTE are the sibling census's own and are stale; they are left as quoted. C33, A15. |
