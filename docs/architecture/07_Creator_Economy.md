...
| **Trust** 0–100 + six public levels | `trust_profiles` (`services/trust/TrustScoreService.ts:276-291`) | safety and reliability of a *person* across nine categories | decays; caps clamp | label only; number only to self (`PassportProjectionService.ts:1059-1067`) |
...
| What it gates | **capabilities** — `public_level` maps through `LEVEL_RANK` to `canHostTrip`, `canCreateLargePlan`, `canUseCrewLocation`, `canContributeLiveIntel`, `canBecomeBuddy` (`services/passport/PassportProjectionService.ts:604-623`) | **ranking only**, and only when a flag is on |
| Visibility | label to everyone, number to self only (`PassportProjectionService.ts:1059-1067`) | never shown to anyone (§6) |
...
- **#467, #453, #454, #455 — display side.** They stop the constant 50 being presented as a
  ...
  no profile is readable (`services/passport/PassportProjectionService.ts:1049`) — and render the
