...
- `layover_sessions.status` is a four-value CHECK (`active|completed|cancelled|expired`), not
  ...
  (`routes/airport.ts:2033#travelers = ((profiles ?? []) as any[]).map((p) => ({`). No code path anywhere writes `'completed'`. The entire
...
| L5 | Every consequential recommendation is versioned, explainable and replayable | N | `layover_recommendations` has no version, snapshot, engine-version or input-hash column (`0127:106-138`), and rows are deleted wholesale and re-inserted on read (`LayoverRecommendationService.ts:793-798#layover_recommendations").delete()`, triggered from `routes/airport.ts:999-1000#isSafetyEnabled`). Nothing can be replayed. |
...
| L97 | Invalidate recommendations whose snapshot is stale or no longer feasible | W | There is wholesale invalidation — `generateRecommendations` DELETEs every row for the session and re-inserts (`LayoverRecommendationService.ts:793-798#layover_recommendations").delete()`), triggered whenever `layover_safety_engine_enabled` is on (`routes/airport.ts:999-1000#isSafetyEnabled`). But it is driven by a client GET, not by an event; it has no staleness test; and it destroys rather than invalidates, so a client holding the previous ids loses them silently. |
...
| L171 | `LayoverSessionService.getActive(userId)` | C | `getActiveSession` (`LayoverSessionService.ts:240-255`) behind `GET /api/airport/sessions/active` (`routes/airport.ts:2102#router.get("/airport/sessions/active", async (req, res) => {`), which expires stale sessions first (`routes/airport.ts:2117#const swept = await expireOldSessions(sc);`). |
...
| L174 | `LayoverSessionService.close(sessionId, **outcome**)` | W | `endSession` exists (`LayoverSessionService.ts:196-219`) but the outcome argument has nowhere to go — `layover_outcomes` is absent (L32) — and the only caller passes the literal `"cancelled"` (`routes/airport.ts:2033#travelers = ((profiles ?? []) as any[]).map((p) => ({`), so a successfully completed layover is recorded as a cancellation. |
...
| L296 | C5. Never certify a recommendation against a snapshot other than the one returned with it | N | There are no snapshots to mismatch. The nearest analogue of the violation is live: `GET /:id/recommendations` regenerates and replaces the whole set whenever `layover_safety_engine_enabled` is on (`routes/airport.ts:999-1000#isSafetyEnabled` → `LayoverRecommendationService.ts:793-798#layover_recommendations").delete()`), so the ids a client holds are destroyed under it and a subsequent `POST /stops/from-recommendation` (`routes/airport.ts:1164`) refers to a row that no longer exists. |
...
| `LayoverPrivacyGuard` gate WIRED into `/presence` and `/overview` | **Yes** | `routes/airport.ts:1854` and `:1508` (`evaluateSharingGate`), `:1858`/`:1869` and `:1513` (`disclosePresence`), `:1382` (`publishableUserIds`) |
...
| `terminal_info` published via `airportRowToProfile` | **Yes, and empty** | `services/airport/AirportProfileService.ts:146`, surfaced at `routes/airport.ts:1768#terminalInfo: a.terminalInfo ?? null,`. **0 of 3,206 production rows carry one**, so it is `null` everywhere |
...
| L143 | N | W | `pinTerminalContext` (`LayoverSafeReturnService.ts:115`) and `terminalInfo` now reach the API (`AirportProfileService.ts:146` → `routes/airport.ts:1768#terminalInfo: a.terminalInfo ?? null,`; also `LayoverSafeReturnService.ts:180` and `LayoverDegradedService.ts:166`). **0 of 3,206 production airports carry one**, so the pinned context is `null` for every session. |
...
| `safeReturn` posture | Published, **not read** | **Read and rendered** | `LayoverOverview.safeReturn` (`layover.ts:351`), `postureHeadline` (`layoverReturnFacts.ts:170#postureHeadline`), used `LayoverSafeReturnCard.tsx:81`; `returnRoutePrimary` hoists the card above the hero (`app/layover/[id].tsx:537#returnRoutePrimary`) |
...
| `POST /:id/return-now` | Wired, **no client function** | **Reachable by gesture** | `returnToAirportNow` (`layover.ts:957#returnToAirportNow`) → `LayoverSafeReturnCard.tsx:96`, mounted at `app/layover/[id].tsx:515#<LayoverSafeReturnCard` |
...
| L146 | W | **C** | §9 said "Reachability is the missing half" in those words. It is closed: `returnToAirportNow` (`layover.ts:957#returnToAirportNow`) calls `POST /:id/return-now`, `LayoverSafeReturnCard.tsx:96` calls it on a **RETURN TO AIRPORT** press, and the card is mounted (`app/layover/[id].tsx:515#<LayoverSafeReturnCard`). A double press is refused by a ref written synchronously (`:77, 92-93`) — state alone loses two presses in one frame. The abort's own `statusCapability` is reported to the traveller rather than swallowed (`:239-240`, `statusCapabilityNote`), so `flag_off` reads as "your layover stays open so you keep the countdown", not as a failure. |
| L114 | W | **C** | §9 ended "The endpoint it lives on is still dark from the app." It is not: `askCompass` (`layover.ts:909#askCompass`) has an importer (`LayoverCompassCard.tsx:24, 48`), the card is mounted (`app/layover/[id].tsx:346`), and the single highest-value clarifying question is the thing actually rendered (`LayoverCompassCard.tsx:83-86`). The computation (`valueOfInformation`, `LayoverCompassService.ts:453`) was already built and pinned; a traveller can now be asked. |
...
| L5, L97, L296 | `routes/airport.ts` lines 560-566 | `routes/airport.ts:999-1000#isSafetyEnabled` | not the regeneration trigger |
| L205, L258 | `routes/airport.ts` lines 751-755 | `routes/airport.ts:1623-1630#return_deadline_set` | the regeneration call, not the audited deadline write |
...
(`routes/airport.ts:890#async function replanAfterSessionEdit`) reads the
...
(`routes/airport.ts:874#const replan = await replanAfterSessionEdit`) and
...
card and the plan (`travel-buddy-standalone/app/layover/[id].tsx:626#<LayoverFlightChangeCard`),
...
8. **THIS PASS MOVED LINES IN THREE COUNTED FILES AND THEREFORE AGED THIS
   ...
   **THIRTEEN ANCHORED citations were caught by `check:doc-citations` and
   ...
   `routes/airport.ts:999-1000#isSafetyEnabled`; three `return_deadline_set`
   citations moved from lines 1105-1112 to
   `routes/airport.ts:1623-1630#return_deadline_set`; `manual_city` moved from
   line 1341 to `routes/airport.ts:1966#manual_city` and `departure_time` from
   line 1345 to `routes/airport.ts:1970#departure_time`; `returnRoutePrimary`
   moved from line 284 to
   `travel-buddy-standalone/app/layover/[id].tsx:537#returnRoutePrimary`; two
   `returnToAirportNow` citations moved from line 608 to
   `travel-buddy-standalone/src/services/layover.ts:957#returnToAirportNow`,
   and `askCompass` from line 560 to
   `travel-buddy-standalone/src/services/layover.ts:909#askCompass`. That is
...
(`travel-buddy-standalone/app/layover/[id].tsx:551#explorationCollapsed === true`,
rendered at
`travel-buddy-standalone/app/layover/[id].tsx:661#layover-exploration-collapsed`).
...
(`travel-buddy-standalone/app/layover/[id].tsx:385#notificationPromptWouldAppear`),
...
`routes/airport.ts:3352#if (!await isFlagEnabled(sc, "rent_buddy_enabled")) {`, read and verified here. That is far
...
| the route writing the traveller's own text, before the event that reports on it | `artifacts/api-server/src/routes/airport.ts:1716#const sent = await postPlainThreadMessage(sc, {` |
| the screen navigating on `posted`, and saying something true when it is false | `travel-buddy-standalone/app/layover/[id].tsx:408#} else if (res.posted && overview.session.tripId) {` |
...
(`artifacts/api-server/src/routes/airport.ts:1812#computePlanFit`).

**The write boundary stops laundering.** `landsideTravelRefusal`
(`artifacts/api-server/src/routes/airport.ts:2286#landsideTravelRefusal`) is
...
(`artifacts/api-server/src/routes/airport.ts:1815#row.duration_min`): a row with
...
`travel-buddy-standalone/app/layover/[id].tsx:259#useSafeReturnAbort`). There is
...
(`travel-buddy-standalone/app/layover/[id].tsx:771#layover-footer-return-now`),
...
(`artifacts/api-server/src/routes/airport.ts:1110#airportIntelligence: airportIntelligence(record)`),
...
(`artifacts/api-server/src/routes/airport.ts:722#NO PASSPORT SEAM HERE`)
and rebuilt on the close
(`artifacts/api-server/src/routes/airport.ts:3649#async function writeElectedLayoverStamp`)
...
(`travel-buddy-standalone/src/services/layover.ts:1032#export async function endLayoverSession`),
...
mounted at
`travel-buddy-standalone/app/layover/[id].tsx:835#<LayoverEndSheet`.
...
| --- | --- | --- |
| L10 | *"`status='completed'` is never written … The system cannot distinguish a safe return from an abandonment."* (the row also names a line in `routes/airport.ts` that was already stale by some 750 lines before this pass; it is quoted without it rather than repointed, for §12.4's reason) | It is written, by a traveller pressing "I made my flight" (`artifacts/api-server/src/routes/airport.ts:3770#const passportStamp = await writeElectedLayoverStamp` sits immediately after the `endSession` that writes it). The verdict does not move — see §17.6. |
...
`artifacts/api-server/src/routes/airport.ts:1794#safeEnvelopeFor` publishes
...
`cityPresence` (`artifacts/api-server/src/routes/airport.ts:1952#cityPresence`)
...
(`artifacts/api-server/src/routes/airport.ts:1794#function safeEnvelopeFor`)
...
(`artifacts/api-server/src/routes/airport.ts:3410#const degradedReasons: string[] = [];`),
published as
`artifacts/api-server/src/routes/airport.ts:3520#degraded: degradedReasons.length > 0,`,
and an availability that is `null` rather than `false` when nobody asked
(`artifacts/api-server/src/routes/airport.ts:3502#availableDuringLayover: availabilityMeasured ? availableSet.has(b.id) : null,`).
...
2. **L273's body row**, the last parseable statement of that verdict:
   ...
   **Two of the three clauses are false at HEAD.** The master flag is consulted
   at `artifacts/api-server/src/routes/airport.ts:3352#if (!await isFlagEnabled(sc, "rent_buddy_enabled")) {`,
   and the safety gate runs BEFORE the profiles are read at
   `artifacts/api-server/src/routes/airport.ts:3371#const { safetyGate, trustRequirement } = layoverBuddyDecision(airport, session);`.
   A compatibility filter also runs
   (`artifacts/api-server/src/routes/airport.ts:3483#rows = filterLayoverCompatible(rows);`).
...
3. **L254's body row**, likewise the last parseable statement:
   ...
   **False at HEAD.** `artifacts/api-server/src/routes/airport.ts:3484#rows = applyBuddyTrustRequirement(rows, trustRequirement);`
...
| `artifacts/api-server/src/routes/airport.ts:3206#const city = crewCityFor(airport, session);` | The join route resolves the joiner's city the SAME way `GET /crew` and `POST /crew` resolve it, and refuses outright when it is unknown — symmetric with both. |
...
`crewAction` in `travel-buddy-standalone/src/services/layover.ts:1602#message: typeof parsed.message === 'string' ? parsed.message : 'That did not work. Please try again.',`
...
`travel-buddy-standalone/app/layover/[id].tsx:739#<LayoverCrewSection`. **No file
