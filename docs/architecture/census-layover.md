...
1. **A one-minute-earlier departure can move the hard return deadline nine minutes LATER.**
   `timeOfDayExtra` (`services/airport/LayoverSafetyEngine.ts:50-53`) is a step function on the
...
| L51 | `security_wait ↑` must never increase `usable_time` | C ⌀ | Monotone by construction: `usableMinutes = availableMin − totalBuffer` and every buffer term is additive (`LayoverSafetyEngine.ts:57-72, 251`). Vacuous — there is no security-wait input to increase. No test asserts it. |
...
| L53 | `departure_time` earlier must never expand the safe envelope | **W** | **Violated by arithmetic.** `timeOfDayExtra` is a step function on the local hour (LayoverSafetyEngine.ts, then-lines 50-53) and `hardReturnTime = cutoff − totalBuffer`, which still holds at `artifacts/api-server/src/services/airport/LayoverSafetyEngine.ts:686#hardReturnTime: new Date(cutoffMs - breakdown.totalBuffer * 60_000)`. Moving a 20:00 departure to 19:59 drops the extra from 10 to 0 while the cutoff drops by 1 minute, so the deadline moves **9 minutes later** and `usableMinutes` grows by 9. Identical at the 22:00 boundary (20 → 10). Untested on main and on PR #463. |
...
| L55 | Safety-critical calculations use a configurable conservative percentile by data maturity/consequence; do not collapse to a single average | N | Every estimate is a single scalar, most of them literals: 45/25/15/20 (`LayoverSafetyEngine.ts:212-216`), 20/10/0 (`:51-53`), 15/25 (`LayoverRecommendationService.ts:213-215`), 30/60/90 (`:219-222`). |

### §7 Temporal Freedom Engine

| id | Requirement | V | Evidence / divergence |
| --- | --- | --- | --- |
| L56 | A generalised engine answering "what can this user do before they must be somewhere else" | N | `computeWindow(airport: AirportProfile, session: LayoverSession, …)` (`LayoverSafetyEngine.ts:233-236`) is airport-typed at its signature. There is no generic engine, and no second adapter. |
...
| L58 | `FreedomWindow { beginsAt, endsAt, origin, requiredDestination, usableMinutes, hardConstraints, softConstraints, riskBudget, uncertaintyBudget, confidence }` | W | `LayoverWindow` (`LayoverSafetyEngine.ts:190-208`) is the analogue and shares two members in substance (`usableMinutes`; `earliestOutTime`/`hardReturnTime` ≈ begins/ends). It has no origin, destination, constraint sets, risk budget, uncertainty budget or confidence, and it is coupled to `AirportProfile`. |
...
| id | Requirement | V | Evidence / divergence |
| --- | --- | --- | --- |
| L60 | Bidirectional reachability: airport→candidate and candidate→airport **under future return-time conditions** | N | Return is modelled as outbound × 2 (`LayoverSafetyEngine.ts:98`) — explicitly symmetric and time-independent. |
...
| L65 | `candidate_feasible iff outbound_arrival + minimum_experience_duration + conservative_return_duration <= hard_return_by` | W | The inequality is implemented (`LayoverSafetyEngine.ts:120-121`), but each term is a category constant: `minimum_experience_duration` is `estimateActivityTime(placeType)` (30/60/90, `LayoverRecommendationService.ts:218-223`) and `conservative_return_duration` equals the outbound constant. **[463]** |
...
| L72 | Return traffic forecast → use future-time estimate, not outbound time | N | Directly contradicted by `travelTimeMin * 2` (`LayoverSafetyEngine.ts:98`). |
...
| L102 | Tool `getLayoverContext(sessionId)` | N | No layover tool exists. `compass/CompassTools.ts:78-222` declares eleven tools (`get_user_profile`, `get_current_trip`, `search_places`, `search_events`, `get_place_details`, `get_circle_activity`, `check_trip_conflicts`, `add_to_trip`, `get_whos_around`, `get_travel_compatibility`, `get_group_recommendation`) — none layover. |
...
| L223 | Overnight → overnight state / expanded options | W | Half covered, and the covered half is well done: `"overnight layover crossing local midnight → overnight"` (`test/airport.test.ts:755-762`) exercises the airport-local day-boundary logic at `LayoverSafetyEngine.ts:256-257`. There are no expanded options — `insideAirportCandidates` adds the same sleep-pod card to every session (`LayoverRecommendationService.ts:145-151`). |
...
inline (`LayoverSafetyEngine.ts:110-129`, `:296-329`), so they cannot be counted, tested,
...
| id | Was | Now | Evidence |
| --- | --- | --- | --- |
| L53 | W | **C** | `9c26efba`: `computeReturnDeadline` (`LayoverSafetyEngine.ts:213`) is the single anchor; 1-Lipschitz ramp; 1,520,640-case sweep in `test/layoverDeadlineMonotonicity.test.ts`. Also closes headline defect 2 (one buffer per response). Not spec-attributable (cites the census, not the spec). |
...
| L41 | N | W | `computeReturnState` (`LayoverSafetyEngine.ts:383`) derives RETURN_SOON at `hardReturn − 30 min` (`RETURN_SOON_LEAD_MIN`, `:84`), wired into every window (`:463`) and serialised to the client by `serializeWindow`. No notification fires and discovery is not de-emphasised — those effects are client/Safe Return work. |
...
| L258 | W | **C** | `recommendation_generated` now records `engineVersion`, the deadline inputs (airport, flags, cutoff, computedAt), the buffer breakdown, `hardReturnTime` and a ratings histogram for every `safety_rating` / `return_buffer_min` / `hard_return_time` written (`LayoverRecommendationService.ts:813#event_type: "recommendation_generated",`; the metadata block runs :805-844 — the earlier citation `:420-440` had rotted onto an unrelated comment about `stableIds`, and an UNANCHORED line range is why nothing caught it). |
...
- `LayoverSafetyEngine.ts:138#* default 30) so the server state and the local reminder agree.` declares `TravelTimeSource =
...
| --- | --- | --- |
| A certified feasibility record with engineVersion, feasibilityVersion, inputHash, verdict, envelope, buffer breakdown, reasons, reason codes, confidence | **Yes** | `services/airport/LayoverFeasibility.ts:352-388` (record), `:69` `LAYOVER_FEASIBILITY_VERSION`, `:325` `feasibilityInputHash`, `:451` `certifyFeasibility` |
| `replayFeasibility` reproduces a record deep-equal | **Yes** | `LayoverFeasibility.ts:553` (`= certifyFeasibility`, no clock, no I/O — `:449-450`), pinned by `test/layoverFeasibilityRecord.test.ts` |
| Five derivations collapsed onto one | **Yes** | The only remaining callers of `computeWindow`/`adviseLeaving`/`computeReturnDeadline` outside the engine are inside `LayoverFeasibility.ts:488-472`. `LayoverRecommendationService.ts:374` still calls `assess` per candidate, but against `certified.deadline` — the same certified deadline, not a second derivation |
...
| L163 | W | W | **Evidence corrected, verdict unchanged.** `layover_events` and `layover_sessions` are in `UNCLASSIFIED_BACKLOG` (`lib/deletionDispositions.ts:378#layover_events`), not the erasure cascade; `layover_plan_stops` and `layover_recommendations` are in `DENOMINATOR_CORRECTION_BACKLOG` (`:571-572`); `airport_profiles` is in `UNCLASSIFIED_BACKLOG` (`:281`). `AccountDeletionService.ts` names no layover table. No layover row is erased by account deletion. |
...
| 5 diff the action universe | `:552#actionUniverseOf`, `:669#diffActionUniverse` | verdict, return state, tier, usable minutes, deadline, feasible candidate ids, reason codes |
| 6 invalidate | `:712#invalidateRecommendations` | decides; writes nothing |
| 7 OpportunityEvent | `:683#opportunityEventFor` | `null` unless materially changed (`:737#MATERIALITY`) |
| 8 notify | `:752#shouldNotify` | strictly narrower than step 7 |

`handleEvent` (`:900#handleEvent`) runs 2 through 8 in order. `disruptionAfter`
...
(`services/airport/LayoverSafetyEngine.ts:281#LiveConditions`) is the one shape
...
(`LayoverSafetyEngine.ts:390#liveExtra:`, summed into the total at
...
INPUT of the certified record (`LayoverFeasibility.ts:333#liveConditions:`), so
...
(`LayoverFeasibility.ts:594#liveExtraEstimate`). Both version constants moved:
...
APPLIED, NO WRITER.** `airport_fact_observations` (`:163#airport_fact_observations`)
...
| L96 | N | C | §11.1 step 5. `actionUniverseOf` + `diffActionUniverse` (`LayoverEventReplanner.ts:626#actionUniverseOf`, `:669#diffActionUniverse`), exercised in both directions — a spike that loses options (`test/layoverEventReplanner.test.ts:423#drops`) and a delay that gains them. Red-first: emptying `candidatesLost` fails 2. |
| L98 | N | C | §11.1 step 7. `null` unless the options materially changed (`LayoverEventReplanner.ts:765#opportunityEventFor`, thresholds `:737#MATERIALITY`); the two flight reason codes are emitted here because only a diff can know them. Red-first: emitting unconditionally fails 1. |
...
| L44 | N | W | A material disruption now has an input: `disruptionAfter` walks §15.2's machine from a flight event (`LayoverEventReplanner.ts:1001#disruptionAfter`), `invalidateRecommendations` names the stale set (`:712#invalidateRecommendations`) and `handleEvent` replans (`:900#handleEvent`). The audit-trail half is not built — nothing writes — and nothing produces the event. |
...
| L288 | N | W | `FLIGHT_MOVED_EARLIER` emitted by the diff, and only for a flight event — a queue spike also moves the deadline earlier and must not claim the flight moved (`LayoverEventReplanner.ts:791#FLIGHT_MOVED_EARLIER`; asserted both ways at `test/layoverEventReplanner.test.ts:593#FLIGHT_MOVED_EARLIER`). Same limit. |
| L289 | N | W | `FLIGHT_DELAY_CREATED_OPPORTUNITY` emitted only when a delay actually widened the window materially (`LayoverEventReplanner.ts:800#FLIGHT_DELAY_CREATED_OPPORTUNITY`) — not on every delay, because a delay into the night band can shrink it. Same limit. |
| L51 | C | C | **Verdict unchanged, vacuity narrowed and NOT removed.** `test/layoverFeasibilityInvariants.test.ts`'s own header says "`security_wait` HAS NO INPUT on this tree". It has one now (`LayoverSafetyEngine.ts:281#LiveConditions`) and the invariant is swept over it directly, 0-240 minutes, in two timezones (`test/layoverLiveConditions.test.ts:188#non-increasing`) instead of over the buffer columns that stood in for it. Still `⌀`: nothing supplies the input. |
...
- **The whole reachability claim rests on one assertion.** Removing
  ...
  (`travel-buddy-standalone/app/layover/__tests__/layoverDashboard.safeReturn.component.test.tsx:272#the dashboard mounts the flight-change card`)
...
(`services/airport/LayoverSafetyEngine.ts:925#export function rankActivities`)
...
| id | was | now | why |
| --- | --- | --- | --- |
| L141 | W | C | "At `CONNECTION_AT_RISK`: exploration surfaces collapse." §10's `W` reason, verbatim: *"`explorationCollapsed` is derived and published, and after this pass it is still read by nothing."* It is read now, and the three exploration surfaces are replaced by a notice — at RETURN_NOW as well as CONNECTION_AT_RISK, which is one rung stricter than the requirement and never looser. Red-first: keying the collapse off `returnRoutePrimary` instead — the field beside it, set from the same boolean — fails 4 (`travel-buddy-standalone/app/layover/__tests__/layoverDashboard.safeReturn.component.test.tsx:363#the collapse is keyed on explorationCollapsed`); forcing the derivation to `false` fails 3; wiring "Show them anyway" to a no-op fails 1. |
| L165 | W | C | "**Notifications** — explain that they are needed to warn when the safe return window changes." The body's `W` reason, verbatim: *"No explanation precedes the OS prompt — there is no rationale sheet or copy anywhere in the layover flow."* There is one, it names the specific risk the permission covers rather than asking for "alerts", and it appears only when a prompt would (`travel-buddy-standalone/app/layover/__tests__/layoverDashboard.safeReturn.component.test.tsx:404#a traveller whose phone is about to ask is told why first`). Red-first: removing the sheet fails 3; showing it unconditionally fails 1 (`travel-buddy-standalone/app/layover/__tests__/layoverDashboard.safeReturn.component.test.tsx:435#a traveller who already granted permission`). |
...
`airportIntelligence()`
(`artifacts/api-server/src/services/airport/LayoverFeasibility.ts:866#export function airportIntelligence`)
derives four rungs
(`artifacts/api-server/src/services/airport/LayoverFeasibility.ts:803#export const AIRPORT_INTELLIGENCE_TIERS`)
...
(`artifacts/api-server/src/services/airport/LayoverFeasibility.ts:862#function airportSuppliedTerms`)
...
(`artifacts/api-server/src/services/airport/LayoverEventReplanner.ts:621#export function candidateIsUnmeasured`),
and the exclusion is **named** rather than folded into "does not fit"
(`artifacts/api-server/src/services/airport/LayoverEventReplanner.ts:638#unmeasuredCandidateIds: candidates.filter`)
...
`artifacts/api-server/src/services/airport/LayoverSafetyEngine.ts:1131#const freedom = TemporalFreedomService.buildFreedomWindow(freedomCtx)`
...
`artifacts/api-server/src/services/airport/LayoverSafetyEngine.ts:1116#const freedomCtx: LayoverFreedomContext`'s
...
(`artifacts/api-server/src/services/airport/LayoverSafetyEngine.ts:1031#shortfallMinutes: number | null`),
it is on the wire
(`travel-buddy-standalone/src/services/layover.ts:134#shortfallMinutes: number | null`)
and it is rendered
(`travel-buddy-standalone/src/components/layover/CanILeaveCard.tsx:140#styles.shortfall`).
...
3. **L56, L58 and L177** point at three ranges in `LayoverSafetyEngine.ts` —
   ...
   `artifacts/api-server/src/services/airport/LayoverSafetyEngine.ts:1017#freedomWindow: FreedomWindow | null`
...
(`artifacts/api-server/src/services/airport/LayoverSessionService.ts:542#expirySweepDisclosure`)
...
`artifacts/api-server/src/services/airport/LayoverFeasibility.ts:543#  const rowClass: EstimateSourceClass = a.id === null ? "STATIC_DEFAULT" : "AIRPORT_PROFILE";`
folded by `artifacts/api-server/src/services/airport/LayoverFeasibility.ts:868#  const airportAddressable = terms.every((t) => t.sourceClass === "AIRPORT_PROFILE");`.
...
## §24 — the ride back was the ride out, in the one list Discovery is allowed to show

Written 2026-09-15 by the LAYOVER TIMING/COVERAGE lane, briefed to close the
timing and coverage rows the owner named as blocking Discovery. **No verdict
moves to `C`.** One defect was found, watched RED, closed and mutated; two rows
carry corrected evidence; and the rest of the timing and coverage family is
reported here with its exact blocker rather than moved.

### 24.1 THE FINDING — a card certified against half a journey

`certifiedActionUniverse` is, by §25 L269, the only list a Discovery surface may
show a traveller in Layover mode. Its admission arithmetic is `candidateFits`,
and for every landside candidate it charged:

```
travelTimeMin * 2 + activityTimeMin <= usableMinutes
```

§12.1's subtraction ladder names that term **"return (future conditions, not
symmetric)"**, and L46's own `W` sentence already quoted the divergence
verbatim: *"they are per-candidate and symmetric (`travelTimeMin * 2`), which is
the exact opposite of 'not symmetric'."* What the census had NOT recorded is
...
instant and once at the instant the window says the traveller must start back —
and publishes `returnLowerBoundMin`, `returnForecastLowerBoundMin` and
`returnDepartsAt`
(`artifacts/api-server/src/services/airport/LayoverEnvelope.ts:415#Once at the same instant and once at the return instant`).
That is a LOWER BOUND: it can refuse a journey and can never certify one. The
admission arithmetic, which CAN certify one, was the half still doubling.

In minutes: a traveller states a 20-minute ride out and a 60-minute visit
against a window with room for 100. `20*2 + 60 = 100` fits, so the card is
ADMITTED. If the ride back at the hour they would actually start back costs 45,
the true cost is `20 + 60 + 45 = 125` and they are 25 minutes past their own
...
**IT IS NOT A SECOND COPY OF `bandCandidate`'s RETURN INSTANT.** That one is the
LATEST instant the window allows a start-back — a bound for a proof. This one is
the traveller's own itinerary for this place. Two different questions with two
different answers; census L6 forbids the copy, not the second question.

### 24.3 What no traveller sees

`LAYOVER_TRAVEL_TIME_PROVIDER` is `noRoutedProvider`, so the port answers
`NO_ROUTED_PROVIDER` to both legs on this deployment and the return term is an
ABSENCE that falls back to the symmetric charge — the same number this surface
served before. Production has 5 layover sessions ever, 0 active, **0 plan
stops**, so there is not one row anywhere that could state either leg. What is
closed is that the contract can now be TOLD the ride back is longer and refuses
when it is; the day a routed provider is configured, this changes by zero lines.

### 24.4 Red first, then fifteen mutations

The defect was watched RED before anything was written: the first case
(`artifacts/api-server/src/services/airport/__tests__/layoverReturnLegAsymmetry.test.ts:97#REFUSES a card whose stated return leg overflows`)
failed with *"a card whose stated ride back overflows the certified window was
ADMITTED"*, and `npx tsc --noEmit` failed on five lines because the field did
not exist. 15 tests, 15 passing.

| # | mutation | killed |
| --- | --- | ---: |
| 1 | `candidateFits` back to `travelTimeMin * 2` | 2 |
| 2 | drop the negative / non-finite guard in `returnLegMin` | 1 |
| 3 | an unstated return leg is free (`return 0`) | 2 |
| 4 | charge airside a landside return | 1 |
| 5 | `candidateIsUnmeasured` also requires a return leg | 3 |
| 6 | ask the port for the ride back at DEPARTURE, not start-back | 1 |
| 7 | ask for the ride back in the OUTBOUND direction | 1 |
| 8 | never resolve the return term | 3 |
| 9 | ask for a landside ride back inside the terminal | 1 |
| 10 | collapse all four return absences to one word | 3 |
| 11 | the gate drops the figure on the way to the universe | 1 |
| 12 | the gate ignores the injected port | 1 |
| 13 | the withheld card hides the term it was refused for | 1 |
| 14 | the snapshot contract drops the field before the replanner | 2 |
| 15 | the snapshot contract defaults the return to the outbound | **0** |

**MUTATION 15 SURVIVED, and it is reported rather than papered over.** Defaulting
`returnTravelTimeMin` to the outbound inside `replanCandidate` is behaviourally
equivalent TODAY, because `candidateFits` already falls back to exactly that. It
is still wrong, and it is a second copy of the fallback rule in a file that does
not own it: the moment any other consumer of `ReplanCandidate` reads the field
directly, a STATED symmetric return and an ABSENT one stop being the same fact
and nothing here would notice. No test was written to kill it, because a test
that pins an equivalent mutant pins the implementation rather than the
behaviour.

### 24.5 Rows re-derived — evidence corrected, NO verdict moved

| id | stays | why |
| --- | --- | --- |
| L46 | `W` | Its stated divergence is now half closed: the return term can be stated and is charged as stated. The other half is untouched — six of §12.1's eleven terms are still one lumped `estimateExitDelay` constant, and outbound/return are still per-candidate rather than in the ladder. By this document's own rule (§13.4's L77, *"three terms of four is not four"*), half a divergence closed is still `W`. |
| L269 | `W` | §17.6's last statement — *"Untouched. A disclosed provenance is not … a certified action universe"* — is **FALSE at this commit** and was false before this pass. A certified action universe exists, is published, and gates all six Discovery serve paths. It stays `W` for a DIFFERENT and larger reason, stated in 24.6. |
| L260 | `W` | Same correction: the pre-filter is on the certified envelope, not the scheduled window. Its own `W` sentence's other half stands. |

### 24.6 WHY L269 STILL DOES NOT MOVE — the flag has no seeding migration

`LAYOVER_DISCOVERY_MODE_FLAG` is `layover_discovery_mode_enabled`
(`artifacts/api-server/src/services/airport/LayoverSnapshot.ts:147#LAYOVER_DISCOVERY_MODE_FLAG`),
and it is FALSE BY ABSENCE. **There is no migration anywhere in this tree that
seeds that row** — `grep -rn 'layover_discovery_mode' src/migrations/` returns
nothing, and the only three hits in the whole repository are the constant, a
test asserting its spelling, and a comment in `checkCensusFreshness.ts`. The
gate therefore reads `absent` on every production request and returns `OFF`, so
Discovery serves the ungated list and L269's requirement is not in force for any
traveller.

That is a blocker this lane will not work around. Seeding a flag is a migration,
migrations belong to the coordinator, and 2893–2970 are unapplied to
portava-ci already. **Stated, not fixed.**

### 24.7 The timing and coverage family, with the blocker each one has

Read against HEAD, not carried forward. None of these was moved.

| id | verdict | requirement, in short | blocker, exactly |
| --- | --- | --- | --- |
| L23 | `N` | `layover_time_budgets` per-snapshot ladder | The breakdown is computed per request and returned in a response body. The table is in migration 2700, which is **applied to no database**. |
| L46 | `W` | the eleven-term subtraction ladder | Six terms lumped into one constant; the two transport terms need a routed provider to be anything but the traveller's own self-report. |
| L79 / L80 / L81 | `W` | airport-fact TTL and freshness | The classes and TTLs exist. **No producer of any fact type**, and `terminal_info` is null on all 3,206 production airports. |
| L161 | `N` | aggregate airport timing may persist after de-identification | `measureCalibration` computes per-hour-band error and p90 coverage, and there are **no outcomes to calibrate against** and nowhere to persist them. |
| L196 | `N` | presence/crew tables with expiration JOBS | Neither table exists (2700/2740/2860 unapplied), and `expireOldSessions` is called inline from two GET handlers rather than scheduled. |
| L215 | `N` | metric `stale_fallback_rate` | The metric IS defined and computed (`artifacts/api-server/src/services/airport/layoverObservability.ts:118#LAYOVER_METRICS`), over `DecisionRecord`s from `layover_certified_computations` — a table **with no writer**. And there is no metrics exporter in this repository to emit it to. |
| L247 | `W` | calibration / p90 coverage | Measures only. No model is calibrated and there are no outcomes. |
| L264 | `W` | CAS the active snapshot, reject stale writes | The `.eq("status","active")` guard is applied consistently. There is **no snapshot to version** — `layover_snapshots` does not exist. |
| L199 | `W` | migration postconditions + authorization-contract coverage | Closing it means editing `0127_layover_system.sql`. Migrations are the coordinator's. |
| L236 / L295 | `W` | database tests against a real CI schema | **No CI database carries a layover schema.** Migrations 2893–2970 are unapplied to portava-ci. |
| L239 | `N` | E2E detection → … → completion | Four of the six stages have no implementation to test. |

### 24.8 WHAT WOULD TURN THIS RED

* **The fallback is load-bearing and invisible.** Every candidate on this tree
  today takes the symmetric path, so the new arithmetic's *stated* branch is
  exercised only by the injected port in one suite. If `returnLegMin` stopped
  falling back correctly, production would not notice and neither would any
  other suite; mutations 3 and 15 are both about that branch and one of them
  survived.
* **`discoveryLayoverGate` gained a fifth parameter.** It is optional and no
  call site in `routes/discovery.ts` passes it, so the production answer is
  unchanged — but it is a seam into the gate that did not exist before, and a
  caller that passed a lenient provider would widen what a traveller is shown.
  Nothing structural stops one.
* **The `terms` key on a withheld card grew a member.** No client in
  `travel-buddy-standalone/` was changed. A client that renders `terms` by
  enumerating its keys will show a member it has no label for.
* **24.7 is derived from this lane's reading plus the census's own stated
  blockers**, not from a measurement of each blocker. A row whose blocker has
  since been removed somewhere in this monorepo would be misfiled here and
  nothing would catch it.

### 24.9 The counted files this pass changed

| file | why |
| --- | --- |
| `artifacts/api-server/src/services/airport/LayoverEventReplanner.ts` | 24.2 — the return term and the charge |
| `artifacts/api-server/src/services/airport/LayoverSnapshot.ts` | 24.2 — carried through the published contract |
| `artifacts/api-server/src/services/airport/__tests__/layoverReturnLegAsymmetry.test.ts` | 24.4 — 15 cases, the first watched RED |
| `artifacts/api-server/src/lib/discoveryLayoverTiming.ts` | 24.2 — the producer and its three absences |
| `artifacts/api-server/src/lib/discoveryLayoverMode.ts` | 24.2 — the gate carries and publishes it |
| `docs/architecture/census-layover.md` | this section |

**`head_commit` is NOT re-declared. It stays `1fe72289b`, and the five files
above are covered by an acknowledgement instead.** This pass first re-declared
it at its own branch commit and was wrong to; §24.10 records why, because the
mistake is the more useful half.

### 24.10 THE RE-DECLARATION THIS SECTION FIRST MADE, AND WHY IT WAS WRONG

Recorded rather than quietly reverted, because it is the same defect this
document has now made twice and the second time was by somebody who had read
the first.

**WHAT WAS DONE.** §24 originally re-declared `head_commit` at `47634baf5`, the
branch commit carrying §24.9's five files, and checked it the way
`check:census-freshness` checks it: `git merge-base --is-ancestor 47634baf5 HEAD`
— which passes, because it is this branch's own second-from-last commit. The
guard went green and the declaration was wrong anyway.

**WHY IT WAS WRONG.** This repository SQUASH-merges. A squash gives `main` ONE
new commit whose parent is `main`'s old head; every commit on the merged branch
is discarded from the line of history. So `47634baf5` is an ancestor of this
branch and of nothing else. Measured, not reasoned about:

```
git merge-base --is-ancestor 47634baf5 origin/main   -> exit 1   (NOT an ancestor)
git merge-base --is-ancestor 1fe72289b origin/main   -> exit 0
git log --format='%h parents=%p' -3 origin/main      -> one parent each
```

The moment #488 lands, `check:census-freshness` would have reported this census
UNREADABLE on `main` — passing in the clone that wrote it and failing in CI,
which is the one shape of failure a guard must not have. **That is exactly what
happened at the #482 squash**, where ten censuses declared pre-merge branch
commits and every one orphaned at once; the `head_commit` row in §1 has carried
a paragraph warning about this hazard since 2026-09-10, and this pass read that
paragraph, reproduced its argument in its own re-declaration, and then made the
error it describes.

**WHAT REPLACES IT.** The declaration stays at `1fe72289b` — `main`'s head, the
commit this branch started from, and a commit that is an ancestor of `main` both
before and after the squash. The five counted files §24.9 lists are covered by an
entry in `artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json`
whose `since` is `1fe72289b` and which NAMES each of the three this census counts.
That is the only pattern available from inside a pull request, and it is what
PR #484 does.

**AND THE ACKNOWLEDGEMENT IS A CLAIM, NOT A SILENCER.** It says these three files
cannot have moved a layover verdict, and §24.5 is the argument: the three rows
they bear on were opened and re-derived, none moved, and
`check:census-integrity` reads the same 73 C / 124 W / 99 N over 296 at both
commits. Had any of them moved, the honest act would have been to leave this
census STALE with the reason rather than write an argument this lane does not
...
## §27 — 2026-09-22: the three crew operations §26.4 left ungraded, and the join that was not scoped to a city

§26.4 named the work this section does, in one sentence: *"The next measuring
pass owns all of that, including whether L185/L186/L188 and L131 move now that
the crew tables are real."*

This pass answers it for L185, L186 and L188. **L131 does not move and is not
graded here** — §26.2's reason is untouched: 2984 ships no chat, deliberately,
because §14's shared-chat half belongs to Telegraph's threads.

It moves **three rows, all `N → C`**, and it closes one live defect found while
grading them. Every old verdict below was read from
`CENSUS_INTEGRITY_DUMP=ALL node --import tsx/esm src/scripts/checkCensusIntegrity.ts`
(§16.6), never from the prose.

### 27.1 THE FINDING — the crew list was scoped to a city and the crew JOIN was not

A crew is a CITY-level thing, and before this pass the route layer said so in
...
- `GET /crew` refuses to LIST crews when the city is unknown, and says why in a
  comment: *"a crew is a CITY-level thing and we do not know the city"*.

`POST /airport/sessions/:id/crew/:crewId/join` asked for no city at all. It took
a crew id from the URL and joined it. **A filtered list in front of an
unfiltered write** — the shape this census has scored `W` repeatedly.

**The first RED is the whole finding in one response body.** Staged against the
real router, a traveller whose airport city is not known *at all* — the case
`GET /crew` refuses to serve a single crew to — joined a Taoyuan crew and got
back:

```
"memberCount": 2,
"sharedReturnBy": "2026-09-22T15:42:23.523Z",
"bindingMemberIds": ["scope-user-b"]
```

`bindingMemberIds` is the harm, not `memberCount`. §14.1's `shared_return_by` is
a MINIMUM over every member's `required_return_by`, so **the remote joiner
became the binding deadline for everyone actually in Taoyuan**. The founder was
told to be back four hours early by somebody who was not there, against a
`meeting_point_label` ("Terminal 2 food court") they could not reach.

There is a second consequence and it is not the larger one, which is worth
saying plainly rather than leading with it. Membership is the FIRST of the three
gates `crewMemberCards` applies; blocks and `publishableUserIds` /
`nameVisibilitySet` still held throughout, so **this was not a raw leak of
names**. It was a WIDENING: those travellers made themselves visible to a crew
in their own city, and an unscoped join let someone in no position to meet them
past the first gate.

**Why the route layer is the only possible place to fix it.** 2984 gives both
crew tables RLS with ZERO policies and ZERO `anon`/`authenticated` grants
(§26.1, re-probed). There is no database-level scope underneath this module that
could have caught an out-of-city join. That is what makes the check
load-bearing rather than belt-and-braces.

### 27.2 The fix, and where each half lives
...
Every mutation was applied by a replacer that REFUSES unless the target text
occurs exactly once, printed the changed line back from disk before the run was
believed, and was restored afterwards.

| # | file | mutation | result |
| --- | --- | --- | --- |
| 54 | `LayoverCrewStore.ts` | the city check made unreachable (`if (false)`) | **1 red** — the other-city case |
| 55 | `LayoverCrewStore.ts` | compare raw, without `canonCity` | **1 red** — the CONTROL |
| 56 | `routes/airport.ts` | drop the route's unknown-city refusal | **1 red** — the unknown-city case |

**54 and 56 red DIFFERENT cases, which is the useful result.** The two halves
are independently load-bearing: the store check is what catches a joiner in a
*different* city, the route check is what catches a joiner in *no known* city.
Neither subsumes the other, and a fix with only one half would have passed a
suite that tested only the other.

**55 kills the CONTROL, which is what makes the control non-vacuous.** A guard
that over-refuses — rejecting a legitimate same-city join because "Taoyuan" and
"taoyuan" compare unequal — is caught by the control rather than by the two
negative cases, and none of the three would have found it without it.

**56 is the WEAKEST of the three and is recorded as such.** It reddens with a
`TypeError: Cannot read properties of null (reading 'trim')` and a 500, not with
a wrongly-permitted join. It proves the route's refusal is REACHED and that
nothing downstream re-derives the city; it does not prove the refusal's *answer*
is right. The unknown-city case's 400 assertion is what does that, and only
while the route keeps refusing rather than coercing.
...
`src/services/layover/__tests__/layoverCrewCityScope.test.ts` (3 cases, two
watched RED first for the right reason — `200 !== 400` — and one control).
Measured together: **16 tests, 16 pass, 0 fail.** Across the whole layover and
airport surface (66 suites): **1145 tests, 1145 pass, 0 fail.**

### 27.5 WHAT THESE THREE ROWS DO NOT CERTIFY

Stated here because §26.2 had to say the same thing about L28/L29, and the
temptation is identical.

- **The evidence is route-level against `fakeLayoverDb`, not against the live
  table.** That double models no unique index, no RLS and no foreign keys. For
  these two tables the gap is narrower than it sounds — 2984 gives them zero
  policies and zero client grants, so there is no database-level behaviour
  underneath the route layer for a live test to observe that this one cannot —
  but it is a real gap, and `(crew_id, user_id)` uniqueness is the specific
  thing being taken on the migration's word. **The live-schema checks
  (`check:write-path-columns`, `check:missing-live-columns`,
  `check:authorization-contract`) CANNOT RUN in this container — they exit 2 at
  `assert-nonprod-supabase` — and this pass does NOT claim they passed.**
- **L131 does not move** (§26.2: no chat, deliberately).
- **L196 does not move**, and its BLOCKER HAS CHANGED. §23.3 filed it under
  "blocked on a migration no database has"; the tables now exist, so what is
  left is the expiration job, and there is no scheduler anywhere in this tree.
  `layover_crews.expires_at` is a read FILTER, so an unswept crew is invisible
  rather than stale-but-live. It is now blocked on an absent platform
  capability, not on a migration.
- **L124 / L132 do not move** (§26.2: the precision ladder has no grant store,
  and 2984 asserts the absence of a coordinate column in a postcondition).
- **Nobody is on this surface.** Production has 5 layover sessions ever, 0
  active, last event 2026-08-12. The defect closed here was reachable by
  nobody, because nobody is here. A `C` over a path nothing reaches is still a
  `C` about the code and still says nothing about use.

### 27.6 Two sentences in this repository's own files were FALSE, and are corrected where they stand

Same precedent as §25 on §24.6 and §26.3 on §23.3: a false sentence is corrected
where it stands rather than left to be read as current. Neither correction moves
a verdict.

1. `artifacts/api-server/src/services/layover/LayoverCrewStore.ts` — its header
   read *"created by migration 2984 (written, NOT applied — see
   docs/BUILD-BACKLOG.md)"*. False since 2026-09-16 (§26.1). Corrected, with the
   struck clause quoted rather than deleted.
2. `artifacts/api-server/src/services/layover/__tests__/layoverCrewSurface.test.ts`
   — its header read *"are NOT APPLIED to any database … it is not evidence that
   the storage exists"*. Same fact, same correction, and the surviving half
   (`fakeLayoverDb` models no index/RLS/FK) is kept because it is still true.

### 27.7 §23.3's partition, re-derived for the `N` population only

§26.3 said the number 82 was not re-derived and did not claim a new one. This
pass re-derives the partition **over the 94 `N` rows only** — not over the `W`
rows, and not as a new aggregate for §23.3's 223.
...
`CENSUS_INTEGRITY_DUMP` reads them today; the crew rows and L196 were
re-derived by hand in this pass because §26 changed the world underneath them.
**It is not a fresh measurement of each blocker.** A row filed under (b) whose
table was quietly applied by some other lane would be misfiled here exactly as
§23.8 warned, and nothing in this section would catch it — that is precisely
the error §26.3 found in §23.3, one pass later.

**75 of the 94 (buckets b + e) wait on something no code in this domain can
supply.**

### 27.8 WHAT WOULD TURN THIS RED

* **A second write path into `layover_crew_members` that does not take a city.**
  The guard is inside `joinCrew`, so anything routing around that function is
  unguarded. There is one write site today and the suite covers it; nothing
  structural stops a second.
* **A crew whose city changes after creation.** Nothing can do that today —
  there is no update path for `layover_crews.city` — and if one is added, the
  check compares against the value at join time and says nothing afterwards.
* **`canonCity` drifting from `cityPresence`'s inline normalisation.** They
  agree today by convention, not by a shared call site, and mutation 55 is the
  only thing that would notice the crew half changing.
* **The live-schema gap in 27.5.** The `(crew_id, user_id)` uniqueness that
  makes a double-tapped join idempotent is taken on 2984's word in this
  container.
* **`check:doc-citations` is RED for three citations this pass moved and did
  NOT repoint** — see 27.10.

### 27.9 Tally

`check:census-integrity` reads **C=78 W=124 N=94 X=0** across 296 rows,
denominator 296. Three rows moved, all `N → C`, all in 27.4.

| Measure | §26 | §27 |
| --- | ---: | ---: |
| BUILT-AND-CORRECT | 75 | **78** |
| BUILT-BUT-WRONG | 124 | **124** |
| NOT-BUILT | 97 | **94** |
| CANNOT-VERIFY | 0 | **0** |
| CONSTRUCTED% | 67.2 % | **68.2 %** (202 / 296) |
| CORRECT% raw | 25.3 % | **26.4 %** (78 / 296) |

78 + 124 + 94 + 0 = 296. **Restated from the rows, not from arithmetic on the
previous headline** — the figures above were read off
`check:census-integrity`, which is the only form another censor can reproduce.
§26.5's numbers were correct when written and are superseded here under this
document's LAST-STATEMENT-WINS rule; they are not deleted.

This is the same KIND of move §26 made and the opposite of §22's: three rows
leave `N` rather than moving within the constructed population, so CONSTRUCTED%
rises for the second pass running. It rises by 1.0 point, and the three rows are
service operations with a mounted client — not table rows.

### 27.10 The counted files this pass changed, and the one it deliberately did not

| file | why |
| --- | --- |
| `artifacts/api-server/src/routes/airport.ts` | 27.2 — the join route resolves and checks the joiner's city |
| `artifacts/api-server/src/services/layover/LayoverCrewStore.ts` | 27.2 — `joinCrew` takes a city and the `city_mismatch` reason; 27.6 item 1 |
| `artifacts/api-server/src/services/layover/__tests__/layoverCrewCityScope.test.ts` | 27.1 — new, 3 cases, two watched RED first |
| `artifacts/api-server/src/services/layover/__tests__/layoverCrewSurface.test.ts` | 27.6 item 2 — header correction only, no case changed |
| `artifacts/api-server/package.json` | registers the new suite in `test` |
| `docs/architecture/census-layover.md` | this section, and ten citations repointed below |

**Ten citations into `routes/airport.ts` moved by this pass's 21 added lines and
have been repointed IN THIS FILE**, each from the line
`check:doc-citations` NAMED rather than by arithmetic offset:
3628→3649, 3749→3770, 3389→3410, 3499→3520, 3481→3502, 3331→3352, 3350→3371,
3462→3483, 3463→3484, and §16's bare `3327#rent_buddy_enabled`, which the
checker reported at TWO candidates (3348 and 3352). The claim settles it: it
says *"the **gate** is at …"*, and the gate is
`if (!await isFlagEnabled(sc, "rent_buddy_enabled")) {` at **3352** — 3348 is a
comment mentioning the flag. It has been given the full anchor so it cannot be
ambiguous again.

**THREE CITATIONS IN `docs/architecture/census-highlights-memories.md` ARE STALE
BECAUSE OF THIS PASS AND HAVE NOT BEEN TOUCHED.** That file belongs to another
lane and this one does not write in it, so the repointing is stated here for
whoever does, mechanically and with no prose or verdict of that census
inspected:

All three cite `artifacts/api-server/src/routes/airport.ts`. The anchor text of
each is UNCHANGED and correct; only the line number moved. **The path, line and
anchor are deliberately NOT written here as a single `path:line#anchor` token**
— `check:doc-citations` would read this table as three more live citations into
that file and fail on them, which is how this section failed its own check once
already.

| its line | anchor, unchanged | line: from | to |
| --- | --- | ---: | ---: |
| 2857 | `sourceType: "layover_session", verificationLevel: "checkin",` | 3691 | **3712** |
| 2973 | `const occurrence = declaredOccurrenceHasHappened(args.session.arrivalTime, Date.now());` | 3665 | **3686** |
...
## §41 — 2026-09-22 (integration): six lanes merged, 16 rows move, and every one was re-derived here rather than accepted

Three lanes handed back proposed verdict moves — LO-API (2), LO-CLIENT (9),
LO-SERVER (5). A lane's proposal is a CLAIM, not a verdict. This section is the
integrating pass, and every citation below was reopened at the merged tree
before the row moved. Two of the sixteen changed their evidence in the reopening
and are recorded with the corrected version, not the proposed one.

Paths are repository-relative here, because this pass spans both workspaces and
the `artifacts/api-server/src/`-relative convention of §13 would make the client
half ambiguous.

### §41.1 The sixteen moves

| id | was | now | why |
| --- | --- | --- | --- |
| L239 | N | W | All six stages walk one session over the REAL router in `artifacts/api-server/src/test/layoverApiLifecycle.test.ts:135#L239` — detection `POST /:id/disruption`, evaluation `GET /:id/safety`, plan `POST /:id/stops`, execution `PATCH /:id/stops/:stopId`, return `POST /:id/return-now`, completion `DELETE /:id`. The body's *"four of the six stages have no implementation to test"* is false; five have a route and detection has no PRODUCER, which is what holds this at W. |
| L221 | N | W | The 6h international case runs over the real router and the abort hands back the return CONTRACT the body says does not exist — `artifacts/api-server/src/services/airport/LayoverSafeReturnService.ts:144#ReturnContract`, driven at `artifacts/api-server/src/test/layoverApiLifecycle.test.ts:260#L221`. The body cited L24, which is about a `layover_return_plans` TABLE; that table really is absent, and a contract is a different artifact. `route` is hard-`null` on `no_routing_provider`, so the traveller is told WHEN and never HOW — W, not C. |
| L116 | N | W | Geography IS drawn from the certified envelope: two edges at a printed metres-per-pixel scale (`travel-buddy-standalone/src/components/layover/LayoverMapCard.tsx:336#layover-map-safe-envelope`, `:381#layover-map-envelope-scale`). W because only two of the four bands have a producer — `BLOCKED` and `UNCERTIFIED` are the only values any server path assigns (`artifacts/api-server/src/services/airport/layoverRankingFeasibility.ts:94#UNCERTIFIED`), against the four the vocabulary declares (`artifacts/api-server/src/services/airport/LayoverEnvelope.ts:92#ENVELOPE_BANDS`). `SAFE`/`TIGHT` are spelled and never produced. |
| L117 | N | W | Blocked pins are kept off the map and listed with the server's own reason (`travel-buddy-standalone/src/components/layover/LayoverMapCard.tsx:426#layover-map-blocked-`). W and not C for a reason this census had not recorded: the band is joined onto a pin by `rec.id` (`travel-buddy-standalone/app/layover/[id].tsx:397#candidateFeasibility`), and a recommendation carries an `id` only when `layover_stable_recommendation_ids_enabled` is ON. It is **False** in the 2026-09-22 production snapshot. Built, and it renders no band in production. |
| L119 | N | W | The return leg and its deadline render whether or not a plan exists, with an explicit uncertified case (`travel-buddy-standalone/src/components/layover/LayoverMapCard.tsx:463#layover-map-route-return-leg`, `:466#layover-map-route-return-deadline`). No LINE is drawn, because there is no routed provider to draw one from — the same purchase decision that holds L142 and L152. |
...
**L117 and L122 were proposed on the strength of the code and are recorded on
the strength of the code AND a flag.** The lane's own report named the ceiling;
what this pass added is that it is not a rehearsal detail. `candidateFeasibility`
builds its lookup with `if (r.id && r.feasibility)`, and
`LayoverRecommendationService` fills `idByKey` only inside `if (opts.stableIds)`.
With `layover_stable_recommendation_ids_enabled` **False** in production, every
card is served with `id: undefined`, so the lookup is empty and NO pin is banded
on any live screen. Both the client work and the server's per-pin feasibility are
real, tested, and invisible to a traveller today. That is recorded here because a
reader of L117 at W would otherwise reasonably assume it renders.

**L204/L246/L249 were proposed as "behind a flag seeded FALSE" and are recorded
as "behind a flag that does not exist on production".** 2977 seeds
`layover_maturity_gate_enabled` FALSE, and 2977 is not in
`production-applied-migrations.json` and not in the 2026-09-22 snapshot's flag
set. The distinction matters for what happens next: a seeded-FALSE flag needs an
owner decision through `public.toggle_feature_flag_with_audit`; an ABSENT one
needs a migration applied first. W is right either way, and the reason written
against the rows is now the true one.

### §41.3 What did NOT move, and was checked

L118 stays N: there is no time-budget chip and no route to replan against a
budget — `POST /api/airport/sessions/:id/simulate` does not exist. L120, L124
and the L164/L168 pair stay exactly where they were; the last two are addressed
in §41.4 for their MARKER, not their verdict. The other eight rows the three
lanes re-measured and declined to move are left as they are, which is the
correct outcome for a re-measurement that confirms.

### §41.4 The `∅` markers on L164 and L168 come off; the verdicts do not

Both rows read `N ∅` — NOT-BUILT, and unguarded, meaning nothing in the tree
would notice if the absence ended. They are now guarded by a source scan over
the whole layover client surface: no `expo-image-picker`, `expo-contacts`,
`expo-camera` or `expo-media-library` import, and no picker or contacts call
anywhere on it. The scan was watched RED — adding a file that imports one turns
it over. `N` is unchanged and correct; the `∅` is not. Both rows are restated here so
that this is their last statement, and the marker is gone from it:
...
### §41.5 Headline after this section

| bucket | was (§40) | now |
| --- | --- | --- |
| BUILT-AND-CORRECT | 79 | 81 |
| BUILT-BUT-WRONG | 131 | 143 |
| NOT-BUILT | 86 | 72 |
| CONTRADICTED | 0 | 0 |
| total | 296 | 296 |

Fourteen rows move `N → W` and two move `W → C`. Verified rather than asserted:
`CENSUS_INTEGRITY_DUMP=ALL` over this file before and after this section differs
on exactly sixteen lines, and they are exactly the sixteen ids named in §41.1.

**Implementation is what moved. Nothing here was verified against a deployed
production surface**: two of the three server rows sit behind a flag whose
...
(`artifacts/api-server/src/services/layover/LayoverDecisionStore.ts:415#DECISION_PERSISTENCE_FLAG`).
...
> **RENUMBERED AT INTEGRATION, 2026-09-23.** The section below was authored as
> **§27** on `main` (PR #525, squash `9018c1966`) and its subsections as 27.1–27.9.
> This branch had independently written its own §27 — *"the three crew operations
> §26.4 left ungraded"* — and eighteen sections after it, so the two collided on
> the same number without either knowing about the other. It is renumbered **§45**
> here, and its subsections **45.1–45.9**, because renumbering one self-contained
> section is safer than renumbering eighteen and every cross-reference between
> them. NOTHING ELSE about it is changed: not a verdict, not a citation, not a
> word of its argument. Where another file still says "§27.4" or "§27.9" meaning
> this section — `src/scripts/checkCensusFreshness.ts`'s CENSUS_SCOPE entry does —
> it means §45.4 and §45.9 and has been updated to say so.

---

## §45 — L48's first half: the border is read. The prohibition is still not invented.

This section moves two rows, supersedes one clause of §5's unrealised delta
block, and records four rows that deliberately did **not** move. It does not
re-measure census-layover and does not re-declare `head_commit`.

### 45.1 What was built

`adviseLeaving` answered *"can I leave the airport?"* from the CLOCK alone. On a
roomy window it returned `verdict: "yes"` while the same response listed *"Visa
or transit-permit requirements for your nationality"* in `unknowns[]` and
emitted `ENTRY_NOT_CONFIRMED` unconditionally — an affirmative and a disclaimer
about the same act, in the same payload. The reason was the one L48's evidence
cell gives: nothing under `routes/airport.ts` or `services/airport/` read
`entry_requirements` or `traveler_passports`, both present since 0169.

It reads them now, through `services/airport/layoverEntryGate.ts`:

- `lib/entryRequirements.ts` gains `readCorridor`, a THREE-state read —
  `found` / `absent` / `unreadable`. supabase-js RESOLVES on a database error,
  so an unreadable table and an uncurated corridor arrived in almost the same
  shape and the old `lookupRequirement` collapsed them into `null`. It still
  does, for the callers that only need the row, and now delegates.
- `resolveLayoverEntry` resolves WHICH corridor a layover is asking about —
  the traveller's own passport (primary, else oldest) into the airport's
  country — and answers `permitted` / `refused` / `unresolved`. It holds no
  corridor data of its own, so **there is no second eligibility mechanism** to
  disagree with `lib/entryRequirements.ts`.
- The permitting statuses are an ALLOW-LIST (`visa_free`, `visa_on_arrival`,
  `transit_visa_free`). A status this module has never seen — a new vocabulary
  entry, a typo, a NULL — refuses, because the failure modes are not symmetric:
  wrongly refusing costs a traveller an afternoon, wrongly permitting costs
  them a denied entry at a foreign border.
- `unresolved` carries FIVE distinct reasons, never merged, because each needs
  something different from a different person: the flag needs an operator, the
  passport needs this traveller, the airport country needs a curator, the
  corridor row needs a curator with different data, and a failed read needs
  nobody — it needs retrying.
- It reaches the verdict as a FACT on `LeaveAdviceFacts`, like
  `travelTimeSource` and `liveConditions` before it, and as a NAMED input on
  `FeasibilityInputs` so it lands in `inputHash` and `replayFeasibility`
  reproduces the record that was certified. **An absent entry fact is
  unresolved, never permitted**: a caller that forgets gets the cautious answer.
- `ENTRY_NOT_CONFIRMED` is now emitted where its CONDITION applies rather than
  always, and the standing visa unknown is dropped — by identity, not by
  position — on the one state that makes it untrue.

The route layer resolves the corridor once per request, beside the one
certification, at every site that certifies the REQUESTER's own session. The
crew wrapper deliberately does not: it certifies other people's sessions, and a
crewmate's passport position is not something that surface may read or publish.
`certifyCrewPlan` reads no verdict — it takes `requiredReturnBy`,
`usableMinutes` and `returnState` — so the crew answer is identical either way.

### 45.2 The two rows

| id | was | now | Evidence |
| --- | --- | --- | --- |
| L34 | N | **C** | `EntryEligibility` (`services/airport/layoverEntryGate.ts`) is the spec's three states under this tree's names — `permitted` = `CONFIRMED_ALLOWED`, `refused` = `CONFIRMED_NOT_ALLOWED`, `unresolved` = `UNKNOWN` — as a discriminated union, so an unhandled state is a type error rather than a default. `EntryUnresolvedReason` refines the third into five reasons that are never collapsed, which is more than §4.1 asks for and nothing it forbids. Exercised by `src/test/layoverEntryGate.test.ts`. |
| L48 | N | **W** | HALF BUILT, and the half that is missing is the one the row's text names. Entry permission state is now read and published: a corridor that is not `CONFIRMED_ALLOWED` costs the traveller the clock's "yes" (`verdict: "entry_unverified"`), and a refusal overrides the clock outright at any amount of spare time. **Landside recommendations are not forbidden.** `certifiedLayoverSnapshot`'s `forbidden` test was made exhaustive so the new verdict could not slip through as fail-open, and `entry_unverified` is explicitly NOT forbidden there — see 45.3. |

### 45.3 Why L48 is `W` and not `C`, stated rather than shrugged

`entry_requirements` has **no INSERT in any migration** — 0169 calls that its
honesty contract. So until somebody curates a corridor, every real corridor is
`no_data_for_corridor`, and forbidding landside on "not CONFIRMED_ALLOWED"
would collapse landside exploration for **every traveller on the app** over a
data gap rather than over a fact about them.

That is a product decision this section does not have the standing to take, and
it is the same decision `LayoverFeasibility.ts` already records for L4's
fail-closed half (*"This record publishes the confidence; it does NOT forbid"*).
So the prohibition is not invented, the divergence is recorded as a divergence,
and the row is `W`: constructed and not what §6.1 asks for. **A curation pass on
`entry_requirements` is the thing that makes `C` available**, and the next
measuring pass owns whether it should then be taken.

What was NOT allowed to happen quietly: `certifiedLayoverSnapshot` read
`verdict === "no" || verdict === "stay_airside"`, which treats any verdict it
does not recognise as NOT forbidden. Adding a value to the union without
touching it would have been a fail-open on the one verdict that says the border
could not be checked. It is an exhaustive switch now, with a `never` default
that forbids.

### 45.4 Four rows that deliberately do not move

- **L230** (§21.1 — unknown entry permission ⇒ no landside recommendation)
  stays **N**. It is L48's prohibition half stated as a scenario, and the
  prohibition is not built. §5's delta block predicted `C` for it; that
  prediction was about a different implementation and is superseded here.
- **L49** (critical unknown ⇒ `confidence = INSUFFICIENT` **and** forbid
  landside) stays **N**. Entry is one critical unknown of several; baggage is
  still unrepresentable (L35) and nothing forbids.
- **L4** stays **W**, where §17 last stated it. Unknown entry now fails closed
  in the sense that it withholds the affirmative; the confidence half that row
  turns on is unchanged by this work.
- **L235** stays **C**, where §6 last stated it. This pass asserts the
  entry gate's own monotonicity property — the gate can only ever WITHHOLD a
  yes, over every entry state crossed with the three sides of the engine's
  thresholds — which is one more instance of a property that row already counts,
  not a new verdict.

### 45.5 §5's delta block is superseded in two clauses, not deleted

§5 tabulated what PR #463 would change and named
`pr/463:src/lib/layoverEntryEligibility.ts` as the module. That module is not
what landed: the gate was rebuilt on `LeaveAdviceFacts` so the eligibility fact
travels with every other certified fact instead of becoming a second
mechanism beside them. Two of that block's cells are wrong at this commit and
are corrected here rather than left to read as current:

- **L48 → C.** It is `W`; the forbid half the row names is not built (45.3).
- **L230 → C.** It is `N`, for the same reason.

The rest of that block's rows are unaffected by this pass and are not re-graded.

### 45.6 What this section does NOT do

It does not re-measure census-layover, re-declare `head_commit`, curate a single
corridor, forbid a single recommendation, or grade any row other than L34 and
L48. It applies no migration and flips no flag —
`passport_entry_intelligence_enabled` was already TRUE in production when the
correction header measured it on 2026-09-07, which means the gate is live for
production traffic the moment this ships, answering `no_data_for_corridor` for
every corridor until one is curated.

### 45.7 Tally

`check:census-integrity` now reads **C=76 W=125 N=95 X=0** across 296 rows,
denominator 296. Two rows moved, both out of `N`, and they moved to different
places: L34 to `C`, L48 to `W`.

| Measure | §26 | §45 |
| --- | ---: | ---: |
| BUILT-AND-CORRECT | 75 | **76** |
| BUILT-BUT-WRONG | 124 | **125** |
| NOT-BUILT | 97 | **95** |
| CANNOT-VERIFY | 0 | **0** |
| CONSTRUCTED% | 67.2 % | **67.9 %** |
| CORRECT% raw | 25.3 % | **25.7 %** |

**Restated from the rows, not from arithmetic on the previous headline** —
`check:census-integrity` read C=76/W=125/N=95 off the table and refused the
`C=75 … N=97` headline until this section was written.

The shape of the move is worth naming, because a pass that moves a row to `W`
looks like a worse outcome than one that moves it to `C` and is not. Both rows
were `N`: nothing existed. One is now a type that says exactly what §4.1 asks
for. The other is a mechanism that reads the fact §6.1 turns on and stops short
of the prohibition §6.1 states, **for a reason written down in 45.3 rather than
for no reason**. The row it leaves is a row a curation pass can close. An `N`
was not.

### 45.8 What this pass edited ABOVE the append-only line, and why

**APPEND-ONLY. Nothing above §17's line is edited by this section**, except
displaced citations repointed by exact original line text — the precedent §17.10
set, for the same reason: this pass moved lines in
`LayoverSafetyEngine.ts`, `LayoverCompassService.ts`, `LayoverFeasibility.ts`,
`LayoverSnapshot.ts`, `routes/airport.ts`, `lib/entryRequirements.ts` and three
more, and a citation left at its old number points at whatever moved into it,
which is worse than a citation that is merely old.

210 citations were repointed across all thirteen censuses, mechanically: an
old→new line map built with `difflib` over the committed content against the
working tree, so a citation follows **the line it named** rather than a guessed
offset, and a line that was EDITED is reported rather than silently moved. Two
were reported. Both are in census-passport's P59 and both were resolved by hand
(the row's own correction note says which and why); one of them named a comment
this pass DELETED, and it was replaced rather than repointed, because the only
surviving occurrence of that phrase is a comment *about* the deletion.

Five citations of `isSafetyEnabled` in `routes/airport.ts` named lines 999-1000
and now name 1003-1004, the two lines that text occupies at this commit — a
range needs BOTH ends moved and the mechanical pass only moves the start, so it
left an inverted range behind and those five were corrected by hand.

Eight citations of `LayoverFeasibility.ts` in THIS file were repointed a second
time, by hand, after the mechanical pass had already moved them — and moved them
wrong. The map that pass used was built against an intermediate working tree,
before the verdict cap was added to `certifyFeasibility`, so every citation
below the cap was short by the lines the cap occupies: `certificationHeader`
went to 740 when it lives at 765, `airportIntelligence` to 841 when it lives at
866. `check:doc-citations` caught all eight, which is the case it exists for.
The correction is the same map rebuilt against the final tree and applied by the
anchor text, so each citation again names the line its anchor is on. **A
mechanical repointing is only as current as the tree it was measured against;
re-run it after the last source edit, not before.**

Not repointed, and named here rather than left silent: this file carries bare
`LayoverFeasibility.ts:NNN` citations — `:103` and `:183` for
`ESTIMATE_CONFIDENCES` and `worstConfidence` in L4 and L37, `:484` in L1, `:553`
in L191 and in §13's table, and the sibling `:182`, `:493`, `:541`, `:449-450`
and `:352-362` beside them — which do not name the line their claim is about,
and **did not before this pass either**. At the commit this pass started from,
`:493` was a routing lookup and not the confidence fold it is cited for, `:352`
was an airport timezone and not the "does NOT forbid" comment, and `:183` was a
percentile field and not `worstConfidence`. They are bare, so no check can see
it: `check:doc-citations` verifies an anchor, and these have none. This pass did
not move them and did not fix them — repointing a dozen citations whose claims
would each have to be re-read belongs in the recount of those rows, not in a
commit about the entry gate. They are recorded here so the next recount does not
have to rediscover them.

`check:doc-citations` is clean at this commit. Measured rather than assumed:
every `path:line` in every document was cross-checked against this diff's
new-side line numbers, and the citations that land inside a changed hunk are
P59's two in census-passport, corrected where they stand, and the eight above.

### 45.9 Seven tests asserted the pre-gate answer. What happened to each.

A change that makes existing tests fail is the case where a census is most
easily falsified quietly: the fastest way to green is to edit the assertion
until it matches the new behaviour, and that erases the record of what the
behaviour used to be. Every one of these was read before it was touched, and
none was weakened to get green.

| Test | Was | Now | Why |
|---|---|---|---|
| `layoverMaturityModel` §22 L243 | `verdict === "yes"` | `verdict === "entry_unverified"` | The row is the DISAGREEMENT between the maturity gate and the engine, and the disagreement is untouched: the gate still forbids landside at L0 and the engine still permits it. Only the word changed. |
| `layoverFeasibilityRecord` certified header | a four-string array of verdicts | `Record<LeaveAdvice["verdict"], true>` | The array silently omitted the new member and the endpoints failed at runtime against a verdict the test called impossible. Keyed by the union, leaving one out is now a type error. |
| `layoverReturnState` reason codes | roomy session ⇒ `"yes"` | roomy session **with a permitted corridor** ⇒ `"yes"` | The case is about INSUFFICIENT_USABLE_TIME. Asserting the new verdict instead would have made it pass for a reason unrelated to usable time; staging the corridor keeps the clock's question and the border's question apart. |
| `layoverReplayDeterminism` positive control | pre-state `"yes"` | pre-state `"entry_unverified"` | The control needs the forged value to DIFFER from the real one. `"no"` still does. The test's own comment already warned about this class of mistake. |
| `layoverScenarioMatrix` L221 / L223 | `"yes"` | `"yes"` with a corridor, **plus** a new case asserting `entry_unverified` without one | The rung is about a window that permits landside. Both halves are now stated instead of one being implied. |
| `layoverScenarioMatrixDisruption` L230 | `"yes"`, and a control sweeping `inputs.airport` / `inputs.session` for an entry field | four cases: disclosure, withdrawal of the caveat on a confirmed corridor, `entry` as a named input, and the absent prohibition | The old control still passed — and that was the defect. `entry` arrived as a TOP-LEVEL input, so the sweep no longer looked where the answer was. A check that cannot see the thing it checks is worse than no check. |
| `layoverUnmeasuredJourney` HTTP `overallRating` | four-verdict map | `entry_unverified → possible_but_risky` | Deliberately DIFFERENT from the map over the raw engine in the same file, which still says `safe`. `assessWindowOnly` reads a clock and knows nothing about borders; `certifyFeasibility` caps the rating at what the verdict allows. The two maps disagreeing is the evidence the cap works. |

One guard also needed a declaration rather than a test change:
`check:flag-polarity` reported the gate's `isFlagEnabled(sc, ENTRY_FLAG)` as an
unresolvable flag argument. The check resolves constants within one file and
does not follow imports, and `routes/entryRequirements.ts` already carries an
`UNRESOLVABLE` entry for the identical call. The gate's entry says the same
...
floor: §45 cites four files and only one of them was in `CENSUS_SCOPE`. Measured
rather than inferred — `origin/main` reads 132 cited / 127 watched, which is
96% exactly, so this census was sitting ON its floor and §45 was simply the
...
`routes/entryRequirements.ts`, whose contents §45.9's claim about the flag
...
verdicts it moves are moved here, in §45.
...
"because nothing in this document grades it yet". §45 grades two rows and names
...
## §46 — 2026-09-23 (integration): the headline restated from the merged rows, and why two correct headlines could not both be

**No verdict moves.** This section moves nothing and grades nothing. It exists
because `check:census-integrity` refused the document, and the refusal was
right: the last headline in the file said **C=76 W=125 N=95** while the rows
underneath it read **C=82 W=144 N=70**. Both sum to 296, so this was not an
arithmetic slip — it was a headline that had stopped describing its own table.

### 46.1 What happened, stated as a fact about two branches rather than a fault

§45 was written in a lane that branched from `main`. §41–§44 were written in
this one. Neither could see the other, and **each lane's headline was correct
for the tree it was measured on**:

| measured on | C | W | N | X | stated where |
| --- | ---: | ---: | ---: | ---: | --- |
| `main` at §45's base (pre-§45) | 75 | 124 | 97 | 0 | §26 |
| `main` with §45 | 76 | 125 | 95 | 0 | §45.7 — correct there |
| this branch before the merge, without §45 | 81 | 143 | 72 | 0 | §44's lane |
| **the merged document, which is what the rows now say** | **82** | **144** | **70** | **0** | here |

The last row is not arithmetic on the row above it. It is what
`CENSUS_INTEGRITY_DUMP=ALL` counts off the merged table, and the first three
rows were measured the same way, by checking out each parent's copy of this
file and counting it. The composition is then an exact check rather than an
assumption:

> 81 C + 143 W + 72 N, plus §45's two moves — **L34 `N → C`** and
> **L48 `N → W`** — gives 82 / 144 / 70. It does, exactly. The same two moves
> take `main`'s 75 / 124 / 97 to the 76 / 125 / 95 §45.7 states.

That both additions land is the evidence that **the merge lost no verdict**.
This matters more than the headline: `§45` is now the LAST section, and this
census is append-only with last-statement-wins, so a tally table in §45 that
restated ids would have silently reverted every row §41 moved. It does not —
§45.7 is a MEASURE table (`BUILT-AND-CORRECT`, `CONSTRUCTED%`), keyed by
nothing, so it cannot supersede a row. Checked, not assumed.

### 46.2 The restated headline

`check:census-integrity` reads **C=82 W=144 N=70 X=0** across 296 rows,
denominator 296.

| Measure | §45 (on `main`) | §44 (on this branch) | **merged** |
| --- | ---: | ---: | ---: |
| BUILT-AND-CORRECT | 76 | 81 | **82** |
| BUILT-BUT-WRONG | 125 | 143 | **144** |
| NOT-BUILT | 95 | 72 | **70** |
| CANNOT-VERIFY | 0 | 0 | **0** |
| CONSTRUCTED% | 67.9 % | 75.7 % | **76.4 %** |
| CORRECT% raw | 25.7 % | 27.4 % | **27.7 %** |

**Restated from the rows, not from arithmetic on either previous headline** —
which is the same discipline §45.7 named, applied to the case §45.7 could not
have known about.

### 46.3 Two things about §45 that the renumbering left, and what was done

- §45 was written as **§27** and renumbered at integration, because §27 through
  §44 already existed here. The note at the head of §45 records that nothing but
  its number changed. Its five self-references (`§27 grades two rows`, the
  `CENSUS_SCOPE` paragraph, the tally table's column header) have been updated
  to say §45, because on this branch **§27 is a different section about crew
  operations with a different tally**, and leaving them would have pointed a
  reader at the wrong table. `checkCensusFreshness.ts` carried four of the same
  references in its `CENSUS_SCOPE` justification and has been updated with them.
- §45.7's table compares §45 against **§26**, and that comparison is left
  standing because it is true of the branch §45 was written on. It is NOT a
  delta over the section that now precedes §45 in this document: §27–§44 sit
  between them and moved rows of their own. A reader taking `§26 → §45` as
  "what the previous pass left" would be reading across a nineteen-section gap.
  The merged delta is 46.2's third column and nowhere else.

### 46.4 What this section does NOT do

It does not re-grade a single row. It does not reopen a citation. Neither lane's
verdicts were re-derived here, because a merge is not a measuring pass and the
one thing worse than a stale headline is a fresh one asserted without looking.
Where §45 and §41–§44 disagreed about a FILE rather than a row — the risk band
