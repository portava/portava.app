...
| P45 | Trust must be domain-specific, confidence-aware and **explainable** | **W** | Domain-specific: yes (`buildDomainTrust:951-972`, six domains). But when there is no trust profile the code **substitutes the neutral 50 for the overall score and for every missing category** (`:965`, `:958`) and `presentationWord(50)` returns "Established" (`:933`). `trust_engine_enabled` is seeded false, so nothing writes `trust_events`, `trust_profiles` is empty, and **every user is described as an "Established" member of the community across all six domains on the strength of a constant**. That is a claim about a person that no evidence supports, and it is the opposite of explainable. PR #467 replaces it with `applicable: false`. |
...
| P50 | Trust confidence matters — an 82 with high evidence is not equivalent to an 82 with little evidence | **W** | A confidence band exists (`buildTrust:983-985`), but it is computed from `stats.stamps + stats.trips * 2 + (verified ? 3 : 0)` — **travel volume, not trust evidence**. A user with many stamps and an empty trust profile reads `confidence: "high"` on a fabricated 50. The spec's exact hypothetical — same number, different evidence, different meaning — is not distinguished, because the number itself is a constant. |
...
| P59 | **canProvideVisaBuddyService** | **N** | A repo-wide search across the server and client trees for `canProvideVisaBuddyService`, `VisaBuddy` and `visa_buddy` returns **nothing**. The capability named by §11 does not exist in any form. |
...
| P98 | **Map** — aggregate or permission-appropriate presence only | **N** | `PassportConsumerVariant` (`PassportConsumerProjections.ts:140#PassportConsumerVariant`) has six members: discovery_card, telegraph, buddy, trips, event, safety. **There is no Map variant.** Map presence is served by `routes/mapTravelers.ts` → `lib/mapTravelers.ts:49#MapTravelerPayload`, which builds its own identity payload; §21's Map row has no Passport projection behind it. **Correction to this row's REASONING (not its verdict), 2026-09-08:** "there is no Map variant" reads like a small omission and it is not one. Two facts the original pass did not state. **(a) It is not a privacy defect.** `mapTravelers` applies the canonical universal display-name rule (`lib/mapTravelers.ts:276#nameVisibilitySet`, the same `nameVisibilitySet` the Passport assembler uses at `PassportProjectionService.ts:1788#nameVisibilitySet`) and honours `show_profile_picture_publicly` (`lib/mapTravelers.ts:299#show_profile_picture_publicly`). It rebuilds the identity, it does not leak more than the Passport would. **(b) Adding a seventh variant does NOT close it.** `buildConsumerProjection` narrows a FULL per-user assembly: `buildPassportProjection` (`PassportProjectionService.ts:1756#buildPassportProjection`) performs ~21 table reads plus `resolvePassportViewerContext`, which runs the interaction-permissions engine (~13 more reads per target). The live map is a BULK, POLLING surface — up to 100 travelers per response (`lib/mapTravelers.ts` `MAX_RESULTS`), polled every 45 s. Calling the per-user path once per traveler is ~3,400 reads per poll per viewer. Closing P98 honestly requires a BATCH projection — one that applies the same rules to N owners in a bounded number of reads — not a variant. That work does not exist, which is why this stays **N** rather than becoming a small follow-up. **Do not confuse this with `PassportMapService.ts`**, which is the Passport's own stamp map (city/neighbourhood markers from `passport_stamps`), a different surface entirely. |
