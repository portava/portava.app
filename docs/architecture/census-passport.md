...
| P98 | **Map** — aggregate or permission-appropriate presence only | **N** | `PassportConsumerVariant` (`:82-88`) has six members: discovery_card, telegraph, buddy, trips, event, safety. **There is no Map variant.** Map presence is served by a separate system (`routes/mapTravelers.ts`) that builds its own identity payload; §21's Map row has no Passport projection behind it. |
