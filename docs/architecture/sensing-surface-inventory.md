...
| `POST /map/observations` | `routes/mapObservations.ts:895` | mounted `routes/index.ts:314#router.use(mapObservationsRouter)`; the only caller of `lib/intelEvidenceCapture.ts` |
| `GET /map/projection/temporal` | `routes/mapProjectionTemporal.ts:412` | mounted `routes/index.ts:312#router.use(mapProjectionTemporalRouter)`; reads `intel_state_snapshot_versions` (`routes/mapProjectionTemporal.ts:397`) |
