...
Six intel routers are mounted in `routes/index.ts` — `routes/intel.ts` (`:291`),
`routes/intelCoverage.ts` (`:293`), `routes/intelApi.ts` (`:294`), `routes/intelReadModels.ts`
(`:295`), `routes/intelOutcomes.ts` (`:298`) and `routes/intelObservability.ts` (`:300`) — carrying
...
| `GET /map/projection/temporal` | `routes/mapProjectionTemporal.ts:412` | mounted `routes/index.ts:263`; reads `intel_state_snapshot_versions` (`:397`) |
