...
**all NOT-BUILT.** The nearest artifact is `services/media/MediaProjectionService.ts:1378#{ key: "gems", label: "Hidden Gems", ownerOnly: false, count: gemsCount, media: [] },`, a
...
(`artifacts/api-server/src/routes/highlights.ts:1780#deleted_at`, a soft delete, not a reversible hide) — **11 BBW**.
...
1. **`highlights` is a Stories table wearing this spec's noun.** §1 lists "building an Instagram
   ...
   Episodes*. `artifacts/api-server/src/routes/highlights.ts:791#media_url: d.mediaUrl` creates a highlight from a raw `mediaUrl` with no source.
...
| `highlights/**` — ranking, lifecycle, projection policy, resurfacing, revocation | **Three of five are wired** | Wired: `highlightResurfacing` (`routes/highlights.ts:18`), `highlightProjectionPolicy` (`:23`), `highlightRevocation` (`:24`, executed `routes/highlights.ts:1740#router.delete("/highlights/:id"`). **Not wired:** `highlightRanking.ts` and `highlightLifecycle.ts` — test-only. |
...
| `highlights.archived_at` wired as a reversible archive | **Yes, and it is live** | `artifacts/api-server/src/routes/highlights.ts:91#archived_at` (projected), `:1877#/highlights/:id/archive` archive, `:1908#/highlights/:id/archive` unarchive, `:1945#/highlights/archived` `GET /highlights/archived`, and `.is("archived_at", null)` on the three list reads (`:956#archived_at`, `:1152#archived_at`, and the following-feed at `:2468#archived_at`). **`archived_at` exists on `public.highlights` in production** (schema snapshot), so this is the one Highlights change in this range that a deployed database can actually hold. |
...
| H81 | NB | **BBW** | `clampLocationToPrecision` (`highlightProjectionPolicy.ts:165`) and `resolveLocationDisclosure` (`:205`), wired into the feeds (`routes/highlights.ts:23`, applied at `:306#applyLocationPrecision`). Unenforced today for exactly the reason the file states: no policy table. | spec |
...
2. **Seven of the nine invariants hold on code no route imports.** Only **H240** and
   **H241** sit on modules a route imports: `artifacts/api-server/src/routes/highlights.ts:40#import`
   pulls in the revocation service and `artifacts/api-server/src/routes/highlights.ts:306#function`
...
| H145 | `HIDE_HIGHLIGHT` | BBW | A reversible archive now exists on a column production has (`highlights.archived_at`, projected at `artifacts/api-server/src/routes/highlights.ts:91#archived_at`, and the three list reads filter it), so the body's "a soft delete, not a reversible hide" is out of date. Still no command boundary |
...
| H241 | Invariant: public location precision cannot exceed owner policy | **BAC** | HELD over the whole ladder: the disclosed field set is monotone toward HIDDEN, HIDDEN discloses nothing, `strictestPrecision` can only tighten, and both an unreadable policy row and an unparseable one clamp to HIDDEN. The module is imported by `artifacts/api-server/src/routes/highlights.ts:306#function`. Mutation 4 turned it red. **Ceiling: 2721 is unapplied, so no rung is ever stored and the clamp never runs** |
...
2. **`memory_search` is not §15.** It is a bounded scan of canonical rows with in-process
   ...
   to infer it.
3. **`getMemoryEvidence` has nothing to read.** §3.6's `memory_evidence` is WRITTEN AND UNAPPLIED:
   `2320_memory_episode_provenance_spine.sql:227` creates it, 2320 is applied nowhere, so the table is absent from production. *(CORRECTED 2026-09-22. This read "has no migration in this tree at all — not written-and-unapplied, absent", which was the opposite of the truth and was inherited by H24's row. H24 is unmoved and H120 is still BBW, for the corrected reason.)* H24 is unmoved and H120 is BBW because of it.
...
`artifacts/api-server/src/routes/airport.ts:3712#sourceType: "layover_session", verificationLevel: "checkin",`.
...
`artifacts/api-server/src/routes/airport.ts:3686#const occurrence = declaredOccurrenceHasHappened(args.session.arrivalTime, Date.now());`,
the fourth term of `artifacts/api-server/src/routes/airport.ts:3649#async function writeElectedLayoverStamp`,
...
| §10 clamp on the third Highlight read | `artifacts/api-server/src/routes/highlights.ts:1068#applyLocationPrecision` | Matches the two existing call sites exactly. **No owner bypass was introduced**, because neither existing call site has one — `GET /highlights/active` clamps the viewer's own Highlights — while the Memory sibling `protectMemoryRow` does bypass. Matching what exists can only narrow disclosure; inventing a bypass on one of three routes would widen it |
...
## M. Two rows the document said were absent and are not, thirteen greens that rest on a credential this repository cannot read, and a single read that reported a Memory as empty — 2026-09-15, the HIGHLIGHTS & MEMORIES lane

Every OLD verdict below was read from `CENSUS_INTEGRITY_DUMP=ALL`, never from this document's
prose. That is not a formality here: **108 of this census's 266 rows have been revised by a later
recount**, more than almost any other census in the corpus, so a body-table verdict is the least
reliable statement about a row that this file contains. Two of the three rows this section touches
were last stated in the BODY, at `ebe72b34`, and had not been looked at since.

**`head_commit` is NOT re-declared.** This section re-measures three rows and reads around a
fourth; it does not re-measure 266. The acknowledgement in
`artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json` was EXTENDED to cover the one
counted file this section changed, with the argument for why it cannot move a verdict.
...
Assignment rule is §D.1's, unchanged: **the first thing standing between the row and CORRECT.**
For the 129 `W` rows this is §F.8's grouping re-checked against the dump rather than taken on
trust; for the 74 `N` rows and the 2 `X` rows it is assigned here for the first time — no earlier
section ever grouped them, which is why the largest bucket in this census had never been counted.

| group | rows | what it means |
|---|---:|---|
| **(a)** closable from code this lane owns | **10** | H6, H10, H189, H190, H193, H202, H104, H105, H198, H1. Nothing external stops any of them; the remedy is typing in `routes/memories.ts`, `services/memory/**` and `services/memoryProjections/**` |
| **(b)** blocked on an unapplied migration or an absent column | **117** | 102 of the `W` rows and 15 of the `N` rows. Migrations 2710, 2711, 2720, 2721, 2722, 2723, 2724, 2730, 2338, 2339 — written, and applied to no database |
| **(c)** blocked on an owner decision, named | **4** | H84 → **D6** (`highlights` and its four children sit in `UNCLASSIFIED_BACKLOG`); H120, H123, H126 → **D-C2** |
| **(d)** another lane's file | **2** | H4 and H239 → `services/passport/StampAwardEngine.ts`, `services/passport/PassportMapService.ts`, `routes/trips.ts`, `routes/hiddenGems.ts`, `lib/stamps/criteria/metrics.ts` — plus migration 2970, applied nowhere (§L.4) |
| **(e)** needs something nobody has written | **70** | 11 `W`, 57 `N`, both `X`. Includes the whole §3 table family that exists in no migration at all (H23, H26, H27, H28, H33, H35, H37), every §24 metric that occurs only in this census and the spec (H211–H218, H222), and H52 and H219 |
| **(f)** the census sentence is already FALSE at HEAD | **3** | H3 and H266, both moved above; and H265, whose sentence is disproved below without its verdict moving |

**(a) is ten rows out of two hundred and five.** The honest headline of this partition is that
**117 of 205 are one `supabase migration up` away and 70 need code nobody has written**, and that
this lane — the one that owns the files — can move ten. That ratio has been stable across four
sections and is the most useful number in this document.
...
3. **H265, line 1484:** *"No summarization of Memories exists to preserve anything through."* —
   ...
   prompt and no such field on any tool result. The requirement is unbuilt; the reason given for
   calling it unbuilt was wrong.
...
### M.4 D-C2 measured around, not resolved

**D-C2 is OPEN and this section does not close it.** What it does is state the chain link by link
and then answer, for each of the thirteen rows §E.7 enumerates, the only question a reader has.

The chain, every link opened at HEAD:
...
So the answer is the same for all thirteen, and it is not a hedge: **if the credential is absent in
production, none of these thirteen rows has ever executed there.** They are marked `⌀` — the
vacuity flag this corpus already uses — rather than moved, because vacuity is a property of the
deployment and not of the code, and this lane may not read the deployment.

| id | verdict | vacuous if `AI_INTEGRATIONS_OPENAI_API_KEY` is absent? |
|---|---|---|
| H115 | **C ⌀** | YES — `memory_get` is reachable by a model tool call and by nothing else |
| H116 | **C ⌀** | YES — `memory_search`, same |
| H117 | **C ⌀** | YES — `memory_get_shared`, same |
| H118 | **C ⌀** | YES — `memory_get_place_history`, same |
| H119 | **C ⌀** | YES — `memory_get_trip_memories`, same |
| H121 | **C ⌀** | YES — `memory_create_draft`, same |
| H122 | **C ⌀** | YES — `memory_suggest_correction`, same |
...
**H207 is NOT in this list and must never be added to it.** §23's `canPublishMemory` is wired to
`POST /memories` and `PATCH /memories/:id` and is reached by an ordinary HTTP request with no model
...
### M.5 The §K.4 standard, applied

§K.4 wrote a five-link chain and verified four, and §L.1 recorded the cost: *"A five-link chain is
only as verified as its weakest link, and I verified four."* Taking that as the rule:

- **This section grades NO row `C`.** There is therefore no multi-step green here to audit, which
  is the cheapest possible way to comply and is said plainly rather than dressed up.
- The two `W` grades above still have every link named and opened, in M.1 and M.4, because a `W`
  built on an unread link is just as wrong — it was §K.4's *unread* link, not its verdict, that
  had to be withdrawn.
- **The link this section did NOT open, named:** whether any SCREEN reaches the Compass ask route.
  The mobile-reachability ledger is pinned per §H.7 and was not re-derived, and M.3 item 4 shows
  that ledger is carrying at least one entry that resolves onto the wrong function. If no screen
  reaches `POST /compass/ask`, the thirteen rows above are vacuous for a second, independent reason
  and D-C2 is not the only thing endangering them.

### M.6 The live defect: a Memory that has photographs, reported as having none

`GET /memories/:id` read six tables in one `Promise.all` and bound **none** of their errors.
supabase-js RESOLVES on a database error, so `items.data` and `tags.data` were `null` for an
unreadable table and the `?? []` beneath turned that into a 200 saying the Memory has no
...
empty item list is — but they now bind and log their errors, so the outage is visible somewhere.

**THE FIRST RED, verbatim** (`src/test/memoriesSingleReadDegraded.test.ts`, before the fix):

```
not ok 1 - answers 503 degraded_unavailable when `memory_items` cannot be read, ...
  error: |-
    expected degraded_unavailable, got 200 {"memory":{ ... "items":[],"tags":[{"userId":"bbbb…
    200 !== 503
# pass 2 # fail 2
```

— with one row in `memory_items` in the fixture. GREEN after: `# pass 4 # fail 0`.

**Every mutation, with kill/survive.**

| mutation | result |
|---|---|
| A. drop `tags.error` from the guard | KILLED — 1 fail |
| B. drop `items.error` from the guard | KILLED — 1 fail |
| C. make the engagement reads refuse too | KILLED by the asymmetry case — 1 fail |
| D. refuse, but still ship a `memory` body | KILLED — 1 fail |
| E. delete the engagement observation loop entirely | **SURVIVED** |
| F. log the engagement failure without binding the table name | KILLED — after the repair below |

**E is the one worth reading.** Deleting the whole observation loop left all four cases green,
because *"does not refuse"* is satisfied just as well by a handler that never looked at the error
at all. The suite was measuring the absence of a refusal and calling it observability. The repair
asserts on SHAPE rather than on wording — it binds `table === "memory_likes"` out of the log object
rather than matching any part of the message, so the assertion cannot be satisfied by the fix's own
prose. E and F are both killed now; F exists only because E revealed the hole.

**What this did NOT find.** Every privacy and visibility read in this lane's files was re-scanned
first, because a failed read on that path fails OPEN, and **every one of them already binds its
error**: the six `blocks` reads in `routes/highlights.ts` and `routes/memories.ts`, both
`circle_memberships` reads, `sharesAcceptedTrip`, `acceptedMemberIdsOfTrip`,
`services/memory/memoryReadPolicy.ts`, `canSeeParticipant`,
...
like reads), `routes/highlights.ts:1034#viewedRows`, `:1052#avatar_url`, `:2581#profileRows` (view/like counts and the author profile).
...
- `docs/architecture/census-telegraph.md` — TWO citations, at its lines 1897 and 1935, both
  ...
  file before this change and exactly once after, and the read-back was done before writing. No
  telegraph verdict was read or moved. Without this repoint `check:doc-citations` goes from
  `RESULT clean` to `RESULT failed — broken anchors 2`, caused entirely by this lane's line shift.
- `artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json` — this census's OWN
  acknowledgement entry, extended with the one file this section changed and the argument for why
  it cannot move a verdict. **It was extended rather than added beside**, because
  `checkCensusFreshness.ts` resolves an acknowledgement with `acks.find` and reads only the FIRST
  entry per census — a second entry is silently ignored, which this lane discovered by writing one.

**Handed over, NOT edited:**

- `docs/architecture/census-telegraph.md`'s staleness entry. `routes/memories.ts` is a counted file
  for that census, so `check:census-freshness` now reports it STALE — 4 problems before this
  section, 5 after, and the fifth is this. The exact remedy: in
  `CENSUS_STALENESS_ACKNOWLEDGED.json`, add `"artifacts/api-server/src/routes/memories.ts"` to the
  `files` array of the FIRST entry whose `census` is `census-telegraph.md` (a second entry will not
  be read), with a reason. The argument that lane may use, offered rather than asserted on their
...
- **H3 → back to `N`** if the owner's bar is that a PROMPT is not a guard. `MEMORY_COMPASS_PROMPT_RULES`
  is advisory text, and this document already says so about H126. The move to `W` rests on the
  mechanical half — approved-tag-only participants, exact-handle identity, an app-composed past-tense
  claim — being real. Delete any one of those three and H3 is `N` again.
- **H266 → back to `N`** if a renderer must render the DATA. A static apology is a fallback and is
  arguably not a renderer at all. This is a product ruling, it is offered here rather than taken,
  and it is the single most contestable judgement in this section.
- **The thirteen `C ⌀` rows → vacuous, and raw CORRECT% from 22.9 % to 18.0 %** (48 of 266) if
  D-C2 resolves to "not set". The figure is stated so the decision can be taken with its size
  visible; it is NOT applied and no row is moved on it here.
- **The thirteen → vacuous for a SECOND reason** if no screen reaches `POST /compass/ask`. That was
  not measured (M.5).
- **M.6's guard → wrong** if a client depends on `GET /memories/:id` answering 200 with an empty
  `items` array during an outage. Nothing in this tree does; a client outside it might, and a 503
  where a 200 used to be is a contract change on an error branch.
- **The whole partition in M.2 → wrong** if §F.8's grouping of the 129 `W` rows no longer
  reproduces. It was re-checked against the dump, but the (b) count of 102 is derived by
  SUBTRACTION from that grouping rather than re-assigned row by row, and a reader who wants the
  per-row list must read §D.1 and §F.8 together.

### M.9 The recomputed headline

Restated from `check:census-integrity`, not counted by hand.

| figure | section L | **now** |
|---|---:|---:|
| Denominator | 266 | **266** |
| BUILT-AND-CORRECT | 61 | **61** |
| BUILT-BUT-WRONG | 129 | **131** |
| NOT-BUILT | 74 | **72** |
| CANNOT-VERIFY | 2 | **2** |
| CONSTRUCTED% | 71.4 % | **72.2 %** (192 / 266 = 72.2 %) |
| CORRECT%, raw | 22.9 % | **22.9 %** (61 / 266 = 22.9 %) |
| **the gap** | **48.5 points** | **49.3 points** |

**CORRECT did not move and CONSTRUCTED did, which is the shape of an honest reading pass.** Nothing
was built into a green here. Two rows the document filed as NOT-BUILT turned out to have been built
...
## O. The blocker on 117 rows was removed on 2026-09-15, ten rows moved, and none of them moved to CORRECT — 2026-09-15, the HIGHLIGHTS & MEMORIES lane

Every OLD verdict below was read from `CENSUS_INTEGRITY_DUMP=ALL`, never from this document's
prose, per §M's rule and for §M's reason.

**`head_commit` is NOT re-declared.** This section re-measures eleven rows and reads around the
rest; it does not re-measure 266. Two entries were ADDED to
`artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json` — this census's, naming all
eight counted files this section changed, and `census-telegraph.md`'s, which §M.7 handed over as a
request and which was never applied. The telegraph entry is named loudly in O.8.

### O.1 The sentence this document repeats more than any other is false at HEAD

§M.2 partitioned the 205 non-correct rows and produced this census's headline number:

> **(b)** blocked on an unapplied migration or an absent column — **117** — *"Migrations 2710,
> 2711, 2720, 2721, 2722, 2723, 2724, 2730, 2338, 2339 — written, and applied to no database."*
> … **"117 of 205 are one `supabase migration up` away"**

**Somebody ran it.** `artifacts/api-server/src/lib/capability/production-applied-migrations.json:183#2710_memory_command_kernel_tables`
records 2710 applied to production, and the same file records 2720 at `:171#2720_highlight_resurfacing_preferences`,
2721 at `:175#2721_highlight_projection_policies`, 2722 at `:179#2722_highlight_sources`,
2730 at `:187#2730_memory_derivative_registry` and 2723 at `:199#2723_highlight_class_lifecycle_and_pin` —
**all ten, all on 2026-09-15**, in the squash merge of PR #482, which is the very commit this
census declares as its `head_commit`.

An entry in a list is bookkeeping, so the SCHEMA was read as well.
`artifacts/api-server/src/lib/capability/snapshots/20260915-production-schema.json` is a generated,
read-only capture of `information_schema.columns` on `travel-buddy (PRODUCTION)` with watermark
`20260915083533`, and it holds every one of the objects:
`:1885#highlight_resurfacing_preferences`, `:1857#highlight_projection_policies`,
...
deployed"*, so one renamed column would silently return an enforced §11 control to being a no-op
with no test failing anywhere. A census sentence cannot be re-checked by a build; that file can.

**19 assertions, 7 mutations, all killed.** 2720 removed from the applied list → 1 fail. The 2720
table removed from the snapshot → 2. One probed column renamed → 1. A gating flag flipped ON → 1.
`highlights` given a trip reference → 1. 2970 added to the applied list → 2. The snapshot watermark
rolled back behind the list → 1.

### O.2 And the rows still did not become CORRECT, which is the finding

**No row in this section moves to `C`.** That is not caution; it is the measurement. The migration
was never the only blocker, and the second one was never named anywhere in §A–§N:

**Nothing writes either control table.** A repository-wide grep for `insert`, `upsert` or `update`
against `highlight_resurfacing_preferences` and `highlight_projection_policies` returns **nothing
outside `src/test/`.** There is no route, no service and no script by which a user can set a §11
resurfacing control or a §10 precision rung. Both tables are deployed and **empty**, and they will
stay empty.

So the §10/§11 enforcement this repository built is live, correct, route-tested and **unreachable
by any user action** — which is a different sentence from "the migration has not been run", and a
worse one, because running a migration is a command and building a preferences surface is product.

The rows this bears on, and what each would now need:

This table RESTATES no verdict — the `standing` column is spelled out rather than lettered so
that nothing here is read as a row move. Every row below keeps the verdict the dump gave it.

| id | standing | the blocker AFTER 2026-09-15 |
|---|---|---|
| H81 | BUILT-BUT-WRONG | `clampLocationToPrecision` is wired into all three Highlight reads and route-tested; no route lets an owner SELECT a rung, so the stored value is always absent |
| H82 | BUILT-BUT-WRONG | same surface, same reason. `strictestPrecision` can only tighten and is proved so by §25's H241 |
| H89 | BUILT-BUT-WRONG | `HIDE_PERSON_FROM_RESURFACING` is enforced on both proactive feeds and route-tested; no route lets a user set it |
| H91 | BUILT-BUT-WRONG | `KEEP_PRIVATE_FOREVER` likewise, and it gained its first route-level coverage in this section |
| H200 | BUILT-BUT-WRONG | the publication policy is read on every feed; nothing writes a policy |
| H201 | BUILT-BUT-WRONG | two blockers, one removed. The ladder is applied on a durable public surface AND 2721 is deployed; it still does not reach a Memory's `location_lat` / `location_lng` |
| H75 | BUILT-BUT-WRONG | blocker CORRECTED: not 2721, which is applied, but that `mayProject` has **no production caller at all** — the only non-test reference in the tree is a comment |
| H210 | BUILT-BUT-WRONG | `personalization` is a named suppressible surface and no personalization path consults it. Unchanged by the deployment |
| H260 | BUILT-BUT-WRONG | no anniversary model and no fatigue model. Unchanged by the deployment |
| H103 | BUILT-BUT-WRONG | blocker CORRECTED: 2339 is applied, so the flag ROW now exists — and `highlights_feed_bounded_enabled` reads `false` in the snapshot's `flags` block, so `isFlagEnabled` still fails closed |
| H1, H76, H79 | BUILT-BUT-WRONG | blocker CORRECTED: 2338 is applied; `memory_location_precision_enabled` and `memory_public_feed_projection_enabled` both read `false` |
...
**I did not build a preferences route to close these.** §E.5 already ruled on that shape —
*"Inventing an endpoint to move two rows is scoring"* — and a `PUT /highlights/:id/resurfacing`
written to make H89 green would be exactly that.

### O.3 Row moves

| **ID** | **was** | **now** | why |
|---|---|---|---|
| **H29** | **N** | **W** | `memory_commands` is deployed as `memory_command_receipts` — the row's only stated blocker was *"which is **not applied**"*, and it is applied (`production-applied-migrations.json:183#2710_memory_command_kernel_tables`, snapshot `artifacts/api-server/src/lib/capability/snapshots/20260915-production-schema.json:2753#memory_command_receipts`). Not `C`: the receipt is written by the KERNEL path and `memory_kernel_enabled` is `false`, so nothing has ever written one; and the name still differs from §3's |
| **H30** | **N** | **W** | `memory_event_outbox` is deployed under §3's **exact** name (snapshot `:2798#memory_event_outbox`), with `published_at`, `attempts` and `last_error`. The row's blocker read *"unapplied, absent from the snapshot"* and both halves are now false. Not `C`: nothing writes it and nothing consumes it (H161) |
| **H32** | **N** | **W** | `highlight_sources` is deployed (snapshot `:1904#highlight_sources`). Not `C` on the row's own second clause, which survives: *"with no TypeScript writer"*. `POST /highlights` still inserts a client-supplied `mediaUrl` and no Highlight has a source Memory |
| **H34** | **N** | **W** | `memory_derivative_registry` is deployed (snapshot `:2764#memory_derivative_registry`) **and is reached in production**: `DELETE /memories/:id` runs `runMemoryDeletionLifecycle`, whose step 3 calls `revokeDerivativesForMemory` against it — see `artifacts/api-server/src/services/memory/memoryDeletionLifecycle.ts:224#which was applied to production on 2026-09-15`. Not `C`: the READ paths still build per request and register nothing |
| **H90** | **W** | **W** | **NOT MOVED, and restated because this section falsified its evidence.** `HIDE_TRIP` is declared, storable, and now ENFORCED on both live feeds — as a withholding. It is not §11's HIDE_TRIP, which hides ONE trip, and it cannot be: **`public.highlights` carries no trip reference at all**, 22 columns and not one of them (snapshot `artifacts/api-server/src/lib/capability/snapshots/20260915-production-schema.json:1918#highlights`). Built and wrong was already this row's verdict and stays it; what changes is that the blocker it named — *"storage is 2720, unapplied"* — is gone and was never the real one |
| **H92** | **N** | **W** | the row's evidence — *"`memory_projections.state='hidden'` hides **and** de-personalizes; the two are not separable"* — is FALSE at HEAD. `CONTROL_EFFECTS.RETAIN_BUT_DO_NOT_PERSONALIZE` suppresses `personalization` and nothing else, `DO_NOT_RESURFACE` suppresses `proactive_resurfacing` and nothing else, and the live feed serves a Highlight carrying the first (asserted in `highlightConsentPolicy.test.ts`). Storable since 2720 landed. Not `C`: no personalization path consults it, which is H210's ceiling and is this row's too |
| **H94** | **N** | **W** | `highlights.lifetime_class` EXISTS in production (snapshot `:1936#lifetime_class`), so `representableLifetimeClasses` no longer answers "the column does not exist" for LIVE. Not `C`: nothing writes the column and `HIGHLIGHT_COLUMNS` does not project it, so `describeHighlightLifetime` still answers `unavailable` on every live read — for the *not projected* reason now, which the function already distinguishes |
| **H95** | **N** | **W** | same column, same reason, for DAY |
| **H96** | **N** | **W** | same column, same reason, for TRIP |
| **H97** | **N** | **W** | same column, same reason, for SEASONAL |
| **H98** | **N** | **N** | **DELIBERATELY NOT MOVED, and this is the one a reader would expect to move with its four siblings.** PERMANENT is not representable no matter what `lifetime_class` says, because `highlights.expires_at` is NOT NULL and a PERMANENT Highlight is one with no expiry. Migration 2723 **deliberately did not change that** — its own header says so at `artifacts/api-server/src/migrations/2723_highlight_class_lifecycle_and_pin.sql:33#IT DOES NOT MAKE`. Four of five moved; the fifth is structurally impossible and stays `N` |

### O.4 The live defect: §M.6's Memory-with-no-photographs, found again on the three paths §M.6 did not read

§M.6 bound the `memory_items` error on `GET /memories/:id` and stated the rule it was binding it
under: *"a wrong like count is not a claim about what happened, an empty item list is."* **The same
table is read by three other handlers in the same file and none of them bound it.**

- `enrichMemories` — the single serialization point for `GET /memories` **and**
  `GET /users/:userId/memories` — read `memory_items` for each row's cover and discarded the error.
  supabase-js resolves on a database error, so `coverRows.data` was `null` and the `?? null`
  beneath turned an outage into a 200 saying **every Memory on the page has no photograph.**
- `GET /trips/:tripId/memory` did the same for its single cover.
- The saved-collection lookup on `GET /memories` was a bare `try { … } catch { /* non-fatal */ }`
  with BOTH `error`s discarded, so an unreadable `collections` made every Memory read
...
```
not ok 1 - GET /memories refuses with degraded_unavailable rather than a null cover
not ok 3 - GET /users/:userId/memories refuses with degraded_unavailable rather than a null cover
not ok 5 - GET /trips/:tripId/memory refuses with degraded_unavailable rather than a null cover
not ok 1 - GET /memories serves the page with a zero like count when `memory_likes` is unreadable
not ok 2 - GET /memories reads `isSaved` as false when the saved-collection lookup fails, and says so
# pass 4 # fail 6
```

GREEN after: `# pass 10 # fail 0`.

**Every mutation, with kill/survive.**

| mutation | result |
|---|---|
| A. drop the `memory_items` guard in `enrichMemories` | KILLED — 2 fail |
| B. drop the `memory_items` guard in `GET /trips/:tripId/memory` | KILLED — 1 fail |
| C. make the engagement reads refuse too | KILLED — 2 fail |
| D. delete the observation loop in `enrichMemories` entirely | KILLED — 2 fail |
| E. log the engagement failure WITHOUT binding the table name | KILLED — 2 fail |
| F. discard the `collections` error again | KILLED — 1 fail |
| G. refuse, but still ship a `memories` body | KILLED — 1 fail |
| H. always refuse (breaks the positive controls) | KILLED — 6 fail |

**E and F exist because of §M.6's mutation E**, which found that *"does not refuse"* is satisfied by
a handler that never looked at the error. Every degrade case here asserts the LOG SHAPE — `table`
bound out of the log object — so no assertion can be satisfied by the fix's own prose.

**What this did NOT find.** The remaining content reads §M.6 listed are now four, not seven:
`routes/highlights.ts` still discards `viewedRows`, `avatar_url` and `profileRows` on the two
proactive feeds (view/like counts and the author profile). They are engagement and identity, not
history, and they are left for the next pass with that reasoning rather than silently.

### O.5 The second live defect: a §11 control that loaded, was recognised, and was never asked about

`applyResurfacingControls` held a hand-typed list, `["DO_NOT_RESURFACE", "KEEP_PRIVATE_FOREVER"]`,
beside a `CONTROL_EFFECTS` table that names a **fourth** control suppressing the same surface.
`isResurfacingControl` accepts `HIDE_TRIP`, so a stored row loaded into a `ready` set, the filter
never consulted it, and the Highlight was resurfaced with a 200 and no log line. Until 2026-09-15
that cost nothing — the table did not exist and every probe answered `absent`. It stopped being
hypothetical the day 2720 landed.

The list is now DERIVED: `artifacts/api-server/src/services/highlights/highlightResurfacing.ts:229#export const FEED_ENFORCEABLE_CONTROLS`
...
rather than skipping them (`artifacts/api-server/src/routes/highlights.ts:277#const unenforceable = unenforceableControls(set)`),
...
**THE FIRST RED, verbatim** (`src/test/highlightsUnenforceableControls.test.ts`):

```
# SyntaxError: The requested module '../services/highlights/highlightResurfacing.js'
  does not provide an export named 'FEED_ENFORCEABLE_CONTROLS'
# pass 0 # fail 1
```

GREEN after: `# pass 8 # fail 0`, with `highlightConsentPolicy.test.ts` unmodified at
`# pass 47 # fail 0`.
...
| mutation | result |
|---|---|
| 1. drop the unenforceable-control check — the original fail-open | KILLED — 2 fail |
| 2. withhold, but do not name the control in the log | KILLED — 1 fail |
| 3. key every control on the Highlight id (loses HIDE_PERSON) | KILLED — 1 fail in `highlightConsentPolicy` |
| 4. call the trip-scoped control enforceable | KILLED — 3 fail |
| 5. name recap/personalization-only controls unenforceable too | KILLED — 2 fail |
| 6. enforce EVERY control on the feed | KILLED — 1 fail |
| 7. stop suppressing entirely | KILLED — 1 fail |

Mutations 5 and 6 are the pair that matters: without both, "derive from CONTROL_EFFECTS" could mean
"enforce everything in the table", which would let `DO_NOT_INCLUDE_IN_RECAPS` empty a feed it has
no business touching.

### O.6 The rows that did NOT move, and were mutation-proved anyway

Five rows were drafted `W → C` on the strength of the deployment and **withdrawn** for O.2's
reason. Their surfaces were mutation-tested before the withdrawal, and the results are recorded
because they are the evidence for what these rows would be worth if a preferences surface existed:

| row | mutation | result |
|---|---|---|
| H89 | `HIDE_PERSON_FROM_RESURFACING` stops suppressing `proactive_resurfacing` | KILLED — 1 fail |
| H89 | `HIDE_PERSON_FROM_RESURFACING` re-scoped from `person` to `highlight` | KILLED — 1 fail |
| H91 | `KEEP_PRIVATE_FOREVER` stops suppressing `proactive_resurfacing` | KILLED — 2 fail |
| H81 | `COUNTRY` stops stripping the venue and city | KILLED — 3 fail |
| H81 | `HIDDEN` leaks the venue name | KILLED — 3 fail |
| H82 | `strictestPrecision` returns its first argument | KILLED — 1 fail, and 2 more in §25's H241 |
| H200 | an unreadable policy stops failing closed | KILLED — 2 fail |
| H200 | every deployment treated as "not deployed" (the pre-2721 world) | KILLED — 4 fail |

**Eight mutations, eight kills, and not one verdict moved.** A lane that wanted the number would
have moved all five.

### O.7 Comments that asserted a false thing about the database, corrected

Seven executable-comment or string sites in this lane's files told a reader that 2720, 2721, 2723
or 2730 is "NOT applied". Each is now false, and a reader trusting them would conclude a live
control is dead. Corrected, with **zero change to any executable branch**:
`artifacts/api-server/src/services/highlights/highlightSchemaAvailability.ts:19#AT THIS COMMIT BOTH ARE DEPLOYED`
(which keeps the superseded reading, because the fact that it changed is why the three-state design
was worth building), `artifacts/api-server/src/services/highlights/highlightLifecycle.ts:48#migration 2723 was applied to production on 2026-09-15`,
`artifacts/api-server/src/services/memory/memoryDeletionLifecycle.ts:224#which was applied to production on 2026-09-15`,
two `storedIn` literals in `highlightRevocation.ts`, one header line in
`memoryParticipantVisibility.ts`, and four `2730 is unapplied` comments in `routes/memories.ts`
(edited line-count-neutrally, so no citation into that file moved for them).

### O.8 Files changed outside this lane, and the request §M.7 handed over

**Changed, and named loudly:**

- `docs/architecture/census-telegraph.md` — THREE citations repointed, at its lines 1897, 1935 and
  3476, all three broken by this section's line shifts in `routes/memories.ts` and
  `routes/highlights.ts`. Every repoint was computed from a line-by-line diff of the file at
  `HEAD` against the working tree, not by eye, and re-verified by `check:doc-citations` going from
  `RESULT failed — broken anchors 55` to `RESULT clean`. **No telegraph verdict was read or moved.**
- `docs/architecture/migration-queue.md` — ONE citation repointed, by the same method, for the same
  reason.
- `artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json` — TWO entries added. One is
  this census's own. **The other is `census-telegraph.md`'s, which §M.7 handed to the integrating
  lane as a request and which was never applied**; `routes/memories.ts` and `routes/highlights.ts`
  are counted files for that census and both changed again here, so the entry is written rather
  than left to fail CI a second time. The argument in it is §M.7's own, extended: telegraph's two
  citations into `memories.ts` land on `GET /memories`'s public-feed state filter, which is
  untouched, and its citation into `highlights.ts` lands on the share-card projection, which is
  untouched.
- `artifacts/api-server/src/scripts/checkMemoryTableOwnership.ts` — one entry added to
  `KERNEL_SIDE`, because this section's new suite names `memory_domain_events`,
  `memory_command_receipts` and `memory_event_outbox` and the guard requires every such file to be
  classified deliberately. It names `memory_events` nowhere, which is the content of the
  classification.

**NOT changed, and deliberately:** no migration, and no entry in
`production-applied-migrations.json` or in any schema snapshot. This section READS both and writes
neither; the migrations are the coordinator's.

### O.9 The blocker this lane could not clear, recorded for the coordinator

`public.stamp_definitions` on `portava-ci` holds **zero rows**, and migration
`2970_stamp_definitions_evidences_presence` is applied **nowhere** —
`artifacts/api-server/src/test/highlightsMemoriesDeployedStorage.test.ts` asserts the second half so
that a reader who sees ten migrations turn green in O.1 cannot infer the eleventh did. §N.5 already
records the consequence: `evidences_presence` reads `undefined` at all four readers, so every stamp
grades as non-presence and the Countries figure reads 0. H4 and H239 are unchanged by this section
and are not this lane's to move.

### O.10 WHAT WOULD TURN THIS RED

- **`production-applied-migrations.json` being wrong.** Everything in O.1 rests on two files in
  this repository, not on a connection to production. The applied list is maintained by hand — its
  own header says *"COMMIT AN ENTRY HERE IN THE SAME CHANGE THAT APPLIES THE MIGRATION"* — and the
  snapshot is generated but committed. If PR #482 recorded ten applies that did not happen, the ten
  blockers are back and O.3's ten row moves go with them. I could not and did not check the
  database; this lane may not.
- **`highlights` acquiring a trip reference**, at which point H90's withholding becomes the wrong
  answer rather than the safe one, and the row wants a real per-trip filter. The assertion in
  `highlightsMemoriesDeployedStorage.test.ts` fires the moment that happens, which is the point of
  writing it as a test.
- **Any of the four flags flipping.** `memory_kernel_enabled`, `memory_location_precision_enabled`,
  `memory_public_feed_projection_enabled` and `highlights_feed_bounded_enabled` are all `false`, and
  roughly thirty rows in this census now have that as their whole blocker. The same suite asserts
  all four, so a flip is a build failure and not a discovery.
- **O.2's "nothing writes either control table" being wrong** — it is a grep over `src/` for three
  verbs against two table names. A writer in a migration's `DO` block, in a Supabase Edge function,
  or in the mobile client would not be found by it.
- **The `HIDE_TRIP` withholding being judged worse than the fail-open.** It blacks out an owner's
  whole proactive surface for one stored row. Given that no route can create that row today it
  cannot fire, but the day a preferences surface lands, this is the branch that decides whether one
  control empties a feed. It is offered as the safe direction, not as the right product answer.
- **The five withdrawn `C`s being over-cautious.** §28's rule 7 grades a prohibition on the surface
  where a violation would live, and someone could argue that a §11 control enforced correctly on
  the only path that serves it is `C` whether or not a user can set it. §M.4 declined to move
  thirteen rows on a vacuity it could not measure; this section declines to move five on one it
  can. Both are judgements and this is the one most worth contesting.

### O.11 The recomputed headline

Restated from `check:census-integrity`, not counted by hand.

| figure | section N | **now** |
|---|---:|---:|
| Denominator | 266 | **266** |
| BUILT-AND-CORRECT | 61 | **61** |
| BUILT-BUT-WRONG | 131 | **140** |
| NOT-BUILT | 72 | **63** |
| CANNOT-VERIFY | 2 | **2** |
| CONSTRUCTED% | 72.2 % | **75.6 %** (201 / 266 = 75.6 %) |
| CORRECT%, raw | 22.9 % | **22.9 %** (61 / 266 = 22.9 %) |
| **the gap** | **49.3 points** | **52.7 points** |

**CORRECT did not move and the GAP grew by 3.4 points**, which is the honest shape of a section
that found a migration had been run. Nine rows stopped being unbuilt because their storage exists;
...
`artifacts/api-server/src/routes/highlights.ts:1534#router.put("/highlights/resurfacing-controls"`, `artifacts/api-server/src/routes/highlights.ts:1567#router.delete("/highlights/resurfacing-controls"`, `artifacts/api-server/src/routes/highlights.ts:1641#router.put("/highlights/:id/projection-policy"`, over `artifacts/api-server/src/services/highlights/highlightControlWrites.ts:235#export async function setResurfacingControl` and `artifacts/api-server/src/services/highlights/highlightControlWrites.ts:443#export async function setProjectionPolicy`. `verifyFlowHighlightControls.test.ts`
...
asks the question: `artifacts/api-server/src/services/highlights/highlightPublicProjection.ts:197#export function publicProjectionVerdict`, with `artifacts/api-server/src/services/highlights/highlightPublicProjection.ts:161#export function controlsSuppressing` DERIVED from `CONTROL_EFFECTS` the way
`FEED_ENFORCEABLE_CONTROLS` is, and `artifacts/api-server/src/services/highlights/highlightPublicProjection.ts:90#export const SURFACE_CONSENT_DIMENSIONS` naming which consent dimension each surface must not have
been refused — RESURFACE and SHARE on a feed, SHARE alone on a surface the viewer navigated to.
Wired at `artifacts/api-server/src/routes/highlights.ts:600#const projectable = filterProjectable([record], viewerId, "public_projection", inputs, log, "resolveViewAccess");` (the five engagement routes), `artifacts/api-server/src/routes/highlights.ts:1023#? filterProjectable(permitted as any[], user.id, "public_projection", inputs, req.log, "GET /users/:userId/highlights")` (the profile listing), `artifacts/api-server/src/routes/highlights.ts:1244#surviving as any[], user.id, "proactive_resurfacing", { controls: suppressed, policies }, req.log, "GET /highlights/active",` and `artifacts/api-server/src/routes/highlights.ts:2561#surviving as any[], user.id, "proactive_resurfacing", { controls: suppressed, policies }, req.log, "GET /highlights/following-feed",`
...
stored `false` and defers a NULL to the row's own `visibility` (`artifacts/api-server/src/services/highlights/highlightPublicProjection.ts:173#export function consentWithholds`). That is not `mayProject`,
...
## Q. Seven rows this document had stale by a whole integration, one writer built, and a consent map the client could not invent — 2026-09-22, the HIGHLIGHTS & MEMORIES lane

**`head_commit` is NOT re-declared.** It stays `1fe72289b`, and the counted files this section
changed are added to this census's existing entry in
`artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json` rather than to a new
declaration — for the reason §N and §P both give: a re-declaration at this branch's own commit is
PRE-SQUASH, would be an orphan in a fresh clone, and would invalidate every sibling census's
acknowledgement in the same file by moving the `since` they are keyed to.

Every OLD verdict below was read from `CENSUS_INTEGRITY_DUMP=ALL`, never from this document's
prose, per §M's rule and for §M's reason. The dump at the section's start read
**266 rows, 61 C / 140 W / 63 N / 2 X**, which is §P.9's claim exactly.

### Q.1 The document was stale, and it is worth saying which way

§P re-measured eleven rows and moved none. What it did not do — it says so in its own header — is
re-read the rest, and between §O and §P an integration landed work that seven rows name their
blocker after. The blockers are quoted from this document and are FALSE at HEAD:

- **H94–H97** (`LIVE` / `DAY` / `TRIP` / `SEASONAL`). §O.3: *"nothing writes the column and
  ...
  `artifacts/api-server/src/routes/highlights.ts:112#const HIGHLIGHT_CLASS_COLUMNS = ["lifetime_class", "lifecycle_state", "pinned_at"] as const;` — a PROBE rather than an append to the column
...
- **H142, H143** (`PIN_HIGHLIGHT` / `UNPIN_HIGHLIGHT`). §B.5: *"no pin column in production, no pin
  ...
  `artifacts/api-server/src/routes/highlights.ts:1321#router.post("/highlights/:id/pin", async (req, res) => {` and
  `artifacts/api-server/src/routes/highlights.ts:1357#router.delete("/highlights/:id/pin", async (req, res) => {`, on a column the 2026-09-15 snapshot
...
that goes stale in the direction of understating what exists is making the same class of error as
one that overstates it — and this one had been understating by seven rows.

### Q.2 What was built, and where

**One writer, and it is the writer §O.3 named as H32's only remaining blocker.**
`artifacts/api-server/src/services/highlights/highlightSources.ts:275#export async function linkHighlightSources(` is the first TypeScript write to
`public.highlight_sources` in this repository. Migration 2722 created the table and it has been on
production since 2026-09-15; §O.3 held H32 at `W` on the row's own surviving clause, *"with no
TypeScript writer"*, and `check:memory-certification` says the same thing from the other side —
H237's report still reads *"highlight_sources is migration 2722, unapplied, with no TypeScript
writer"*, of which the first half was already false.

`POST /highlights` now accepts `sourceMemoryIds`
(`artifacts/api-server/src/routes/highlights.ts:710#sourceMemoryIds: z.array(z.string().uuid()).max(MAX_HIGHLIGHT_SOURCES).optional(),`), and the ORDER
is the design: `artifacts/api-server/src/routes/highlights.ts:763#const ready = await sourceStoreReady(client);` probes the link table and
`artifacts/api-server/src/services/highlights/highlightSources.ts:213#export async function verifyMemorySources(` checks every named id against `memories` BEFORE the
...
`artifacts/api-server/src/routes/highlights.ts:873#const linked = await linkHighlightSources(client, {`, which soft-deletes the Highlight and
refuses. A Highlight standing with a provenance it does not have is worse than no Highlight:
`artifacts/api-server/src/services/highlights/highlightSources.ts:411#export async function highlightIdsProjecting(` is the reverse index §21 revocation walks, so a
bad link is a stranger's deletion reaching this row, or this owner's deletion missing it.

**All or nothing, and that is the load-bearing choice.** One unverifiable source refuses the whole
...
**The read is owner-only**, at `artifacts/api-server/src/routes/highlights.ts:1705#router.get("/highlights/:id/sources", async (req, res) => {`, and that is 2722's decision rather
...
**EPISODE sources are refused BY NAME.** 2722's CHECK admits them and §12 names them, and
`memory_episodes` (H23) has no `CREATE TABLE` anywhere in this tree, so an EPISODE id could be
verified against nothing. Writing one would be the fabricated link 2722's own header says the
missing foreign key exists to avoid.

### Q.3 Red-first: every mutation, and the four that survived

`artifacts/api-server/src/test/highlightSourceLinks.test.ts:120#describe("§12 a Highlight created from Memories records what it projects", () => {` was written
before the route was wired and ran **10 fail / 10 pass** — the ten failures being the route half
(201 with no link rows, 404 for a route that did not exist) and the ten passes the module half,
which existed by then. After wiring: 23 / 0.

Ten mutations, each applied to the merged tree and reverted:

| # | mutation | result |
|---|---|---|
| 1 | the create path stops verifying source ownership | 3 red |
| 2 | `verifyMemorySources` reads an unreadable `memories` as "none of these are yours" | 1 red |
| 3 | the failed link no longer compensates the Highlight it left behind | 1 red |
| 4 | `GET /highlights/:id/sources` stops checking the owner | 1 red |
| 5 | `highlightIdsProjecting` answers "nothing projects it" for an unreadable index | 1 red |
| 6 | the pre-write probe is removed, so an undeployed 2722 creates a Highlight first | 1 red |
| 7 | an EPISODE source is written unverified instead of refused | 1 red |
| 8 | the upsert names the primary key instead of 2722's unique index | 1 red |
| 9 | a partial link write is reported as success | 1 red |
| 10 | a deleted source Memory is admitted | 1 red |

**Three of those ten were green on the first run, and the suite was wrong, not the mutations.**
Mutations 2, 5 and 9 survived because the shared fake resolves a failed read as `{ data: null }`,
so the `!Array.isArray(data)` guard fired and the `.error` binding — the guard actually under test
— was never reached; and because the fake's upsert always stores what it was given, so a short
write could not be expressed at all. Both are properties a store-shaped fake cannot produce. Those
three cases now drive a RESPONSE-shaped client instead, and all three mutations are red.

**A fourth survivor, in a suite this section did not write.** Replacing `deterministicMatchCount`
with `hits.length` on `POST /memories/search` left `memorySearchRoute.test.ts` green. Its assertion
was `hits.length <= deterministicMatchCount`, and with one matching Memory in the fixture the two
...
Eight further mutations covering the stale tranche, all red: `lifetime_class` dropped from the
insert (2 red), the class columns dropped from the projection (2 red), the class inferred from the
expiry instead of stored-or-unknown (1 red), the per-dimension contributions dropped from the
search response (1 red), the count replaced by the page size (1 red), the caller's `ownerId`
honoured on a `mine` search (1 red), a REVOKED derivative rebuilt instead of refused (1 red),
`pinned_at` no longer written (3 red).

### Q.3b A defect this section's own build shipped, caught by a guard rather than by a reviewer

Worth recording because it is the one place in this section where the lane was wrong and something
else noticed. `POST /highlights` computes the expiry from `Date.now()`; the compensating
soft-delete added in Q.2 stamped `deleted_at` from a second, no-arg `new Date()`. `splitClockGuard`
refuses exactly that — two independent clock reads inside one function — and it went red on the
full run with the handler named by line. Both now derive from one `const nowMs = Date.now()`.

The guard is right and the reason is not pedantry: an expiry computed at one instant, on a row
stamped at another, is a Highlight whose lifetime does not match its own record, and the drift is
invisible until it is a support ticket. The fix moved seven lines, which is why the citation repoint
in Q.8 had to be done twice.

### Q.4 Row moves

| id | was | now | why |
|---|---|---|---|
| H32 | W | **C** | `highlight_sources` is deployed AND has a writer. The row's surviving clause was *"with no TypeScript writer"*; `linkHighlightSources` is one, reachable by an authenticated `POST /highlights` and route-tested, and `readHighlightSources` / `highlightIdsProjecting` are the two reads 2722's own header says the table exists to serve. Graded on §K.3's precedent — reachable by an HTTP request and by no screen — applied consistently |
| H94 | W | **C** | the class is stored by the create path, projected through a probe, and reported with a provenance that distinguishes "the owner chose LIVE" from "nobody assigned a class" from "this deployment cannot hold one". §O.3's stated blocker is false at HEAD in both halves |
| H95 | W | **C** | same column, same writer, same projection, for DAY |
| H96 | W | **C** | same, for TRIP |
| H97 | W | **C** | same, for SEASONAL |
| H110 | W | **C** | `POST /memories/search` imports the module the row is about and hands §15's parameter set to it. The row's only stated blocker was *"No route imports the module"* |
| H111 | W | **C** | the deterministic filters decide membership, `semanticQuery` may only reorder what they selected, and `deterministicMatchCount` reports what they selected BEFORE ranking or limit — now pinned strictly, see Q.3. The row's *"Unreachable"* is false |
| H142 | N | **W** | `pinned_at` is on production, `POST /highlights/:id/pin` writes it, and the profile read orders on it. Not correct, and not for a client reason: §17's requirement is the COMMAND BOUNDARY, and PIN_HIGHLIGHT is still absent from `MEMORY_COMMAND_TYPES` — no idempotency key, no receipt, no audit row, no outbox insert. That is H144's and H145's ceiling exactly, and these two now sit on it |
| H143 | N | **W** | same route pair, same ceiling |

### Q.5 The client lane's request, verified rather than taken on trust, and served

The Highlights CLIENT lane stopped building a consent section and asked for one field, and its
reasoning was right. `GET /highlights/:id/projection-policy` publishes all five §10 dimensions and
`PUT` stores a patch for any of them, so a client had two bad options: render five switches of
which three do nothing, or hard-code the enforced pair and own a second copy of a vocabulary the
server defines.

**Re-measured here rather than accepted.** `consentFromRow` has exactly ONE non-test caller in the
tree — `consentWithholds` — and that function is only ever asked for the dimensions in
`artifacts/api-server/src/services/highlights/highlightPublicProjection.ts:90#export const SURFACE_CONSENT_DIMENSIONS: Readonly<Record<NonOwnerSurface, readonly MemoryConsentDimension[]>> =`,
which is RESURFACE + SHARE on a feed and SHARE alone on a navigated-to surface. `mayProject` — the
function that would ask the other three — still has no production caller, which is §O.2's own
finding on H75. So exactly two of the five bite, and the lane's claim holds.

`artifacts/api-server/src/services/highlights/highlightPublicProjection.ts:139#export function consentEnforcement(): ConsentEnforcementMap {` is the answer, DERIVED from that
table rather than listed beside it, and served at
`artifacts/api-server/src/routes/highlights.ts:1627#consentEnforcement: consentEnforcement(),`. It is the same shape `unenforceableOnFeed` already
...
is the assertion most worth having — a dimension that fell out of BOTH lists would be rendered by
nobody and enforced by nobody, and a test checking only `enforced` would not notice. Five
mutations, all red, including one that publishes a hard-coded copy of the correct answer: the suite
compares the WIRE against the module, so a route that froze today's answer fails tomorrow's.

**NO ROW MOVES ON THIS.** H75 and H200 stay BUILT-BUT-WRONG on §P.2's owner decision, unchanged.
Serving the map does not enforce a dimension; it stops a client claiming one is enforced.

### Q.6 Rows read and NOT moved, with what stops each

This table RESTATES no verdict — `standing` is spelled out so nothing here is read as a move.

| id | standing | the blocker AFTER 2026-09-22 |
|---|---|---|
| H93 | BUILT-BUT-WRONG | a Highlight can now RECORD what it projects, and is still not required to project anything: `POST /highlights` accepts a client-supplied `mediaUrl` with no source, and every Highlight on production is sourceless. §28.12's rebuild does not exist either — knowing the sources is its precondition, not the thing itself. Asserted deliberately, by the case that says a sourceless create still succeeds |
| H98 | NOT-BUILT | unchanged and still structurally impossible: `2975_highlights_permanent_lifetime.sql` is NOT in `production-applied-migrations.json`, so `expires_at` is still NOT NULL and a PERMANENT Highlight is one with no expiry. The CODE is correct on both sides of that migration and refuses PERMANENT by name when the database cannot hold it |
...
### Q.7 A gap found reading H114, and the reason the obvious fix is wrong

§21: *"Privacy changes must revoke derived artifacts and cached projections, not merely stop
producing new ones."* Measured at HEAD: `revokeDerivativesForMemory` has exactly one non-test
caller, `memoryDeletionLifecycle.ts`'s `DERIVATIVES_PURGED` step. **A visibility NARROWING does not
reach it.** `PATCH /memories/:id` calls `revokeMemoryAudienceCaches`, which evicts Compass feed
caches; the §18 derivative registry is not touched.

**This is not a leak, and saying why matters.** `projectionStaleness` keys on each source Memory's
`updated_at`, the PATCH writes `updated_at`, so the registration goes STALE and `ensureDerivative`
rebuilds it before the next search — and the rebuilt `PublicMemoryProjection` filters
`visibility === "public"`, so the narrowed Memory is gone from it. The outcome is right. The
mechanism is staleness-then-rebuild, which is *"merely stop producing new ones"*, and the row asks
for revocation.

**And the obvious fix would be a defect.** A revoked registration is deliberately NEVER rebuilt —
that is what makes H114's other half mean anything. Calling `revokeDerivativesForMemory` on every
narrowing would therefore permanently destroy that owner's public search index on the first
public → private → public round trip, because the registry is scoped per OWNER and not per Memory.
Making it per-Memory is a schema decision; making the revocation reversible is a lifecycle
decision. Neither is this lane's to take, so H114 stays BUILT-BUT-WRONG with its blocker named
precisely instead of a fix that trades a wrong sentence for a broken index.

### Q.8 Files changed outside this lane, named loudly

- `artifacts/api-server/src/test/highlightsSpecHarness.ts` — the shared Highlights fake. Its
  generated primary key was `new-<random hex>`, which is not a UUID, so a handler that takes the id
  an insert returned and validates it failed HERE and only here, for a shape production cannot
  produce. It now generates a real UUID, which is what every `id` it stands in for is. No other
  suite references the old shape; the full suite is green.
- `artifacts/api-server/src/test/memorySearchRoute.test.ts` — one case added (Q.3's fourth
  survivor). Nothing existing was changed or removed.
- `artifacts/api-server/src/scripts/checkCensusFreshness.ts` — five test files added to this
  census's `CENSUS_SCOPE`, all of them evidence this section cites. Additive.
- `artifacts/api-server/src/scripts/CENSUS_STALENESS_ACKNOWLEDGED.json` — this census's existing
  entry's `files` list extended. Additive; `since` is untouched.
- `artifacts/api-server/package.json` — two new suites registered under `test`. Additive.
- `artifacts/api-server/scripts/check-citation-targets.mjs` — `MAX_DEAD_TARGETS` LOWERED 183 → 182,
  ...
  H91's `routes/highlights.ts:83`, which had been pointing at a BLANK LINE for two head_commits.
  ...
  ANCHORED, which removes it from that population for good. `check:citation-symbols` reported NO
  gain — 35 / 35, unchanged — and was not touched.
- `docs/architecture/census-telegraph.md`, `docs/architecture/census-wall.md` and
  `docs/architecture/migration-queue.md` — ONE citation each, REPOINTED ONLY. No verdict, sentence
  or judgement in any of the three was read or changed; each named a line in `routes/highlights.ts`
  or `checkCensusFreshness.ts` that this section's edits moved, and `check:doc-citations` named the
  new line for each. Named loudly because a lane editing another census's document is the thing
  that should always be explained, even when it is only a pointer.
- `docs/architecture/telegraph-phase0-inventory.md` — ONE generated cell, `routes/highlights.ts`
  22 → 23 Supabase table touches. Not authored: `check:telegraph-inventory` RE-DERIVES that table
  on every run and fails if the committed file disagrees with the tree, so the extra `.from()` in
  Q.2's compensating soft-delete has to appear there or that check goes red. Re-derived, not edited
  by hand, and the check reports 205 lines identical afterwards.

The citation repoint was done TWICE, and the second time is the instructive one: Q.3b's split-clock
fix moved seven more lines after the first pass had already repaired 37 pointers, so every anchor
below the create handler shifted again. Repointed from what `check:doc-citations` NAMED on each
run, never by adding a constant — the first pass's shifts inside ONE file were +9, +143, +201 and
+1, and an offset applied uniformly would have been wrong in three of those four places.

### Q.9 What would turn this red

- **`rankHighlights` acquiring a caller.** H99 and H100 stop being "unreachable" and start being
  "unenforced on three of four surfaces", which is a different and worse sentence.
- **2975 landing.** H98 becomes representable and the PERMANENT refusal branch this section did not
  move becomes dead code that must be re-read rather than trusted.
- **A per-Memory derivative scope.** Q.7's blocker dissolves, and H114's fix becomes a wiring
  change rather than a product decision.
- **A third consent dimension being wired.** `consentEnforcement` moves it from `unenforced` to
  `enforced` with no edit, which is the design — but H75's ceiling changes and §P.2's owner
  decision has to be taken before it means anything.
- **`memory_episodes` landing.** `linkHighlightSources`' refusal of EPISODE sources stops being the
  honest answer and becomes a gap.

### Q.10 The recomputed headline

Restated from `check:census-integrity`, not counted by hand.

| figure | section P | **now** |
|---|---:|---:|
| Denominator | 266 | **266** |
| BUILT-AND-CORRECT | 61 | **68** |
| BUILT-BUT-WRONG | 140 | **135** |
| NOT-BUILT | 63 | **61** |
| CANNOT-VERIFY | 2 | **2** |
| CONSTRUCTED% | 75.6 % | **76.3 %** (203 / 266 = 76.3 %) |
| CORRECT%, raw | 22.9 % | **25.6 %** (68 / 266 = 25.6 %) |
| **the gap** | **52.7 points** | **50.7 points** |

**CORRECT moved by seven rows and only one of them was built here.** Six of the seven were work
