...
| H10 | MemoryPrivacyService | BBW | `routes/memories.ts:127` `canViewMemory` and `lib/highlightPermissions.ts:37` `canViewHighlight` are per-route read helpers; no audience/precision/resurfacing/personalization/publication policy service | pre |
...
**all NOT-BUILT.** The nearest artifact is `services/media/MediaProjectionService.ts:1091#{ key: "gems", label: "Hidden Gems", ownerOnly: false, count: gemsCount, media: [] },`, a
...
| Seven routes cross the boundary | **Yes** | `routes/memories.ts:481#CREATE_MEMORY` (CREATE), `:937#dispatchMemoryCommand` (PATCH → ARCHIVE / CONFIRM / CHANGE_VISIBILITY / CHANGE_PLACE / UPDATE via `commandTypeForPatch` in `MemoryDomainService.ts`), `routes/memories.ts:1015#DELETE_MEMORY` (DELETE), `:1088#ADD_MEDIA` (ADD_MEDIA), `:1180#REMOVE_MEDIA` (REMOVE_MEDIA), `:1349#ADD_PERSON` (ADD_PERSON / REMOVE_PERSON) |
...
- **H79** (public search must query a derivative, never canonical plus post-filtering) stays
  **BBW**, but not for section A.5's reason. A.5 says the feed is "still the service client
  ...
  longer true: at `artifacts/api-server/src/routes/memories.ts:552#router.get` every privacy
...
and again at `artifacts/api-server/src/routes/memories.ts:934#place_correction_rate`, both
...
| H215 | `place_correction_rate` | NB | Named in two comments — `artifacts/api-server/src/services/memory/MemoryDomainService.ts:219#place_correction_rate` and `artifacts/api-server/src/routes/memories.ts:934#place_correction_rate` — both saying the command exists so the metric COULD be counted. No counter is incremented anywhere |
...
| H264 | §28.11 — never swallow projection or schema failures into plausible-looking empty history without a structured error state | **BAC** | `artifacts/api-server/src/routes/memories.ts:605#req.log.error` returns `db_error` on a block-lookup failure rather than serving an unfiltered feed, and `loadMemoryGemContext` records `determined: false` and coarsens rather than passing silently. §25's `PROJECTION_WORKER_OUTAGE` (H247) now makes the same rule executable one layer down: an unreadable registry refuses with a named reason instead of returning zero rows |
...
| Artifact | Where | What it is |
|---|---|---|
| §23's ladder, no longer module-private | `artifacts/api-server/src/services/memory/memoryReadPolicy.ts:90#canReadMemory` | Moved out of `routes/memories.ts` comment for comment. Nothing in the ladder changed. `routes/memories.ts:182#memoryReadPolicy.js` imports it back, so the repository has one copy where it had one private one. |
...
| §23's audience predicate | `artifacts/api-server/src/services/memory/memoryReadPolicy.ts:360#canPublishMemory` | Refuses the three combinations `canReadMemory` denies to every non-owner. Reasons and user-facing messages at `artifacts/api-server/src/services/memory/memoryReadPolicy.ts:339#PUBLISH_REFUSAL_MESSAGE`. Wired at `artifacts/api-server/src/routes/memories.ts:426#canPublishMemory` (POST) and `:901#canPublishMemory` (PATCH, on the MERGED row). |
...
`artifacts/api-server/src/routes/memories.ts:1813#asHistoricalMemoryPayload` applies it at `mapMemory` — the
...
| H83 | C | **C** | **Evidence falsified and replaced; verdict unmoved.** The row proves "being tagged does not make another user a co-owner" by citing `userId !== user.id → 403` — a line that no longer exists. `artifacts/api-server/src/routes/memories.ts:1350#authorizeParticipantCommand` is now `authorizeParticipantCommand`, under which the **owner** may also remove a tag. The verdict survives for a different reason than the one written: a tagged user still gets no edit right and no audience right; what changed is that the owner gained one |
...
file moved it to `artifacts/api-server/src/routes/memories.ts:1772#state`, and
...
`artifacts/api-server/src/routes/memories.ts:802#loadParticipantVisibility` and
`artifacts/api-server/src/routes/memories.ts:823#participants.participants`, and
`artifacts/api-server/src/routes/memories.ts:1307#participants.participants`.
...
`artifacts/api-server/src/routes/memories.ts:937#dispatchMemoryCommand` — was resolved by exact
...
## F. §25's H239, worked from the surface end — and the planned layover that was minting a checked-in stamp — 2026-09-13, measured at HEAD `46156b375`
...
false** — the two causes that account for 105 of this document's 129 remaining W rows. It is a
...
### F.2 The five seams, enumerated — because "the live stamp path" is not one path

A repository-wide search for callers of `createStamp`
(`artifacts/api-server/src/services/passport/PassportStampService.ts:63#export async function createStamp`)
and of `createSuggestedMemory` returns **five** route seams, not the two H4's evidence names.
Each is listed with the occurrence evidence it requires, because "it enforces the rule" is a
claim about a set:
...
| `POST /api/location/...` city stamp | `artifacts/api-server/src/routes/location.ts:382#if (stampType === "city_visit" && city && trustLevel === "gps_verified") {` — GPS-verified or nothing | enforces |
| hidden-gem check-in | `artifacts/api-server/src/routes/hiddenGems.ts:957#const result = await recordGpsCheckin` and then only when the check-in is not flagged suspicious | enforces |
| Safe Return confirm | the traveller's own explicit "I am safe" (`artifacts/api-server/src/routes/safeReturn.ts:562#stampType: "safe_return",`), which is §1's *user confirmation* limb | enforces |
| `POST /api/airport/sessions` | **nothing.** The session is created and the stamp follows | **does not enforce** |

Four of five hold. The fifth is the one nobody had opened, and it is the only one whose
requirement is not in the route at all.
...
| id | was | now | why |
|---|---|---|---|
| H239 | W | **C** | §25's invariant "planned activity without occurrence cannot earn a visit Memory/Stamp". Its BBW rested on two things and this section changed both: the rule is now true on ALL FIVE live seams (§F.2), and it is COVERED on the live surface by a registered suite rather than only on `evidence.ts`. That is §B.5's own rule for grading a §25 invariant verbatim — *"BAC only when the property is proved on a path production serves — either the module is imported by a route, **or a separate registered suite covers the live surface**"* — and it is the rule H236 and H240 are already C under. **CEILING, and it is why this is not a clean green.** Two of the five seams are driven end to end through their real routers; the other three are pinned only as a SET, by the enumeration in §F.2 read at this commit, and a sixth seam appearing would not turn anything red. The engine half is unchanged: `evidence.ts` is still imported by no route, so what moved is the surface, not the pipeline. |
| H4 | C | **C** | **NO NET MOVE, AND THAT IS THE WORST WAY TO READ THIS ROW.** Its C was a **FALSE GREEN** at `6d4fd1a06`: the stated evidence is two routes — `routes/geofence.ts` and `routes/location.ts` — and the sentence it supports is universal, *"Planned/saved/nearby never represented as 'experienced'"*. There was a third seam, it was flag-open in production, and it wrote `verification_level: 'checkin'` for a layover the same handler had just validated as being in the future. The row ends green because the gate was added (§F.3), **not because the sentence was rewritten**. Evidence replaced: the rule now holds on all five seams and all five are named. |
| H264 | C | **C** | **A SECOND FALSE GREEN, CLOSED THE SAME WAY.** §28.11 — "never swallow projection/schema failures into plausible-looking empty history without structured error state" — was C on `routes/memories.ts`'s block-lookup branch. Rule 7 of this census's own counting method grades a prohibition **on the surface where a violation would live**, and a violating path existed: `toolMemoryGetEvidence` reported an unreadable participant table as `confirmed_participants: 0`. Read the object, not the sentence about it. Green now because the two reads bind their errors. |
| H205 | C | **C** | **Evidence corrected, verdict unmoved.** §B.2 states the row's ceiling as *"two other surfaces re-derive the rule instead of calling it"*, and §C names the same two. There are **three**: `artifacts/api-server/src/services/telegraph/shareables.ts:452#const loadMemory` is a fourth reader that says nothing about mirroring anything and that DISAGREED with the predicate on blocks until §F.3. The verdict survives because the requirement is the predicate and the predicate exists with its surface parameter; what is now true and was not is that one of the three transcriptions had drifted, which is the failure mode the ceiling was written to warn about, arriving. |
...
**No other row moved, and no row in another lane's census was moved.** §F.7 names the two
census-layover rows and the two census-compass rows this work bears on and leaves all four alone.
...
2. **Three of the five seams are read, not driven.** `routes/location.ts`, `routes/hiddenGems.ts`
   ...
   section has.
3. **The airport refusal is terminal.** A traveller who sets up a layover in advance now earns no
...
|---|---:|---:|---|
| **(a)** logic wrong or incomplete in code | 7 | 7 | H6, H10, H84, H189, H190, H193, H202 — §E.5 opened all seven and named what stops each |
...
|---|---:|---:|
| Denominator | 266 | **266** |
| BUILT-AND-CORRECT | 56 | **57** |
| BUILT-BUT-WRONG | 130 | **129** |
| NOT-BUILT | 78 | **78** |
| CANNOT-VERIFY | 2 | **2** |
| CONSTRUCTED% | 69.9 % | **69.9 %** |
| CORRECT%, raw | 21.1 % | **21.4 %** |
| **the gap** | **48.8 points** | **48.5 points** |

**Three tenths of a point, and the number is the least interesting thing here again.** What this
section is worth is not in that table. It is that a route which validates its own event as being
in the FUTURE was, on two open production flags, writing a Passport row that said the traveller
had checked in — and that the census carrying the rule against it had been reporting that rule
BUILT-AND-CORRECT since the body, through five re-measurements, on evidence naming two of the
five places the rule has to hold.
