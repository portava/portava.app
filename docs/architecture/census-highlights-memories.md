...
`artifacts/api-server/src/routes/airport.ts:3025#sourceType: "layover_session", verificationLevel: "checkin",`.
...
`artifacts/api-server/src/routes/airport.ts:2999#const occurrence = declaredOccurrenceHasHappened(args.session.arrivalTime, Date.now());`,
the fourth term of `artifacts/api-server/src/routes/airport.ts:2962#async function writeElectedLayoverStamp`,
