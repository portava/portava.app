...
|---|---|---|
| R1 — `routes/admin.ts:2254#targetType === "message"`, drop `targetType === "message"` from the resolve guard | wiring `pulse_post_reported` (or anything) at resolve without subsuming `content_removed` | **# fail 3, EXIT 1** — §6 tests 1 and 2, plus §4 "upheld on a NON-message report" |
