...
| C30 | `grep` + `artifacts/api-server/src/index.ts:82#process.on("unhandledRejection", (reason) => {` | **Stays `C`, COUNT CORRECTED.** The cell says "the 19 unguarded `void recordTrustEvent(...)` sites"; there are now **22**. The `unhandledRejection` backstop still covers all of them, so the requirement holds and the number in the cell does not. |
...
3. **§14.6, TRV2-08** — *"`getRestrictionState` still has exactly **five** non-Trust callers"*.
   ...
   `services/passport/PassportProjectionService.ts:2126#getRestrictionState(sc, userId),`, `lib/calls/callGatewayAdapter.ts:265`,
