...
| C14 | Every cap a moderation finding created is lifted when the finding is reversed (`:93-108`) | C | `liftCapsBySourceEvents:110-128`; wired through `revokeModerationTrustConsequences:112-155` from `routes/admin.ts:1826#void revokeModerationTrustConsequences(sc, adminUserId, userId, reason ?? "Account restore`. |
...
`routes/discoverySearch.ts:2634#buildConsumerProjection(sc,`, `routes/compass.ts:4572#buildConsumerProjection(sc,`, `routes/telegraph.ts:370#buildConsumerProjection(sc,`,
...
| TV-4a | Admin-only queue: list open reports, filter by category/status, view subject content snapshot, act (warn / remove content / suspend with expiry / ban / dismiss), every action writing `moderation_actions` | **W** | Five criteria, one and a half pass. **List ✓** `routes/admin.ts:2094-2156#router.get("/admin/moderation/reports", async (req, res) => {`, `requireAdmin` first at `routes/admin.ts:2095#requireAdmin(req, res, { withDisplayName: true })`, with a client at `travel-buddy-standalone/app/admin/content-reports.tsx`. **Filter ✗ (half)** — the route filters `subject_type` and `status` (`routes/admin.ts:2114-2117#query = query.eq("subject_type", subjectType);`); there is **no category filter**, which is the axis the plan names and the axis a moderator triages on. **Subject content snapshot ✗** — only `place` reports are enriched, with name and address (`routes/admin.ts:2126-2153#const placeIds = [`); a reported post, comment, message, event, review or buddy listing arrives as a bare UUID. **Act ✗** — no route acts on a `moderation_reports` row at all: `POST /admin/reports/:id/resolve` and `/dismiss` operate on the separate legacy `reports` table, and nothing updates `moderation_reports.status`, so the queue can only grow. **Every action writing `moderation_actions` ✓ at user level, ✗ as specified** — `/admin/users/:userId/{warn,restrict,suspend,ban,restore}` each call `logModerationAction` (`routes/admin.ts:1674#const auditR = await logModerationAction(sc, userId, adminUserId, "warn", reason, {` (warn), `routes/admin.ts:1703#const auditR = await logModerationAction(sc, userId, adminUserId, "message_limit", reason);` (restrict), `routes/admin.ts:1724#const auditR = await logModerationAction(sc, userId, adminUserId, "temporary_suspension", reason);` (suspend), `routes/admin.ts:1770#const auditR = await logModerationAction(sc, userId, adminUserId, "permanent_ban", reason);` (ban), `routes/admin.ts:1816#const auditR = await logModerationAction(sc, userId, adminUserId, "account_restored", reason);` (restore)`), but they are reached from a user, not from a report, and the row cannot name the report because the column does not exist (TV-0a). "Suspend with expiry" is likewise unrecordable: `routes/admin.ts:1720#null = (req.body as any)?.expires_at ?? null;` accepts `expires_at` and stores it in `user_account_states`, while `moderation_actions` has no `expires_at` at all. |
...
| TV-4c | `unverify` left the one column the booking gate reads | `routes/admin.ts:1646#verification_level: "none",` | `test/adminUnverifyRevokesIdLevel.test.ts` | **RED 5 / 3 / 2** → **GREEN 5/5**. M4 (revert the single added field): back to **3/2**. |
...
(`routes/admin.ts:3568#router.get("/admin/verification/attempt-metrics", async (req, res) => {`), which is why it sits at the end of the file rather than beside the
