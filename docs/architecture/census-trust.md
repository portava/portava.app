...
| C30 | `grep` + `artifacts/api-server/src/index.ts:83#process.on("unhandledRejection", (reason) => {` | **Stays `C`, COUNT CORRECTED.** The cell says "the 19 unguarded `void recordTrustEvent(...)` sites"; there are now **22**. The `unhandledRejection` backstop still covers all of them, so the requirement holds and the number in the cell does not. |
