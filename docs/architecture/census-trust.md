...
3. **§14.6, TRV2-08** — *"`getRestrictionState` still has exactly **five** non-Trust callers"*.
   ...
   `routes/messaging.ts:692`, and `routes/trips.ts:288` and `:1335`. **TRV2-08 does not move**, and
...
| `artifacts/api-server/src/routes/messaging.ts:692#const senderRestrictions = await getRestrictionState(sc, user.id);` — §16's `getRestrictionState` caller — is byte-identical | same | **identical** |
