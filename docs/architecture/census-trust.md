...
| C17 | `expireOldRestrictions` — "call from cleanup job" (`:264`) | **W → C** | Had no caller. Enforcement already ignored expired rows, so nothing was over-enforced, but the row stayed `lifted_at IS NULL` and every admin view listed a lapsed restriction as active. Now step 1b of the pass (`artifacts/api-server/src/lib/trustMaintenanceScheduler.ts:662#const sweep = await expireOldRestrictions(db);`) and the function binds its `error` on BOTH halves it now has — the due-set read and the lift (`artifacts/api-server/src/services/trust/TrustRestrictionService.ts:364#export async function expireOldRestrictions(`). `trustCensusRepairs.test.ts` §4, and six more cases in `trustAsymmetryAndMaintenance.test.ts` §"the sweep that had no caller". **Re-derived in §22**, which also records the per-pass bound (`RESTRICTION_EXPIRY_BATCH` = 500) that this row does not claim past: having a caller is what C17 asks, and full coverage in one pass is not. |
...
| C17 | `W → C` | C | `artifacts/api-server/src/services/trust/TrustRestrictionService.ts:364#export async function expireOldRestrictions(` still binds its own error — and now reports a failed sweep as `failed`, distinct from lifting nothing — and it still has the caller it lacked: `lib/trustMaintenanceScheduler.ts:662#const sweep = await expireOldRestrictions(db);`. |
...
(`artifacts/api-server/src/services/trust/TrustRestrictionService.ts:325#export interface ExpireRestrictionsResult {`)
...
`artifacts/api-server/src/services/trust/TrustRestrictionService.ts:323#export const RESTRICTION_EXPIRY_BATCH = 500;`
and the scheduler carries the failure out of the pass at
`artifacts/api-server/src/lib/trustMaintenanceScheduler.ts:404#/** True when the restriction sweep could not tell — DISTINCT from 0 expired. */`.
...
|---|---|---|---|
| `C17` | whether `expireOldRestrictions` has a caller and binds its `error` | `git grep` over `artifacts/api-server/src`: the identifier appears on **32** lines, of which exactly **one** is a call in non-test source — `artifacts/api-server/src/lib/trustMaintenanceScheduler.ts:662#const sweep = await expireOldRestrictions(db);`. `error` is bound on both halves now instead of one. | **C, unmoved.** Evidence repointed; the bound is named. |
...
| `C5`, `C27` | `trustCensusRepairs.test.ts` §1/§2/§5 | the only edit to that file is inside §4's single `it` (`artifacts/api-server/src/test/trustCensusRepairs.test.ts:356#const r = await expireOldRestrictions(db);`). No `describe` is added, removed or re-ordered, so the § numbering those two rows cite is unchanged. | **unmoved.** |
| `C9`, `TV-U7` | `trustAsymmetryAndMaintenance.test.ts` | the file gains two `describe` blocks at its END (`:480`, `:569`). TV-U7's anchor at `:188` and C9's unnumbered citation both still resolve, and the eight named fixture classes are still eight — a ninth subject in the same FILE is not a ninth class. | **unmoved.** |
...
(`artifacts/api-server/src/services/trust/TrustRestrictionService.ts:400#.is("lifted_at", null)`),
...
§20's and §21's reason unchanged: this grades the rows resting on six counted files, not 108
requirements. *Numbered §24 on purpose — sibling branches PR #450 and PR #449 append a §22 and a
§23 to this same document, and leaving the gaps lets the three land without renumbering.*

### 24.1 What changed

Six files, one theme: supabase-js RESOLVES `{ data, error }`, so a failed read and an empty one
arrive identically unless `error` is read, and across the engine each unread `error` became a
confident negative claim. The change gives each of them a distinguishable shape.

| where | the old confident claim | the new signal |
|---|---|---|
| `TrustCapService.liftCapsBySourceEvents` | a failed lift returned `0`, identical to "nothing to lift" | `artifacts/api-server/src/services/trust/TrustCapService.ts:198#failed: boolean;` |
| `TrustAdminService.revokeModerationTrustConsequences` | every failure returned `{0, 0}`, and the dismissal returned `ids.length` whether or not a row moved | `artifacts/api-server/src/services/trust/TrustAdminService.ts:163#incomplete: boolean;` |
| `TrustRecoveryService.getRecoveryStatus` | an unreadable `trust_profiles` produced `onProbation: false` — fail-OPEN on a sanction | `artifacts/api-server/src/services/trust/TrustRecoveryService.ts:45#probationUnknown: boolean;` |
| the same, for caps | `activeCapsCount: 0` meant both "no ceilings" and "could not tell" | `artifacts/api-server/src/services/trust/TrustRecoveryService.ts:50#activeCapsUnknown: boolean;` |
| `GET /admin/trust/users/:userId` | a failed `trust_caps` read rendered to a moderator as an uncapped account | `artifacts/api-server/src/routes/trust-admin.ts:257#capsUnavailable: capsRead.state !== "ok",` |
| the three `recalculateTrustScore` loaders | already threw on `origin/main`; the throw is now a typed class | `artifacts/api-server/src/services/trust/TrustScoreService.ts:46#export class TrustInputUnavailableError extends Error {` |

### 24.2 Where each new signal actually goes — the measurement that matters most here

Consumers were counted, not assumed. `grep -rn` over `artifacts/api-server/src` excluding
`src/test/`:

| signal | non-test consumers | reaches a response body? |
|---|---|---|
...
uses for `getTrustMaintenanceStatus` and `adminRemoveOverride`. That is not an argument against
them — the shape has to exist before a consumer can read it, and the alternative shape was a lie
— but a census that recorded "failure is now visible" without saying WHERE it is visible would be
overstating the tree. The visible surfaces today are one admin field, the logs, and one audit
metadata blob.

### 24.3 The rows, and what each was measured on

| row | measured at this head | verdict |
|---|---|---|
| `C14` (every cap a reversed finding created is lifted) | the row this change is FOR. The lift is deliberately non-fatal — *"a reversal must not fail because its bookkeeping did"* — so a failed lift still leaves a `behavior_confirmed` ceiling of 40 with no `expires_at` standing, and nothing else lifts it. What changed is that it can no longer read as "nothing to lift". **The limit, measured:** §24.2 — the sole caller voids the result, so `POST` restore still answers success whatever happened. | **C, unmoved**, limit named in the row. |
| `C11` (a failed profile read is not a missing profile) | last moved to `C` on `getTrustProfileResult` being the read at every consumer. This change extends the SAME three-state discipline to the cap read in the same dossier. | **C, unmoved, strengthened.** |
| `C16` (degraded reads are labelled fail-open vs fail-closed) | `getRestrictionState` is byte-identical; `probationUnknown` is deliberately shaped like `restrictionsDegraded`, set only when true. | **C, unmoved.** |
| `C18` (recovery status) | last moved to `C`, partly on *"the probation read beside it had an unbound error … it is bound"*. It is now also REPORTED, not just bound. | **C, unmoved, strengthened.** |
| `C20` (reporter identity never exposed; raw deltas not returned; `pending_review` invisible to the subject) | the only addition to `SafeTrustSummary` is a boolean about read health. It carries no reporter id, no delta, no internal score and no event. `onProbation` was already in this payload, so a flag saying the probation read FAILED reveals strictly less about the subject than the field it qualifies — and §24.2 shows neither leaves `getSafeTrustSummary`. | **C, unmoved.** Citations repointed. |
| `C21` (every admin write creates a `trust_admin_actions` row) | `logAdminAction` is still called on the reversal path, now with `eventsTargeted` and `incomplete` in its metadata. No admin write loses its row. | **C, unmoved.** Route-level ranges repointed. |
| `C26` (every `/admin/trust/*` route is admin-guarded) | `trust-admin.ts` gains NO route. `const admin = await requireAdmin(req, res);` appears **14** times at `origin/main` and **14** at this head; every site shifted by at most 5 lines. | **C, unmoved.** |
| `C4`, `C8`, `C9`, `C12`, `A3` | the `TrustScoreService` change is confined to the three loaders and a new exception class; the three throws already existed on `origin/main` as plain `Error` and `TrustInputUnavailableError extends Error`, so nothing that catches an `Error` behaves differently. No caller matches on the message, and the two that inspect the outcome count a rejection either way. | **unmoved.** |
| `A2`, `A4`, `A9`, `A15`, `A17` | claims untouched; their citations moved with the files and were repointed to the IDENTICAL source lines, verified by `git show origin/main:` against this head line by line. | **unmoved.** |

### 24.4 What this section does NOT claim, and what it deliberately did not repair

- **The other rows are not re-measured**, and no measurement here was made against a database.
  §3's deployment facts still hold: `trust_admin_actions` is **0** and `TrustAdminService` has
  never been invoked in production, so none of this failure handling has ever run live.
- **Two bare citation LISTS were already stale at `origin/main` and are left alone rather than
  quietly swept in.** `C21`'s `logAdminAction` sites read `:87,184,200,214,247,282,311`; the real
  sites at `origin/main` are `115,193,248,264,278,396,484,585`. `C26`'s guard list has twelve
  entries; there are **fourteen** guarded handlers, at `origin/main` and here alike. Neither was
  caused by this branch — both were checked at `origin/main` — so repointing them belongs to the
  pass that re-grades those two rows. They are named so the next reader does not rediscover them.
- **`A17`'s `:353-364` was shifted, not corrected.** It moved with the file and now points at the
  identical lines it pointed at before; those lines were ALREADY not the header sentence the
  claim quotes, which lives at line 477. Correcting it is a re-grade of `A17`, not a line repair.
- **One citation in a SIBLING census was repaired, and it was this pass's own fault.** Adding the
  scope line above pushed `census-wall.md`'s pointer into `checkCensusFreshness.ts` down six
  lines; `check:doc-citations` named it by anchor and it was repointed from `:1141` to `:1147`,
  the same comment, unchanged. No census-wall verdict is touched and none is claimed.
- **`check:citation-targets` reports 209 against a ceiling of 210 at this tree.** The ceiling is
  NOT lowered here: the gain came in with `origin/main`, not from this branch, and lowering a
  shared ratchet from one branch would redden its siblings.

### 24.5 Guards and suites at this tree
