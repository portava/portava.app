...
| C14 | Every cap a moderation finding created is lifted when the finding is reversed (`:93-108`) | C | `liftCapsBySourceEvents:110-128`; wired through `revokeModerationTrustConsequences:112-155` from `routes/admin.ts:1818#void revokeModerationTrustConsequences(sc, adminUserId, userId, reason ?? "Account restore`. |
...
| (second pass) `plan_attendance_events` is read/written by four files | `routes/geofence.ts:154` writes; `routes/admin.ts:620#.from("plan_attendance_events")` and `TrustGamingDetectionService.ts:130` read; `lib/crowdFlowProducer.ts` names it in comments only (`:152`, `:361`). 0 rows in production. |
...
| C5 | `W → C` | C | `artifacts/api-server/src/services/trust/TrustEventService.ts:374#async function queueEventForReview` still writes the open `event_review` row, called on the pending-review path, and the admin queue that reads it is still routed at `artifacts/api-server/src/routes/trust-admin.ts:154#router.get("/admin/trust/events/pending", async (req, res) => {`. |
...
| C27 | `W → C` | C | `artifacts/api-server/src/routes/trust-admin.ts:85#const SETTING_BOUNDS: Record<string, SettingBound> = {` still bounds each key structurally and `artifacts/api-server/src/routes/trust-admin.ts:102#export function trustSettingRejection(key: string, value: unknown): string` still rejects a value outside it before the write. |
...
| TV-1e | Rate limits: max 3 session creations per user per 24 h | **C** | `routes/verification.ts:32-33#const VERIFICATION_SESSION_LIMIT = 3;` (`VERIFICATION_SESSION_LIMIT = 3`, 24 h window), enforced first in the handler at `routes/verification.ts:181-187#const rl = checkRateLimit("verification_session", user.id, VERIFICATION_SESSION_LIMIT, VER` with a `Retry-After` header and an explicit retry timestamp. Covered by `test/verification.test.ts`. |
...
| TV-4a | Admin-only queue: list open reports, filter by category/status, view subject content snapshot, act (warn / remove content / suspend with expiry / ban / dismiss), every action writing `moderation_actions` | **W** | Five criteria, one and a half pass. **List ✓** `routes/admin.ts:2086-2148#router.get("/admin/moderation/reports", async (req, res) => {`, `requireAdmin` first at `routes/admin.ts:2087#requireAdmin(req, res, { withDisplayName: true })`, with a client at `travel-buddy-standalone/app/admin/content-reports.tsx`. **Filter ✗ (half)** — the route filters `subject_type` and `status` (`routes/admin.ts:2106-2109#query = query.eq("subject_type", subjectType);`); there is **no category filter**, which is the axis the plan names and the axis a moderator triages on. **Subject content snapshot ✗** — only `place` reports are enriched, with name and address (`routes/admin.ts:2118-2145#const placeIds = [`); a reported post, comment, message, event, review or buddy listing arrives as a bare UUID. **Act ✗** — no route acts on a `moderation_reports` row at all: `POST /admin/reports/:id/resolve` and `/dismiss` operate on the separate legacy `reports` table, and nothing updates `moderation_reports.status`, so the queue can only grow. **Every action writing `moderation_actions` ✓ at user level, ✗ as specified** — `/admin/users/:userId/{warn,restrict,suspend,ban,restore}` each call `logModerationAction` (`routes/admin.ts:1666#const auditR = await logModerationAction(sc, userId, adminUserId, "warn", reason, {` (warn), `routes/admin.ts:1695#const auditR = await logModerationAction(sc, userId, adminUserId, "message_limit", reason);` (restrict), `routes/admin.ts:1716#const auditR = await logModerationAction(sc, userId, adminUserId, "temporary_suspension", reason);` (suspend), `routes/admin.ts:1762#const auditR = await logModerationAction(sc, userId, adminUserId, "permanent_ban", reason);` (ban), `routes/admin.ts:1808#const auditR = await logModerationAction(sc, userId, adminUserId, "account_restored", reason);` (restore)`), but they are reached from a user, not from a report, and the row cannot name the report because the column does not exist (TV-0a). "Suspend with expiry" is likewise unrecordable: `routes/admin.ts:1712#null = (req.body as any)?.expires_at ?? null;` accepts `expires_at` and stores it in `user_account_states`, while `moderation_actions` has no `expires_at` at all. |
...
| TV-P5 | the webhook answered 200 when the provider factory refused, discarding every delivery | `routes/verification.ts:339#res.sendStatus(503);` | `test/verificationWebhookProviderUnavailable.test.ts` | **RED 5 / 2 / 3** → **GREEN 5/5**, premise and control green throughout. |
| TV-4c | `unverify` left the one column the booking gate reads | `routes/admin.ts:1638#verification_level: "none",` | `test/adminUnverifyRevokesIdLevel.test.ts` | **RED 5 / 3 / 2** → **GREEN 5/5**. M4 (revert the single added field): back to **3/2**. |
...
`src/test/trust-integration.test.ts:1720#describe("D-OVERRIDE: the ceiling the owner ruled for must PERSIST"` —
...
|---|---|---|
| 1 | `src/test/trust-integration.test.ts:1746#it("a downward ceiling reaches trust_profiles, and the call REPORTS that it bound"` | the ceiling is on the row, `overall_score` fell with it, and `ceilingBinding` is true |
| 2 | `src/test/trust-integration.test.ts:1774#it("an UPWARD override reports that it bound NOTHING — CAP semantics, said out loud"` | `persistedScore` is the natural score and `ceilingBinding` is **false**, in the result AND in the audit metadata |
| 3 | `src/test/trust-integration.test.ts:1796#it("a failed recalculation leaves NO raw admin number on the row, and is never audited as applied"` | **the defect.** With `trust_events` unreadable: rejects, no raw number on the row, the row stays internally consistent, zero `score_override` audit rows, and the cap row stands |
