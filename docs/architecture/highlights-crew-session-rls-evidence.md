...
| --- | --- | --- |
| `2530_highlights_trip_only_accepted_crew.sql` | **ready_for_manual_apply** — shape-agnostic; on production apply after 2334 → 2337; on CI apply now. **Then rebase PR #461's 2313 before it is applied anywhere 2530 has run.** | 2337 |
...
| `2532_pg_policies_snapshot_v2.sql` | **ready_for_manual_apply** — additive, service_role-only RPC; until it lands on CI the FOR ALL rule fails by design | 2199 (convention only) |
