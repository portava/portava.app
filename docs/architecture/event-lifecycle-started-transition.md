...
| …and that phase is "published, live, not yet begun" | migration `2033_rls_hardening.sql:269` `state IN ('open','full','waitlist','started','completed')`; `EventPassportService.ts:68` and `PassportProjectionService.ts:1787#const LIVE_EVENT_STATES` `LIVE_EVENT_STATES = {open, full, waitlist, started}`; `BROWSE_STATES` `routes/events.ts:221`. In every one of these, `started` is the only "live" member that is *after* the start. |
