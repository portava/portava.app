...
| G89 | Global Search: recent searches, around-you-now, current Trip, Saved | W | **The Saved arm now fires**, and it is the only one of the four that is live in production: `lib/inputAssistance/gateway.ts:305#buildSavedPlaceSuggestions` offers the viewer's saved places at zero characters for `global_search`, whose policy names `place` (`src/test/inputAssistanceSavedEntities.test.ts:193#offers it in global_search`). The recent-searches branch still fires only through `input_selection_history` and is the dead path of G75 (`☠prod`). There is still no around-you-now and no current-Trip zero-state for `global_search`. |
...
| G95 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:493#SearchQueryContext` passes `{lat, lng, userCity}` into the city boost. **The old pointer, line 380, is stale by 16 — this pass moved it.** |
| G126 | `C ᵖ` (unreadable) | **C** | ᵖ | Age: `lib/inputAssistance/gateway.ts:486#ageRestrictedSet`, fail-closed at `:491#blockedSet`. Membership/role: `routes/discoverySearch.ts:921#show_in_discovery`. Trust/invite have no separate gate and no path exposes an invite-scoped object. |
| G128 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:486#ageRestrictedSet` and `:491#blockedSet` — a null set from either suppresses every entity row; the picker branch repeats it at `:787#fetchBlockedSet`. |
...
| G184 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:486#ageRestrictedSet` and `:787#fetchBlockedSet`; the null-set refusal is the mutation-proven case in `src/test/inputAssistanceGateway.test.ts:384#suppresses`. |
...
| G218 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:450#dispatchTypes` → `routes/discoverySearch.ts:2292#dispatchSearch`. |
...
| id | was | now | why |
| --- | --- | --- | --- |
| G97 | N | **C** | §15 **TemporalFit** now has a producer. `extractTemporal` was already normalising "tonight" / "tomorrow morning" / "Friday after dinner" into an ISO window; the window went into a search STRING and was discarded. It is now resolved once per request at `lib/inputAssistance/gateway.ts:195#TemporalWindow` and handed to the projection at `:471#temporalWindow`, where `lib/inputAssistance/rankingSignals.ts:104#applyTemporalFit` boosts a row that starts inside it and demotes one that starts outside. Deliberately a RANKING term, not a filter — see the ceiling note in §8.7. |
...
spelling always gets the first query; `lib/inputAssistance/gateway.ts:430#let correctionHelped = false`
...
opening `lib/inputAssistance/gateway.ts:430#let correctionHelped = false` and
...
(`artifacts/api-server/src/lib/inputAssistance/gateway.ts:450#if (wantsEntities && dispatchTypes.length > 0 && q.length >= 2) {`), so a
