...
and nothing else.
`travel-buddy-standalone/src/platform/input-assistance/components/SmartInput.tsx:105#selectionAnnouncement`
is the sentence and
`travel-buddy-standalone/src/platform/input-assistance/components/SmartInput.tsx:247#announceForAccessibility`
...
`SmartInput.tsx:310#input_opened`, `useInputAssistance.ts:216#suggestion_request_started`
...
| event | call site |
| --- | --- |
| `suggestion_rendered` | `components/SmartInput.tsx:198#emitSuggestionsRendered(telemetryField, suggestions)` |
| `validation_shown` | `components/SmartInput.tsx:200#if (validations > 0) emitValidationShown(telemetryField, validations)` |
| `suggestion_dismissed` | `components/SmartInput.tsx:189#emitSuggestionsDismissed(telemetryField, prev.count, focused ? 'no_results' : 'blur')` and `components/SmartInput.tsx:277#emitSuggestionsDismissed(telemetryField, shownRef.current.count, 'escape')` |
| `manual_value_kept` | `components/SmartInput.tsx:319#emitManualValueKept(telemetryField, value.trim().length)` |
| `raw_search_submitted` | `components/SmartInput.tsx:230#emitRawSearchSubmitted(telemetryField, (s.replacementText ?? value ?? '').length, true)` and `components/SmartInput.tsx:328#emitRawSearchSubmitted(telemetryField, value.trim().length, false)` |
| `correction_accepted` | `components/SmartInput.tsx:227#if (s.type === 'correction') emitCorrectionAccepted(telemetryField, s)` |
| `disambiguation_selected` | `components/SmartInput.tsx:228#if (s.type === 'disambiguation') emitDisambiguationSelected(telemetryField, s)` |
...
`components/SmartInput.tsx:141#const shownRef` and its companion `acceptedRef`
...
## 14. Phase 14 — the scattered non-`C` rows of §27–§56, worked
...
### 14.2 Five rows whose §4 cell disagreed with this document's own verdict
...
### 14.3 Rows examined and left, with the blocker named
...
### 14.4 Mutations applied, watched go red, reverted, `cmp`-verified
...
### 14.5 What this pass did NOT do, deliberately
...
### 14.6 Pointer repairs, and the two guards this section cannot make green
