...
| G68 | Hidden Gem: separate identity, protection and approximate-location rules | C ᵖ | `discoverySearch.ts:1486#sensitivity_level,` selects `sensitivity_level, approx_latitude, approx_longitude` and the exact pair is "deliberately absent"; `:286-300#gemSearchPosition` returns `hidden` for a denied or unparseable sensitivity level. Pre-existing Discovery work that the gateway inherits. |
...
| G126 | Age, trust, membership, role or invite restrictions | C ᵖ | Age: `fetchAgeRestrictedSet` fail-closed (`gateway.ts:374-378`). Membership/role: `discoverySearch.ts:1078-1081#admitted:` (plans via parent-trip ownership), `:917#visibility` + `:918#show_in_discovery` (trips). Trust/invite have no separate gate but no path exposes an invite-scoped object either. Pre-existing. |
...
| G129 | Protected or sensitive locations whose exact position cannot be surfaced | C ᵖ | Structural: `InputSuggestion` has **no coordinate field at all** (`types.ts:206-244`) and `discoverySearch.ts:1486#sensitivity_level,` never selects a gem's exact pair. Deep-scanned by `test/inputAssistanceCertification.test.ts:265-327`. |
...
| G185 | Private Trips/Events/Plans excluded unless the viewer is eligible | C ᵖ | `discoverySearch.ts:736#visibility`, `:917#visibility`, `:918#show_in_discovery`, `:1078-1081#admitted:`. Proven **through the gateway** by `test/inputAssistanceCertification.test.ts:330-366`. |
...
| G126 | `C ᵖ` (unreadable) | **C** | ᵖ | Age: `lib/inputAssistance/gateway.ts:456#ageRestrictedSet`, fail-closed at `:461#blockedSet`. Membership/role: `routes/discoverySearch.ts:918#show_in_discovery`. Trust/invite have no separate gate and no path exposes an invite-scoped object. |
...
| G129 | `C ᵖ` (unreadable) | **C** | ᵖ | Structural: `lib/inputAssistance/types.ts:230#InputSuggestion` has no coordinate field, and `routes/discoverySearch.ts:1486#sensitivity_level,` never selects a gem's exact pair. Deep-scanned by `src/test/inputAssistanceCertification.test.ts:289#findCoordLeaks`. **Phase 9 widened the projection by three fields and this deep scan still passes** (§8.4). |
...
`searchSaved` does put `lat` / `lng` on `metadata` (`discoverySearch.ts:1418#lat:`
and `discoverySearch.ts:1452#lat:`), but no `saved` row can arrive at
...
at `discoverySearch.ts:703#verified:` and `isOfficial` still comes straight off
the row at `discoverySearch.ts:704#isOfficial:`. §8.4's producer for G101,
...
| --- | --- | --- | --- |
| G31 | line 980 | `routes/discoverySearch.ts:1486#sensitivity_level,` | a bare `);` |
...
| G129 | line 980 | `routes/discoverySearch.ts:1486#sensitivity_level,` | a bare `);` |
