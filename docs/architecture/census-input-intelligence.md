...
| G23 | `allowPersonalization` gates memory read and write | C | Read `gateway.ts:227`; write refused at `personalization.ts:464-466`; the client's mirror is pinned to the server's by `test/inputPolicyContractParity.test.ts:110`. |
...
| G26 | `allowAI` gates AI assistance | C | `gateway.ts:586`, `:632`; `aiWriting.ts` requires it plus the per-request opt-in plus the flag. |
...
| G217 | Prefer local: request cancellation / state | C | `services/raceGuard.ts`; `useInputAssistance.ts:212`, `:282`. |
...
| G300 | `set_structured_value` | C | `projection.ts:191`, `:222`, `:245`; `socialIdentity.ts:335`, `:428`, `:501`; `validationSuite.ts:210`, `:245`. |
| G301 | `submit_search` | C | `projection.ts:285`; `semanticIntent.ts:175`, `:204`; `validationSuite.ts:347`. |
...
| G308 | `query_length_changed` | C | `useInputAssistance.ts:279`. |
...
(`artifacts/api-server/src/lib/inputAssistance/telemetry.ts:442#function classifyWriteFailure`).
...
which confirms both the SQLSTATE and that the constraint NAME arrives **inside
the message** rather than in a separate field — PostgREST does not reliably
surface `constraint`, so the message fallback in `constraintName`
(`telemetry.ts:383#function constraintName`) is the path that will actually fire
in production, and it was chosen from a measurement rather than from the docs.

Six tests in `artifacts/api-server/src/test/inputTelemetryPrivacyGuard.test.ts`
cover it, including a control that an unknown failure stays retryable, and one
asserting no `props` **values** reach the log line.
