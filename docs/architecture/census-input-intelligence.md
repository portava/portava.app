...
| G367 | Manual fallback rate | **N** | Both events now fire — `manual_value_kept` from `components/SmartInput.tsx:310#emitManualValueKept(telemetryField, value.trim().length)` and `raw_search_submitted` from two call sites (G314/G315) — which corrects this row's stated evidence. The RATE is still not computed and the events reach a no-op sink. WHAT WOULD TURN THIS RED: see G263. |
...
| G128 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:456#ageRestrictedSet` and `:461#blockedSet` — a null set from either suppresses every entity row; the picker branch repeats it at `:766#fetchBlockedSet`. |
...
| G184 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:456#ageRestrictedSet` and `:766#fetchBlockedSet`; the null-set refusal is the mutation-proven case in `src/test/inputAssistanceGateway.test.ts:384#suppresses`. |
...
and nothing else.
`travel-buddy-standalone/src/platform/input-assistance/components/SmartInput.tsx:98#selectionAnnouncement`
is the sentence and
`travel-buddy-standalone/src/platform/input-assistance/components/SmartInput.tsx:229#announceForAccessibility`
...
`SmartInput.tsx:301#input_opened`, `useInputAssistance.ts:264#suggestion_request_started`
...
| event | call site |
| --- | --- |
| `suggestion_rendered` | `components/SmartInput.tsx:180#emitSuggestionsRendered(telemetryField, suggestions)` |
| `validation_shown` | `components/SmartInput.tsx:182#if (validations > 0) emitValidationShown(telemetryField, validations)` |
| `suggestion_dismissed` | `components/SmartInput.tsx:171#emitSuggestionsDismissed(telemetryField, prev.count, focused ? 'no_results' : 'blur')` and `components/SmartInput.tsx:268#emitSuggestionsDismissed(telemetryField, shownRef.current.count, 'escape')` |
| `manual_value_kept` | `components/SmartInput.tsx:310#emitManualValueKept(telemetryField, value.trim().length)` |
| `raw_search_submitted` | `components/SmartInput.tsx:212#emitRawSearchSubmitted(telemetryField, (s.replacementText ?? value ?? '').length, true)` and `components/SmartInput.tsx:319#emitRawSearchSubmitted(telemetryField, value.trim().length, false)` |
| `correction_accepted` | `components/SmartInput.tsx:209#if (s.type === 'correction') emitCorrectionAccepted(telemetryField, s)` |
| `disambiguation_selected` | `components/SmartInput.tsx:210#if (s.type === 'disambiguation') emitDisambiguationSelected(telemetryField, s)` |
...
`components/SmartInput.tsx:132#const shownRef` and its companion `acceptedRef`
