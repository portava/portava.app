...
| G128 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:501#ageRestrictedSet` and `:506#blockedSet` — a null set from either suppresses every entity row; the picker branch repeats it at `:802#fetchBlockedSet`. |
...
| G184 | `C ᵖ` (unreadable) | **C** | ᵖ | `lib/inputAssistance/gateway.ts:501#ageRestrictedSet` and `:802#fetchBlockedSet`; the null-set refusal is the mutation-proven case in `src/test/inputAssistanceGateway.test.ts:384#suppresses`. |
