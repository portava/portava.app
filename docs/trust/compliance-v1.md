...
| VF-7a | Account deletion calls `requestProviderDeletion()` **then** deletes the rows | TV-7a `C` | SATISFIED | `providerErasure.ts:58#export async function requestProviderDeletionForUser(` reads the refs, `AccountDeletionService.ts:1004#const provErasureOk = await step(steps, "request_provide` runs before the delete step; *"reads the refs BEFORE deleting the rows that hold them"* and *"an unreadable table is not 'nothing to redact' — it throws"* both green (6/6). |
...
| VF-7a | `artifacts/api-server/src/services/identityVerification/providerErasure.ts:5#verified-foundation-plan.md V-7 reads:` — *"verified-foundation-plan.md V-7 reads: …"*; `artifacts/api-server/src/services/accountDeletion/AccountDeletionService.ts:984#verified-foundation-plan.md V-7:`; `artifacts/api-server/src/test/verificationProviderErasure.test.ts:5#verified-foundation-plan.md V-7:` |
