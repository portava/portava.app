...
| RM-06 | `:15` no fake live data | `CX-04` | PARTIAL † | The output boundary exists (`artifacts/api-server/src/compass/CompassGroundingEnvelope.ts:233#if (!evidence.hasVerifiedLive && N`) but is turn-scoped, not claim-scoped — see CCL-12. |
...
| V2-16 | CPV2-04 `:26` current Experience value and switching cost | `CX-05` | PARTIAL | The substance is exactly the clause (`artifacts/api-server/src/lib/compassDecision.ts:91#export const SWITCHING_COST = 0.25`) and it is behind a FALSE-seeded flag. |
