...
> | **3** | **COMPLETE coverage matrix, 7 destinations × 4 categories** — **outranks starting any new feature**. **PRESERVE PER-CITY AND PER-CATEGORY NUMBERS ALONGSIDE THE AGGREGATE; the result must NOT be reduced to one percentage.** If the default endpoint stays unreachable, **use the mirror and pace it** — same OSM database, and a complete matrix from it beats a partial one from the canonical host. **Record which endpoint produced the numbers** | **DONE — 28/28 cells, 2121 places, zero failed queries.** And it moved two headline numbers: **`wikidata` 3.2% → 12.0%**, outdoor seating 33.0% → 15.0%. [`osm-tier1-coverage-findings.md`](osm-tier1-coverage-findings.md) |
...
> | **5** | **Change the default test city away from Cebu — but KEEP Cebu as an explicit low-coverage fixture. Do not solve Cebu by hiding it.** Two standing tests: does enrichment work when the source has data, and does the product degrade gracefully when it does not | **DONE** — Berlin (high-coverage, the only city with all six fields non-zero) + Cebu (low-coverage, kept). Guard test red-proved. [`test-destination-fixtures.md`](test-destination-fixtures.md) |
> | **6** | **Paris is SEPARATE.** 0.0% from arrondissement tagging semantics is a **NORMALISATION problem** — not evidence Tier 1 failed, not evidence for Tier 2. File as a **narrow geography adapter** so it does not contaminate the enrichment decision | **FILED** — classification kept; **scope corrected: 6 of 7 cities are at ~0%, so it is not narrow.** Berlin is the exception, not Paris. [`paris-geography-adapter.md`](paris-geography-adapter.md) |
>
...
| `test-destination-fixtures.md` | **Step 5** — the two standing QA destinations: **Berlin** (does enrichment work when the source has data) and **Cebu**, kept (does the product degrade gracefully when it does not) |
| `paris-geography-adapter.md` | **Step 6, FILED not started** — the neighbourhood normalisation gap. Classification unchanged; **scope corrected — 6 of 7 measured cities return ~0%** |
