...
Compass block at `routes/discovery.ts:2104#callerUserId`. *(Line citations
...
| **C1** | `NOT STARTED` (correct, but for the wrong reason) | The **Stage-0 serve-point log** at points 4/5 has existed since 2026-08-14 (`489d26b8a`) — `routes/discovery.ts:2149#CACHE_B_HIT` and `:2239#COMPASS_FRESH_RANK`. **That is not C1.** C1 is the *shadow* comparison at 4/5, and `logDiscoveryShadowServe` still has exactly one call site — `routes/discovery.ts:1975#logDiscoveryShadowServe`, inside `serveCachedPlaces`, so serve points **1–3 only**. C1 is genuinely NOT STARTED; the instrumentation that looks like it is a different phase's work. |
...
| **C3** | `NOT STARTED` | **IN PROGRESS** — PR #252, 2026-08-31, **5 of 6 requirements met**. `lib/discoveryDivergenceReport.ts` (pure aggregation), `src/scripts/reportDiscoveryDivergence.ts` (read-only CLI behind the audit front door), `src/test/discoveryDivergenceReport.test.ts`, `package.json` → `report:discovery-divergence`, registered in `READ_ONLY_AUDIT_ENTRY_POINTS` with a written reason (`scripts/check-guard-coverage.mjs:264-271#reportDiscoveryDivergence`). Requirement 2 is unmet — see below. |
| **D** | `ON EXPLICIT HOLD` | The **machinery landed inert** — PR #250, 2026-08-31, `routes/discovery.ts:1853#pdeCohort`. The hold is real and unchanged, but it is a hold on the *flip*, not on the build; reading the row as "nothing exists" was wrong in a way that would have caused someone to build it twice. |
...
else) — at `lib/discoveryDivergenceReport.ts:78-82#classifyServePoint`. So 1–3 can never be summed
...
| **Shadow for any cohort** | Same flag; and the D6 cohort gate fails closed to nobody (`lib/discoveryCohort.ts`, applied at `routes/discovery.ts:1958#shadowCohort`) | Phase F gate 1 — **not ruled** |
...
|---|---|---|
| **impression-path exposure denominator** (#365) | `content_distribution_stats.eligible_impressions` is incremented where the impression is written (`lib/rankLog.ts:154#recordImpressionDistributionStats`, `:248`; `lib/discoveryServeLog.ts:466#recordImpressionDistributionStats`), and the outcome route no longer touches it (`routes/rankEvents.ts:290#content_distribution_stats`). The denominator was previously a count of **conversions** | Fixes the `03_Trending` defect the architecture status list carried as open |
...
| **capped `local_momentum`** (#366) | `lib/discoveryLocalMomentum.ts:183#velocity` computes a 48h-vs-baseline place velocity; `portavaRank.ts:152` caps its contribution at `LOCAL_MOMENTUM_MAX_CONTRIBUTION = 0.15`, clamped at `:337-338` | **Step 7** — *"capped `local_momentum` as modifiers only"*, built to the words |
...
> | Unit | State | Evidence |
> |---|---|---|
> | **C1** — shadow at serve points 4–5 | **NOT STARTED** | `logDiscoveryShadowServe` has one call site, `routes/discovery.ts:1975#logDiscoveryShadowServe`, inside `serveCachedPlaces` ⇒ serve points **1–3 only** |
...
`routes/discovery.ts:2104#callerUserId` *(the same block was at 1253 when this
...
> `logDiscoveryServe` at `routes/discovery.ts:2149#CACHE_B_HIT` and `:2004`, landed
...
| 2 — breaks down by `serve_point`, never sums 1–3 with 4–5 | **HALF MET** | It groups by serve-point **class** — `cache_a`(1/2/3) · `cold_rank`(6) · `other`(the rest) — `discoveryDivergenceReport.ts:78-82#classifyServePoint`. 1–3 can never be summed with 4–5, so the prohibition holds; the per-point breakdown does not exist, and **4/5 would land in an unlabelled `other` bucket alongside 7–10** |
| 3 — breaks down by `cohort_reason` | **MET** | Part of the group key, `discoveryDivergenceReport.ts:148-149#groupKey` |
| 4 — separates `sort_by` | **MET** | Also part of the group key, same line; `null` renders as `default` |
| 5 — surfaces `pde_suppressed_writes` | **MET** | `meanSuppressedWrites` per group, printed on the cost line (`discoveryDivergenceReport.ts:241-250#formatGroup`, in `formatGroup`) |
| 6 — read-only, front door, registered with a reason | **MET** | Guard imported at `reportDiscoveryDivergence.ts:15#ciProdReadOnlyAuditGuard`; registered with a written reason at `scripts/check-guard-coverage.mjs:264-271#reportDiscoveryDivergence` |
...
>
> `routes/discovery.ts:1853#pdeCohort` — inside `serveCachedPlaces`, when the mode
...
`routes/discovery.ts:1914`) and the pde branch ranks them per request. **The
...
(`routes/discovery.ts:1733-1749`) — a second impression row per served item would
...
|---|---|---|---|
| Enabling `shadow` for any cohort | **Owner** | **not ruled** | `DISCOVERY_ENGINE_MODE` seeded `enabled=false`, `metadata.mode='legacy'` (`2091:70-73`); the D6 cohort fails closed to nobody (`lib/discoveryCohort.ts`, applied at `routes/discovery.ts:1958#shadowCohort`) |
