...
| **C14** | **`/discovery/suggest` is fail-soft: any internal error → `200 { groups: [] }`** | **`docs/specs/discovery-v1/11_API_Specification.md:103#A failure must not masquerade as success.` and §9's six distinguishable failure classes — the specification says the OPPOSITE of the header** | **C → W. Moved by census §11.6; RE-EXECUTED HERE and confirmed.** All three exits still collapse to one empty success: `artifacts/api-server/src/routes/discoverySearch.ts:2920#discoveryRefusal("validation", "query_too_short", "GET /discovery/suggest")`, `artifacts/api-server/src/routes/discoverySearch.ts:2966#discoveryRefusal("transient_db", "visibility_state_unreadable", "GET /discovery/suggest")`, `artifacts/api-server/src/routes/discoverySearch.ts:3050#discoveryRefusal("transient_db", "suggest_failed", "GET /discovery/suggest")`. The failure IS observable server-side (`logger.warn`) and is not observable to the caller, which is what §9 asks for. Owner decision D11 stands. |
