...
export async function createTrip(input: CreateTripInput): Promise<TripRow | null> {
  ...
  if (userErr || !u?.user?.id) {
    console.warn('[createTrip] no authenticated user:', userErr?.message);
    return null;
  }
  const uid = u.user.id;
  const { data: sess } = await supabase.auth.getSession();
  const tok = sess.session?.access_token;
  if (!tok) { console.warn('[createTrip] no access token'); return null; }
  ...
  if (error) { console.warn('[createTrip] insert error:', error.message, error); return null; }
  if (!data) return null;
  return mapTrip(data);
}
