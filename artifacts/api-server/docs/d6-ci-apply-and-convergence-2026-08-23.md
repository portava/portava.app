...
> **62 foreign keys point at `public.profiles` with `ON DELETE NO ACTION` or
...
into profiles, and 62 constraints would reject it. That is not "deletion works
properly"; it is the same failure this quarter has been removing, moved from
five edges to sixty-two.
...
(54 on CI, 62 on production — CI lacks the `journey_*` family.)
...
1. Resolve the 62 dormant blockers under the D6 rulings (each becomes ERASE,
   ANONYMIZE, or a documented retention — the same five rulings, applied to FK
   rules instead of tables).
...
- **The 62 dormant blockers** need the same treatment the five got.
