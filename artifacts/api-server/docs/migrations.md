...
| --- | --- | --- |
| `2103_journey_segment_shadow.sql` | 2026-08-21 (applied live through the Supabase Management API) | Created the shadow-only segment revision table and its original service RPCs with every Journey feature flag still disabled. Successor migration 2123 immediately sealed direct service-role table access and superseded the original read/write authority. |
| `2123_journey_shadow_controlled_rollout.sql` | 2026-08-21 (applied live through the Supabase Management API) | Installed the default-off staged rollout, centralized SQL authorization, quality-aware ingestion, sealed raw/derived read-write-delete paths, finite retention, global stop, and aggregate-only QA evidence tables/RPCs. Live verification found zero stages, assignments, issuances, observations, segments, truth rows, and QA reports; all Journey capability flags remained false; direct service-role privileges on raw and derived tables were absent; the first post-restart unified retention cycle was HEALTHY with zero backlog, lag, retries, or failures. |
| `2121_account_deletion_tombstone_contract.sql` | 2026-08-21 (applied live through the Supabase Management API) | Removed the profile-to-Auth cascade, rebound deletion requests to retained profile tombstones with `ON DELETE RESTRICT`, added leased execution-claim fields and integrity checks, and excluded deleted profiles from direct profile RLS reads. Verified from live catalog definitions and with a disposable PostgreSQL race/RLS rehearsal. |
| `2122_account_deletion_journey_revocation_compat.sql` | 2026-08-21 (applied live through the Supabase Management API) | Installed the service-only atomic Journey revocation RPC needed by account deletion and made the existing consent-revocation trigger tolerate the intentionally absent optional segment table. Applied twice in disposable PostgreSQL with the optional table both absent and present. Live verification confirmed no segment table was created, anonymous/authenticated execution is denied, service execution is granted, and both Journey flags remain disabled. |
