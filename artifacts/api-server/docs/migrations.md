...
## 2026-08-28 — Every feedback kind does something (2196)

- `2196_memory_feedback_completeness.sql` — closes the last two real gaps in the memory system. **Applied to CI 2026-08-28. Prod press pending owner.**
  - **`incorrect` was a silent no-op.** `memory_feedback` accepts five kinds and the route returns 201 for all of them, but retrieval only ever suppressed on `hide`/`forget`/`already_known`/`not_interested`. A user reporting *"this memory is wrong"* got a success response and the memory kept being served — and kept being injected into the Compass prompt. **The same failure shape as the original hide/forget blocker (2190): the API says yes, the read path never asks.** `incorrect` now suppresses on both read paths.
  - **The state vocabulary was dead.** `state` permitted `'hidden'` and `'forgotten'` and *nothing ever wrote them*. Suppression worked entirely through the feedback join, so behaviour was right — but anyone reading the schema would reasonably believe a suppressed memory is marked as such. A trigger now records the user's decision on the row, and `'disputed'` is added as a state distinct from `'forgotten'`: *"I never want to see this"* and *"this is factually wrong"* are different statements, and flattening them would lose the signal that the **derivation** is faulty.
  - Both mechanisms are kept deliberately: the feedback join stays the **authority** because it is durable across re-projection (2190), which a column on the row cannot be; the state column is a denormalised record for export, admin and direct readers.
  - **Verified on CI (rolled back):** compass `3 → 1` — `incorrect` and `hide` suppress, while `already_known` correctly still shows (it is a **discovery-scoped novelty signal**, not a request to forget); rediscovery `0`; states written `disputed` / `hidden` / `active` respectively. Persistence confirmed from a **separate transaction** (the 2195 lesson), and `anon`/`authenticated` still denied on both DROP/CREATEd functions (the 2190 lesson). Locally: typecheck clean, 88/88.
## 2026-08-28 — Production press of the memory chain (2182–2197)

Applied 2182 and 2183–2197 to **production** (`ajrurzioarfkagpuxfnb`). Each migration was run txn-wrapped with its own preconditions and postconditions, and verified from a separate connection afterwards. Three real defects surfaced during the replay — all three were invisible on CI:

- **`2190` was not deployable as committed.** Its postcondition read `SELECT oid FROM pg_proc p JOIN pg_namespace n ...`; `pg_proc` and `pg_namespace` both have an `oid` column, so it raised `42702 column reference "oid" is ambiguous` and aborted the whole migration. Both databases run PostgreSQL 17.6, so what had been applied to CI was **not** the text that got committed. Fixed in place (`p.oid`) — a forward migration cannot repair a file that will not run for the next person replaying it.
- **`2190`'s broad anon-grant guard was vacuous.** It matched `LIKE 'memory\\_%'` (doubled backslash). With `standard_conforming_strings` on, LIKE reads `\\` as a literal backslash, so the pattern only matched names *containing a backslash* — i.e. nothing. It passed by matching nothing, not by finding nothing wrong. Corrected to a single backslash; it then immediately caught a genuine problem (below).
- **`memory_events_no_update()` carried `anon`/`authenticated` EXECUTE on prod.** 2183 creates it with `CREATE OR REPLACE` and Supabase's `ALTER DEFAULT PRIVILEGES` grants EXECUTE on every new public function — trigger functions included. 2192 revoked it, but only two migrations later, so a strict in-order replay fails at 2190. The revoke now lives in **2183**, at the point of creation, with a matching postcondition. Impact was low (the body only raises), but the grant contradicted 2183's own least-privilege stance.

- `2197_memory_reset_category_scope.sql` — **new.** The replay also exposed a behavioural bug: 2194's event delete used `(p_memory_types IS NULL OR subject_type = ANY (p_memory_types) OR true)`. The trailing `OR true` made it unconditional, so *resetting a single category deleted the user's entire L2 ledger*. The filter could never have worked regardless — `memory_type` is `episodic|semantic|social|place|intent` while `memory_events.subject_type` is `city|user|place`, two different vocabularies, which is presumably why someone disabled it rather than fixed it. 2197 translates between them (`episodic→city`, `place→place`, `social→user`; semantic/intent have no ledger events). **Proved on CI:** seeded `city,place,user`; `reset(['social'])` returned `events=1` and left `city,place`; a full reset still clears everything.
...
## 2026-08-28 — Messaging RLS: recursion, correlation, and a policy that granted what it meant to subtract (2198)

- `2198_messaging_rls_recursion_and_correlation.sql` — **NOT yet applied to CI or prod** (see "Apply status" at the end of this entry). Three defects on the messaging read path, found while applying 2182. None was caused by 2182: no policy involved references any function 2182 relocated, and all three expressions are byte-identical in the 2026-08-19 baseline dump.

  Numbered **2198, not 2197**: `2197_memory_reset_category_scope.sql` was landed on `memory-prod-press-fixes` while this work was in progress. Two files with the same prefix in one directory is the collision this repo has already paid for once (the Phase-0 2100–2102/2109 renumber), so this took the next free number rather than the next number it started with. A draft of *this* entry, still carrying the 2197 number, was picked up by that branch's commit and merged to main in #193 — it described a migration that does not exist under that number. This change removes it; the memory prod-press entry it was wedged into is untouched.

  **D1 — messaging reads were dead in production (42P17).** `mtm_select` on `message_thread_members` contained `EXISTS (SELECT 1 FROM message_thread_members self …)`. RLS applies to tables named inside a policy expression, so evaluating the policy required evaluating the policy. Every read of `messages`, `message_threads` and `message_thread_members` raised `infinite recursion detected in policy for relation "message_thread_members"` — verified live as role `anon` on `SELECT count(*) FROM public.messages`.

  **D2 — the correlation was a tautology.** Both `mtm_select` and `msg_select` compared a column to *itself*: `mtm.thread_id = mtm.thread_id`, which is always true. The predicate asked *"is this user a member of ANY thread"*, not *"of THIS thread"*. **Fixing D1 alone would have converted a hard error into a silent cross-thread message leak**, which is why both are in one file.

  The root cause is worth recording because it is invisible on the page. The source reads `WHERE mtm.thread_id = thread_id`. An unqualified name inside a subquery resolves in the **innermost** scope that provides it, and `message_thread_members` has a `thread_id`, so it bound to the subquery's own alias — no ambiguity, no warning. The sibling policy in the same file wrote `WHERE mtm.thread_id = id` and is **correct**, purely because `message_thread_members` has no `id` column and the name could only resolve outward. The bug appears exactly where the inner and outer columns share a name — which is exactly where a correlation is needed. A scan of the live catalog for `x.c = x.c` finds these two policy expressions and nothing else in the schema.

  **D3 — `messages_hide_blocked_sender` was PERMISSIVE.** Not in the report that opened this work; found while auditing what else grants SELECT on `messages`. `supabase/migrations/0015_blocks.sql` created it to *remove* blocked senders' messages, reasoning in its own comment that "message thread access is already gated by message_thread_members". **Permissive policies OR; they do not AND.** So it independently *granted* every message whose sender had not blocked the caller — every message in the table, to every caller, `anon` included (`is_blocked(NULL, sender)` is false, `NOT false` is true, and anon holds `GRANT ALL` on `public.messages`). D1 masked it: the recursive policy errored before this one could grant. **Repairing D1+D2 and leaving D3 would have handed every message in the database to every anonymous caller** — the same trap as D1-without-D2, one layer down. It is rebuilt `AS RESTRICTIVE`, which is both the fix and the behaviour 0015 described.

  **The fix** is one SECURITY DEFINER predicate, `authz.is_thread_member(thread, user)`, used by both SELECT policies. It breaks the recursion (the body does not re-enter RLS) and it fixes the correlation *structurally*: the outer `thread_id` is passed as an **argument**, so there is no unqualified name left to mis-resolve. It lives in `authz`, not `public`, for the reason 2182 exists — a SECURITY DEFINER authz predicate in `public` is a PostgREST oracle answering "who is in which conversation" to anyone with the publishable key. `EXECUTE` is granted to `authenticated` because a policy expression is evaluated **as the caller**, so revoking it would break RLS rather than tighten it (the 2182 lesson); `authz` is not exposed by PostgREST, so the grant is not an endpoint. Requires `function:is_thread_member` in the `auditMigrationsVsLive` allowlist, alongside `function:is_blocked` and for the same reason.

  **Two behavioural changes stated plainly.** (1) `mtm_select` gains `left_at IS NULL` on the *viewer's* membership: 0010 added `left_at` and updated `mt_select` and `msg_select` but not `mtm_select`, so a user who left a thread already could not read its messages but could still read its roster. Their own row stays visible via the `auth.uid() = user_id` branch. (2) All three policies bind `TO authenticated` instead of PUBLIC — the predicates already required `auth.uid()`, so this grants nothing new and makes anon's exclusion structural (the 2143 convention).

  **What it deliberately does not do.** `mt_select` on `message_threads` is untouched: its correlation is correct, and its subquery resolves through the repaired `mtm_select`, so it stops raising 42P17 on its own. The roster read stays available to co-members — dropping the `EXISTS` branch outright was the other available repair, and it would have broken **four** live call sites that read *other* members' rows directly over PostgREST with a user JWT: `travel-buddy-standalone/app/messages/[id].tsx` at :1231 (DM read receipt), :1245 (group member count) and :1432 (group read-receipt avatars), plus `src/components/GroupChatScreen.tsx:571`. Three of them are explicitly `.neq('user_id', <me>)`, so `auth.uid() = user_id` alone cannot serve them, and they would have failed *silently* — empty sets, not errors. That is why this takes the SECURITY DEFINER route. Nothing in either app reads `public.messages` directly and there is no Realtime subscription on it; messages come from the API server on the service role.

  **The postconditions are the interesting part.** Beyond the usual existence checks: a back-referencing regex (`([a-z_]+)\.([a-z_]+) = \1\.\2`) over the **live** policy expressions on all three messaging tables, so D2 cannot be reintroduced through the apply path; an assertion that each table has **exactly one** permissive SELECT-granting policy, because a leftover permissive policy ORs and is precisely what D3 was; an assertion that `messages_hide_blocked_sender` is `RESTRICTIVE`; and an assertion that the helper's owner **equals the table's owner**, because SECURITY DEFINER only skips RLS when the owner is exempt — if a migration is ever applied by a different role, that catches it at apply time instead of letting 42P17 return at read time. `FORCE ROW LEVEL SECURITY` on `message_thread_members` is refused in the preconditions for the same reason.

  Finally, a **runtime probe** in the same transaction: it `SET LOCAL ROLE authenticated` with a fabricated stranger identity and reads all three tables, asserting no 42P17 and **zero rows**. Before this migration that same identity would have seen every message via D3. Its exception handler wraps *only* the role switch, so a 42P17 propagates and aborts rather than being swallowed as a skip. Per the 2195 lesson this is not the proof of record — an assertion inside the transaction being verified proves nothing about what persisted — it is a fail-closed gate. The proof of record is `src/test/messagingThreadIsolation.test.ts`, which runs against the live database from its own connection.

  **The regression test's design.** `test:messaging-thread-isolation`, wired into `live-db.yml`. Four fixture users, two threads: Alice and Bob in T1, **Carol in T2 only**, **Dave in no thread**. Carol is the D2 guard — a suite whose only negative case is a user with no threads passes happily under the tautology, because the tautology's failure mode is that membership becomes *global*, so only a user who is in some *other* thread can distinguish "in this thread" from "in any thread". Dave is the D3 guard, because a permissive blocked-sender policy grants regardless of membership. Neither case alone covers both leaks. The suite also pins the co-member roster read (so a future tightening cannot silently break the app), asserts a read error is never scored as "saw nothing" (42P17 *is* an error), and checks that a block still subtracts inside a shared thread — converting the policy to restrictive had to preserve what 0015 intended, not merely stop it granting.

  **Apply status.** Written and checked offline only. The session that produced it had no route to a database — no Supabase MCP, no `psql`, no Supabase CLI, and no credentials it was permitted to read — so **neither the CI apply nor the separate-transaction verification has been done**, and the "Verified on CI" line every other entry above carries is deliberately absent here rather than implied. Prod remains the owner's press, as always, and must follow a CI apply, not precede one. One precondition is already satisfied there: the entry above records 2182 as applied to production, so `authz.is_blocked` exists and the drift-tolerant `public`/`authz` lookup will resolve to `authz`.
