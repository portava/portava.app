    ...
    return {
      ...
      _resolve(): any {
        ...
        if (this._ins !== null) {
          ...
          const made = rows.map((r: any) => ({ id: `m-${state.messages.length + 1}`, ...r }));
...
describe("send path — the block guard's input", () => {
  ...
  it("an unreadable thread roster REFUSES the send instead of skipping the block check", async () => {
    ...
    assert.equal(state.messages.length, 0, "no message may reach a person whose block status is unknown");
