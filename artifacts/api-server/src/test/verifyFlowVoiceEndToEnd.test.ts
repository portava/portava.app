...
describe("FLOW 1 — record → upload → send → read → drawer, as one traveller's action", () => {
  ...
  it("a voice REPLY carries reply_to_id IN THE INSERT — a seam with the ordinary send path", async () => {
    ...
    //
    // UPDATED at integration: the target is now SEEDED INTO THIS THREAD. When
    // this case was written the voice route accepted any UUID-shaped
    // `replyToId` after a shape test and nothing more, so an invented id was a
    // legal fixture. It is not any more — `routes/telegraphVoice.ts` now checks
    // the reference against the thread, as `POST /threads/:id/messages` always
    // has, because `reply_to_id` is rendered as the quoted message's body
    // ATTRIBUTED TO ITS ORIGINAL AUTHOR and the read resolves it on the
    // BYPASSRLS service client. Replying to a message that does not exist is
    // not the divergence this case is about, so the fixture is made legitimate
    // rather than the guard relaxed; the assertion below is unchanged.
    ...
    (store.messages ??= []).push({
      ...
      thread_id: THREAD,
      sender_id: BOB,
      body: "what did you think of the place?",
      msg_type: "text",
      created_at: "2026-01-01T00:00:00.000Z",
    });
