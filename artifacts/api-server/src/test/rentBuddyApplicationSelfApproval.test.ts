...
async function makeUser(tag: string): Promise<{ id: string; token: string }> {
  ...
  const { error: pErr } = await sc.from("profiles").upsert({ id, handle: fixtureLabel(`${PREFIX}${tag}`), username: fixtureLabel(`${PREFIX}${tag}`), name: `rba ${tag}` }, { onConflict: "id" });
...
before(async () => {
  ...
  await purgeFixtureUsers(adminClient(), [fixtureEmail(`${PREFIX}applicant@example.com`)]);
