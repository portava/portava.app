...
/**
 * ── THE ZONE NAMES ARE RUN-SCOPED, AND THE TEARDOWN NO LONGER USES `like` ───
 *
 * DEFECT, before this: every zone this suite creates was named by a constant
 * (`geo_zone_test_zone`, `geo_zone_test_system`, …) and the `after` hook ran
 *
 *     await sc.from(TABLE).delete().like("name", `${PREFIX}%`);
 *
 * That pattern matches EVERY run's zones, so run A's teardown deleted run B's
 * live `geo_zones` rows mid-suite. It is a worse failure than the unique-handle
 * collision this commit also fixes: a 23505 stops the run loudly, whereas a
 * peer-delete leaves B asserting against zones that no longer exist —
 * `readZone()` returns null and "the zone must survive" fails for a reason that
 * has nothing to do with RLS.
 *
 * The fix is the same two halves as everywhere else. `fixtureLabel` appends
 * `_r<run id>` so a name belongs to exactly one run, and the sweep goes through
 * `purgeFixtureRowsDetailed` → `decideSweep`, which takes this run's rows at any
 * age (the teardown path) and a peer's only once it is too old for any live run
 * to still own it. `ZONE_BASES` holds the UNSCOPED bases because that is what
 * the sweeper's prefix prefilter needs; every read and write uses the scoped
 * name beside it.
 */
const ZONE_BASES = [
  `${PREFIX}zone`, `${PREFIX}system`, `${PREFIX}forged`,
  `${PREFIX}vf`, `${PREFIX}ups`, `${PREFIX}svc`,
];
const Z_MAIN = fixtureLabel(`${PREFIX}zone`);
const Z_SYSTEM = fixtureLabel(`${PREFIX}system`);
const Z_FORGED = fixtureLabel(`${PREFIX}forged`);
const Z_VF = fixtureLabel(`${PREFIX}vf`);
const Z_UPS = fixtureLabel(`${PREFIX}ups`);
const Z_SVC = fixtureLabel(`${PREFIX}svc`);
...
async function makeUser(tag: string): Promise<{ id: string; token: string }> {
  ...
  const id = c.user.id;
  const { error: pErr } = await sc.from("profiles").upsert({ id, handle: fixtureLabel(`${PREFIX}${tag}`), username: fixtureLabel(`${PREFIX}${tag}`), name: `gz ${tag}` }, { onConflict: "id" });
...
before(async () => {
  ...
  const sc = adminClient();
  const { data, error } = await sc.from(TABLE).insert({ name: Z_MAIN, zone_type: "city", verified: false, featured: false }).select("id").single();
  if (error) throw new Error(`fixture: could not seed zone: ${error.message}`);
  zoneId = (data as any).id;
  const { data: sys, error: sErr } = await sc.from(TABLE).insert({ name: Z_SYSTEM, zone_type: "city", is_system: true }).select("id").single();
...
after(async () => {
  ...
  // Was `.delete().like("name", `${PREFIX}%`)`, which took a concurrent run's
  // zones with it. This sweeps by exact run-scoped label through decideSweep.
  await purgeFixtureRowsDetailed(sc, TABLE, "name", ZONE_BASES);
...
describe("geo_zones client-write boundary", { skip: !CREDS_AVAILABLE }, () => {
  it("authenticated cannot INSERT a zone (even content-only)", async () => {
    const { error } = await userClient(ownerToken).from(TABLE).insert({ name: Z_FORGED, zone_type: "city" });
    assertDenied(error, "insert");
    const { data } = await adminClient().from(TABLE).select("id").eq("name", Z_FORGED);
  ...
  it("authenticated cannot INSERT a forged verified+featured zone", async () => {
    const { error } = await userClient(ownerToken).from(TABLE).insert({ name: Z_VF, zone_type: "city", verified: true, featured: true });
  ...
  it("authenticated cannot edit a system zone (closes the owner_update OR is_system defect)", async () => {
    ...
    assert.equal((await readZone(systemZoneId))?.name, Z_SYSTEM);
  });
  ...
  it("an upsert (alternate form) does not bypass the boundary", async () => {
    const { error } = await userClient(ownerToken).from(TABLE).upsert({ name: Z_UPS, zone_type: "city", verified: true });
  ...
  it("the service role can still create and verify zones", async () => {
    const sc = adminClient();
    const { data, error } = await sc.from(TABLE).insert({ name: Z_SVC, zone_type: "city", verified: false }).select("id").single();
