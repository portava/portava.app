 ...
 *      city/time-slice keys". `pruneOrphanedExperienceNodes` removes it, and
 *      REFUSES to remove anything when the read that would decide it failed —
 *      which is the case that separates a prune from a truncation.
 *
...
import { pruneOrphanedExperienceNodes } from "../compass/CompassGraphEngine.js";
...
describe("B. CH-03 — an experience node whose Memory is gone is pruned", () => {
  it("B1 — a deleted Memory's node and its edges are removed; a live Memory's are not", async () => {
    ...
    const r = await pruneOrphanedExperienceNodes(client);

    assert.equal(r.nodesChecked, 2);
    assert.equal(r.nodesDeleted, 1);
    assert.equal(r.edgesDeleted, 2);
    assert.equal(r.undecidedKeys, 0);
  ...
  it("B2 — a Memory switched to only_me or unpublished is pruned as well: the builder would not write it today", async () => {
    const store: Record<string, Row[]> = {
      compass_graph_nodes: [NODE("n1", "mem-private"), NODE("n2", "mem-draft")],
      memories: [
    ...
    const { client } = makeFake(store);
    const r = await pruneOrphanedExperienceNodes(client);
    assert.equal(r.nodesDeleted, 2);
    assert.equal(store.compass_graph_nodes!.length, 0);
  });

  it("B3 — AN UNREADABLE `memories` TABLE DELETES NOTHING, and says so", async () => {
    // This is the case that separates a prune from a truncation: an
    // eligibility-filtered read that FAILS returns the same empty set as one
    // where every memory is gone.
    const store: Record<string, Row[]> = {
    ...
    assert.equal(r.undecidedKeys, 2);
  ...
  it("B4 — an unreadable node table is not an empty one, AND the report can tell them apart", async () => {
    // Written weak first, and the mutation said so: dropping the `nodeErr`
    // binding left this test green, because an unreadable table and an empty
    // one produced the identical zero report. `sourceUnavailable` is what
    // makes the branch observable — the same fix `GraphRebuildReport`
    // already carries as `nodesFailed`.
    ...
    const unreadable = await pruneOrphanedExperienceNodes(brokenFake.client);
    assert.deepEqual(unreadable, { nodesChecked: 0, nodesDeleted: 0, edgesDeleted: 0, undecidedKeys: 0, sourceUnavailable: true });
    ...
    const genuinelyEmpty = await pruneOrphanedExperienceNodes(emptyFake.client);
    assert.equal(genuinelyEmpty.sourceUnavailable, false);
  ...
  it("B5 — a graph with no experience nodes is a no-op, not a scan of every edge", async () => {
    ...
    const r = await pruneOrphanedExperienceNodes(client);
    assert.equal(r.nodesChecked, 0);
    assert.equal(deletes.length, 0);
    assert.equal(store.compass_graph_edges!.length, 1);
  });

  it("B6 — an unreadable EDGE table still prunes the nodes and reports zero edges, rather than aborting", async () => {
    const store: Record<string, Row[]> = {
...
describe("C. both builds are WIRED, not merely written", () => {
  ...
  it("C2 — the scheduled rebuild prunes BEFORE it derives anything from the graph", () => {
    ...
    const pruneAt = rebuild.indexOf("pruneOrphanedExperienceNodes(db)");
    const modelsAt = rebuild.indexOf("buildCityWorldModels(db)");
    const confidenceAt = rebuild.indexOf("computeCityConfidenceIndex(db)");
    assert.ok(pruneAt > 0, "rebuildIntelligenceGraph does not prune at all");
    assert.ok(pruneAt < modelsAt, "world models are derived before the prune");
    assert.ok(pruneAt < confidenceAt, "the confidence index is derived before the prune");
