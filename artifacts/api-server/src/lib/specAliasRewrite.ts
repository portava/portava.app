...
// Exported so route tests can mount the exact same rewrite and exercise the
// URLs the mobile client actually calls (e.g. /api/buddy-bookings/*).
export const specAliasRewrite: RequestHandler = (req, _res, next) => {
  const u = req.url;
