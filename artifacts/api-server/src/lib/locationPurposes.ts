...
export const LOCATION_PURPOSES: readonly LocationPurpose[] = [
  ...
  },
  {
    ...
    lawfulBasis: "consent",
    retentionSeconds: null,
    retentionNote: "circle_presence carries TTL columns (stale_after_secs, expires_at, is_stale). The hourly IntelRetentionScheduler invokes runPresenceCleanup behind presence_cleanup_enabled; the existing internal route remains an operator-triggered fallback. circle_checkins is the append-only log behind it and stores venue/approximate labels, never coordinates.",
