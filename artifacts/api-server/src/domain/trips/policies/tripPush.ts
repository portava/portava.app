 ...
 * server/trips/projectionWorkers/tripReminderScheduler.ts called lib/pushWithRetry.sendPushWithRetry
 ...
 *      quiet hours) with domain/trips/policies/TripAttentionPolicy.decideAttention;
...
import { sendPushWithRetry, type PushRecipient } from "../../../lib/pushWithRetry.js";
...
import { incrementTripMetric, readTripMetric } from "../services/tripMetrics.js";
import {
  decideAttention, mayPush, TRIP_PUSH_EVENT_PROFILES,
  type AttentionDecision, type AttentionLevel,
} from "./TripAttentionPolicy.js";
import type { AttentionState } from "../services/TripSignals.js";
...
export async function sendTripPush(
  ...
  if (!profile) throw new Error(`sendTripPush: '${kind}' has no TRIP_PUSH_EVENT_PROFILES row; add one (domain/trips/policies/TripAttentionPolicy.ts) before pushing it`);
