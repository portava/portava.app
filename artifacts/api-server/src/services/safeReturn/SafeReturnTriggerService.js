...
exports.getSuggestionReason = getSuggestionReason;
/** Hour (0–23) at which we consider a start "late night". */
var LATE_NIGHT_HOUR = 21;
 ...
 */
function shouldSuggest(planItem, _userId, userCtx) {
    if (userCtx === void 0) { userCtx = {}; }
    var reasons = [];
    ...
    // 2. Late-night start
    var hour = extractHour(planItem.startsAt);
    ...
    var shouldSuggest = reasons.length > 0;
    var confidence = "low";
    if (reasons.length >= 3)
        confidence = "high";
    else if (reasons.length >= 2)
        confidence = "medium";
    return { shouldSuggest: shouldSuggest, reasons: reasons, confidence: confidence };
}
/**
 * Human-readable summary of why Safe Return was suggested.
 * Suitable for display in the suggestion callout.
 */
function getSuggestionReason(reasons) {
    if (reasons.length === 0)
        return "";
    var labels = {
    ...
    if (reasons.length === 1)
        return labels[reasons[0]];
    var all = reasons.map(function (r) { return labels[r]; });
    var last = all.pop();
    return "".concat(all.join(", "), " and ").concat(last);
}
