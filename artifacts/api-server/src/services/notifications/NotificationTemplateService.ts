...
export const TEMPLATES: NotificationTemplate[] = [
  ...
  // ── Compass Sense (Phase 11) — proactive nudges, always sent with explicit
  // title/body from the evaluator; these templates provide honest fallbacks
  // plus category/priority/channel defaults.
  tpl({
    eventType: 'compass.sense.saved_event_starting',
    category: 'compass',
    defaultPriority: 'normal',
    defaultChannels: ['in_app', 'push'],
    title: () => 'Saved event starting soon',
    body: ({ eventTitle }) => eventTitle ? `${eventTitle} starts soon` : 'An event you saved starts soon',
    actionUrl: ({ eventId }) => eventId ? `/event/${eventId}` : '/',
  }),
  tpl({
    eventType: 'compass.sense.leave_earlier',
    category: 'compass',
    defaultPriority: 'normal',
    defaultChannels: ['in_app', 'push'],
    title: () => 'Leave earlier to make your next stop',
    body: ({ stopTitle }) => stopTitle ? `Travel to ${stopTitle} takes longer than the time left` : 'You may need to leave earlier for your next stop',
    actionUrl: ({ routePlanId }) => routePlanId ? `/route-plan/${routePlanId}` : '/',
  }),
  tpl({
    eventType: 'compass.sense.weather_change',
    category: 'compass',
    defaultPriority: 'normal',
    defaultChannels: ['in_app', 'push'],
    title: () => 'Weather update for your plans',
    body: ({ summary }) => summary ?? 'The forecast changed for your plans today',
    actionUrl: ({ tripId }) => tripId ? `/trip/${tripId}` : '/',
  }),
  tpl({
    eventType: 'compass.sense.circle_plan_change',
    category: 'compass',
    defaultPriority: 'normal',
    defaultChannels: ['in_app', 'push'],
    title: () => 'Your Circle plans changed',
    body: ({ meetupTitle }) => meetupTitle ? `${meetupTitle} was updated` : 'A meetup you joined was updated',
    actionUrl: ({ meetupId }) => meetupId ? `/meetup/${meetupId}` : '/',
  }),
  tpl({
    eventType: 'compass.sense.free_time_block',
    category: 'compass',
    defaultPriority: 'low',
    defaultChannels: ['in_app'],
    title: () => 'You have a free block today',
    body: () => 'Nothing on your plan for a while — want ideas nearby?',
    actionUrl: ({ tripId }) => tripId ? `/trip/${tripId}` : '/',
  }),
  tpl({
  ...
  tpl({
    ...
    defaultPriority: 'low',
    defaultChannels: ['in_app'],
    title: () => 'Your stamp was admired ✨',
    body: ({ stamp }) => `Someone admired your ${stamp ?? 'travel'} stamp`,
    actionUrl: ({ stampId }) => stampId ? `/stamp/${stampId}` : '/(tabs)/passport?tab=stamps',
  }),
  tpl({
