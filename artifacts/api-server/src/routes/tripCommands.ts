...
router.post("/trips/:tripId/commands", asyncHandler(async (req, res) => {
  ...
  const status = FORBIDDEN_REASONS.has(reason) ? 403
    ...
    : reason === "TRIP_VERSION_CONFLICT" || reason === "TRIP_PLAN_VERSION_CONFLICT"
      || reason === "TRIP_IDENTITY_ALREADY_EXISTS" || reason === "TRIP_PARTICIPANT_ALREADY_EXISTS"
      || reason === "TRIP_STAGE_SEQUENCE_TAKEN" || reason === "TRIP_PROPOSAL_NOT_PENDING" ? 409
    : 400;
