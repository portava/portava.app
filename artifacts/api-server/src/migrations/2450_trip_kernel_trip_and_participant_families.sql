-- 2450_trip_kernel_trip_and_participant_families.sql
--
-- Trip Kernel contract v2: the trip-level, participant-level, admin and system
-- command families, added to public.trip_kernel_execute. 2420 built the kernel
-- for the plan-item family only; 40 of the 47 direct canonical writers could
-- not migrate because the command they needed did not exist. This file adds
-- the commands. It does NOT move any writer — that is TypeScript, flag-gated,
-- and stays exactly what it was while trip_kernel_enabled is FALSE.
--
-- Spec: docs/specs/Portava_Trips_Development_Architecture_Spec_v4.txt
--   §3.1  primary lifecycle; terminal states
--   §4.1  TripCommand envelope; "actor capability" is one of the six checks
--   §4.2  trip.participant_joined, trip.trip_completed are spec-named events
--   §5.1  trip_participants(trip_id, user_id, role, membership_state) — our
--         deployed shape is trip_members(role member_role, status text)
--   §6.1  canInviteParticipant / canEditTrip — "roles are not permissions";
--         the function derives capability from role + trip + membership row
--   §6.2  "service role is not business authorization": every command is
--         re-authorized inside the function even though the route already did
--   Appendix B  TRIP_AUTH_*, TRIP_VERSION_*, TRIP_TEMPORAL_*, TRIP_IDENTITY_*
--
-- POST-CUTOVER CANONICAL FORWARD MIGRATION (2100-2999 band). Lane 2450.
--
-- ══════════════════════════════════════════════════════════════════════════════
-- DEPENDENCY ORDER — READ BEFORE APPLYING ANYWHERE
-- ══════════════════════════════════════════════════════════════════════════════
--   2334  authz schema + authz.is_trip_crew(uuid)
--   2337  authz.is_accepted_trip_member(uuid, uuid)   <- 2420 calls this
...
--   2450  THIS FILE — replaces trip_kernel_execute with v2, adds actor_role
--
-- Production (ajrurzioarfkagpuxfnb) measured 2026-09-07: NONE of 2334, 2337,
-- 2420 is applied there (0 authz helpers, no trips.version, no kernel tables,
-- no schema_migration_ledger). Apply 2334 -> 2337 -> 2420 -> 2450, in that
-- order, or the precondition block below RAISES and nothing changes.
-- portava-ci (hwokxgbmezheskbzskfr) has 2334, 2337 and 2420 applied.
--
-- ══════════════════════════════════════════════════════════════════════════════
...
-- direct write, byte for byte (measured; see the lane report).
--
-- ══════════════════════════════════════════════════════════════════════════════
-- CONTRACT v2 — WHAT CHANGES IN SHAPE
-- ══════════════════════════════════════════════════════════════════════════════
--   Envelope: + actor_role text in ('user','admin','system'), default 'user'.
--             actor_user_id may be NULL only when actor_role = 'system'.
--   Tables:   trip_events.actor_role, trip_command_receipts.actor_role (both
--             NOT NULL DEFAULT 'user'); trip_command_receipts.actor_user_id
--             becomes nullable with CHECK (actor_user_id IS NOT NULL OR
--             actor_role = 'system'). Every v1 row is a valid v2 row.
--   Return:   + contract_version: 2 on every response.
--   Events:   schema_version stays 1 — no existing event payload changes shape;
--             the new event types start at 1.
--
-- ══════════════════════════════════════════════════════════════════════════════
-- ACTOR CAPABILITY PER COMMAND (§4.1 "actor capability", §6.1)
-- ══════════════════════════════════════════════════════════════════════════════
--   crew     authz.is_accepted_trip_member(trip, actor)     — the 2337 rule,
--            i.e. lib/http.ts requireTripMember
--   owner    trips.owner_id = actor OR an accepted trip_members row with role
--            'owner' (the same two sources requireTripMember consults)
--   invited  a trip_members row for the actor with role 'invited' — the ONLY
--            capability an accepted-crew check cannot express; this is why
--            ACCEPT_INVITE / DECLINE_INVITE could not exist in v1
--   admin    actor_role = 'admin' AND profiles.role = 'admin' for the actor —
--            the predicate lib/requireAdmin.ts applies (DEFAULT_ROLES)
--   system   actor_role = 'system', no user; only SET_TRIP_COVER accepts it
--   none     CREATE_TRIP — there is no aggregate to be a member of yet; the
--            route's requireUser (ban gate) is the authorization
--
--   Plan family (2420):  ADD_PLAN UPDATE_PLAN MOVE_PLAN CONFIRM_PLAN CANCEL_PLAN
--                        COMPLETE_ACTIVITY REMOVE_PLAN REORDER_PLAN
--                        LINK_PLAN_ROUTE_STOP                         -> crew
--   Trip family:         CREATE_TRIP -> none · UPDATE_TRIP CANCEL_TRIP
--                        COMPLETE_TRIP ARCHIVE_TRIP                   -> owner
--   Participant family:  INVITE_PARTICIPANT ADD_PARTICIPANT
--                        SET_PARTICIPANT_ROLE REMOVE_PARTICIPANT      -> owner
--                        ACCEPT_INVITE DECLINE_INVITE                 -> invited
--   Admin family:        ADMIN_HIDE_TRIP                              -> admin
--   System family:       SET_TRIP_COVER                               -> system
--
-- ══════════════════════════════════════════════════════════════════════════════
-- WHAT THIS DELIBERATELY DOES NOT DO
-- ══════════════════════════════════════════════════════════════════════════════
--   * No lifecycle COMPUTATION. trips.status is computed by lib/tripStatus.ts
--     (timezone-aware "today"); the kernel VALIDATES the transition it is
--     handed (§3.1 terminal states are sticky) and records it. Computing it
--     here would need Intl-style timezone day boundaries in SQL, a second
--     implementation of a rule the census already found diverging once.
--   * No change to trip_members' row shape for an invite. Legacy writes
--     role='invited' and leaves status at its default 'accepted' (yes — a
--     pending invite row says status 'accepted'; requireTripMember excludes it
--     by ROLE). The kernel writes the same row so every reader, including the
--     enforce_trip_max_members trigger that counts status='accepted', sees
--     what it saw before. Fixing the shape is a separate, reader-by-reader
--     change and is recorded in the lane report, not smuggled in here.
--   * No reminder bookkeeping commands. trips.reminder_sent_at /
--     reminder_retry_count / reminder_delivered_at are scheduler claim columns,
--     not aggregate state; a version bump per reminder would hand every client
--     a spurious TRIP_VERSION_CONFLICT. lib/tripReminderScheduler.ts stays a
--     direct writer by classification (tripKernelWriterBaseline.ts).
--   * Still no temporal/spatial engine, no commitment model, no outbox worker,
--     no projection worker (2420's list stands). The ONE temporal check added
--     is start_date <= end_date, because it is the check the routes already
--     make and it is genuinely temporal; it is not the §7 engine.
...
DO $$
BEGIN
  IF to_regprocedure('authz.is_accepted_trip_member(uuid, uuid)') IS NULL THEN
    RAISE EXCEPTION 'PRECONDITION FAILED: authz.is_accepted_trip_member(uuid, uuid) missing -- apply 2334 then 2337 first.';
  END IF;
  IF to_regprocedure('public.trip_kernel_execute(jsonb)') IS NULL
     OR to_regclass('public.trip_events') IS NULL
     OR to_regclass('public.trip_command_receipts') IS NULL
    ...
    RAISE EXCEPTION 'PRECONDITION FAILED: 2420 (trip kernel foundation) is not applied.';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'profiles' AND column_name = 'role') THEN
    RAISE EXCEPTION 'PRECONDITION FAILED: profiles.role missing -- the admin capability cannot be expressed.';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'trip_members' AND column_name = 'status') THEN
    RAISE EXCEPTION 'PRECONDITION FAILED: trip_members.status missing.';
  END IF;
END $$;

-- ── 1. Contract v2 columns (additive; every v1 row remains valid) ─────────────
ALTER TABLE public.trip_events
  ADD COLUMN IF NOT EXISTS actor_role text NOT NULL DEFAULT 'user';
ALTER TABLE public.trip_events DROP CONSTRAINT IF EXISTS trip_events_actor_role_check;
ALTER TABLE public.trip_events
  ADD CONSTRAINT trip_events_actor_role_check CHECK (actor_role IN ('user', 'admin', 'system'));
COMMENT ON COLUMN public.trip_events.actor_role IS
  'Trip Kernel contract v2 (migration 2450): who the actor was acting AS — user (crew/owner/invitee), admin (moderation), system (no user; automated job). Rows written by contract v1 carry the default ''user''.';

ALTER TABLE public.trip_command_receipts
  ADD COLUMN IF NOT EXISTS actor_role text NOT NULL DEFAULT 'user';
ALTER TABLE public.trip_command_receipts DROP CONSTRAINT IF EXISTS trip_command_receipts_actor_role_check;
ALTER TABLE public.trip_command_receipts
  ADD CONSTRAINT trip_command_receipts_actor_role_check CHECK (actor_role IN ('user', 'admin', 'system'));
ALTER TABLE public.trip_command_receipts ALTER COLUMN actor_user_id DROP NOT NULL;
ALTER TABLE public.trip_command_receipts DROP CONSTRAINT IF EXISTS trip_command_receipts_actor_present;
ALTER TABLE public.trip_command_receipts
  ADD CONSTRAINT trip_command_receipts_actor_present CHECK (actor_user_id IS NOT NULL OR actor_role = 'system');

-- ── 2. The kernel write path, contract v2 ─────────────────────────────────────
-- p_command (snake_case §4.1 envelope):
--   command_id uuid · trip_id uuid · actor_user_id uuid (NULL only for system)
--   actor_role text? ('user' default | 'admin' | 'system')
--   expected_trip_version bigint? · idempotency_key text · type text
--   payload jsonb · client_observed_at timestamptz? · correlation_id uuid?
-- Returns jsonb: { ok, duplicate, version, event_id, sequence, result,
-- contract_version } or { ok:false, reason, ..., contract_version }.
-- Rejections are RETURNED, never RAISED, and every rejection happens before the
-- first write; the two trigger-raised errors that CAN happen mid-apply
-- (trip_full from enforce_trip_max_members) are caught in a subtransaction and
-- returned as TRIP_PARTICIPANT_CAPACITY_REACHED.
...
COMMENT ON FUNCTION public.trip_kernel_execute(jsonb) IS
  'Trip Kernel write path, contract v2 (migrations 2420 + 2450; Trips spec §4.1/§4.3/§4.4/§22.4). Applies one TripCommand from the plan, trip, participant, admin or system family: locks the trip, honours the idempotency receipt, re-checks the actor capability the command needs (crew / owner / invited / admin / system), checks expected_trip_version, applies the state change, bumps trips.version, writes trip_events + trip_outbox + trip_command_receipts. service_role only; application authorization runs before the call.';
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'trip_events' AND column_name = 'actor_role')
     OR NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'trip_command_receipts' AND column_name = 'actor_role') THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: actor_role column missing';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns
              WHERE table_schema = 'public' AND table_name = 'trip_command_receipts'
                AND column_name = 'actor_user_id' AND is_nullable = 'NO') THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: trip_command_receipts.actor_user_id still NOT NULL';
  END IF;

  -- The kernel function is still service-role only.
  ...
  -- Behaviour, without touching a row: every probe below is refused BEFORE any write.
  r := public.trip_kernel_execute('{}'::jsonb);
  IF r->>'reason' IS DISTINCT FROM 'TRIP_COMMAND_MALFORMED' OR (r->>'contract_version')::int <> 2 THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: {} => % (expected TRIP_COMMAND_MALFORMED, contract 2)', r;
  END IF;
  r := public.trip_kernel_execute(jsonb_build_object(
         ...
         'idempotency_key', 'post-1', 'type', 'NOT_A_COMMAND', 'payload', '{}'::jsonb));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_COMMAND_UNKNOWN_TYPE' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: unknown type => %', r;
  END IF;
  -- A user-role envelope cannot issue an admin command; an admin-role envelope
  -- cannot create a trip; a system envelope with no actor is well-formed but
  -- cannot issue a user command.
  r := public.trip_kernel_execute(jsonb_build_object(
         'command_id', gen_random_uuid(), 'trip_id', gen_random_uuid(), 'actor_user_id', gen_random_uuid(),
         'idempotency_key', 'post-2', 'type', 'ADMIN_HIDE_TRIP', 'payload', '{}'::jsonb));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_AUTH_ROLE_NOT_PERMITTED' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: user->ADMIN_HIDE_TRIP => %', r;
  END IF;
  r := public.trip_kernel_execute(jsonb_build_object(
         'command_id', gen_random_uuid(), 'trip_id', gen_random_uuid(), 'actor_user_id', gen_random_uuid(),
         'actor_role', 'admin', 'idempotency_key', 'post-3', 'type', 'CREATE_TRIP', 'payload', '{}'::jsonb));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_AUTH_ROLE_NOT_PERMITTED' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: admin->CREATE_TRIP => %', r;
  END IF;
  r := public.trip_kernel_execute(jsonb_build_object(
         'command_id', gen_random_uuid(), 'trip_id', gen_random_uuid(),
         'actor_role', 'system', 'idempotency_key', 'post-4', 'type', 'ACCEPT_INVITE', 'payload', '{}'::jsonb));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_AUTH_ROLE_NOT_PERMITTED' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: system->ACCEPT_INVITE => %', r;
  END IF;
  -- A missing actor on a user command is malformed, not a system command.
  r := public.trip_kernel_execute(jsonb_build_object(
         'command_id', gen_random_uuid(), 'trip_id', gen_random_uuid(),
         'idempotency_key', 'post-5', 'type', 'UPDATE_TRIP', 'payload', '{}'::jsonb));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_COMMAND_MALFORMED' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: actorless UPDATE_TRIP => %', r;
  END IF;
  -- Nothing above wrote anything.
  IF EXISTS (SELECT 1 FROM public.trip_command_receipts WHERE idempotency_key LIKE 'post-%') THEN
