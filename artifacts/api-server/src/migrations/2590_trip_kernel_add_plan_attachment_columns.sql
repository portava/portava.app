-- 2590_trip_kernel_add_plan_attachment_columns.sql
--
-- Trip Kernel, contract v2 extended (ADDITIVE, fourth pass): ADD_PLAN learns
-- the four trip_plan_items columns one legacy writer fills and the kernel could
-- not — added_by, description, city, country — and its location_is_private
-- default becomes the TABLE's default. No envelope change, no response change,
-- no new column, no new command type, no capability change: contract_version
-- stays 2. Every ADD_PLAN a 2500 database accepts, this one accepts with the
-- same result; only a payload carrying one of the four keys behaves differently
-- (2500 ignored them silently, 2590 writes them).
--
...
--   §4.1 ADD_PLAN; "the command service validates schema"
--   §24  Phase 1 ratchet: the satellite plan-item writers (routes/plan.ts,
--        routes/telegraphChat.ts, routes/hiddenGems.ts, routes/tripReservations.ts,
--        routes/compass.ts, compass/CompassAutopilotEngine.ts, lib/visuals/service.ts
--        gated in this pass; routes/events.ts and routes/airport.ts handed over)
--
-- POST-CUTOVER CANONICAL FORWARD MIGRATION (2100-2999 band). Lane 2590 (Trips).
--
-- ══════════════════════════════════════════════════════════════════════════════
...
--   2500  v2 + JOIN_VIA_LINK + host
--   2590  THIS FILE — replaces the 2500 body with 2500 + the ADD_PLAN edits
--   Also needs (already everywhere, measured 2026-09-07 on both databases):
--   trip_plan_items.added_by / description / city / country / location_is_private.
--
-- Measured 2026-09-07:
--   production (ajrurzioarfkagpuxfnb): NONE of 2420 / 2450 / 2500 applied (no
--     trips.version, no kernel function, no trip_events, no flag row). Apply
--     2334 -> 2337 -> 2420 -> 2450 -> 2500 -> 2590, in that order, or the
--     precondition block RAISES and nothing changes.
--   portava-ci (hwokxgbmezheskbzskfr): 2420 applied; 2450 and 2500 NOT applied.
--     This file was rehearsed there inside ONE transaction that ran 2450, then
--     2500, then this file, then behavioural probes, and ended in ROLLBACK;
--     nothing was left behind (the lane report records the probe output).
--
-- ══════════════════════════════════════════════════════════════════════════════
...
-- direct write, byte for byte (src/test/tripKernelSatellites.test.ts pins each
-- one as the exact table / verb / payload / filters the fake client observed).
--
-- ══════════════════════════════════════════════════════════════════════════════
-- WHAT CHANGES IN THE FUNCTION (a diff of 2500's body; nothing else moves)
-- ══════════════════════════════════════════════════════════════════════════════
--   ~ ADD_PLAN: INSERT column list + added_by, description, city, country,
--               read from the payload (NULL when absent — the table default).
--   + ADD_PLAN: added_by, when present and not null, must equal the actor —
--               attribution is of the actor, never of a third party; a
--               non-uuid is TRIP_COMMAND_MALFORMED. Checked BEFORE the write.
--   ~ ADD_PLAN: coalesce(location_is_private, FALSE) -> coalesce(..., TRUE).
--
-- DECISION RECORDED, NOT TAKEN SILENTLY — the location_is_private default
-- ====================================================================
-- 2420 wrote coalesce(payload.location_is_private, false) because its only
-- caller, routes/trips.ts POST /plan/items, sends `b.locationIsPrivate ?? false`
-- explicitly — so no caller has ever hit the kernel default. The TABLE default
-- (0011_trip_plan_coords) is TRUE, and four of the writers this pass moves
-- (events, telegraph, compass, airport) omit the column and so rely on it:
-- routing them through a kernel that defaulted to FALSE would have made their
-- rows coordinate-revealing (lib/plan.ts filterPlanItemForViewer exposes lat/lng
-- when location_is_private is false) where the legacy insert hid them. The
-- routes ALSO send the key explicitly, so the kernel row equals the legacy row
-- regardless of this default; the default is changed so the NEXT omitted key
-- fails private, not public. UPDATE_PLAN is untouched: a patch that names the
-- key with null still writes false, exactly as 2420 did.
--
-- WHY added_by / description / city / country ARE AGGREGATE STATE
-- ==============================================================
-- They are columns of trip_plan_items, a canonical table (§2.2). A column the
-- kernel cannot write is a column that forces a direct writer to stay — that
-- was routes/hiddenGems.ts's position. Widening ADD_PLAN's column list is the
-- change that removes the reason; no new command family is needed because the
-- semantics (a crew member adds an item to the plan) are exactly ADD_PLAN's.
--
-- Idempotent: CREATE OR REPLACE; the rollback restores the 2500 body verbatim
-- (db/rollback/2026-09-07-2590-trip-kernel-add-plan-attachment-columns-rollback.sql).
...
BEGIN
  IF to_regprocedure('public.trip_kernel_execute(jsonb)') IS NULL THEN
    RAISE EXCEPTION 'PRECONDITION FAILED: trip_kernel_execute missing -- apply 2334 -> 2337 -> 2420 -> 2450 -> 2500 first.';
  END IF;
  ...
  IF position('JOIN_VIA_LINK' in def) = 0 OR position('TRIP_AUTH_NOT_HOST' in def) = 0
    ...
    RAISE EXCEPTION 'PRECONDITION FAILED: 2500 (JOIN_VIA_LINK + host) is not applied -- apply 2450 then 2500 first.';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'trip_plan_items' AND column_name = 'added_by')
     OR NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'trip_plan_items' AND column_name = 'description')
     OR NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'trip_plan_items' AND column_name = 'city')
     OR NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'trip_plan_items' AND column_name = 'country')
     OR NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'trip_plan_items' AND column_name = 'location_is_private') THEN
    RAISE EXCEPTION 'PRECONDITION FAILED: trip_plan_items.added_by / description / city / country / location_is_private missing.';
  END IF;
  -- The table default this file adopts must be what it was measured to be.
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'trip_plan_items'
                    AND column_name = 'location_is_private' AND column_default = 'true') THEN
    RAISE EXCEPTION 'PRECONDITION FAILED: trip_plan_items.location_is_private default is not true -- the kernel default below would not match the table.';
  END IF;
END $$;

-- ── 1. The kernel write path, contract v2 + 2500 + the ADD_PLAN edits ─────────
-- The body below is 2500's body with exactly the edits listed in the header.
  ...
  ELSE
    ...
    CASE v_type
      ...
      WHEN 'ADD_PLAN' THEN
        ...
        END IF;
        -- 2590: added_by is attribution of the same actor, never a third party.
        BEGIN
          IF v_payload ? 'added_by' AND (v_payload->>'added_by') IS NOT NULL
             AND (v_payload->>'added_by')::uuid IS DISTINCT FROM v_actor THEN
            RETURN jsonb_build_object('ok', false, 'reason', 'TRIP_COMMAND_MALFORMED', 'detail', 'added_by must be the actor', 'contract_version', 2);
          END IF;
        EXCEPTION WHEN OTHERS THEN
          RETURN jsonb_build_object('ok', false, 'reason', 'TRIP_COMMAND_MALFORMED', 'detail', 'added_by must be a uuid', 'contract_version', 2);
        END;
        ...
        VALUES (
          ...
          -- 2590: the TABLE default (0011_trip_plan_coords: true). 2420 defaulted the kernel to
          -- false, which no caller relied on (routes/trips.ts always sends the
          -- key) and which would have turned a legacy insert that omitted the
          -- column into a coordinate-revealing row on the kernel path.
          coalesce((v_payload->>'location_is_private')::boolean, true),
          v_payload->>'notes',
          coalesce((v_payload->>'sort_order')::integer, 0),
          coalesce(v_payload->>'lock_type', 'flexible'),
          coalesce(v_payload->>'visibility', 'members'),
          -- 2590: the hidden-gem attachment shape (routes/hiddenGems.ts POST
          -- /hidden-gems/:id/plan). added_by must be the actor or absent.
          (v_payload->>'added_by')::uuid,
...
UPDATE public.feature_flags
   SET description = 'Trip Kernel (Trips spec §4), contract v2 + 2500 + 2590: plan-item, trip (create/patch/settings/cancel/complete/archive/delete), participant (invite/accept/decline/add/role/remove, join-request approve, invite-link join), route-plan-link, and the satellite plan-item writes (meetup/place/event/hidden-gem/telegraph/reservation/compass/layover add-to-plan, autopilot apply) plus the generated trip cover in routes/trips.ts, routes/trips-expansion.ts, routes/requests.ts, routes/routePlan.ts, routes/plan.ts, routes/events.ts, routes/hiddenGems.ts, routes/telegraphChat.ts, routes/tripReservations.ts, routes/compass.ts, routes/airport.ts, compass/CompassAutopilotEngine.ts and lib/visuals/service.ts go through public.trip_kernel_execute (aggregate version, domain event, outbox row, idempotency receipt). FALSE = every write path is exactly what it was.'
...
BEGIN
  def := pg_get_functiondef('public.trip_kernel_execute(jsonb)'::regprocedure);
  -- The 2590 body is installed: the four columns are in ADD_PLAN's INSERT and
  -- the kernel default for location_is_private is the table's.
  IF position('added_by, description, city, country)' in def) = 0
     OR position('added_by must be the actor' in def) = 0
     OR position('coalesce((v_payload->>''location_is_private'')::boolean, true)' in def) = 0 THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: the 2590 body is not installed';
  END IF;
  -- Nothing 2500 installed was lost.
  IF position('JOIN_VIA_LINK' in def) = 0 OR position('TRIP_AUTH_NOT_HOST' in def) = 0
     OR position('TRIP_AUTH_NOT_LINK_HOLDER' in def) = 0 OR position('ACCEPT_INVITE' in def) = 0 THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: a 2450/2500 branch is missing from the 2590 body';
  END IF;
  ...
  -- ADD_PLAN against a random trip still reaches the aggregate lock (known type).
  r := public.trip_kernel_execute(jsonb_build_object(
         ...
         'idempotency_key', 'post-2590-1', 'type', 'ADD_PLAN',
         'payload', jsonb_build_object('title', 'x', 'added_by', gen_random_uuid(), 'city', 'Cebu')));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_NOT_FOUND' OR (r->>'contract_version')::int <> 2 THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: ADD_PLAN on a missing trip => % (expected TRIP_NOT_FOUND, contract 2)', r;
  END IF;
  -- Everything 2500 refused is still refused.
  ...
  r := public.trip_kernel_execute(jsonb_build_object(
         'command_id', gen_random_uuid(), 'trip_id', gen_random_uuid(), 'actor_user_id', gen_random_uuid(),
         'idempotency_key', 'post-2590-2', 'type', 'NOT_A_COMMAND', 'payload', '{}'::jsonb));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_COMMAND_UNKNOWN_TYPE' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: unknown type => %', r;
  END IF;
  r := public.trip_kernel_execute(jsonb_build_object(
         'command_id', gen_random_uuid(), 'trip_id', gen_random_uuid(), 'actor_user_id', gen_random_uuid(),
         'idempotency_key', 'post-2590-3', 'type', 'JOIN_VIA_LINK', 'payload', jsonb_build_object('invite_link_id', gen_random_uuid())));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_NOT_FOUND' THEN
    RAISE EXCEPTION 'POSTCONDITION FAILED: JOIN_VIA_LINK on a missing trip => %', r;
  END IF;
  -- Nothing above wrote anything.
  IF EXISTS (SELECT 1 FROM public.trip_command_receipts WHERE idempotency_key LIKE 'post-2590-%') THEN
