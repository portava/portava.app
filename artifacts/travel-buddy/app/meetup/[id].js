...
function MeetupScreen() {
    ...
    // Calendar
    var _m = (0, react_1.useState)(false), calendarAdded = _m[0], setCalendarAdded = _m[1];
    var _o = (0, react_1.useState)(false), calendarActioning = _o[0], setCalendarActioning = _o[1];
    var _p = (0, react_1.useState)(false), showCalendarDenied = _p[0], setShowCalendarDenied = _p[1];
    var toastOpacity = (0, react_1.useRef)(new react_native_1.Animated.Value(0)).current;
    var toastTimer = (0, react_1.useRef)(null);
    function showToast() {
        if (toastTimer.current)
            clearTimeout(toastTimer.current);
        react_native_1.Animated.sequence([
            react_native_1.Animated.timing(toastOpacity, { toValue: 1, duration: 200, useNativeDriver: true }),
            react_native_1.Animated.delay(2000),
            react_native_1.Animated.timing(toastOpacity, { toValue: 0, duration: 300, useNativeDriver: true }),
        ]).start();
    }
    ...
    // Edit mode state
    var _q = (0, react_1.useState)(false), editing = _q[0], setEditing = _q[1];
    var _r = (0, react_1.useState)(''), editTitle = _r[0], setEditTitle = _r[1];
    var _s = (0, react_1.useState)(''), editLocation = _s[0], setEditLocation = _s[1];
    var _t = (0, react_1.useState)(''), editDesc = _t[0], setEditDesc = _t[1];
    var _u = (0, react_1.useState)(null), editDate = _u[0], setEditDate = _u[1];
    var _v = (0, react_1.useState)(null), editExactTime = _v[0], setEditExactTime = _v[1];
    var _w = (0, react_1.useState)(null), editTimeBlock = _w[0], setEditTimeBlock = _w[1];
    var _x = (0, react_1.useState)('title'), editFocusField = _x[0], setEditFocusField = _x[1];
    ...
    function handleAddToCalendar() {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        if (!meetup || calendarActioning || calendarAdded)
                            return [2 /*return*/];
                        setCalendarActioning(true);
                        return [4 /*yield*/, (0, calendar_1.addMeetupToCalendar)(meetup)];
                    case 1:
                        result = _b.sent();
                        ...
                        if (result.ok) {
                            ...
                            react_native_1.Alert.alert('Calendar error', (_a = result.message) !== null && _a !== void 0 ? _a : 'Could not add event to calendar.');
                        }
                        return [2 /*return*/];
                }
            });
    ...
    return (<react_native_1.View style={{ flex: 1, backgroundColor: tokens_1.color.paper }}>
      ...
      <react_native_1.ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {meetup.status === 'confirmed' && !!meetup.startsAt && react_native_1.Platform.OS !== 'web' && (<react_native_1.Pressable style={[s.calBtn, calendarAdded && s.calBtnAdded]} onPress={handleAddToCalendar} disabled={calendarAdded || calendarActioning}>
            {calendarActioning ? (<react_native_1.ActivityIndicator size="small" color={calendarAdded ? tokens_1.color.onInk : tokens_1.color.signal}/>) : calendarAdded ? (<lucide_react_native_1.Check size={16} color={tokens_1.color.onInk}/>) : (<lucide_react_native_1.CalendarDays size={16} color={tokens_1.color.signal}/>)}
            <react_native_1.Text style={[s.calBtnText, calendarAdded && s.calBtnTextAdded]}>
      ...
      {/* Success toast */}
      <react_native_1.Animated.View style={[s.toast, { opacity: toastOpacity }]} pointerEvents="none">
        <lucide_react_native_1.Check size={14} color="#fff"/>
        <react_native_1.Text style={s.toastText}>Added to your calendar</react_native_1.Text>
      </react_native_1.Animated.View>

      {/* Permission-denied bottom sheet */}
      <react_native_1.Modal visible={showCalendarDenied} transparent animationType="slide" onRequestClose={function () { return setShowCalendarDenied(false); }}>
        <react_native_1.Pressable style={s.modalBackdrop} onPress={function () { return setShowCalendarDenied(false); }}/>
        <react_native_1.View style={[s.modalSheet, { paddingBottom: insets.bottom + tokens_1.space.md }]}>
          <react_native_1.View style={s.modalHandle}/>
          <lucide_react_native_1.CalendarDays size={32} color={tokens_1.color.signal} style={{ alignSelf: 'center', marginBottom: tokens_1.space.sm }}/>
          <react_native_1.Text style={s.modalTitle}>Calendar Access Required</react_native_1.Text>
          <react_native_1.Text style={s.modalBody}>
          ...
          <react_native_1.Pressable style={s.modalSettingsBtn} onPress={function () { setShowCalendarDenied(false); react_native_1.Linking.openSettings(); }}>
            <react_native_1.Text style={s.modalSettingsBtnText}>Open Settings</react_native_1.Text>
          </react_native_1.Pressable>
          <react_native_1.Pressable style={s.modalDismissBtn} onPress={function () { return setShowCalendarDenied(false); }}>
            <react_native_1.Text style={s.modalDismissText}>Not now</react_native_1.Text>
          </react_native_1.Pressable>
        </react_native_1.View>
...
var s = react_native_1.StyleSheet.create({
    ...
    calBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: tokens_1.color.paper, borderRadius: tokens_1.radius.md, borderWidth: 1.5, borderColor: tokens_1.color.signal, paddingVertical: tokens_1.space.md },
    calBtnAdded: { backgroundColor: tokens_1.color.signal, borderColor: tokens_1.color.signal },
    calBtnText: __assign(__assign({}, tokens_1.type.bodyStrong), { color: tokens_1.color.signal }),
    calBtnTextAdded: { color: tokens_1.color.onInk },
    modalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.35)' },
    modalSheet: __assign({ backgroundColor: tokens_1.color.paperRaised, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: tokens_1.space.xl, gap: tokens_1.space.md }, tokens_1.shadow.card),
    modalHandle: { width: 36, height: 4, borderRadius: 2, backgroundColor: tokens_1.color.haze, alignSelf: 'center', marginBottom: tokens_1.space.sm },
    modalTitle: __assign(__assign({}, tokens_1.type.title), { color: tokens_1.color.ink, fontWeight: '700', textAlign: 'center', fontSize: 18 }),
    modalBody: __assign(__assign({}, tokens_1.type.body), { color: tokens_1.color.mute, textAlign: 'center', lineHeight: 22 }),
    modalSettingsBtn: { backgroundColor: tokens_1.color.signal, borderRadius: tokens_1.radius.md, paddingVertical: tokens_1.space.md, alignItems: 'center' },
    modalSettingsBtnText: __assign(__assign({}, tokens_1.type.bodyStrong), { color: tokens_1.color.onInk }),
    modalDismissBtn: { alignItems: 'center', paddingVertical: tokens_1.space.sm },
    modalDismissText: __assign(__assign({}, tokens_1.type.body), { color: tokens_1.color.mute }),
    toast: __assign({ position: 'absolute', bottom: 80, alignSelf: 'center', flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#16A34A', paddingHorizontal: tokens_1.space.lg, paddingVertical: tokens_1.space.sm + 2, borderRadius: tokens_1.radius.pill }, tokens_1.shadow.card),
    toastText: __assign(__assign({}, tokens_1.type.bodyStrong), { color: '#fff', fontSize: 14 }),
});
