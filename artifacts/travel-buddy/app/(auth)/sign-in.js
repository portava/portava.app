...
function SignIn() {
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 1:
                        ...
                        return [4 /*yield*/, (0, auth_1.lookupUsernameByEmail)(email.trim())];
                    case 2:
                        res = _c.sent();
                        if (res.error || !res.handle) {
                            setError((_b = res.error) !== null && _b !== void 0 ? _b : 'No account found with that email address.');
                            return [2 /*return*/];
                        }
    ...
    return (<react_native_1.KeyboardAvoidingView style={{ flex: 1, backgroundColor: tokens_1.color.paper }} behavior={react_native_1.Platform.OS === 'ios' ? 'padding' : undefined}>
      <react_native_1.ScrollView contentContainerStyle={[s.wrap, { paddingTop: insets.top + tokens_1.space.xxxl, paddingBottom: insets.bottom + tokens_1.space.xl }]} keyboardShouldPersistTaps="handled">
        <react_native_1.View style={s.centreBox}>
          ...
          <react_native_1.View style={s.card}>
            ...
            {/* ── Forgot-flow header ── */}
            {isForgot && (<react_native_1.View style={s.forgotHeader}>
                <react_native_1.Pressable onPress={function () { return switchMode('signin'); }} style={s.backBtn} hitSlop={8}>
                  <lucide_react_native_1.ArrowLeft size={18} color={tokens_1.color.mute}/>
                </react_native_1.Pressable>
                <react_native_1.Text style={s.forgotTitle}>
            ...
            <react_native_1.Pressable style={[s.submit, busy ? s.submitBusy : null]} onPress={mode === 'forgot-password' ? sendPasswordReset : mode === 'forgot-username' ? lookupUsername : submit} disabled={busy}>
              {busy
            ? <react_native_1.ActivityIndicator color={tokens_1.color.onInk}/>
            : <react_native_1.Text style={s.submitText}>
                    {mode === 'signin' ? 'Sign in'
...
var s = react_native_1.StyleSheet.create({
    wrap: { flexGrow: 1, paddingHorizontal: tokens_1.space.lg, justifyContent: 'center', gap: tokens_1.space.xl, alignItems: 'center' },
    centreBox: { width: '100%', maxWidth: 480, gap: tokens_1.space.xl },
    ...
    forgotRow: { flexDirection: 'row', alignItems: 'center', gap: tokens_1.space.sm, marginTop: -tokens_1.space.xs },
    forgotLink: __assign(__assign({}, tokens_1.type.small), { color: tokens_1.color.signal, fontWeight: '600' }),
    forgotSep: __assign(__assign({}, tokens_1.type.small), { color: tokens_1.color.faint }),
    forgotHeader: { flexDirection: 'row', alignItems: 'center', gap: tokens_1.space.md, marginBottom: tokens_1.space.xs },
    backBtn: { padding: 4 },
    forgotTitle: __assign(__assign({}, tokens_1.type.bodyStrong), { color: tokens_1.color.ink }),
    forgotHint: __assign(__assign({}, tokens_1.type.small), { color: tokens_1.color.mute, lineHeight: 18 }),
