/**
 * LanguagesScreen — dirty/save/guard tests.
 *
 * Run with: pnpm test:component
 *
 * ## What's covered
 *
 * 1. Save button is disabled until a language chip is toggled.
 * 2. A successful updateMyProfile (ok: true) calls updateMyProfile with the
 *    updated languages array and transitions the SaveBar to the 'Saved' state.
 ...
 *
 * languages.tsx uses useUnsavedGuard + useSavedThenBack with a ChipGrid.
...
import LanguagesScreen from '../languages.tsx';
...
describe('LanguagesScreen — dirty/save/guard', () => {
  ...
  it('a successful save calls updateMyProfile and transitions the bar to Saved', async () => {
    ...
    mockUpdateMyProfile.mockResolvedValue({
      ok: true,
      data: makeProfile({ spokenLanguages: ['English', 'French'] }),
    });
    ...
    // Toggle French — form is now dirty.
    await act(async () => { fireEvent.press(screen.getByText('French')); });
    ...
    expect(mockUpdateMyProfile).toHaveBeenCalledWith(
      expect.objectContaining({ spokenLanguages: expect.any(Array) }),
    );
  ...
  it('toggling a chip marks the form dirty and the guard fires on back-navigation', async () => {
    ...
    // Toggle French — form becomes dirty.
    await act(async () => { fireEvent.press(screen.getByText('French')); });
