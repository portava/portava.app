...
function DiscoveryHub() {
    ...
    var showMapToggle = react_native_1.Platform.OS !== 'web' && activeTab !== 'for_you';
    ...
    return (<react_native_1.View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      <react_native_1.View style={styles.tabRow}>
        <react_native_1.ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabBar} contentContainerStyle={styles.tabBarContent}>
          {TABS.map(function (tab) {
            ...
            return (<react_native_1.Pressable key={tab.key} style={[styles.tab, active && styles.tabActive]} onPress={function () { return handleTabChange(tab.key); }}>
        ...
        {showMapToggle && (<react_native_1.View style={styles.viewToggle}>
            <react_native_1.Pressable style={[styles.toggleBtn, viewMode === 'list' && styles.toggleBtnActive]} onPress={function () { return setViewMode('list'); }}>
              <react_native_1.Text style={[styles.toggleBtnText, viewMode === 'list' && styles.toggleBtnTextActive]}>
                List
              </react_native_1.Text>
            </react_native_1.Pressable>
            <react_native_1.Pressable style={[styles.toggleBtn, viewMode === 'map' && styles.toggleBtnActive]} onPress={function () { return setViewMode('map'); }}>
              <lucide_react_native_1.MapPin size={11} color={viewMode === 'map' ? tokens_1.color.signal : tokens_1.color.mute}/>
              <react_native_1.Text style={[styles.toggleBtnText, viewMode === 'map' && styles.toggleBtnTextActive]}>
                Map
              </react_native_1.Text>
            </react_native_1.Pressable>
          </react_native_1.View>)}
      ...
      <react_native_1.View style={{ flex: 1 }}>
        {activeTab === 'for_you' ? (<ForYouTab_1.ForYouTab key={"".concat(destination, "-").concat(contextMode)} destination={destination} onAddToPlan={handleAddToPlan} contextMode={contextMode}/>) : (<DiscoveryCategoryTab_1.DiscoveryCategoryTab key={"".concat(activeTab, "-").concat(destination, "-").concat(contextMode)} category={activeTab} destination={destination} onSelectPlace={handleSelectPlace} onAddToPlan={handleAddToPlanFromPlace} onPickDestination={handlePickDestination} contextMode={contextMode} viewMode={viewMode}/>)}
      </react_native_1.View>
