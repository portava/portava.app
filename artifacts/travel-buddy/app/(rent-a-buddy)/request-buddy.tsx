...
const CATEGORIES = ['city', 'nightlife', 'language', 'arrival', 'content', 'shopping', 'food', 'adventure', 'other'];
      ...
      <ScrollView style={s.body} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.chips}>
          {DURATIONS.map((d) => (
            <TravelChip
              key={d.value}
              label={d.label}
              active={durationMinutes === d.value}
        ...
        <View style={s.row}>
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <Pressable
              key={n}
              style={[s.numBtn, groupSize === n && s.numBtnSel]}
        ...
        <View style={s.toggleRow}>
          ...
          <Switch
            value={publicOnly}
            onValueChange={setPublicOnly}
            trackColor={{ false: color.haze, true: color.deep }}
          />
        </View>

        <Pressable
          style={[s.submitBtn, (loading || !city.trim()) && s.submitBtnDisabled]}
          onPress={submit}
