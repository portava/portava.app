...
// ── Mock freshToken (apiToken.ts) ─────────────────────────────────────────────
// The verification service obtains auth headers via freshToken() from apiToken.ts,
// not directly from supabase.auth.getSession() (see getSession bypass guard).
// NOTE: Intentionally exhaustive — apiToken.ts exports only `freshToken`; this
// mock stays in sync with the module so no real exports are silently dropped.
jest.mock('../apiToken.ts', () => ({
  freshToken: () => Promise.resolve('test-token'),
}));
