/**
 * WatchItemOverlay — Stamp It guard: already-liked item (long-press → press lifecycle).
 *
 * ## What's covered
 *
 * Long-pressing then pressing the Like button on an *already-liked* item calls
 * onLike zero times — the internal guard blocks both the Stamp It path
 * (already liked, nothing to do) and the spurious release press. No accidental unlike.
 *
...
it('long-press then press on already-liked item: onLike never fires — no accidental unlike', async () => {
  ...
  // Long-press: guard sets flag, isLiked=true so handleStampIt skips onLike.
  fireEvent(unlikeBtn, 'longPress');
  // Subsequent press: guard flag consumed, handleLikePress returns early.
  fireEvent.press(unlikeBtn); // press budget: 1 of ~2 (only press in this file)
