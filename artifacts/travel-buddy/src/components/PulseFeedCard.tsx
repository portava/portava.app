    ...
    >
      ...
      <View style={[s.postMedia, { height: mediaHeight }]}>
        ...
        {/* Passport-stamp label — city name top-left.
            When both the featured badge and this label are present, shift
            the label down so it clears the featured badge (~20px) + gap. */}
        <View style={[s.postcardLabel, item.featuredByPortava ? { top: 38 } : undefined]}>
          <MapPin size={9} color={color.onInk} />
          <Text style={s.postcardLabelText}>{item.city?.toUpperCase() ?? 'POSTCARD'}</Text>
        </View>
        {/* Date mark top-right */}
