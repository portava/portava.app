...
function DiscoveryUserAvatar(_a) {
    var _b, _c;
    var userId = _a.userId, avatarUrl = _a.avatarUrl, size = _a.size;
    var ringState = (0, useHighlightRingState_1.useHighlightRingState)(userId !== null && userId !== void 0 ? userId : null);
    var _d = (0, react_1.useState)(false), viewerOpen = _d[0], setViewerOpen = _d[1];
    var onPress = (ringState === null || ringState === void 0 ? void 0 : ringState.hasActive)
        ? function () { return setViewerOpen(true); }
        : userId
            ? function () { return expo_router_1.router.push("/profile/".concat(userId)); }
            : undefined;
    return (<>
      <HighlightRing_1.HighlightRing hasActive={(_b = ringState === null || ringState === void 0 ? void 0 : ringState.hasActive) !== null && _b !== void 0 ? _b : false} allViewed={(_c = ringState === null || ringState === void 0 ? void 0 : ringState.allViewed) !== null && _c !== void 0 ? _c : false} size={size} ringWidth={1.5} gap={1.5} onPress={onPress}>
        <react_native_1.Image source={{ uri: avatarUrl }} style={{ width: size, height: size, borderRadius: size / 2, backgroundColor: tokens_1.color.haze }}/>
...
function HiddenGemCard(_a) {
    ...
    var _e = (0, react_1.useState)(false), shareVisible = _e[0], setShareVisible = _e[1];
    var sharePayload = {
        sourceId: gem.id,
        sourceType: 'hidden_gem',
        title: gem.name,
        category: (_b = gem.category) !== null && _b !== void 0 ? _b : 'Hidden Gem',
        city: (_d = (_c = gem.city) !== null && _c !== void 0 ? _c : gem.neighborhood) !== null && _d !== void 0 ? _d : '',
        blurb: gem.blurb,
    };
    return (<>
      <react_native_1.View style={g.card}>
        <react_native_1.View style={g.media}>
          ...
          <react_native_1.Pressable style={g.saveIcon} hitSlop={tokens_1.layout.hitSlop} onPress={function () { return react_native_1.Alert.alert('Coming Soon', 'Saving gems is coming in a future update.'); }}><lucide_react_native_1.Bookmark size={15} color={tokens_1.color.onInk}/></react_native_1.Pressable>
        </react_native_1.View>
        <react_native_1.View style={g.body}>
          ...
          {gem.submittedBy ? (<react_native_1.View style={g.byRow}>
              ...
              <react_native_1.Pressable hitSlop={tokens_1.layout.hitSlop} onPress={gem.submittedBy.id ? function () { return expo_router_1.router.push("/profile/".concat(gem.submittedBy.id)); } : undefined}>
    ...
    }} onPress={function () { return planPicker.open({ id: gem.id, type: 'hidden_gem', title: gem.name, city: gem.city, category: 'Hidden Gem' }); }}>
              <react_native_1.Text style={g.addText}>Add to Plan</react_native_1.Text>
            </react_native_1.Pressable>
            <react_native_1.Pressable style={function (_a) {
        var pressed = _a.pressed;
        return [g.shareBtn, pressed && { opacity: tokens_1.layout.pressedOpacity }];
    }} hitSlop={tokens_1.layout.hitSlop} onPress={function () { return setShareVisible(true); }}>
              <lucide_react_native_1.Share2 size={13} color={tokens_1.color.mute}/>
            </react_native_1.Pressable>
          </react_native_1.View>
        </react_native_1.View>
      </react_native_1.View>
      <DiscoveryShareSheet_1.DiscoveryShareSheet visible={shareVisible} item={sharePayload} onClose={function () { return setShareVisible(false); }}/>
    </>);
}
...
function TravelerPickCard(_a) {
    ...
    return (<react_native_1.View style={tp.card}>
      <react_native_1.View style={tp.head}>
        ...
        <react_native_1.View style={{ flex: 1 }}>
          <react_native_1.Pressable hitSlop={tokens_1.layout.hitSlop} onPress={pick.user.id ? function () { return expo_router_1.router.push("/profile/".concat(pick.user.id)); } : undefined}>
    ...
    }} hitSlop={tokens_1.layout.hitSlop} onPress={function () { return react_native_1.Alert.alert('Coming Soon', 'Saving traveler picks is coming in a future update.'); }}>
    ...
    }} onPress={function () { return planPicker.open({ id: pick.id, type: 'place', title: pick.place, city: pick.city, category: pick.tag }); }}>
    ...
    return (<react_native_1.View>
      ...
      {items.length === 0 ? (<primitives_1.TravelEmptyState title="Nothing saved yet" sub="Save places, gems, and experiences to build your trip." action="Explore the city" onAction={function () { return expo_router_1.router.push('/(tabs)/discovery'); }}/>) : (<react_native_1.View style={sv.list}>
          {items.map(function (it) { return (<react_native_1.View key={it.id} style={sv.row}>
            ...
            }} onPress={function () { return planPicker.open({ id: it.id, type: 'place', title: it.name, city: it.neighborhood, category: it.type }); }}>
                <lucide_react_native_1.Plus size={13} color={tokens_1.color.signal}/><react_native_1.Text style={sv.addText}>Add to Plan</react_native_1.Text>
              </react_native_1.Pressable>
              <react_native_1.Pressable hitSlop={tokens_1.layout.hitSlop} onPress={function () { return react_native_1.Alert.alert('Coming Soon', 'Saving places is coming in a future update.'); }}><lucide_react_native_1.Bookmark size={17} color={tokens_1.color.signal} fill={tokens_1.color.signal}/></react_native_1.Pressable>
            </react_native_1.View>); })}
        </react_native_1.View>)}
    </react_native_1.View>);
}
...
var g = react_native_1.StyleSheet.create({
    ...
    btnRow: { marginTop: tokens_1.space.sm, flexDirection: 'row', alignItems: 'center', gap: tokens_1.space.xs },
    ...
    shareBtn: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center', borderRadius: tokens_1.radius.sm, borderWidth: 1, borderColor: tokens_1.color.haze },
});
