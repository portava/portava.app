 ...
 * set start/end points, pick a route style, and tap "Generate route".
 */
...
import { X, Route, GripVertical, Plus, Trash2 } from 'lucide-react-native';
...
export function RouteBuilderSheet({ visible, onClose, onRouteCreated, initialStops = [], tripId }: Props) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (missing.length > 0) {
      Alert.alert('Missing locations', `${missing.length} stop(s) are missing coordinates and will be skipped.`);
    }

    setGenerating(true);
    try {
      const candidateStops: CandidateStopInput[] = stops.map((s) => ({
        title: s.title,
        lat: s.lat,
        lng: s.lng,
  ...
  }, [stops, routeStyle, title, tripId, onRouteCreated]);
