"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AVATAR_TARGET_PX = exports.COVER_MAX_PX = exports.MAX_ORIGINAL_BYTES = void 0;
exports.renderAvatarImage = renderAvatarImage;
exports.renderCoverImage = renderCoverImage;
/**
 * Client-side image compression pipeline for profile photos.
 *
 * Uses expo-image-manipulator to resize + compress before upload so large
 * originals (up to 25 MB) never hit the API server's size caps.
 *
 * Variants produced:
 *   avatar   — 512 × 512 JPEG (q 0.82) — nav chips, passport cards, feed
 *   cover    — max 1 200 px wide, aspect-preserved JPEG (q 0.80) — cover banner
 */
var ImageManipulator = require("expo-image-manipulator");
var expo_image_manipulator_1 = require("expo-image-manipulator");
/** 25 MB — client gate; originals larger than this are rejected before rendering */
exports.MAX_ORIGINAL_BYTES = 25 * 1024 * 1024;
/** Target long-side for cover photos */
exports.COVER_MAX_PX = 1200;
/** Square side for avatar/profile variants */
exports.AVATAR_TARGET_PX = 512;
/**
 * Compress an avatar to 512 × 512 JPEG (quality 0.82).
 *
 * Expects a square-cropped input: expo-image-picker with
 * `allowsEditing: true, aspect: [1, 1]` already handles the crop.
 *
 * Falls back to the original URI if ImageManipulator fails so the upload
 * can still succeed (server's 5 MB cap is the final safety net).
 */
function renderAvatarImage(uri) {
    return __awaiter(this, void 0, void 0, function () {
        var result, _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, ImageManipulator.manipulateAsync(uri, [{ resize: { width: exports.AVATAR_TARGET_PX, height: exports.AVATAR_TARGET_PX } }], { compress: 0.82, format: expo_image_manipulator_1.SaveFormat.JPEG })];
                case 1:
                    result = _b.sent();
                    return [2 /*return*/, { uri: result.uri, width: result.width, height: result.height, mimeType: 'image/jpeg' }];
                case 2:
                    _a = _b.sent();
                    return [2 /*return*/, { uri: uri, width: exports.AVATAR_TARGET_PX, height: exports.AVATAR_TARGET_PX, mimeType: 'image/jpeg' }];
                case 3: return [2 /*return*/];
            }
        });
    });
}
/**
 * Compress a cover photo to at most 1 200 px on the longest side.
 *
 * Never upscales. Pass `originalWidth` from the expo-image-picker asset so we
 * only resize when the image is actually larger than the target.
 *
 * Falls back to the original URI on manipulator failure.
 */
function renderCoverImage(uri, originalWidth) {
    return __awaiter(this, void 0, void 0, function () {
        var needsResize, actions, result, _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _b.trys.push([0, 2, , 3]);
                    needsResize = originalWidth > exports.COVER_MAX_PX;
                    actions = needsResize
                        ? [{ resize: { width: exports.COVER_MAX_PX } }]
                        : [];
                    return [4 /*yield*/, ImageManipulator.manipulateAsync(uri, actions, { compress: 0.80, format: expo_image_manipulator_1.SaveFormat.JPEG })];
                case 1:
                    result = _b.sent();
                    return [2 /*return*/, { uri: result.uri, width: result.width, height: result.height, mimeType: 'image/jpeg' }];
                case 2:
                    _a = _b.sent();
                    return [2 /*return*/, { uri: uri, width: originalWidth, height: 0, mimeType: 'image/jpeg' }];
                case 3: return [2 /*return*/];
            }
        });
    });
}
