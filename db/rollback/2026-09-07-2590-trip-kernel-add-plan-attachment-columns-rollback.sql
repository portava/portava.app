-- Rollback for 2590_trip_kernel_add_plan_attachment_columns.sql
-- NOT applied anywhere as of 2026-09-07: not to portava-ci (hwokxgbmezheskbzskfr,
-- which has 2420 only — 2450 and 2500 are also unapplied there), not to
-- travel-buddy production (ajrurzioarfkagpuxfnb, which has no kernel at all).
-- Probed on portava-ci inside a rolled-back transaction only.
--
-- WHAT 2590 DID
-- =============
--   * CREATE OR REPLACE FUNCTION trip_kernel_execute(jsonb) — 2500's body with
--     ADD_PLAN writing added_by/description/city/country, refusing an added_by
--     that is not the actor, and defaulting location_is_private to TRUE.
...
-- =======================
-- Restores the 2500 function body VERBATIM — generated from
-- artifacts/api-server/src/migrations/2500_trip_kernel_join_via_link_and_host.sql
-- section 1; if that file and this one ever disagree, 2500 is authoritative —
-- and the 2500 COMMENT and flag description.
--
-- Nothing a user can observe changes while trip_kernel_enabled is FALSE (its
-- seeded value; 2590 does not touch it). If the flag was ever TRUE on this
-- database, routes/hiddenGems.ts POST /hidden-gems/:id/plan will find the 2500
-- body silently DROPPING added_by/description/city/country from the row, and
-- any ADD_PLAN that omits location_is_private will write FALSE (revealing
-- coordinates) — the routes this pass converted all send the key explicitly,
-- so only a future caller is exposed. Flip the flag FIRST regardless.
--
-- Data: NOTHING is deleted. Plan items written with the four columns stay as
-- written (they are ordinary rows). Events and receipts stay as the durable
-- record. Canonical rows are not touched.
--
-- Idempotent: CREATE OR REPLACE.

BEGIN;

-- 1. Contract v2 + 2500 function body (2500 §1), verbatim.
...
-- 2. Postcondition: the 2590 body is gone, 2500 still answers.
DO $$
DECLARE r jsonb; def text;
BEGIN
  def := pg_get_functiondef('public.trip_kernel_execute(jsonb)'::regprocedure);
  IF position('added_by' in def) > 0 THEN
    RAISE EXCEPTION 'ROLLBACK POSTCONDITION FAILED: the 2590 body is still installed';
  END IF;
  IF position('JOIN_VIA_LINK' in def) = 0 OR position('TRIP_AUTH_NOT_HOST' in def) = 0 THEN
    RAISE EXCEPTION 'ROLLBACK POSTCONDITION FAILED: the 2500 body is not what was restored';
  END IF;
  r := public.trip_kernel_execute(jsonb_build_object(
         ...
         'idempotency_key', 'rb-2590-1', 'type', 'ADD_PLAN', 'payload', jsonb_build_object('title', 'x')));
  IF r->>'reason' IS DISTINCT FROM 'TRIP_NOT_FOUND' THEN
    RAISE EXCEPTION 'ROLLBACK POSTCONDITION FAILED: ADD_PLAN => % (expected TRIP_NOT_FOUND)', r;
  END IF;
