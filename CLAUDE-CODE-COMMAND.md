# Portava — Claude Code Task: Apply Audit Fix Wave 1 + Complete Remaining Production Items

**Context for Claude Code:** You are working in the Portava (Travel Buddy) repo root. A full production audit was run on this codebase (see `PORTAVA-PRODUCTION-AUDIT.md`). 26 findings have already been FIXED in the attached bundle (`portava-fixes-files.zip` — 49 files, unzip over the repo root; or apply `portava-fixes.patch` with `git apply --binary`). Your job: **apply the bundle, verify it, then complete the remaining items listed in Part 3.** Do not re-litigate the fixes; verify and integrate.
