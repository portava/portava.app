...
**DB tables** — declared in `artifacts/api-server/src/migrations/0168_discovery_cache_ddl.sql`. (There is no `0154_discovery_cache.sql`; 0154 is `0154_rank_events_compass_surface.sql`.) 0168 is a *reconciliation*: the code read and wrote these tables before any migration declared them, and the L2 layer fails soft, so a missing table degrades silently to L1-only. Confirm they exist live rather than assuming the file means applied — see [migration-applied-vs-committed.md](migration-applied-vs-committed.md).
