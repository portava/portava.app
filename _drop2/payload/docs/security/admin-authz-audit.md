...
**Status as of 2026-08-09 (added after the fact; the body below is the original
audit plus dated corrections):**

| Item | Status |
|---|---|
| Finding 17 — `profiles.role` self-writable | **RESOLVED** by `2078_profiles_role_not_self_writable.sql` (§2). Verified live; 12 tests, each negative assertion proved capable of failing |
| Guard consolidation | **In progress** — 33 guards, 24 converted, **9 outstanding** across 8 route files (§2, "Two things this fix does NOT do", item 2). The "6 held back" figure in §1e and §3 was superseded |
| Finding 19 — `is_official` grant asymmetry | Open, **verified not exploitable** (§2). Not a privilege-escalation path |
| Regression detection for finding 17 | See the ⚠ subsection at the end of §2 |
...
6. *(Superseded later the same day: the denominator was wrong too. The real
figure is **33 guards, 24 converted, 9 outstanding** — `checkAdminGuard.ts`
matched `requireAdmin\w*` and never saw `requireVisualAdmin`,
`checkRentBuddyAccess` or `canEditEntity`. See §2, "Two things this fix does NOT
do", item 2. The dimensional argument in this section is unaffected; only the
count is.)* None is a security finding — every one still fails closed. They are cases
...
## 2. Finding 17 — `profiles.role` was self-writable (was CRITICAL; RESOLVED 2026-08-09)

> **Status: RESOLVED.** Fixed by `2078_profiles_role_not_self_writable.sql`
> (column-level `REVOKE UPDATE (role)` plus `trg_profiles_role_privileged`).
> Verified live and by tests proved capable of failing — see "RESOLVED
> 2026-08-09" below. It was CRITICAL and decision-ready when written on
> 2026-08-08; the original finding is kept verbatim below because the remedy
> analysis and the evidence are what make the fix reviewable.

The rest of §2 up to "RESOLVED 2026-08-09" is the finding exactly as recorded on
2026-08-08, when it was open. It is preserved, not amended.
...
### What was NOT done *by this session* (2026-08-08)

> **Overtaken by events on 2026-08-09.** The escalation *was* executed — by the
> concurrent session, against the unpatched schema, before `2078` was applied.
> `role` read back `'admin'` and 8 assertions failed. See "Evidence that the
> protection did not pre-exist" below. The paragraph beneath remains an accurate
> account of *this* session's read-only limits; it is no longer the state of
> knowledge about the finding.
...
### Why it was decision-ready and not fixed here (2026-08-08)

> Superseded by the resolution below: options **1 + 2 together** were chosen and
> shipped on 2026-08-09. Option 4 was discarded, option 3 deferred. The remedy
> menu is kept because it is what makes the chosen remedy reviewable.
...
### RESOLVED 2026-08-09 — fix shipped, verified live, and covered by tests proven capable of failing

**The column is no longer self-writable. Verified live, and verified by tests
that are proven capable of failing.**

> This heading previously read "*with a caveat that matters more than the fix*".
> That caveat — live protection existing in no migration — was **retracted** the
> same day (see the struck-through subsection below); it was a misreading of a
> concurrent session's uncommitted work. The retraction is kept; the heading no
> longer advertises the retracted claim above the fix. What survives of it is a
> narrower, real point about drift, recorded in that subsection.
...
#### ⚠ These tests did not run automatically anywhere as of 2026-08-09. Checked, not assumed.

> **Re-checked 2026-08-10 — facts 1 and 2 below have moved; do not read them as
> current.** `.github/workflows/` now exists in the working tree with `ci.yml`,
> `live-db.yml` and `unwired-checks.yml`, and `live-db.yml` invokes both
> credentialled suites by name (`test:rls-hardening`,
> `test:profile-role-not-self-writable`). `check:all` now runs seven checks, not
> six — `check:rank-events-surfaces` was added — but still runs no tests, so
> fact 2's substance holds and only its count is stale.
>
> **This is not yet a resolution and is deliberately not written up as one.**
> The entire `.github/` tree is **untracked** (`git ls-files .github` → 0 files;
> not gitignored). Per this document's own practical rule, presence in the tree
> is not evidence of history, and absence from history is not evidence that
> nothing was written — the shape is indistinguishable from a concurrent
> session's uncommitted work. Nothing here establishes that these workflows have
> ever executed, or that the repo they would run in has the credentials
> `live-db.yml` needs. Whoever picks this up: confirm the commit and confirm a
> run, then update finding 18 (`docs/security/no-automated-enforcement.md`,
> still marked **OPEN** and still asserting fact 1) and this subsection together.

The instruction was to check rather than ask. The result, as checked on
2026-08-09, was worse than "CI lacks credentials":
...
**Consequence, stated plainly (as of 2026-08-09; see the re-check note above):
finding 17 is fixed and verified, but nothing will tell you if it regresses.**
Dropping the trigger or re-granting the column
...
> **Corrected 2026-08-10.** Item 1 was still presenting finding 17 as an open
> choice long after §2 recorded it RESOLVED, and item 5 still carried the
> superseded 24/6 count. Both are restated below. The original wording is kept
> struck through rather than deleted, and the numbering is unchanged, because
> other notes and commit messages refer to these items by number. Items 2–5 are
> still genuine open decisions — that is why this section keeps its name.
...
1. ~~**Finding 17 — `profiles.role` self-writable.** Pick a remedy from §2.
   ...
   column.~~
   **RESOLVED 2026-08-09 — no decision outstanding.** The remedy was chosen and
   shipped: options 1 + 2 together (column-level `REVOKE UPDATE (role)` plus
   `trg_profiles_role_privileged`, with `admin_set_profile_role()` as the
   explicit privileged path) in
   `artifacts/api-server/src/migrations/2078_profiles_role_not_self_writable.sql`.
   Verified live, and covered by 12 tests each of whose negative assertions was
   proved capable of failing (§2). What is left here is **regression detection,
   not remedy selection** — see the ⚠ subsection at the end of §2.
...
5. ~~**Six further routes need their own reconciliation** (§1e)~~ —
   **corrected to nine guards across eight route files.** The six §1e named are
   `circle.ts`, `placesCanonical.ts`, `rentABuddySpec.ts`,
   `rentABuddyMarketplace.ts`, `compassGraph.ts`, and `rentABuddyRollout.ts`;
   the three `checkAdminGuard.ts` was structurally blind to are
   `requireVisualAdmin` (`adminVisuals.ts`), `checkRentBuddyAccess`
   (`rentABuddyRollout.ts`, a second guard in an already-listed file) and
   `canEditEntity` (`visuals.ts`). Per-guard assessment:
   `admin-guard-consolidation.md`, "Assessment of the 9 outstanding guards".
   ...
   Corrected status: **33 guards, 24 converted, 9 outstanding** across 8 route
   files. See §1e's parenthetical and §2, "Two things this fix does NOT do",
   item 2.

## 4. Verification note

- All 30 implementations were read in full, not sampled — but **30 was not the
  population.** There are 33; the detector and this audit shared one blind spot
  (§1e, §2 item 2). Reading every implementation you found is not the same as
  finding every implementation.
...
- ~~**Not verified:** that the escalation actually succeeds end to end. See
  §2.~~ **Superseded 2026-08-09 — it was verified, by execution.** This was
  accurate for *this* session, which was read-only and could not write to
  production. A concurrent session then ran the guard tests against the
  unpatched schema and **the escalation succeeded**: 8 assertions failed with
  `role` reading back `'admin'` via `.update()`, raw PostgREST `PATCH`,
  supabase-js upsert, PostgREST merge-duplicates upsert, and self-`INSERT`. See
  §2, "Evidence that the protection did not pre-exist". The original line is
  kept because it records the honest limit of the read-only audit at the time.
