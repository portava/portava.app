...
**Also:** the mobile mock-lint (`scripts/check-test-mocks.mjs`, runs inside `pnpm test`) rejects bare object-literal `jest.mock` factories — either spread `jest.requireActual(...)` or put a `// NOTE: ...` comment directly above the mock explaining why it stays exhaustive.
