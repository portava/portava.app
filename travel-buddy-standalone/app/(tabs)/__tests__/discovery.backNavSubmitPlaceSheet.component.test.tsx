/**
 * DiscoveryHub — Back-nav closes the Submit Place sheet (web guard)
 *
 * Task 3661 confirmed that the back-nav guard for SubmitPlaceSheet uses the
 * same capture-phase + dismissedByBack pattern as the PlaceDetailSheet guard
 * (fixed in Task 3657).  This test covers the popstate path for submitPlaceOpen.
 *
 * Confirmed behaviours:
 *   1. The sheet becomes visible after the submit-place button (testID
 *      "submit-place-btn") is pressed.
...
jest.mock('../../../src/context/SessionContext', () => ({
  // isAuthed=true so the submit-place button (testID "submit-place-btn") renders.
...
// ── Prop-capture stub ─────────────────────────────────────────────────────────
// Capture visible from SubmitPlaceSheet so the test can assert it changes.
// Renders a sentinel testID when visible so the test can confirm the sheet opened.
let capturedSubmitPlaceVisible: boolean | undefined = undefined;

jest.mock('../../../src/components/discovery/SubmitPlaceSheet', () => {
  const R = require('react');
  const actual = jest.requireActual('react-native');
  return {
    SubmitPlaceSheet: (props: { visible?: boolean; onClose?: () => void }) => {
      capturedSubmitPlaceVisible = props.visible;
      return props.visible
        ? R.createElement(actual.View, { testID: 'submit-place-sheet' })
        : null;
    },
  };
});
...
beforeEach(() => {
  jest.clearAllMocks();
  capturedSubmitPlaceVisible = undefined;
  ...
  // history.state (so the cleanup branch `if (!dismissedByBack && state?._submitPlaceSheet)`
...
describe('DiscoveryHub — back-nav closes Submit Place sheet (web guard)', () => {
  it('popstate closes the submit-place sheet and does not call history.back() a second time', async () => {
    const view = await render(<DiscoveryHub />);
    await act(async () => {});

    // ── Precondition: Discovery screen rendered, submit-place sheet closed ─────
    expect(view.getByText('Discover')).toBeTruthy();
    expect(capturedSubmitPlaceVisible).toBe(false);
    expect(view.queryByTestId('submit-place-sheet')).toBeNull();

    // submit-place-btn is the PlusCircle Pressable in the Discovery header.
    // It renders when isAuthed=true (confirmed by our SessionContext stub).
    expect(view.getByTestId('submit-place-btn')).toBeTruthy();

    // ── Open the Submit Place sheet via the header button ─────────────────────
    // One fireEvent.press is within the per-file RNTL + React 19 press budget.
    await act(async () => {
      fireEvent.press(view.getByTestId('submit-place-btn'));
    });

    // Sheet sentinel must be present — SubmitPlaceSheet received visible=true.
    expect(view.queryByTestId('submit-place-sheet')).not.toBeNull();
    expect(capturedSubmitPlaceVisible).toBe(true);
    ...
    // submitPlaceOpen became true.  Dispatching via our minimal window emitter
