/**
 * Passport — ⋯ menu button opens owner menu (fresh-load state)
 *
 * Confirms that pressing the ⋯ button while compactBarInteractive=false
 * (fresh load — compact bar hidden, pointerEvents='none') results in
 * PassportOwnerMenuSheet becoming visible.
 *
 * Sibling file passport.ownerMenuScrolled.component.test.tsx covers the
 * compactBarInteractive=true case in a separate renderer instance (each file
...
describe('Passport — ⋯ menu button (fresh-load state)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    // Fresh-load state: compact bar is transparent / hidden.
    // pointerEvents='none' on the bar so scroll content stays interactive.
  ...
  it('opens PassportOwnerMenuSheet when ⋯ is pressed on fresh load (compactBarInteractive=false)', async () => {
    ...
    // Pressing ⋯ must make the owner menu sheet visible.
