 ...
 * waitlist position, host info, and host action menu (if host/co-host).
 */
...
import {
  ...
  acceptWaitlistOffer, requestToJoinEvent, joinEventChat, updateEvent,
...
export default function EventDetailScreen() {
  ...
  async function handleJoinChat() {
    ...
    if (res.data?.threadId) {
      router.push(`/messages/${res.data.threadId}` as any);
    }
  }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        {isHost && (
          <Pressable style={styles.dashBtn} onPress={() => setShowDashboard(true)} hitSlop={8}>
      ...
      {loading && !event ? (
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <View style={styles.body}>
            ...
            {/* Gates */}
            {(event.trustScoreMin || event.ageMin || event.ageMax || event.verifiedOnly) && (
      ...
      {event && !isBanned && !loading && (
        <View style={[styles.actionBar, { paddingBottom: insets.bottom + space.md }]}>
          {event.state === 'cancelled' || event.state === 'archived' ? (
            ...
            <View style={styles.rsvpRow}>
              ...
              >
                <Text style={styles.rsvpCurrentText}>
                  {RSVP_OPTIONS.find((o) => o.key === event.myRsvp)?.label ?? 'Going'} {RSVP_OPTIONS.find((o) => o.key === event.myRsvp)?.emoji}
                </Text>
            ...
            /* Waitlisted with an active spot offer — show accept + leave */
            ...
            /* On waitlist but no active offer yet */
            <Pressable style={styles.leaveWaitlistBtn} onPress={handleLeaveWaitlist} disabled={rsvpLoading}>
              <Text style={styles.leaveWaitlistText}>Leave waitlist</Text>
            </Pressable>
