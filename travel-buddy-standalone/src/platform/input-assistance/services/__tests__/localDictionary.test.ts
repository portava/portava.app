...
test('§32: the surface map is keyed EXACTLY by the surfaces the authority licenses', () => {
  // This assertion pins the map's key set to the licensed set.
  //
  // CORRECTED 2026-09-21. This comment used to say the `if` in
  // `localDictionaryFor` was redundant "because the map lookup already answers
  // `undefined` for an unlicensed surface", on the evidence that deleting it
  // changed no test result. That evidence was incomplete and the conclusion was
  // wrong: the map is an OBJECT LITERAL, so a bare `[key]` lookup returns a
  // TRUTHY inherited function for `constructor`, `toString`, `__proto__` and
  // `hasOwnProperty`, and for those four the guard below is skipped and
  // `classes.has(...)` throws. The `if` is restored and the block at the end of
  // this file is the mutation that proves it. A mutation score only proves what
  // its inputs enumerate, and that one enumerated only values the union names.
  // Adding `server_required` to that map reddens this
...
// ── THE GATE THAT WAS DELETED, AND THE INPUTS THAT PROVE IT IS NOT VACUOUS ───
//
// `localDictionaryFor` briefly dropped its `offlineSurfaceAllowed` call on the
// argument that the SURFACE_ENTITY_CLASSES lookup already answered `undefined`
// for every unlicensed surface. It does not: the map is an object literal, so
// it inherits from Object.prototype and a bare `[key]` lookup returns a TRUTHY
// function for four inherited names. These cases are the mutation that should
// have been run and was not — remove the gate and every one of them fails.
//
// MUTATION LOG: deleting `if (!offlineSurfaceAllowed(...)) return []` from
// localDictionaryFor turns the four inherited-key cases RED (each throws
// `classes.has is not a function`) and leaves the rest green. That is what
// makes this block non-vacuous, and it is the run the original mutation missed
// because it only tried surfaces the union can name.
// Reuses the file's own typed fixture rather than a fresh literal: a widening
// cast here would defeat `typecheck:tests`, which exists precisely to stop a
// fixture describing a shape production never emits.
const LICENSABLE: LocalDictionaryPolicy = COUNTRY_PICKER;

test('a LICENSED surface still answers — the control, so the refusals below are not vacuous', () => {
  const got = localDictionaryFor({ ...LICENSABLE, offlinePolicy: 'static_dictionary' as OfflineInputPolicy });
  assert.ok(got.length > 0, 'static_dictionary must still serve, or this whole block proves nothing');
});

for (const refused of ['server_required', 'unavailable'] as const) {
  test(`an unlicensed surface is refused: ${refused}`, () => {
    assert.deepEqual(localDictionaryFor({ ...LICENSABLE, offlinePolicy: refused as OfflineInputPolicy }), []);
  });
}

// The four the map lookup alone does NOT refuse, because they are inherited
// from Object.prototype and answer TRUTHY.
for (const inherited of ['constructor', 'toString', '__proto__', 'hasOwnProperty'] as const) {
  test(`the inherited key "${inherited}" is refused and does not throw`, () => {
    const got = localDictionaryFor({ ...LICENSABLE, offlinePolicy: inherited as unknown as OfflineInputPolicy });
    assert.deepEqual(got, [], `"${inherited}" reached the map lookup — the licence gate is missing`);
  });
}
