/**
 * Pulse Wall feed seed (mock). Mixed item types. Clearly labeled by `source`:
 * - user/circle items are sample content (replaceable by backend; not real users)
 * - compass/city_note are provisional/explicit-reason (no fake ranking)
 * - editorial items are inspiration, labeled isEditorial
 *
...
export const pulseFeed: PulseFeedItem[] = [
  {
...
export function withEditorial(items: PulseFeedItem[]): PulseFeedItem[] {
