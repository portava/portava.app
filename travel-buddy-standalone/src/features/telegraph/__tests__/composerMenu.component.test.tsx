 ...
 *
 * SHOWN RED before commit: the VOICE entry's `available` flipped to true →
 * "names all EIGHT, and says which cannot complete" and the inline-reason test
 * RED.
 */
...
describe('§6.1 — the menu is §6.1s eight, with honest availability', () => {
  it('names all EIGHT of §6.1, in the spec order, and says which cannot complete', () => {
    ...
    expect(unavailable.map((e) => e.id).sort()).toEqual(['GIF', 'PORTAVA', 'VOICE']);
    ...
    expect(availableEntryCount()).toBe(5);
  });

  it('VOICE names the constraint that blocks it', () => {
    expect(composerEntry('VOICE').unavailableReason).toContain('media_type');
  });
  ...
  it('renders every entry, and an unavailable one states its reason INLINE', async () => {
    ...
    expect(screen.getByTestId('telegraph-composer-reason-VOICE')).toBeTruthy();
    ...
    expect(onUnavailable).toHaveBeenCalledWith(expect.objectContaining({ id: 'VOICE' }));
