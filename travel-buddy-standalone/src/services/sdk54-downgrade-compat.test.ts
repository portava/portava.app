...
describe('SDK 54 downgrade — peer dep sync between artifacts/travel-buddy and travel-buddy-standalone', () => {
  // artifacts/travel-buddy/package.json is 3 levels up from src/services (standalone tree).
  ...
  // travel-buddy-standalone/package.json is 2 levels up from src/services (standalone tree).
...
describe('Lockfile-resolved transitive peer dep versions — @expo/config-plugins and expo-modules-core', () => {
  // travel-buddy-standalone/pnpm-lock.yaml is 2 levels up from src/services/ (standalone tree).
  const standaloneLockText = readFileSync(pathResolve(__dir, '../../pnpm-lock.yaml'), 'utf8');
  // monorepo root pnpm-lock.yaml is 3 levels up from src/services/ (standalone tree).
