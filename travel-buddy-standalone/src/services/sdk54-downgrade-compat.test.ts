...
describe('SDK 54 downgrade — package version pins', () => {
  ...
  it('versions are in sync between artifacts/travel-buddy and travel-buddy-standalone', () => {
    ...
    const mono = readPkg('../../../artifacts/travel-buddy/package.json');
    const monoDeps: Record<string, string> = {
      ...mono.dependencies,
      ...mono.devDependencies,
    };
    const toCheck = [
      'expo-notifications',
      'expo-dev-client',
    ...
    for (const name of toCheck) {
      assert.equal(
        deps[name],
        monoDeps[name],
        `${name} version mismatch: artifacts/travel-buddy="${monoDeps[name]}" travel-buddy-standalone="${deps[name]}"`,
      );
    }
  });
});
