...
describe('SDK 54 downgrade — package version pins', () => {
  // This file lives in the standalone tree, so ../../package.json (2 levels up
  // from src/services) is travel-buddy-standalone/package.json — the standalone
  ...
  it('versions are in sync between artifacts/travel-buddy and travel-buddy-standalone', () => {
    ...
    // (travel-buddy-standalone/src/services → workspace root → artifacts/travel-buddy).
    const monorepoCopy = readPkg('../../../artifacts/travel-buddy/package.json');
    const tbDeps: Record<string, string> = {
      ...monorepoCopy.dependencies,
      ...monorepoCopy.devDependencies,
    };
    const toCheck = [
      'expo-notifications',
      'expo-dev-client',
    ...
    for (const name of toCheck) {
      assert.equal(
        tbDeps[name],
        deps[name],
        `${name} version mismatch: artifacts/travel-buddy="${tbDeps[name]}" travel-buddy-standalone="${deps[name]}"`,
      );
    }
  });
});
...
describe('Lockfile-resolved transitive peer dep versions — @babel/core, @expo/config-plugins, expo-modules-core, and metro bundler family', () => {
  // This file lives in travel-buddy-standalone/src/services/, so the standalone
  // lockfile is 2 levels up (travel-buddy-standalone/pnpm-lock.yaml)…
  const standaloneLockText = readFileSync(pathResolve(__dir, '../../pnpm-lock.yaml'), 'utf8');
  // …and the monorepo root pnpm-lock.yaml is 3 levels up (workspace root).
