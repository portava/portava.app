...
describe('Lockfile-resolved transitive peer dep versions — @expo/config-plugins and expo-modules-core', () => {
  // travel-buddy-standalone/pnpm-lock.yaml is 2 levels up from src/services/ within this standalone tree
  const standaloneLockText = readFileSync(pathResolve(__dir, '../../pnpm-lock.yaml'), 'utf8');
  // monorepo root pnpm-lock.yaml is 3 levels up from src/services/ (standalone root → workspace root)
