    ...
    <View style={cp.wrap}>
      <Pressable style={cp.hero} onPress={() => router.push('/(tabs)/ai')}>
        ...
        <View style={cp.heroBody}>
          ...
          <Text style={cp.heroSub}>{pick.category.replace(/[_-]/g, ' ')} · Compass Pick</Text>
          <View style={cp.locRow}><MapPin size={13} color={color.onInk} /><Text style={cp.heroLoc}>{pick.neighborhood}, {pick.city}</Text></View>
          <View style={cp.matchRow}><Info size={13} color={color.onInk} /><Text style={cp.matchText} numberOfLines={2}>{pick.blurb}</Text></View>
          <View style={cp.heroBtns}>
            <Pressable style={cp.ghostBtn} onPress={() => router.push('/(tabs)/ai')}><Text style={cp.ghostText}>View Details</Text></Pressable>
...
const h = StyleSheet.create({
  ...
  iconBtn: { width: avatar.s42, height: avatar.s42, borderRadius: avatar.s42 / 2, borderWidth: 1, borderColor: color.haze, alignItems: 'center', justifyContent: 'center', backgroundColor: color.paperRaised },
...
const g = StyleSheet.create({
  ...
  gemBadge: { width: avatar.s28, height: avatar.s28, borderRadius: avatar.s28 / 2, backgroundColor: color.success, alignItems: 'center', justifyContent: 'center' },
  saveIcon: { width: avatar.s28, height: avatar.s28, borderRadius: avatar.s28 / 2, backgroundColor: 'rgba(17,17,15,0.4)', alignItems: 'center', justifyContent: 'center' },
  body: { padding: space.md, gap: 3 },
