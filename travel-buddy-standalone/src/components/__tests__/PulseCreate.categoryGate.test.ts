 ...
 *
 * validateCategoryGate() is that guard; PulseCreate.tsx's validate() calls it
 * so that handleSubmit() blocks and surfaces feedback instead of submitting a
 * category-less post.
 *
 ...
 *      validate() → handleSubmit() blocks instead of submitting a post with
 *      no category.
 *
...
//
// PulseCreate.tsx's validate() calls validateCategoryGate() so that any new
// post type without a TYPE_CATEGORY entry is automatically blocked at submit.
