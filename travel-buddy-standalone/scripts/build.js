...
const requestedMetroPort = Number(process.env.METRO_PORT || 8081);
let metroPort = requestedMetroPort;
...
async function chooseMetroPort() {
  for (let port = requestedMetroPort; port < requestedMetroPort + 20; port++) {
  ...
  throw new Error(
    `Could not find an available Metro port starting at ${requestedMetroPort}`,
  );
}
