...
import { toLegacy } from './stamps/StampCard';
...
export function StampsTab({ stamps: _legacyStamps = [], viewingUsername, viewingUserId, isOwner = false }: StampsTabProps) {
  ...
  // Featured: most recently earned stamp, shown full-width above the grid.
  const featured = displayed.length > 0
    ? displayed.slice().sort((a, b) => (b.earnedAt ?? '').localeCompare(a.earnedAt ?? ''))[0]
    : null;
  ...
  return (
    <View style={styles.wrap}>
      ...
      {/* Featured — most recent stamp, full-width */}
      {featured ? (
        <Pressable
          style={styles.featuredCard}
          onPress={() => setSelected(featured)}
          accessibilityRole="button"
          accessibilityLabel={`Newest stamp: ${featured.titleOverride ?? featured.definition?.name ?? featured.city ?? featured.country ?? 'stamp'}`}
        >
          <View style={styles.featuredArt}>
            <StampArtwork stamp={toLegacy(featured)} size={72} />
          </View>
          <View style={styles.featuredBody}>
            <Text style={styles.featuredKicker}>Newest Stamp</Text>
            <Text style={styles.featuredTitle} numberOfLines={1}>
              {featured.titleOverride ?? featured.definition?.name ?? featured.city ?? featured.country ?? featured.stampType.replace(/_/g, ' ')}
            </Text>
            ...
            {featured.earnedAt ? (
              <Text style={styles.featuredMeta}>
                {new Date(featured.earnedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
              </Text>
            ) : null}
          </View>
...
const styles = StyleSheet.create({
  featuredCard: {
    marginHorizontal: space.md, marginTop: space.sm, marginBottom: space.xs,
    flexDirection: 'row', alignItems: 'center', gap: space.md,
    borderRadius: radius.lg, borderWidth: 1, borderColor: color.haze,
    backgroundColor: color.paperRaised, padding: space.md,
  },
  featuredArt: { width: 72, height: 72, alignItems: 'center', justifyContent: 'center' },
  featuredBody: { flex: 1, minWidth: 0, gap: 1 },
  featuredKicker: { fontSize: 11, fontWeight: '700', letterSpacing: 0.6, color: color.mute, textTransform: 'uppercase' },
  featuredTitle: { ...t.bodyStrong, fontSize: 15.5, color: color.ink },
  featuredMeta: { fontSize: 12, color: color.mute },
