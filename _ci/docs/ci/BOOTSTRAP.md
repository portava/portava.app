...
no credential at all. And it is **not guarded**: it does not import
`ciSupabaseGuard.mjs` (confirmed against the importer list in §3 step 1), so nothing
...
You are about to use a service-role key. Note that `seed-portava-account.ts` does
**not** import `ciSupabaseGuard.mjs`. Verified by grepping the tree for the import —
the guard has exactly **nine** importers:

| | |
|---|---|
| `src/test/rlsHardening.test.ts` | `:47` |
| `src/test/isOfficialPrivileged.test.ts` | `:65` |
| `src/test/profileRoleNotSelfWritable.test.ts` | `:57` |
| `src/scripts/auditMigrationsVsLive.ts` | `:50` |
| `src/scripts/checkMissingLiveColumns.ts` | `:51` |
| `src/scripts/checkWritePathColumns.ts` | `:79` |
| `src/scripts/checkRankEventsSurfaces.ts` | `:273` |
| `src/scripts/checkMediaObjects.ts` | `:55` |
| `src/scripts/checkDiscoveryCacheKeys.ts` | `:331` |

(`ciSupabaseGuard.mjs`'s own header and `docs/ci/README.md` both say "eight" and
enumerate eight — they predate `checkDiscoveryCacheKeys`. The import list above is
what is on disk. Trust the grep, not the prose, including this document's.)
...
**Four environment variables, not two.** All four commands below import
`ciSupabaseGuard.mjs` as their first import, which spawns
...
`process.exit(2)` (`ciSupabaseGuard.mjs:219`–`:227`). Export only `SUPABASE_URL` and
...
A first line of `[ciSupabaseGuard] Supabase allowlist asserted in-process…` is how
you know the guard passed rather than the script simply not needing it. If you see
`[ciSupabaseGuard] REFUSED`, the exports are wrong and nothing downstream ran.
...
provoke. Confirm `[ciSupabaseGuard] … Proceeding.` appears in each run.
...
1. Confirm the CI project ref. You are the only guard on the seeders — the guard has
   nine importers and no seeder is among them (§3 step 1).
...
- **Production drift itself.** Once CI points at `hwokxgbmezheskbzskfr`, no scheduled
  ...
  workflow will not notice. If you want production watched, that is a separate,
  read-only job that does not exist yet — and it must never reuse
  `scripts/pre-release-check.sh`, which soft-skips the audit to exit 0 when no token
  is present.
