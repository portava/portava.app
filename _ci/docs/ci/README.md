...
| `artifacts/api-server/src/lib/ciSupabaseGuard.mjs` | — | **the chokepoint.** Imported first by all eight CI entry points that can reach Supabase; runs the allowlist policy before any client is constructed or query issued. This is what keeps CI off production |
...
inside every process that can reach Supabase:
`artifacts/api-server/src/lib/ciSupabaseGuard.mjs`, imported as the first
`import` in each entry point. Because ES modules evaluate their imports before
the importing module's body — and before every sibling import, including
`@supabase/supabase-js` — a refusal means the entry point's own code never runs
at all: no client is constructed, no Management API call is made, no query is
issued, no auth user is created.

The eight entry points, which are every path by which a CI job actually reaches
Supabase:

| live-db.yml job | invokes | entry point | how it reaches Supabase |
| --- | --- | --- | --- |
| `api-server-check-all` | `check:all` → `scripts/run-all-checks.sh` | `src/scripts/checkWritePathColumns.ts` | Management API query |
| `api-server-check-all` | `check:all` → `scripts/run-all-checks.sh` | `src/scripts/checkMissingLiveColumns.ts` | Management API query |
| `api-server-check-all` | `check:all` → `scripts/run-all-checks.sh` | `src/scripts/checkRankEventsSurfaces.ts` | Management API; **real `INSERT` probe**, rolled back |
| `schema-drift` | `audit:schema` | `src/scripts/auditMigrationsVsLive.ts` | Management API query |
| `schema-drift` | `check:media-objects` | `src/scripts/checkMediaObjects.ts` | Management API query |
| `live-db-security-suites` | `run-live-suite.sh` | `src/test/rlsHardening.test.ts` | `supabase-js`; anon + service-role reads |
| `live-db-security-suites` | `run-live-suite.sh` | `src/test/profileRoleNotSelfWritable.test.ts` | `supabase-js`; **creates/deletes auth users**, mutates `profiles.role` |
| `live-db-security-suites` | `run-live-suite.sh` | `src/test/isOfficialPrivileged.test.ts` | `supabase-js`; **creates/deletes auth users**, mutates `profiles.is_official` |

The other four checks in `run-all-checks.sh` — `check:frozen-dir`,
`check:async-handlers`, `check:migration-prefixes`, `check:test-runner-flags` —
read only files on disk, reach no database, and deliberately do not import the
guard.

#### One policy, one guard module, eight import sites — and why not one call site

These eight paths **do not converge on a common connection helper**, and that is
a fact about the tree rather than a preference. The five scripts each talk to
the Supabase **Management API** by hand — `process.env.SUPABASE_URL`,
`new URL(...).hostname.split(".")[0]`, then `fetch(https://api.supabase.com/…)`
— five independent copies sharing nothing. The three suites each call
...
none of the eight use it, and putting the guard there would refuse to let the
production API server boot — a process that is *supposed* to talk to production.

So convergence was *created* rather than assumed: one guard module, which does
not restate the policy but **runs
`.github/scripts/assert-nonprod-supabase.sh`**, imported first at each of the
eight entry points. There is one implementation of the rule and one
implementation of the policy.

#### Fail closed, always, with no opt-out

Every outcome other than "the resolved ref is the sanctioned ref" is a refusal,
and there is deliberately **no environment variable that disables the guard**
and **no "credentials absent, so skip" branch**:
...
| the policy script is missing, `bash` cannot be spawned, or the child is killed | refuse |
...
now refuse to run **anywhere** — including a developer laptop — unless
`CI_SUPABASE_PROJECT_REF` and `KNOWN_PROD_PROJECT_REF` are set. That is the
point. These processes create and delete real auth users and write-probe a real
table; the caller must state which project that may happen in, and what
...
Deleting the `import "../lib/ciSupabaseGuard.mjs";` line from an entry point
disables the assertion for that entry point, and nothing mechanical catches it.
...
- **deleting the guard module itself is loud and fail-closed**: all eight entry
  points then fail to load with `ERR_MODULE_NOT_FOUND`.
...
The routes are a **floor, not a proof**. A job is caught if it references
`secrets.`, references a name defined transitively from `secrets.` in a
workflow-level or job-level `env:` block, references one of the Supabase
credential env var names, passes `secrets:` to a reusable workflow, or declares
an `environment:`. What is still outside them:
...
**Per PROCESS that can reach Supabase —
`artifacts/api-server/src/lib/ciSupabaseGuard.mjs`.** The chokepoint, and the
thing that actually keeps CI off production. Imported first by all eight entry
points; runs `.github/scripts/assert-nonprod-supabase.sh` before any client is
constructed or query issued, and exits **2** on any refusal: unset or malformed
`KNOWN_PROD_PROJECT_REF`, unset or malformed `CI_SUPABASE_PROJECT_REF`,
unparseable `SUPABASE_URL`, a resolved ref that is not the sanctioned one, a
sanctioned ref that is production, or an inability to evaluate the policy at
all. **No YAML edit can skip it** — a job that omits, disables, reorders or
never had the workflow step is still refused at runtime.
...
- **Whether an entry point still imports the chokepoint.** Deleting
  `import "../lib/ciSupabaseGuard.mjs";` from one of the eight entry points
  disables the assertion for that process, and nothing mechanical catches it.
  Deleting the guard *module* is loud and fail-closed — all eight then fail with
  `ERR_MODULE_NOT_FOUND` — but the one-line import removal is not. Writing a
  scanner for it would be the sixth round of the mistake this change exists to
  end; the difference is that the edit is now a source diff sitting directly
  above the code that queries the database, not a workflow-configuration change
  that reads as plumbing.
